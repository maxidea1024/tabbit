...
enum class EKeyword : uint8
{
    ...
    Package = 2 UMETA(DisplayName = "Package"),
...
struct X_API FPackageRow
{
    ...
    /** 예약어 라벨을 쓰는 enum 필드 */
    UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Package")
    EKeyword Kind = static_cast<EKeyword>(0);
...
public:
     ...
     * reference, a key that came from user input.
     */
    const FPackageRow* FindByIndex(int32 Key) const;
...
public:
     ...
     * a real one.
     */
    UFUNCTION(BlueprintPure, Category = "Tabbit|Template",
    ...
    /**
     * The Package row with the given Index.
     *
     * bFound rather than a pointer, because Blueprint has no null struct - a graph that
     * ignored a failure would otherwise carry a default row it could not tell apart from
     * a real one.
     */
    UFUNCTION(BlueprintPure, Category = "Tabbit|Package",
