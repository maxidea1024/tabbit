...
export class PackageRecord {
  ...
  /** 예약어 라벨을 쓰는 enum 필드 */
  public get kind(): Keyword { return this._kind }

  public _index: number = 0
  public _label: string = ''
  public _kind: Keyword = 0 as Keyword
...
export class PackageTable {
   ...
   */
  public [Symbol.iterator](): IterableIterator<PackageRecord> {
    return this._records[Symbol.iterator]()
  }

  // Indexing by 'index'
  public get recordsByIndex(): Map<number, PackageRecord> { return this._recordsByIndex }
  private _recordsByIndex: Map<number, PackageRecord> = new Map<number, PackageRecord>()
   ...
   */
  public findByIndex(key: number): PackageRecord | undefined {
   ...
   */
  public getByIndexOrThrow(key: number): PackageRecord {
    const found = this._recordsByIndex.get(key)
    if (!found)
      throw new Error(`There is no record in table "Package" that corresponds to field "index" value ${key}`)
   ...
   */
  public *entries(): IterableIterator<[number, PackageRecord]> {
  ...
  public readBinaryFrom(fileBytes: Uint8Array): void {
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          tabbit.checkColumn(column, 'Package.Index', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Package.Index')
          ...
          break
        case 2:
          tabbit.checkColumn(column, 'Package.Label', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_STRING])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Package.Label')
          for (let i = 0; i < rowCount; ) {
          ...
          break
        case 3:
          tabbit.checkColumn(column, 'Package.Kind', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Package.Kind')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
              records[i]._kind = value as Keyword
          }
          break
        default:
  ...
  private publish(records: PackageRecord[]): void {
    const recordsByIndex = new Map<number, PackageRecord>()
