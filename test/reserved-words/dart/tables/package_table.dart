...
class PackageRecord {
  ...
  String label = '';
  /// 예약어 라벨을 쓰는 enum 필드
  Keyword kind = Keyword.of(0);

}

/// Every row of Package.
class PackageTable extends Iterable<PackageRecord> {
  ...
  @override
  Iterator<PackageRecord> get iterator => _records.iterator;

  Map<int, PackageRecord> _byIndex = {};

  /// The row with this Index, or null when the table has none.
  ///
  /// The lookup to reach for when a missing row is an ordinary answer - an optional
  /// reference, a key that came from user input.
  PackageRecord? findByIndex(int key) => _byIndex[key];
  ...
  PackageRecord getByIndexOrThrow(int key) {
  ...
  PackageRecord? operator [](int key) => findByIndex(key);
  ...
  Iterable<MapEntry<int, PackageRecord>> get entries =>
  ...
  void read(String filename) {
    ...
    final loadedByIndex = <int, PackageRecord>{};
      ...
      switch (column.tag) {
        ...
        case 3:
          ...
          for (var i = 0; i < count; ) {
            final (n, value) = cursor.nextSameI32(count - i);
            for (var left = n; left > 0; left--, i++) {
              loaded[i].kind = Keyword.of(value);
            }
          }
          break;
        default:
