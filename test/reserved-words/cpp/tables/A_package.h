...
#ifndef TABBIT_GENERATED_A_PACKAGE_H
#define TABBIT_GENERATED_A_PACKAGE_H
...
struct PackageRecord {
  ...
  std::string label;
  /// 예약어 라벨을 쓰는 enum 필드
  Keyword kind = static_cast<Keyword>(0);
};
...
class PackageTable {
 public:
  ...
  typename std::vector<PackageRecord>::const_iterator begin() const {
    return records_.begin();
  }

  typename std::vector<PackageRecord>::const_iterator end() const {
    return records_.end();
  }
  ...
  const PackageRecord* find_by_index(std::int32_t key) const {
  ...
  const PackageRecord& get_by_index_or_throw(std::int32_t key) const {
    const PackageRecord* found = find_by_index(key);
        ...
        case 3: {
          ...
          for (std::size_t i = 0; i < row_count; ) {
            std::int32_t n = cursor.next_same_i32(
                static_cast<std::int32_t>(row_count - i), value);
            for (; n > 0; --n, ++i) {
              records[i].kind = static_cast<Keyword>(value);
            }
          }
          break;
        }
...
#endif  // TABBIT_GENERATED_A_PACKAGE_H
