...
func (t *TemplateTable) All() iter.Seq[*TemplateRecord] {
	records := t.records

	return func(yield func(*TemplateRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *TemplateTable) Entries() iter.Seq2[int32, *TemplateRecord] {
	records := t.records

	return func(yield func(int32, *TemplateRecord) bool) {
		for i := range records {
