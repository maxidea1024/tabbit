    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        ...
        case 2:
          ...
          break
        case 3:
          tabbit.checkColumn(column, 'Item.CategoryName', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Item.CategoryName')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
              records[i]._categoryName_ItemCategory_index = value
          }
          break
        default:
