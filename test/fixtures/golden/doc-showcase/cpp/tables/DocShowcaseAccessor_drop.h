...
#ifndef TABBIT_GENERATED_DOCSHOWCASEACCESSOR_DROP_H
#define TABBIT_GENERATED_DOCSHOWCASEACCESSOR_DROP_H
  ...
  void read(const std::string& filename) {
    ...
    for (const tabbit::Column& column : header.columns) {
      ...
      switch (column.tag) {
        ...
        case 2: {
          tabbit::check_column(column, "Drop.Grades", tabbit::kKindArray, false, {tabbit::kElementVarint});
          tabbit::TcbColumnCursor cursor(reader, column, header.row_count, "Drop.Grades");
          for (std::size_t i = 0; i < row_count; ++i) {
            ...
            for (std::int32_t j = 0; j < element_count; ++j) {
              record.grades[static_cast<std::size_t>(j)] = static_cast<Rarity>(cursor.next_i32());
            }
          }
          break;
        }
        case 3: {
          tabbit::check_column(column, "Drop.Counts", tabbit::kKindArray, false, {tabbit::kElementI32, tabbit::kElementVarint}, true);
          element_presence = tabbit::read_element_presence(reader, column);
          element_at = 0;
          tabbit::TcbColumnCursor cursor(reader, column, header.row_count, "Drop.Counts");
          for (std::size_t i = 0; i < row_count; ++i) {
            auto& record = records[i];
            const std::int32_t element_count = cursor.next_length();
            record.counts.resize(static_cast<std::size_t>(element_count));
            record.has_counts_at_.assign(
                static_cast<std::size_t>(element_count), true);
            for (std::int32_t j = 0; j < element_count; ++j) {
              record.counts[static_cast<std::size_t>(j)] = cursor.next_i32();
              record.has_counts_at_[static_cast<std::size_t>(j)] =
...
#endif  // TABBIT_GENERATED_DOCSHOWCASEACCESSOR_DROP_H
