...
#ifndef TABBIT_GENERATED_DOCSHOWCASEACCESSOR_LOOT_H
#define TABBIT_GENERATED_DOCSHOWCASEACCESSOR_LOOT_H
...
class LootTable {
 public:
  ...
  typename std::vector<LootRecord>::const_iterator begin() const {
    return records_.begin();
  }

  typename std::vector<LootRecord>::const_iterator end() const {
    return records_.end();
  }
  ...
  void read(const std::string& filename) {
    ...
    for (const tabbit::Column& column : header.columns) {
      ...
      switch (column.tag) {
        ...
        case 2: {
          tabbit::check_column(column, "Loot.Tags", tabbit::kKindArray, false, {tabbit::kElementString});
          tabbit::TcbColumnCursor cursor(reader, column, header.row_count, "Loot.Tags");
          for (std::size_t i = 0; i < row_count; ++i) {
            ...
            for (std::int32_t j = 0; j < element_count; ++j) {
              record.tags[static_cast<std::size_t>(j)] = cursor.next_string();
            }
          }
          break;
        }
        case 3: {
          tabbit::check_column(column, "Loot.Weight", tabbit::kKindArray, false, {tabbit::kElementI32, tabbit::kElementVarint});
          tabbit::TcbColumnCursor cursor(reader, column, header.row_count, "Loot.Weight");
          for (std::size_t i = 0; i < row_count; ++i) {
            auto& record = records[i];
            const std::int32_t element_count = cursor.next_length();
            record.weight.resize(static_cast<std::size_t>(element_count));
            for (std::int32_t j = 0; j < element_count; ++j) {
              record.weight[static_cast<std::size_t>(j)] = cursor.next_i32();
            }
          }
          break;
        }
...
#endif  // TABBIT_GENERATED_DOCSHOWCASEACCESSOR_LOOT_H
