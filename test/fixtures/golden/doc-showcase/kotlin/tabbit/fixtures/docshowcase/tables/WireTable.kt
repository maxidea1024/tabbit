...
/** Every row of Wire. */
class WireTable : Iterable<WireRecord> {
     ...
     */
    var records: MutableList<WireRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<WireRecord> = records.iterator()

    private var byIndex: HashMap<Int, WireRecord> = HashMap()
     ...
     */
    fun findByIndex(key: Int): WireRecord? = byIndex[key]
     ...
     */
    fun getByIndexOrThrow(key: Int): WireRecord =
     ...
     */
    operator fun get(key: Int): WireRecord? = findByIndex(key)
     ...
     */
    val entries: Sequence<Pair<Int, WireRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<WireRecord>(count)
        val loadedByIndex = HashMap<Int, WireRecord>(count * 2)

        repeat(count) { loaded.add(WireRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "Wire.Index", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Wire.Index")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "Wire.Name", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "Wire.Name")
                    var at = 0
                    while (at < count) {
                ...
                4 -> {
                    checkColumn(column, "Wire.Price", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Wire.Price")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
