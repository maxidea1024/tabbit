...
class ServerTuningTable : Iterable<ServerTuningRecord> {
     ...
     */
    var records: MutableList<ServerTuningRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<ServerTuningRecord> = records.iterator()

    private var byIndex: HashMap<Int, ServerTuningRecord> = HashMap()
     ...
     */
    fun findByIndex(key: Int): ServerTuningRecord? = byIndex[key]
     ...
     */
    fun getByIndexOrThrow(key: Int): ServerTuningRecord =
     ...
     */
    operator fun get(key: Int): ServerTuningRecord? = findByIndex(key)
     ...
     */
    val entries: Sequence<Pair<Int, ServerTuningRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<ServerTuningRecord>(count)
        val loadedByIndex = HashMap<Int, ServerTuningRecord>(count * 2)

        repeat(count) { loaded.add(ServerTuningRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "ServerTuning.Index", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "ServerTuning.Index")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "ServerTuning.Knob", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "ServerTuning.Knob")
                    var at = 0
                    while (at < count) {
                ...
                3 -> {
                    checkColumn(column, "ServerTuning.Value", KIND_SCALAR, false, ELEMENT_F32)
                    val cursor = ColumnCursor(reader, column, count, "ServerTuning.Value")
