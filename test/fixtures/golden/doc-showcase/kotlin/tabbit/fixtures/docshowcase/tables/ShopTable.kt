...
class ShopTable : Iterable<ShopRecord> {
    ...
    private var byIndex: HashMap<Int, ShopRecord> = HashMap()
     ...
     */
    fun findByIndex(key: Int): ShopRecord? = byIndex[key]
     ...
     */
    fun getByIndexOrThrow(key: Int): ShopRecord =
     ...
     */
    operator fun get(key: Int): ShopRecord? = findByIndex(key)
    ...
    fun read(filename: String) {
        ...
        val loadedByIndex = HashMap<Int, ShopRecord>(count * 2)
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "Shop.Index", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Shop.Index")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "Shop.Name", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "Shop.Name")
                    var at = 0
                    while (at < count) {
