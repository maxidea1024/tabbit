...
class QuestTable : Iterable<QuestRecord> {
    ...
    private var byIndex: HashMap<Int, QuestRecord> = HashMap()
     ...
     */
    fun findByIndex(key: Int): QuestRecord? = byIndex[key]
     ...
     */
    fun getByIndexOrThrow(key: Int): QuestRecord =
     ...
     */
    operator fun get(key: Int): QuestRecord? = findByIndex(key)
    ...
    fun read(filename: String) {
        ...
        val loadedByIndex = HashMap<Int, QuestRecord>(count * 2)
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "Quest.Index", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Quest.Index")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "Quest.Name", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "Quest.Name")
                    var at = 0
                    while (at < count) {
                ...
                3 -> {
                    checkColumn(column, "Quest.Reward.Id", KIND_ARRAY, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Quest.Reward.Id")
                    for (record in loaded) {
                        ...
                        record.reward =
                            MutableList(elementCount.coerceAtLeast(0)) { QuestRecord.RewardEntry() }
                ...
                4 -> {
                    checkColumn(column, "Quest.Reward.Count", KIND_ARRAY, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Quest.Reward.Count")
                    for (record in loaded) {
                        val elementCount = cursor.nextLength()
                        if (record.reward.size != elementCount) {
                            throw TcbException(
