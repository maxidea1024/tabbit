...
/** Every row of Deck. */
class DeckTable : Iterable<DeckRecord> {
     ...
     */
    var records: MutableList<DeckRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<DeckRecord> = records.iterator()

    private var byIndex: HashMap<Int, DeckRecord> = HashMap()
     ...
     */
    fun findByIndex(key: Int): DeckRecord? = byIndex[key]
     ...
     */
    fun getByIndexOrThrow(key: Int): DeckRecord =
     ...
     */
    operator fun get(key: Int): DeckRecord? = findByIndex(key)
     ...
     */
    val entries: Sequence<Pair<Int, DeckRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<DeckRecord>(count)
        val loadedByIndex = HashMap<Int, DeckRecord>(count * 2)

        repeat(count) { loaded.add(DeckRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "Deck.Index", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Deck.Index")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "Deck.Slot.Id", KIND_ARRAY, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Deck.Slot.Id")
                    for (record in loaded) {
                        ...
                        record.slot =
                            MutableList(elementCount.coerceAtLeast(0)) { DeckRecord.SlotEntry() }
                ...
                3 -> {
                    checkColumn(column, "Deck.Slot.Label", KIND_ARRAY, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "Deck.Slot.Label")
                    for (record in loaded) {
                        val elementCount = cursor.nextLength()
                ...
                4 -> {
                    checkColumn(column, "Deck.Home.At.X", KIND_SCALAR, false, ELEMENT_F32)
                    val cursor = ColumnCursor(reader, column, count, "Deck.Home.At.X")
                    for (record in loaded) {
                        record.home.at.x = cursor.nextF32()
                    }
                }
                5 -> {
                    checkColumn(column, "Deck.Home.At.Y", KIND_SCALAR, false, ELEMENT_F32)
                    val cursor = ColumnCursor(reader, column, count, "Deck.Home.At.Y")
                    for (record in loaded) {
