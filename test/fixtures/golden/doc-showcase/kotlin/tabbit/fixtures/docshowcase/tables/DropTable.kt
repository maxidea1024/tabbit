    ...
    fun read(filename: String) {
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                ...
                2 -> {
                    checkColumn(column, "Drop.Grades", KIND_ARRAY, false, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Drop.Grades")
                    for (record in loaded) {
                        val elementCount = cursor.nextLength()
                        record.grades = ArrayList(elementCount.coerceAtLeast(0))
                        repeat(elementCount) { record.grades.add(Rarity.of(cursor.nextI32())) }
                    }
                }
                3 -> {
                    checkColumnWithElements(column, "Drop.Counts", KIND_ARRAY, false, ELEMENT_I32, ELEMENT_VARINT)
                    // Behind the row bitmap and in front of the values, walked with a counter
                    // that steps once per element of every row.
                    // spec/types/nullable-array-elements.md.
                    val elementPresence = readElementPresence(reader, column)
                    var elementAt = 0
                    val cursor = ColumnCursor(reader, column, count, "Drop.Counts")
                    for (record in loaded) {
                        val elementCount = cursor.nextLength()
                        record.counts = ArrayList(elementCount.coerceAtLeast(0))
                        record.hasCountsAt =
                            ArrayList(elementCount.coerceAtLeast(0))
                        repeat(elementCount) {
                            record.counts.add(cursor.nextI32())
                            record.hasCountsAt
