...
/** Every row of Access. */
class AccessTable : Iterable<AccessRecord> {
     ...
     */
    var records: MutableList<AccessRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<AccessRecord> = records.iterator()

    private var byIndex: HashMap<Int, AccessRecord> = HashMap()
     ...
     */
    fun findByIndex(key: Int): AccessRecord? = byIndex[key]
     ...
     */
    fun getByIndexOrThrow(key: Int): AccessRecord =
     ...
     */
    operator fun get(key: Int): AccessRecord? = findByIndex(key)
     ...
     */
    val entries: Sequence<Pair<Int, AccessRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<AccessRecord>(count)
        val loadedByIndex = HashMap<Int, AccessRecord>(count * 2)

        repeat(count) { loaded.add(AccessRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "Access.Index", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Access.Index")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "Access.Flags", KIND_SCALAR, false, ELEMENT_I64, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Access.Flags")
                    for (record in loaded) {
                        record.flags = cursor.nextI64()
                    }
                }
                3 -> {
                    checkColumn(column, "Access.Same", KIND_SCALAR, false, ELEMENT_I64, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Access.Same")
                    for (record in loaded) {
