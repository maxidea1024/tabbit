...
/** Every row of Potion. */
class PotionTable : Iterable<PotionRecord> {
     ...
     */
    var records: MutableList<PotionRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<PotionRecord> = records.iterator()

    private var byIndex: HashMap<Int, PotionRecord> = HashMap()
     ...
     */
    fun findByIndex(key: Int): PotionRecord? = byIndex[key]
     ...
     */
    fun getByIndexOrThrow(key: Int): PotionRecord =
     ...
     */
    operator fun get(key: Int): PotionRecord? = findByIndex(key)
     ...
     */
    val entries: Sequence<Pair<Int, PotionRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<PotionRecord>(count)
        val loadedByIndex = HashMap<Int, PotionRecord>(count * 2)

        repeat(count) { loaded.add(PotionRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "Potion.Index", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Potion.Index")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "Potion.Name", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "Potion.Name")
                    var at = 0
                    while (at < count) {
                ...
                3 -> {
                    checkColumn(column, "Potion.Rarity", KIND_SCALAR, false, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Potion.Rarity")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                4 -> {
                    checkColumn(column, "Potion.Price", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Potion.Price")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
