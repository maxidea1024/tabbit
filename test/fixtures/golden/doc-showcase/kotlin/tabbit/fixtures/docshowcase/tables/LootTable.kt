    ...
    fun read(filename: String) {
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                ...
                2 -> {
                    checkColumn(column, "Loot.Tags", KIND_ARRAY, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "Loot.Tags")
                    for (record in loaded) {
                        val elementCount = cursor.nextLength()
                        record.tags = ArrayList(elementCount.coerceAtLeast(0))
                        repeat(elementCount) { record.tags.add(cursor.nextString()) }
                    }
                }
                3 -> {
                    checkColumn(column, "Loot.Weight", KIND_ARRAY, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Loot.Weight")
                    for (record in loaded) {
                        val elementCount = cursor.nextLength()
                        record.weight = ArrayList(elementCount.coerceAtLeast(0))
                        repeat(elementCount) { record.weight.add(cursor.nextI32()) }
                    }
                }
                else ->
