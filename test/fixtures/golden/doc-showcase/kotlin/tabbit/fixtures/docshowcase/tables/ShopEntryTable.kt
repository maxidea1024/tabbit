...
class ShopEntryTable : Iterable<ShopEntryRecord> {
     ...
     */
    var records: MutableList<ShopEntryRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<ShopEntryRecord> = records.iterator()

    private var byIndex: HashMap<Int, ShopEntryRecord> = HashMap()
     ...
     */
    fun findByIndex(key: Int): ShopEntryRecord? = byIndex[key]
     ...
     */
    fun getByIndexOrThrow(key: Int): ShopEntryRecord =
     ...
     */
    operator fun get(key: Int): ShopEntryRecord? = findByIndex(key)
     ...
     */
    val entries: Sequence<Pair<Int, ShopEntryRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<ShopEntryRecord>(count)
        val loadedByIndex = HashMap<Int, ShopEntryRecord>(count * 2)

        repeat(count) { loaded.add(ShopEntryRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "ShopEntry.Index", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "ShopEntry.Index")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "ShopEntry.ShopId", KIND_SCALAR, false, ELEMENT_I32)
                    val cursor = ColumnCursor(reader, column, count, "ShopEntry.ShopId")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                3 -> {
                    checkColumn(column, "ShopEntry.PotionId", KIND_SCALAR, false, ELEMENT_I32)
                    val cursor = ColumnCursor(reader, column, count, "ShopEntry.PotionId")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                4 -> {
                    checkColumn(column, "ShopEntry.Stock", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "ShopEntry.Stock")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
