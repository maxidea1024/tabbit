...
/** Every row of Sample. */
class SampleTable : Iterable<SampleRecord> {
     ...
     */
    var records: MutableList<SampleRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<SampleRecord> = records.iterator()

    private var byIndex: HashMap<Int, SampleRecord> = HashMap()
     ...
     */
    fun findByIndex(key: Int): SampleRecord? = byIndex[key]
     ...
     */
    fun getByIndexOrThrow(key: Int): SampleRecord =
     ...
     */
    operator fun get(key: Int): SampleRecord? = findByIndex(key)
     ...
     */
    val entries: Sequence<Pair<Int, SampleRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<SampleRecord>(count)
        val loadedByIndex = HashMap<Int, SampleRecord>(count * 2)

        repeat(count) { loaded.add(SampleRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "Sample.Index", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Sample.Index")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "Sample.Count", KIND_SCALAR, false, ELEMENT_I64, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Sample.Count")
                    for (record in loaded) {
                        record.count = cursor.nextI64()
                    }
                }
                3 -> {
                    checkColumn(column, "Sample.Ratio", KIND_SCALAR, false, ELEMENT_F32)
                    val cursor = ColumnCursor(reader, column, count, "Sample.Ratio")
                    for (record in loaded) {
                        record.ratio = cursor.nextF32()
                    }
                }
                4 -> {
                    checkColumn(column, "Sample.Precise", KIND_SCALAR, false, ELEMENT_F64, ELEMENT_F32, ELEMENT_I32)
                    val cursor = ColumnCursor(reader, column, count, "Sample.Precise")
                    for (record in loaded) {
                        record.precise = cursor.nextF64()
                    }
                }
                5 -> {
                    checkColumn(column, "Sample.Enabled", KIND_SCALAR, false, ELEMENT_BOOL)
                    val cursor = ColumnCursor(reader, column, count, "Sample.Enabled")
                    for (record in loaded) {
                        record.enabled = cursor.nextBool()
                    }
                }
                6 -> {
                    checkColumn(column, "Sample.Label", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "Sample.Label")
                    var at = 0
                    while (at < count) {
                ...
                7 -> {
                    checkColumn(column, "Sample.At", KIND_SCALAR, false, ELEMENT_I64)
                    val cursor = ColumnCursor(reader, column, count, "Sample.At")
                    for (record in loaded) {
                        record.at = cursor.nextI64()
                    }
                }
                8 -> {
                    checkColumn(column, "Sample.Span", KIND_SCALAR, false, ELEMENT_I64)
                    val cursor = ColumnCursor(reader, column, count, "Sample.Span")
                    for (record in loaded) {
                        record.span = cursor.nextI64()
                    }
                }
                9 -> {
                    checkColumn(column, "Sample.Id", KIND_SCALAR, false, ELEMENT_UUID)
                    for (record in loaded) {
