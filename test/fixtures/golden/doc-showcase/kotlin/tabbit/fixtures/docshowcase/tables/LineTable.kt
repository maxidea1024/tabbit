...
/** Every row of Line. */
class LineTable : Iterable<LineRecord> {
     ...
     */
    var records: MutableList<LineRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<LineRecord> = records.iterator()

    private var byIndex: HashMap<Int, LineRecord> = HashMap()
     ...
     */
    fun findByIndex(key: Int): LineRecord? = byIndex[key]
     ...
     */
    fun getByIndexOrThrow(key: Int): LineRecord =
     ...
     */
    operator fun get(key: Int): LineRecord? = findByIndex(key)
     ...
     */
    val entries: Sequence<Pair<Int, LineRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<LineRecord>(count)
        val loadedByIndex = HashMap<Int, LineRecord>(count * 2)

        repeat(count) { loaded.add(LineRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "Line.Index", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Line.Index")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "Line.Body", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "Line.Body")
                    var at = 0
                    while (at < count) {
                ...
                3 -> {
                    checkColumn(column, "Line.Icon", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "Line.Icon")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameString(count - at)
                        while (n > 0) {
