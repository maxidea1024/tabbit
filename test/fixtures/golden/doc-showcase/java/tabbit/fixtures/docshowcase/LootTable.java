...
/** Every row of Loot. */
public final class LootTable implements Iterable<LootRecord> {
    private List<LootRecord> records = new ArrayList<>();
    private Map<Integer, LootRecord> byIndex = new HashMap<>();
     ...
     */
    public List<LootRecord> records() {
        return records;
    }
     ...
     */
    @Override
    public Iterator<LootRecord> iterator() {
    ...
    public LootRecord getByIndexOrThrow(int key) {
        LootRecord record = byIndex.get(key);

        if (record == null) {
            throw new TcbReader.RecordNotFoundException(
                "there is no record in table `Loot` that corresponds to field "
                + "`Index` value " + key);
        }
     ...
     */
    public Iterable<Map.Entry<Integer, LootRecord>> entries() {
        final List<LootRecord> rows = records;

        return () -> new Iterator<Map.Entry<Integer, LootRecord>>() {
            private final Iterator<LootRecord> source = rows.iterator();
            ...
            @Override
            public Map.Entry<Integer, LootRecord> next() {
                final LootRecord row = source.next();
    ...
    public void read(Path filename) {
        ...
        List<LootRecord> loaded = new ArrayList<>(count);
        Map<Integer, LootRecord> loadedByIndex = new HashMap<>(count * 2);

        for (int i = 0; i < count; i++) {
            loaded.add(new LootRecord());
        }
        ...
        for (TcbReader.Column column : header.columns) {
            ...
            switch (column.tag) {
                ...
                case 2: {
                    TcbReader.checkColumn(column, "Loot.Tags", TcbReader.KIND_ARRAY, false, TcbReader.ELEMENT_STRING);
                    cursor = new TcbReader.ColumnCursor(reader, column, count, "Loot.Tags");
                    for (LootRecord record : loaded) {
                        int elementCount;
                        elementCount = cursor.nextLength();
                        record.tags = new String[elementCount];
                        for (int j = 0; j < elementCount; j++) {
                            record.tags[j] = cursor.nextString();
                        }
                    }
                    break;
                }
                case 3: {
                    TcbReader.checkColumn(column, "Loot.Weight", TcbReader.KIND_ARRAY, false, TcbReader.ELEMENT_I32, TcbReader.ELEMENT_VARINT);
                    cursor = new TcbReader.ColumnCursor(reader, column, count, "Loot.Weight");
                    for (LootRecord record : loaded) {
                        int elementCount;
                        elementCount = cursor.nextLength();
                        record.weight = new int[elementCount];
                        for (int j = 0; j < elementCount; j++) {
                            record.weight[j] = cursor.nextI32();
                        }
                    }
                    break;
                }
