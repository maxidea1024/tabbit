    ...
    public void read(Path filename) {
        ...
        for (TcbReader.Column column : header.columns) {
            ...
            switch (column.tag) {
                ...
                case 2: {
                    TcbReader.checkColumn(column, "Drop.Grades", TcbReader.KIND_ARRAY, false, TcbReader.ELEMENT_VARINT);
                    cursor = new TcbReader.ColumnCursor(reader, column, count, "Drop.Grades");
                    for (DropRecord record : loaded) {
                        int elementCount;
                        elementCount = cursor.nextLength();
                        record.grades = new Rarity[elementCount];
                        for (int j = 0; j < elementCount; j++) {
                            record.grades[j] = Rarity.of(cursor.nextI32());
                        }
                    }
                    break;
                }
                case 3: {
                    TcbReader.checkColumnWithElements(column, "Drop.Counts", TcbReader.KIND_ARRAY, false, TcbReader.ELEMENT_I32, TcbReader.ELEMENT_VARINT);
                    // Behind the row bitmap and in front of the values, walked with a counter
                    // that steps once per element of every row.
                    // spec/types/nullable-array-elements.md.
                    elementPresence = TcbReader.readElementPresence(reader, column);
                    elementAt = 0;
                    cursor = new TcbReader.ColumnCursor(reader, column, count, "Drop.Counts");
                    for (DropRecord record : loaded) {
                        int elementCount;
                        elementCount = cursor.nextLength();
                        record.counts = new int[elementCount];
                        record.hasCountsAt = new boolean[elementCount];
                        for (int j = 0; j < elementCount; j++) {
                            record.counts[j] = cursor.nextI32();
                            record.hasCountsAt[j] =
