...
---@field index integer
---@field name string
local ShopRecordMeta = tcb.strictType("a `Shop` row", { "index", "name" })

---@return ShopRecord
local function newShopRecord()
  return setmetatable({
    index = 0,
    name = "",
  }, ShopRecordMeta)
...
---@class ShopTable
---@field records ShopRecord[]
local ShopTable = {}

local ShopTableMeta = tcb.strictInstance("the `Shop` table", ShopTable, { "records", "byIndex" })

function ShopTable.new()
  return setmetatable({
    records = {},
    byIndex = {},
  }, ShopTableMeta)
...
function ShopTable:containsIndex(key)
...
function ShopTable:readBytes(data)
  ...
  for c = 1, #columns do
    ...
    if column.tag == 1 then
      tcb.checkColumn(column, "Shop.Index", tcb.KIND_SCALAR, false, { tcb.ELEMENT_I32, tcb.ELEMENT_VARINT })
      local cursor = tcb.newCursor(reader, column, count, "Shop.Index")
      local at = 0
    ...
    elseif column.tag == 2 then
      tcb.checkColumn(column, "Shop.Name", tcb.KIND_SCALAR, false, { tcb.ELEMENT_STRING })
      local cursor = tcb.newCursor(reader, column, count, "Shop.Name")
      local at = 0
...
function ShopTable:read(filename)
