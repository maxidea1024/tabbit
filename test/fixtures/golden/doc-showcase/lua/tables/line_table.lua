...
---@class LineTable
---@field records LineRecord[]
local LineTable = {}

local LineTableMeta = tcb.strictInstance("the `Line` table", LineTable, { "records", "byIndex" })

function LineTable.new()
  return setmetatable({
    records = {},
    byIndex = {},
  }, LineTableMeta)
...
function LineTable:containsIndex(key)
...
function LineTable:readBytes(data)
  ...
  for c = 1, #columns do
    ...
    if column.tag == 1 then
      tcb.checkColumn(column, "Line.Index", tcb.KIND_SCALAR, false, { tcb.ELEMENT_I32, tcb.ELEMENT_VARINT })
      local cursor = tcb.newCursor(reader, column, count, "Line.Index")
      local at = 0
    ...
    elseif column.tag == 2 then
      tcb.checkColumn(column, "Line.Body", tcb.KIND_SCALAR, false, { tcb.ELEMENT_STRING })
      local cursor = tcb.newCursor(reader, column, count, "Line.Body")
      local at = 0
    ...
    elseif column.tag == 3 then
      tcb.checkColumn(column, "Line.Icon", tcb.KIND_SCALAR, false, { tcb.ELEMENT_STRING })
      local cursor = tcb.newCursor(reader, column, count, "Line.Icon")
      local at = 0
...
function LineTable:read(filename)
