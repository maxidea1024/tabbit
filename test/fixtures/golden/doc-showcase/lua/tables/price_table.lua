...
---@class PriceTable
---@field records PriceRecord[]
local PriceTable = {}

local PriceTableMeta = tcb.strictInstance("the `Price` table", PriceTable, { "records", "byIndex" })

function PriceTable.new()
  return setmetatable({
    records = {},
    byIndex = {},
  }, PriceTableMeta)
...
function PriceTable:containsIndex(key)
...
function PriceTable:readBytes(data)
  ...
  for c = 1, #columns do
    ...
    if column.tag == 1 then
      tcb.checkColumn(column, "Price.Index", tcb.KIND_SCALAR, false, { tcb.ELEMENT_I32, tcb.ELEMENT_VARINT })
      local cursor = tcb.newCursor(reader, column, count, "Price.Index")
      local at = 0
    ...
    elseif column.tag == 2 then
      tcb.checkColumn(column, "Price.Name", tcb.KIND_SCALAR, false, { tcb.ELEMENT_STRING })
      local cursor = tcb.newCursor(reader, column, count, "Price.Name")
      local at = 0
    ...
    elseif column.tag == 3 then
      tcb.checkColumn(column, "Price.Amount", tcb.KIND_SCALAR, false, { tcb.ELEMENT_I32, tcb.ELEMENT_VARINT })
      local cursor = tcb.newCursor(reader, column, count, "Price.Amount")
      local at = 0
...
function PriceTable:read(filename)
