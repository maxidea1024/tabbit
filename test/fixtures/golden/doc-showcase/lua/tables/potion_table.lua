...
---@return PotionRecord
local function newPotionRecord()
  return setmetatable({
    index = 0,
    name = "",
    rarity = 0,
...
local PotionTableMeta = tcb.strictInstance("the `Potion` table", PotionTable, { "records", "byIndex" })

function PotionTable.new()
  return setmetatable({
    records = {},
    byIndex = {},
  }, PotionTableMeta)
...
function PotionTable:containsIndex(key)
...
function PotionTable:readBytes(data)
  ...
  for c = 1, #columns do
    ...
    if column.tag == 1 then
      tcb.checkColumn(column, "Potion.Index", tcb.KIND_SCALAR, false, { tcb.ELEMENT_I32, tcb.ELEMENT_VARINT })
      local cursor = tcb.newCursor(reader, column, count, "Potion.Index")
      local at = 0
    ...
    elseif column.tag == 2 then
      tcb.checkColumn(column, "Potion.Name", tcb.KIND_SCALAR, false, { tcb.ELEMENT_STRING })
      local cursor = tcb.newCursor(reader, column, count, "Potion.Name")
      local at = 0
    ...
    elseif column.tag == 3 then
      tcb.checkColumn(column, "Potion.Rarity", tcb.KIND_SCALAR, false, { tcb.ELEMENT_VARINT })
      local cursor = tcb.newCursor(reader, column, count, "Potion.Rarity")
      local at = 0
    ...
    elseif column.tag == 4 then
      tcb.checkColumn(column, "Potion.Price", tcb.KIND_SCALAR, false, { tcb.ELEMENT_I32, tcb.ELEMENT_VARINT })
      local cursor = tcb.newCursor(reader, column, count, "Potion.Price")
      local at = 0
...
function PotionTable:read(filename)
