...
local function newAnimationRecord()
  return setmetatable({
    index = "",
    slot = 0,
    blend = 0.0,
...
---@field records AnimationRecord[]
local AnimationTable = {}

local AnimationTableMeta = tcb.strictInstance("the `Animation` table", AnimationTable, { "records", "byIndex", "bySlot" })

function AnimationTable.new()
  return setmetatable({
    records = {},
    byIndex = {},
    bySlot = {},
...
function AnimationTable:containsIndex(key)
...
function AnimationTable:entries()
  local records = self.records
  local i = 0
...
-- The row with this Slot, or nil when the table has none: the lookup to
-- reach for when a missing row is an ordinary answer.
function AnimationTable:findBySlot(key)
  return self.bySlot[key]
end

-- The row with this Slot, or a raised error naming what was missing -
-- for a key that has to be there.
function AnimationTable:getBySlotOrThrow(key)
  local record = self:findBySlot(key)

  if record == nil then
    tcb.fail("there is no record in table `Animation` that corresponds to " ..
      "field `Slot` value %s", tostring(key))
  end
...
function AnimationTable:containsSlot(key)
  return self:findBySlot(key) ~= nil
end
...
function AnimationTable:readBytes(data)
  ...
  for c = 1, #columns do
    ...
    if column.tag == 1 then
      tcb.checkColumn(column, "Animation.Index", tcb.KIND_SCALAR, false, { tcb.ELEMENT_STRING })
      local cursor = tcb.newCursor(reader, column, count, "Animation.Index")
      local at = 0
    ...
    elseif column.tag == 2 then
      tcb.checkColumn(column, "Animation.Slot", tcb.KIND_SCALAR, false, { tcb.ELEMENT_I32, tcb.ELEMENT_VARINT })
      local cursor = tcb.newCursor(reader, column, count, "Animation.Slot")
      local at = 0
    ...
    elseif column.tag == 3 then
      tcb.checkColumn(column, "Animation.Blend", tcb.KIND_SCALAR, false, { tcb.ELEMENT_F32 })
      local cursor = tcb.newCursor(reader, column, count, "Animation.Blend")
      for i = 1, count do
        local record = records[i]
  ...
  for i = 1, #records do
    bySlot[records[i].slot] = records[i]
  end
...
function AnimationTable:read(filename)
