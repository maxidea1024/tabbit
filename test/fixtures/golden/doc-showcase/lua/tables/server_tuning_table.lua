...
---@return ServerTuningRecord
local function newServerTuningRecord()
  return setmetatable({
    index = 0,
    knob = "",
    value = 0.0,
  }, ServerTuningRecordMeta)
end

---@class ServerTuningTable
---@field records ServerTuningRecord[]
local ServerTuningTable = {}

local ServerTuningTableMeta = tcb.strictInstance("the `ServerTuning` table", ServerTuningTable, { "records", "byIndex" })

function ServerTuningTable.new()
  return setmetatable({
    records = {},
    byIndex = {},
  }, ServerTuningTableMeta)
end
...
function ServerTuningTable:count()
  return #self.records
end
...
function ServerTuningTable:iter()
...
function ServerTuningTable:findByIndex(key)
  return self.byIndex[key]
end

-- The row with this Index, or a raised error naming what was missing -
-- for a key that has to be there.
function ServerTuningTable:getByIndexOrThrow(key)
  local record = self:findByIndex(key)

  if record == nil then
    tcb.fail("there is no record in table `ServerTuning` that corresponds to " ..
...
function ServerTuningTable:containsIndex(key)
...
function ServerTuningTable:entries()
  local records = self.records
  local i = 0
...
function ServerTuningTable:readBytes(data)
  ...
  for i = 1, count do
    records[i] = newServerTuningRecord()
  end
  ...
  for c = 1, #columns do
    ...
    if column.tag == 1 then
      tcb.checkColumn(column, "ServerTuning.Index", tcb.KIND_SCALAR, false, { tcb.ELEMENT_I32, tcb.ELEMENT_VARINT })
      local cursor = tcb.newCursor(reader, column, count, "ServerTuning.Index")
      local at = 0
    ...
    elseif column.tag == 2 then
      tcb.checkColumn(column, "ServerTuning.Knob", tcb.KIND_SCALAR, false, { tcb.ELEMENT_STRING })
      local cursor = tcb.newCursor(reader, column, count, "ServerTuning.Knob")
      local at = 0
      ...
      while at < count do
        ...
        for i = at + 1, at + n do
          records[i].knob = value
        end

        at = at + n
      end
    elseif column.tag == 3 then
      tcb.checkColumn(column, "ServerTuning.Value", tcb.KIND_SCALAR, false, { tcb.ELEMENT_F32 })
      local cursor = tcb.newCursor(reader, column, count, "ServerTuning.Value")
      for i = 1, count do
        local record = records[i]
...
function ServerTuningTable:read(filename)
  self:readBytes(tcb.readAllBytes(filename))
end

return ServerTuningTable
