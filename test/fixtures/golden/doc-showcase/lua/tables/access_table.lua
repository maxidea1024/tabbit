...
---@return AccessRecord
local function newAccessRecord()
  return setmetatable({
    index = 0,
    flags = 0,
    same = 0,
...
---@class AccessTable
---@field records AccessRecord[]
local AccessTable = {}

local AccessTableMeta = tcb.strictInstance("the `Access` table", AccessTable, { "records", "byIndex" })

function AccessTable.new()
  return setmetatable({
    records = {},
    byIndex = {},
  }, AccessTableMeta)
...
function AccessTable:containsIndex(key)
...
function AccessTable:readBytes(data)
  ...
  for c = 1, #columns do
    ...
    if column.tag == 1 then
      tcb.checkColumn(column, "Access.Index", tcb.KIND_SCALAR, false, { tcb.ELEMENT_I32, tcb.ELEMENT_VARINT })
      local cursor = tcb.newCursor(reader, column, count, "Access.Index")
      local at = 0
    ...
    elseif column.tag == 2 then
      tcb.checkColumn(column, "Access.Flags", tcb.KIND_SCALAR, false, { tcb.ELEMENT_I64, tcb.ELEMENT_I32, tcb.ELEMENT_VARINT })
      local cursor = tcb.newCursor(reader, column, count, "Access.Flags")
      for i = 1, count do
        local record = records[i]
        record.flags = cursor:nextI64()
      end
    elseif column.tag == 3 then
      tcb.checkColumn(column, "Access.Same", tcb.KIND_SCALAR, false, { tcb.ELEMENT_I64, tcb.ELEMENT_I32, tcb.ELEMENT_VARINT })
      local cursor = tcb.newCursor(reader, column, count, "Access.Same")
      for i = 1, count do
        local record = records[i]
...
function AccessTable:read(filename)
