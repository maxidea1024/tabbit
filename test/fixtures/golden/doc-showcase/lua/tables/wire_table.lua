...
---@class WireTable
---@field records WireRecord[]
local WireTable = {}

local WireTableMeta = tcb.strictInstance("the `Wire` table", WireTable, { "records", "byIndex" })

function WireTable.new()
  return setmetatable({
    records = {},
    byIndex = {},
  }, WireTableMeta)
...
function WireTable:containsIndex(key)
...
function WireTable:readBytes(data)
  ...
  for c = 1, #columns do
    ...
    if column.tag == 1 then
      tcb.checkColumn(column, "Wire.Index", tcb.KIND_SCALAR, false, { tcb.ELEMENT_I32, tcb.ELEMENT_VARINT })
      local cursor = tcb.newCursor(reader, column, count, "Wire.Index")
      local at = 0
    ...
    elseif column.tag == 2 then
      tcb.checkColumn(column, "Wire.Name", tcb.KIND_SCALAR, false, { tcb.ELEMENT_STRING })
      local cursor = tcb.newCursor(reader, column, count, "Wire.Name")
      local at = 0
    ...
    elseif column.tag == 4 then
      tcb.checkColumn(column, "Wire.Price", tcb.KIND_SCALAR, false, { tcb.ELEMENT_I32, tcb.ELEMENT_VARINT })
      local cursor = tcb.newCursor(reader, column, count, "Wire.Price")
      local at = 0
...
function WireTable:read(filename)
