    ...
    def read(filename)
      ...
      columns.each do |column|
        ...
        when 2
          ...
          cursor = Tabbit::ColumnCursor.new(reader, column, count, 'Drop.Grades')
          records.each do |record|
            element_count = cursor.next_length
            record.grades = Array.new(element_count) { cursor.next_i32 }
          end
        when 3
          ...
          element_at = 0
          cursor = Tabbit::ColumnCursor.new(reader, column, count, 'Drop.Counts')
          records.each do |record|
            element_count = cursor.next_length
            record.counts = Array.new(element_count) { cursor.next_i32 }
            record.has_counts_at = Array.new(element_count) do |at|
