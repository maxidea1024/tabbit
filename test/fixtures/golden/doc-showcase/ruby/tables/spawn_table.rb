...
module DocShowcase
  ...
  class SpawnTable
    ...
    def read(filename)
      ...
      records = Array.new(count) { SpawnRecord.new }
      ...
      columns.each do |column|
        ...
        when 1
          ...
          cursor = Tabbit::ColumnCursor.new(reader, column, count, 'Spawn.Index')
          at = 0
          while at < count
        ...
        when 2
          ...
          cursor = Tabbit::ColumnCursor.new(reader, column, count, 'Spawn.At.X')
          records.each do |record|
            record.at.x = cursor.next_f32
          end
        when 3
          Tabbit.check_column(column, 'Spawn.At.Y', Tabbit::KIND_SCALAR, false, [Tabbit::ELEMENT_F32])
          cursor = Tabbit::ColumnCursor.new(reader, column, count, 'Spawn.At.Y')
          records.each do |record|
        ...
        when 4
          ...
          cursor = Tabbit::ColumnCursor.new(reader, column, count, 'Spawn.Radius')
          records.each do |record|
          ...
          records.each_with_index do |record, row|
            record.has_radius = Tabbit.present?(presence, row)
            record.radius = 0.0 unless record.has_radius
          end
        else
