    ...
    def read(filename)
      ...
      columns.each do |column|
        ...
        when 2
          ...
          cursor = Tabbit::ColumnCursor.new(reader, column, count, 'Loot.Tags')
          records.each do |record|
            element_count = cursor.next_length
            record.tags = Array.new(element_count) { cursor.next_string }
          end
        when 3
          Tabbit.check_column(column, 'Loot.Weight', Tabbit::KIND_ARRAY, false, [Tabbit::ELEMENT_I32, Tabbit::ELEMENT_VARINT])
          cursor = Tabbit::ColumnCursor.new(reader, column, count, 'Loot.Weight')
          records.each do |record|
            element_count = cursor.next_length
            record.weight = Array.new(element_count) { cursor.next_i32 }
          end
        else
