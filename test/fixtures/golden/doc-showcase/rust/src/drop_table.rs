    ...
    pub fn read(&mut self, filename: &Path) -> tabbit::Result<()> {
        ...
        for column in &header.columns {
            ...
            match column.tag {
                ...
                2 => {
                    tabbit::check_column(column, "Drop.Grades", tabbit::KIND_ARRAY, false, &[tabbit::ELEMENT_VARINT])?;
                    let mut cursor = tabbit::TcbColumnCursor::new(&mut reader, column, header.row_count, "Drop.Grades")?;
                    for record in records.iter_mut() {
                        let element_count = cursor.next_length()?.max(0) as usize;
                        record.grades = Vec::with_capacity(element_count.min(65536));
                        for _ in 0..element_count {
                            record.grades.push(Rarity::from_value(cursor.next_i32()?).unwrap_or_default());
                        }
                    }
                }
                3 => {
                    tabbit::check_column_with_elements(column, "Drop.Counts", tabbit::KIND_ARRAY, false, &[tabbit::ELEMENT_I32, tabbit::ELEMENT_VARINT])?;
                    let element_presence = tabbit::read_element_presence(&mut reader, column)?;
                    let mut element_at: usize = 0;
                    let mut cursor = tabbit::TcbColumnCursor::new(&mut reader, column, header.row_count, "Drop.Counts")?;
                    for record in records.iter_mut() {
                        let element_count = cursor.next_length()?.max(0) as usize;
                        record.counts = Vec::with_capacity(element_count.min(65536));
                        record.has_counts_at =
                            Vec::with_capacity(element_count.min(65536));
                        for _ in 0..element_count {
                            record.counts.push(cursor.next_i32()?);
                            record.has_counts_at
