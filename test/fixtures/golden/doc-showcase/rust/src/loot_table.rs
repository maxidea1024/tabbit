    ...
    pub fn read(&mut self, filename: &Path) -> tabbit::Result<()> {
        ...
        for column in &header.columns {
            ...
            match column.tag {
                ...
                2 => {
                    tabbit::check_column(column, "Loot.Tags", tabbit::KIND_ARRAY, false, &[tabbit::ELEMENT_STRING])?;
                    let mut cursor = tabbit::TcbColumnCursor::new(&mut reader, column, header.row_count, "Loot.Tags")?;
                    for record in records.iter_mut() {
                        let element_count = cursor.next_length()?.max(0) as usize;
                        record.tags = Vec::with_capacity(element_count.min(65536));
                        for _ in 0..element_count {
                            record.tags.push(cursor.next_string()?);
                        }
                    }
                }
                3 => {
                    tabbit::check_column(column, "Loot.Weight", tabbit::KIND_ARRAY, false, &[tabbit::ELEMENT_I32, tabbit::ELEMENT_VARINT])?;
                    let mut cursor = tabbit::TcbColumnCursor::new(&mut reader, column, header.row_count, "Loot.Weight")?;
                    for record in records.iter_mut() {
                        let element_count = cursor.next_length()?.max(0) as usize;
                        record.weight = Vec::with_capacity(element_count.min(65536));
                        for _ in 0..element_count {
                            record.weight.push(cursor.next_i32()?);
                        }
                    }
                }
                _ => {
