...
pub struct PotionTable {
    records: Vec<PotionRecord>,
    by_index: HashMap<i32, usize>,
}

impl PotionTable {
    /// Every row, in the order the sheet declared them.
    pub fn records(&self) -> &[PotionRecord] {
        &self.records
    }
    ...
    pub fn iter(&self) -> std::slice::Iter<'_, PotionRecord> {
        self.records.iter()
    }
    ...
    pub fn find_by_index(&self, key: i32) -> Option<&PotionRecord> {
    ...
    /// error it returns.
    pub fn get_by_index_or_error(&self, key: i32) -> tabbit::Result<&PotionRecord> {
        self.by_index
            .get(&key)
    ...
    pub fn entries(&self) -> impl Iterator<Item = (&i32, &PotionRecord)> + '_ {
    ...
    pub fn read(&mut self, filename: &Path) -> tabbit::Result<()> {
        ...
        let mut records = vec![PotionRecord::default(); count];
        ...
        for column in &header.columns {
            ...
            match column.tag {
                1 => {
                    tabbit::check_column(column, "Potion.Index", tabbit::KIND_SCALAR, false, &[tabbit::ELEMENT_I32, tabbit::ELEMENT_VARINT])?;
                    let mut cursor = tabbit::TcbColumnCursor::new(&mut reader, column, header.row_count, "Potion.Index")?;
                    let mut at = 0usize;
                ...
                2 => {
                    tabbit::check_column(column, "Potion.Name", tabbit::KIND_SCALAR, false, &[tabbit::ELEMENT_STRING])?;
                    let mut cursor = tabbit::TcbColumnCursor::new(&mut reader, column, header.row_count, "Potion.Name")?;
                    let mut at = 0usize;
                    while at < records.len() {
                ...
                3 => {
                    tabbit::check_column(column, "Potion.Rarity", tabbit::KIND_SCALAR, false, &[tabbit::ELEMENT_VARINT])?;
                    let mut cursor = tabbit::TcbColumnCursor::new(&mut reader, column, header.row_count, "Potion.Rarity")?;
                    let mut at = 0usize;
                    while at < records.len() {
                        let (n, value) = cursor.next_same_i32((records.len() - at) as i32)?;
                        for _ in 0..n {
                            records[at].rarity = Rarity::from_value(value).unwrap_or_default();
                            at += 1;
                        }
                    }
                }
                4 => {
                    tabbit::check_column(column, "Potion.Price", tabbit::KIND_SCALAR, false, &[tabbit::ELEMENT_I32, tabbit::ELEMENT_VARINT])?;
                    let mut cursor = tabbit::TcbColumnCursor::new(&mut reader, column, header.row_count, "Potion.Price")?;
                    let mut at = 0usize;
                    while at < records.len() {
                        let (n, value) = cursor.next_same_i32((records.len() - at) as i32)?;
                        for _ in 0..n {
...
impl<'a> IntoIterator for &'a PotionTable {
    type Item = &'a PotionRecord;
    type IntoIter = std::slice::Iter<'a, PotionRecord>;
