...
impl AnimationTable {
    ...
    pub fn iter(&self) -> std::slice::Iter<'_, AnimationRecord> {
        self.records.iter()
    }
    ...
    pub fn find_by_index(&self, key: &str) -> Option<&AnimationRecord> {
    ...
    /// error it returns.
    pub fn get_by_index_or_error(&self, key: &str) -> tabbit::Result<&AnimationRecord> {
        self.by_index
            .get(key)
    ...
    pub fn contains_index(&self, key: &str) -> bool {
    ...
    pub fn entries(&self) -> impl Iterator<Item = (&String, &AnimationRecord)> + '_ {
    ...
    /// The lookup to reach for when a missing row is an ordinary answer - an optional
    /// reference, a key that came from user input.
    pub fn find_by_slot(&self, key: i32) -> Option<&AnimationRecord> {
    ...
    /// exception to throw, so where the other languages name a throw this one names the
    /// error it returns.
    pub fn get_by_slot_or_error(&self, key: i32) -> tabbit::Result<&AnimationRecord> {
        self.by_slot
            .get(&key)
            .map(|position| &self.records[*position])
            .ok_or_else(|| tabbit::Error::RecordNotFound {
    ...
    pub fn contains_slot(&self, key: i32) -> bool {
    ...
    pub fn read(&mut self, filename: &Path) -> tabbit::Result<()> {
        ...
        let mut records = vec![AnimationRecord::default(); count];
        ...
        for column in &header.columns {
            ...
            match column.tag {
                1 => {
                    tabbit::check_column(column, "Animation.Index", tabbit::KIND_SCALAR, false, &[tabbit::ELEMENT_STRING])?;
                    let mut cursor = tabbit::TcbColumnCursor::new(&mut reader, column, header.row_count, "Animation.Index")?;
                    let mut at = 0usize;
                    while at < records.len() {
                        let (n, value) = cursor.next_same_string((records.len() - at) as i32)?;
                        for _ in 0..n {
                            records[at].index = value.clone();
                            at += 1;
                        }
                    }
                }
                2 => {
                    tabbit::check_column(column, "Animation.Slot", tabbit::KIND_SCALAR, false, &[tabbit::ELEMENT_I32, tabbit::ELEMENT_VARINT])?;
                    let mut cursor = tabbit::TcbColumnCursor::new(&mut reader, column, header.row_count, "Animation.Slot")?;
                    let mut at = 0usize;
                    while at < records.len() {
                        let (n, value) = cursor.next_same_i32((records.len() - at) as i32)?;
                        for _ in 0..n {
                            records[at].slot = value;
                            at += 1;
                        }
                    }
                }
                3 => {
                    tabbit::check_column(column, "Animation.Blend", tabbit::KIND_SCALAR, false, &[tabbit::ELEMENT_F32])?;
                    let mut cursor = tabbit::TcbColumnCursor::new(&mut reader, column, header.row_count, "Animation.Blend")?;
...
impl<'a> IntoIterator for &'a AnimationTable {
    type Item = &'a AnimationRecord;
    type IntoIter = std::slice::Iter<'a, AnimationRecord>;
