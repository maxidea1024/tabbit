...
impl ServerTuningRecord {
}

/// Every row of ServerTuning.
#[derive(Clone, Debug, Default)]
pub struct ServerTuningTable {
    records: Vec<ServerTuningRecord>,
    by_index: HashMap<i32, usize>,
}

impl ServerTuningTable {
    /// Every row, in the order the sheet declared them.
    pub fn records(&self) -> &[ServerTuningRecord] {
        &self.records
    }
    ...
    pub fn iter(&self) -> std::slice::Iter<'_, ServerTuningRecord> {
        self.records.iter()
    }
    ...
    pub fn find_by_index(&self, key: i32) -> Option<&ServerTuningRecord> {
    ...
    /// error it returns.
    pub fn get_by_index_or_error(&self, key: i32) -> tabbit::Result<&ServerTuningRecord> {
        self.by_index
            .get(&key)
    ...
    pub fn entries(&self) -> impl Iterator<Item = (&i32, &ServerTuningRecord)> + '_ {
    ...
    pub fn read(&mut self, filename: &Path) -> tabbit::Result<()> {
        ...
        let mut records = vec![ServerTuningRecord::default(); count];
        ...
        for column in &header.columns {
            ...
            match column.tag {
                1 => {
                    tabbit::check_column(column, "ServerTuning.Index", tabbit::KIND_SCALAR, false, &[tabbit::ELEMENT_I32, tabbit::ELEMENT_VARINT])?;
                    let mut cursor = tabbit::TcbColumnCursor::new(&mut reader, column, header.row_count, "ServerTuning.Index")?;
                    let mut at = 0usize;
                ...
                2 => {
                    tabbit::check_column(column, "ServerTuning.Knob", tabbit::KIND_SCALAR, false, &[tabbit::ELEMENT_STRING])?;
                    let mut cursor = tabbit::TcbColumnCursor::new(&mut reader, column, header.row_count, "ServerTuning.Knob")?;
                    let mut at = 0usize;
                    while at < records.len() {
                        let (n, value) = cursor.next_same_string((records.len() - at) as i32)?;
                        for _ in 0..n {
                            records[at].knob = value.clone();
                            at += 1;
                        }
                    }
                }
                3 => {
                    tabbit::check_column(column, "ServerTuning.Value", tabbit::KIND_SCALAR, false, &[tabbit::ELEMENT_F32])?;
                    let mut cursor = tabbit::TcbColumnCursor::new(&mut reader, column, header.row_count, "ServerTuning.Value")?;
                    for record in records.iter_mut() {
                        record.value = cursor.next_f32()?;
                    }
                }
                _ => {
...
impl<'a> IntoIterator for &'a ServerTuningTable {
    type Item = &'a ServerTuningRecord;
    type IntoIter = std::slice::Iter<'a, ServerTuningRecord>;
