...
class AnimationTable extends Iterable<AnimationRecord> {
  ...
  List<AnimationRecord> get records => _records;
  List<AnimationRecord> _records = [];
  ...
  @override
  Iterator<AnimationRecord> get iterator => _records.iterator;

  Map<String, AnimationRecord> _byIndex = {};
  Map<int, AnimationRecord> _bySlot = {};

  /// The row with this Index, or null when the table has none.
  ///
  /// The lookup to reach for when a missing row is an ordinary answer - an optional
  /// reference, a key that came from user input.
  AnimationRecord? findByIndex(String key) => _byIndex[key];
  ...
  AnimationRecord getByIndexOrThrow(String key) {
  ...
  bool containsIndex(String key) => _byIndex.containsKey(key);
  ...
  AnimationRecord? operator [](String key) => findByIndex(key);
  ...
  Iterable<MapEntry<String, AnimationRecord>> get entries =>
  ...
  /// The lookup to reach for when a missing row is an ordinary answer - an optional
  /// reference, a key that came from user input.
  AnimationRecord? findBySlot(int key) => _bySlot[key];
  ...
  /// cannot otherwise tell whether the next line is a null check or a catch.
  AnimationRecord getBySlotOrThrow(int key) {
  ...
  bool containsSlot(int key) => _bySlot.containsKey(key);
  ...
  void read(String filename) {
    ...
    final loaded = <AnimationRecord>[];
    final loadedByIndex = <String, AnimationRecord>{};
    final loadedBySlot = <int, AnimationRecord>{};

    for (var i = 0; i < count; i++) {
      loaded.add(AnimationRecord());
    }
    ...
    for (final column in header.columns) {
      ...
      switch (column.tag) {
        case 1:
          checkColumn(column, 'Animation.Index', kindScalar, false, [elementString]);
          cursor = TcbColumnCursor(reader, column, count, 'Animation.Index');
          for (var i = 0; i < count; ) {
          ...
          break;
        case 2:
          checkColumn(column, 'Animation.Slot', kindScalar, false, [elementI32, elementVarint]);
          cursor = TcbColumnCursor(reader, column, count, 'Animation.Slot');
          for (var i = 0; i < count; ) {
          ...
          break;
        case 3:
          checkColumn(column, 'Animation.Blend', kindScalar, false, [elementF32]);
          cursor = TcbColumnCursor(reader, column, count, 'Animation.Blend');
