...
/// Every row of Line.
class LineTable extends Iterable<LineRecord> {
  ...
  List<LineRecord> get records => _records;
  List<LineRecord> _records = [];
  ...
  @override
  Iterator<LineRecord> get iterator => _records.iterator;

  Map<int, LineRecord> _byIndex = {};

  /// The row with this Index, or null when the table has none.
  ///
  /// The lookup to reach for when a missing row is an ordinary answer - an optional
  /// reference, a key that came from user input.
  LineRecord? findByIndex(int key) => _byIndex[key];
  ...
  LineRecord getByIndexOrThrow(int key) {
  ...
  LineRecord? operator [](int key) => findByIndex(key);
  ...
  Iterable<MapEntry<int, LineRecord>> get entries =>
  ...
  void read(String filename) {
    ...
    final loaded = <LineRecord>[];
    final loadedByIndex = <int, LineRecord>{};

    for (var i = 0; i < count; i++) {
      loaded.add(LineRecord());
    }
    ...
    for (final column in header.columns) {
      ...
      switch (column.tag) {
        case 1:
          checkColumn(column, 'Line.Index', kindScalar, false, [elementI32, elementVarint]);
          cursor = TcbColumnCursor(reader, column, count, 'Line.Index');
          for (var i = 0; i < count; ) {
          ...
          break;
        case 2:
          checkColumn(column, 'Line.Body', kindScalar, false, [elementString]);
          cursor = TcbColumnCursor(reader, column, count, 'Line.Body');
          for (var i = 0; i < count; ) {
          ...
          break;
        case 3:
          checkColumn(column, 'Line.Icon', kindScalar, false, [elementString]);
          cursor = TcbColumnCursor(reader, column, count, 'Line.Icon');
          for (var i = 0; i < count; ) {
            final (n, value) = cursor.nextSameString(count - i);
            for (var left = n; left > 0; left--, i++) {
              loaded[i].icon = value;
