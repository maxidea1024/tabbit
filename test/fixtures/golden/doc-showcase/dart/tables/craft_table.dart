...
/// Every row of Craft.
class CraftTable extends Iterable<CraftRecord> {
  ...
  List<CraftRecord> get records => _records;
  List<CraftRecord> _records = [];
  ...
  @override
  Iterator<CraftRecord> get iterator => _records.iterator;

  Map<int, CraftRecord> _byIndex = {};

  /// The row with this Index, or null when the table has none.
  ///
  /// The lookup to reach for when a missing row is an ordinary answer - an optional
  /// reference, a key that came from user input.
  CraftRecord? findByIndex(int key) => _byIndex[key];
  ...
  CraftRecord getByIndexOrThrow(int key) {
  ...
  CraftRecord? operator [](int key) => findByIndex(key);
  ...
  Iterable<MapEntry<int, CraftRecord>> get entries =>
  ...
  void read(String filename) {
    ...
    final loaded = <CraftRecord>[];
    final loadedByIndex = <int, CraftRecord>{};

    for (var i = 0; i < count; i++) {
      loaded.add(CraftRecord());
    }
    ...
    for (final column in header.columns) {
      ...
      switch (column.tag) {
        case 1:
          checkColumn(column, 'Craft.Index', kindScalar, false, [elementI32, elementVarint]);
          cursor = TcbColumnCursor(reader, column, count, 'Craft.Index');
          for (var i = 0; i < count; ) {
          ...
          break;
        case 2:
          checkColumn(column, 'Craft.Result', kindScalar, false, [elementI32]);
          cursor = TcbColumnCursor(reader, column, count, 'Craft.Result');
          for (var i = 0; i < count; ) {
            final (n, value) = cursor.nextSameI32(count - i);
            for (var left = n; left > 0; left--, i++) {
          ...
          break;
        case 3:
          checkColumn(column, 'Craft.ResultName', kindScalar, false, [elementI32]);
          cursor = TcbColumnCursor(reader, column, count, 'Craft.ResultName');
          for (var i = 0; i < count; ) {
            final (n, value) = cursor.nextSameI32(count - i);
            for (var left = n; left > 0; left--, i++) {
              loaded[i].resultNameIndex = value;
            }
          }
          break;
        case 4:
          checkColumn(column, 'Craft.Parts', kindArray, false, [elementI32]);
          cursor = TcbColumnCursor(reader, column, count, 'Craft.Parts');
          for (final record in loaded) {
            final elementCount = cursor.nextLength();
            record.parts = List.generate(elementCount, (_) => cursor.nextI32());
            record.potionByParts = List.filled(elementCount, null);
          }
          break;
        case 5:
          checkColumn(column, 'Craft.Substitute', kindScalar, true, [elementI32]);
          ...
          cursor = TcbColumnCursor(reader, column, count, 'Craft.Substitute');
          for (var i = 0; i < count; ) {
            final (n, value) = cursor.nextSameI32(count - i);
            for (var left = n; left > 0; left--, i++) {
              loaded[i].substitute = value;
            }
          }
