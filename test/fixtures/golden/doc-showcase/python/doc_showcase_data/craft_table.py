    ...
    def read(self, filename):
        ...
        for column in columns:
            ...
            if column.tag == 1:
                ...
                cursor = tabbit.ColumnCursor(reader, column, count, "Craft.Result")
                at = 0
                while at < count:
                    n, value = cursor.next_same_i32(count - at)
                    for i in range(at, at + n):
                ...
                cursor = tabbit.ColumnCursor(reader, column, count, "Craft.ResultName")
                at = 0
                while at < count:
                    n, value = cursor.next_same_i32(count - at)
                    for i in range(at, at + n):
                ...
                cursor = tabbit.ColumnCursor(reader, column, count, "Craft.Parts")
                for record in records:
                    element_count = cursor.next_length()
                    record.parts = [cursor.next_i32() for _ in range(element_count)]
                    record.potion_by_parts = [None] * element_count
            elif column.tag == 5:
                ...
                cursor = tabbit.ColumnCursor(reader, column, count, "Craft.Substitute")
                at = 0
                while at < count:
                    n, value = cursor.next_same_i32(count - at)
                    for i in range(at, at + n):
                ...
                for row, record in enumerate(records):
                    ...
                    if not record.has_substitute:
                        record.substitute = None
            else:
