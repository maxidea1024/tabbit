    ...
    def read(self, filename):
        ...
        for column in columns:
            ...
            if column.tag == 1:
                ...
                cursor = tabbit.ColumnCursor(reader, column, count, "Drop.Grades")
                for record in records:
                    element_count = cursor.next_length()
                    record.grades = [Rarity(cursor.next_i32()) for _ in range(element_count)]
            elif column.tag == 3:
                tabbit.check_column(column, "Drop.Counts", tabbit.KIND_ARRAY, False, (tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT), True)
                # Behind the row bitmap and in front of the values, walked with a counter
                # that steps once per element of every row.
                # spec/types/nullable-array-elements.md.
                element_presence = tabbit.read_element_presence(reader, column)
                element_at = 0
                cursor = tabbit.ColumnCursor(reader, column, count, "Drop.Counts")
                for record in records:
                    element_count = cursor.next_length()
                    record.counts = [cursor.next_i32() for _ in range(element_count)]
