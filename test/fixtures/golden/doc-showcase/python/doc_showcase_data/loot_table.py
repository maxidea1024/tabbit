    ...
    def read(self, filename):
        ...
        for column in columns:
            ...
            if column.tag == 1:
                ...
                cursor = tabbit.ColumnCursor(reader, column, count, "Loot.Tags")
                for record in records:
                    element_count = cursor.next_length()
                    record.tags = [cursor.next_string() for _ in range(element_count)]
            elif column.tag == 3:
                tabbit.check_column(column, "Loot.Weight", tabbit.KIND_ARRAY, False, (tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT))
                cursor = tabbit.ColumnCursor(reader, column, count, "Loot.Weight")
                for record in records:
                    element_count = cursor.next_length()
                    record.weight = [cursor.next_i32() for _ in range(element_count)]
            else:
