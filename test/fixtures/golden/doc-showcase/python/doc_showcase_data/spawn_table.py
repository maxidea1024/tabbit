...
class SpawnAtEntry:
    ...
    def __repr__(self):
        return "SpawnAtEntry(x=%r, y=%r)" % (self.x, self.y)
    ...
    def read(self, filename):
        ...
        for column in columns:
            ...
            if column.tag == 1:
                ...
                cursor = tabbit.ColumnCursor(reader, column, count, "Spawn.At.X")
                for record in records:
                    record.at.x = cursor.next_f32()
            elif column.tag == 3:
                tabbit.check_column(column, "Spawn.At.Y", tabbit.KIND_SCALAR, False, (tabbit.ELEMENT_F32,))
                cursor = tabbit.ColumnCursor(reader, column, count, "Spawn.At.Y")
                for record in records:
                ...
                cursor = tabbit.ColumnCursor(reader, column, count, "Spawn.Radius")
                for record in records:
