...
static bool DocShowcase_ShopParse(DocShowcase_ShopTable_t* table, tb_reader* reader) {
  int32_t row;
  int32_t at;
  ...
  if (table->count > 0) {
    table->records = (DocShowcase_ShopRecord_t*)tb_arena_alloc(
  ...
  for (row = 0; row < table->count; ++row) {
    DocShowcase_ShopRecord_t* record = &table->records[row];
  ...
  for (at = 0; at < column_count && !tb_failed(reader); ++at) {
    ...
    case 1:
      (void)tb_check_column(reader, column, "Shop.Index", TB_KIND_SCALAR, false, TB_ELEMENT_MASK(TB_ELEMENT_I32) | TB_ELEMENT_MASK(TB_ELEMENT_VARINT));

      (void)tb_cursor_init(&cursor, reader, column, table->count, "Shop.Index");
    ...
    case 2:
      (void)tb_check_column(reader, column, "Shop.Name", TB_KIND_SCALAR, false, TB_ELEMENT_MASK(TB_ELEMENT_STRING));

      (void)tb_cursor_init(&cursor, reader, column, table->count, "Shop.Name");
...
bool DocShowcase_ShopLoad(DocShowcase_ShopTable_t* table, const char* filename,
  ...
  DocShowcase_ShopTable_t loaded;
...
void DocShowcase_ShopFree(DocShowcase_ShopTable_t* table) {
  tb_arena_free(&table->arena);

  memset(table, 0, sizeof *table);
}

const DocShowcase_ShopRecord_t* DocShowcase_ShopFindByIndex(
  const DocShowcase_ShopTable_t* table, int32_t key) {
  int32_t position = tb_index_find(table->by_index, table->count, key);

  return position < 0 ? NULL : &table->records[position];
}

bool DocShowcase_ShopContainsIndex(
  const DocShowcase_ShopTable_t* table, int32_t key) {
