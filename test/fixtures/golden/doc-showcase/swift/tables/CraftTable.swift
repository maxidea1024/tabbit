...
/// Every row of Craft.
public final class CraftTable: Sequence {
    ...
    public func makeIterator() -> Array<CraftRecord>.Iterator {
    ...
    public private(set) var records: [CraftRecord] = []

    private var byIndex: [Int32: CraftRecord] = [:]

    /// The row with this Index, or nil when the table has none.
    ///
    /// The lookup to reach for when a missing row is an ordinary answer - an optional
    /// reference, a key that came from user input.
    public func findByIndex(_ key: Int32) -> CraftRecord? {
        byIndex[key]
    }
    ...
    public func getByIndexOrThrow(_ key: Int32) throws -> CraftRecord {
    ...
    public subscript(_ key: Int32) -> CraftRecord? {
        findByIndex(key)
    }
    ...
    /// row with.
    public var entries: LazyMapSequence<[CraftRecord], (key: Int32, row: CraftRecord)> {
    ...
    public func read(
        ...
        var loaded = [CraftRecord]()
        loaded.reserveCapacity(count)

        for _ in 0 ..< count { loaded.append(CraftRecord()) }
        var loadedByIndex = [Int32: CraftRecord](
        ...
        for column in header.columns {
            ...
            switch column.tag {
            case 1:
                try Tcb.checkColumn(column, "Craft.Index", Tcb.kindScalar, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Craft.Index")
                var at = 0
                while at < count {
            ...
            case 2:
                try Tcb.checkColumn(column, "Craft.Result", Tcb.kindScalar, false, Tcb.elementI32)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Craft.Result")
                var at = 0
                while at < count {
                    var n = try cursor.nextSameI32(count - at)
                    while n > 0 {
            ...
            case 3:
                try Tcb.checkColumn(column, "Craft.ResultName", Tcb.kindScalar, false, Tcb.elementI32)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Craft.ResultName")
                var at = 0
                while at < count {
                    var n = try cursor.nextSameI32(count - at)
                    while n > 0 {
            ...
            case 4:
                try Tcb.checkColumn(column, "Craft.Parts", Tcb.kindArray, false, Tcb.elementI32)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Craft.Parts")
                for record in loaded {
                    let elementCount = Swift.max(0, try cursor.nextLength())
                    record.parts = []
                    record.parts.reserveCapacity(elementCount)

                    for _ in 0 ..< elementCount {
                        record.parts.append(try cursor.nextI32())
                    }
                }
            case 5:
                try Tcb.checkColumn(column, "Craft.Substitute", Tcb.kindScalar, true, Tcb.elementI32)
                ...
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Craft.Substitute")
                var at = 0
                while at < count {
                    var n = try cursor.nextSameI32(count - at)
                    while n > 0 {
                ...
                for row in 0 ..< count {
                    ...
                    record.hasSubstitute = Tcb.isPresent(presence, row)
