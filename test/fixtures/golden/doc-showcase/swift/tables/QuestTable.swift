...
public final class QuestTable: Sequence {
    ...
    private var byIndex: [Int32: QuestRecord] = [:]

    /// The row with this Index, or nil when the table has none.
    ///
    /// The lookup to reach for when a missing row is an ordinary answer - an optional
    /// reference, a key that came from user input.
    public func findByIndex(_ key: Int32) -> QuestRecord? {
        byIndex[key]
    }
    ...
    public func getByIndexOrThrow(_ key: Int32) throws -> QuestRecord {
    ...
    public func read(
        ...
        var loadedByIndex = [Int32: QuestRecord](
        ...
        for column in header.columns {
            ...
            switch column.tag {
            case 1:
                try Tcb.checkColumn(column, "Quest.Index", Tcb.kindScalar, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Quest.Index")
                var at = 0
                while at < count {
            ...
            case 2:
                try Tcb.checkColumn(column, "Quest.Name", Tcb.kindScalar, false, Tcb.elementString)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Quest.Name")
                var at = 0
                while at < count {
            ...
            case 3:
                try Tcb.checkColumn(column, "Quest.Reward.Id", Tcb.kindArray, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Quest.Reward.Id")
                for record in loaded {
                    ...
                    record.reward = [QuestRecord.RewardEntry](
                        repeating: QuestRecord.RewardEntry(), count: elementCount)

                    for element in 0 ..< elementCount {
                        record.reward[element].id = try cursor.nextI32()
                    }
                }
            case 4:
                try Tcb.checkColumn(column, "Quest.Reward.Count", Tcb.kindArray, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Quest.Reward.Count")
                for record in loaded {
                    let elementCount = Swift.max(0, try cursor.nextLength())
                    if record.reward.count != elementCount {
                        throw TcbError(
                            "Quest.reward: the file gives one "
                            + "member of this record a different element count than another")
                    }

                    for element in 0 ..< elementCount {
                        record.reward[element].count = try cursor.nextI32()
                    }
                }
            default:
