...
public final class ShopEntryTable: Sequence {
    ...
    public func makeIterator() -> Array<ShopEntryRecord>.Iterator {
    ...
    public private(set) var records: [ShopEntryRecord] = []

    private var byIndex: [Int32: ShopEntryRecord] = [:]

    /// The row with this Index, or nil when the table has none.
    ///
    /// The lookup to reach for when a missing row is an ordinary answer - an optional
    /// reference, a key that came from user input.
    public func findByIndex(_ key: Int32) -> ShopEntryRecord? {
        byIndex[key]
    }
    ...
    public func getByIndexOrThrow(_ key: Int32) throws -> ShopEntryRecord {
        guard let record = byIndex[key] else {
            throw RecordNotFoundError(
                "there is no record in table `ShopEntry` that corresponds to "
    ...
    public subscript(_ key: Int32) -> ShopEntryRecord? {
        findByIndex(key)
    }
    ...
    /// row with.
    public var entries: LazyMapSequence<[ShopEntryRecord], (key: Int32, row: ShopEntryRecord)> {
    ...
    public func read(
        ...
        var loaded = [ShopEntryRecord]()
        loaded.reserveCapacity(count)

        for _ in 0 ..< count { loaded.append(ShopEntryRecord()) }
        var loadedByIndex = [Int32: ShopEntryRecord](
        ...
        for column in header.columns {
            ...
            switch column.tag {
            case 1:
                try Tcb.checkColumn(column, "ShopEntry.Index", Tcb.kindScalar, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "ShopEntry.Index")
                var at = 0
                while at < count {
            ...
            case 2:
                try Tcb.checkColumn(column, "ShopEntry.ShopId", Tcb.kindScalar, false, Tcb.elementI32)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "ShopEntry.ShopId")
                var at = 0
                while at < count {
                    var n = try cursor.nextSameI32(count - at)
                    while n > 0 {
            ...
            case 3:
                try Tcb.checkColumn(column, "ShopEntry.PotionId", Tcb.kindScalar, false, Tcb.elementI32)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "ShopEntry.PotionId")
                var at = 0
                while at < count {
                    var n = try cursor.nextSameI32(count - at)
                    while n > 0 {
                        let i = at
                        loaded[i].potionId = cursor.runSameValue
                        at += 1
                        n -= 1
                    }
                }
            case 4:
                try Tcb.checkColumn(column, "ShopEntry.Stock", Tcb.kindScalar, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "ShopEntry.Stock")
                var at = 0
                while at < count {
                    var n = try cursor.nextSameI32(count - at)
                    while n > 0 {
                        let i = at
                        loaded[i].stock = cursor.runSameValue
                        at += 1
                        n -= 1
                    }
                }
