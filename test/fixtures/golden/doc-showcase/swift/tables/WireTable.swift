...
/// Every row of Wire.
public final class WireTable: Sequence {
    ...
    public func makeIterator() -> Array<WireRecord>.Iterator {
    ...
    public private(set) var records: [WireRecord] = []

    private var byIndex: [Int32: WireRecord] = [:]

    /// The row with this Index, or nil when the table has none.
    ///
    /// The lookup to reach for when a missing row is an ordinary answer - an optional
    /// reference, a key that came from user input.
    public func findByIndex(_ key: Int32) -> WireRecord? {
        byIndex[key]
    }
    ...
    public func getByIndexOrThrow(_ key: Int32) throws -> WireRecord {
    ...
    public subscript(_ key: Int32) -> WireRecord? {
        findByIndex(key)
    }
    ...
    /// row with.
    public var entries: LazyMapSequence<[WireRecord], (key: Int32, row: WireRecord)> {
    ...
    public func read(
        ...
        var loaded = [WireRecord]()
        loaded.reserveCapacity(count)

        for _ in 0 ..< count { loaded.append(WireRecord()) }
        var loadedByIndex = [Int32: WireRecord](
        ...
        for column in header.columns {
            ...
            switch column.tag {
            case 1:
                try Tcb.checkColumn(column, "Wire.Index", Tcb.kindScalar, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Wire.Index")
                var at = 0
                while at < count {
            ...
            case 2:
                try Tcb.checkColumn(column, "Wire.Name", Tcb.kindScalar, false, Tcb.elementString)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Wire.Name")
                var at = 0
                while at < count {
            ...
            case 4:
                try Tcb.checkColumn(column, "Wire.Price", Tcb.kindScalar, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Wire.Price")
                var at = 0
                while at < count {
                    var n = try cursor.nextSameI32(count - at)
                    while n > 0 {
