...
/// Every row of Deck.
public final class DeckTable: Sequence {
    ...
    public func makeIterator() -> Array<DeckRecord>.Iterator {
    ...
    public private(set) var records: [DeckRecord] = []

    private var byIndex: [Int32: DeckRecord] = [:]

    /// The row with this Index, or nil when the table has none.
    ///
    /// The lookup to reach for when a missing row is an ordinary answer - an optional
    /// reference, a key that came from user input.
    public func findByIndex(_ key: Int32) -> DeckRecord? {
        byIndex[key]
    }
    ...
    public func getByIndexOrThrow(_ key: Int32) throws -> DeckRecord {
        guard let record = byIndex[key] else {
            throw RecordNotFoundError(
                "there is no record in table `Deck` that corresponds to "
    ...
    public subscript(_ key: Int32) -> DeckRecord? {
        findByIndex(key)
    }
    ...
    /// row with.
    public var entries: LazyMapSequence<[DeckRecord], (key: Int32, row: DeckRecord)> {
    ...
    public func read(
        ...
        var loaded = [DeckRecord]()
        loaded.reserveCapacity(count)

        for _ in 0 ..< count { loaded.append(DeckRecord()) }
        var loadedByIndex = [Int32: DeckRecord](
        ...
        for column in header.columns {
            ...
            switch column.tag {
            case 1:
                try Tcb.checkColumn(column, "Deck.Index", Tcb.kindScalar, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Deck.Index")
                var at = 0
                while at < count {
            ...
            case 2:
                try Tcb.checkColumn(column, "Deck.Slot.Id", Tcb.kindArray, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Deck.Slot.Id")
                for record in loaded {
                    ...
                    record.slot = [DeckRecord.SlotEntry](
                        repeating: DeckRecord.SlotEntry(), count: elementCount)
            ...
            case 3:
                try Tcb.checkColumn(column, "Deck.Slot.Label", Tcb.kindArray, false, Tcb.elementString)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Deck.Slot.Label")
                for record in loaded {
                    let elementCount = Swift.max(0, try cursor.nextLength())
                    if record.slot.count != elementCount {
                        throw TcbError(
                            "Deck.slot: the file gives one "
            ...
            case 4:
                try Tcb.checkColumn(column, "Deck.Home.At.X", Tcb.kindScalar, false, Tcb.elementF32)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Deck.Home.At.X")
                for record in loaded {
                    record.home.at.x = try cursor.nextF32()
                }
            case 5:
                try Tcb.checkColumn(column, "Deck.Home.At.Y", Tcb.kindScalar, false, Tcb.elementF32)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Deck.Home.At.Y")
                for record in loaded {
                    record.home.at.y = try cursor.nextF32()
                }
            default:
