...
/// Every row of Loot.
public final class LootTable: Sequence {
    ...
    public func makeIterator() -> Array<LootRecord>.Iterator {
    ...
    public private(set) var records: [LootRecord] = []

    private var byIndex: [Int32: LootRecord] = [:]

    /// The row with this Index, or nil when the table has none.
    ///
    /// The lookup to reach for when a missing row is an ordinary answer - an optional
    /// reference, a key that came from user input.
    public func findByIndex(_ key: Int32) -> LootRecord? {
        byIndex[key]
    }
    ...
    public func getByIndexOrThrow(_ key: Int32) throws -> LootRecord {
    ...
    public subscript(_ key: Int32) -> LootRecord? {
        findByIndex(key)
    }
    ...
    /// row with.
    public var entries: LazyMapSequence<[LootRecord], (key: Int32, row: LootRecord)> {
    ...
    public func read(
        ...
        var loaded = [LootRecord]()
        loaded.reserveCapacity(count)

        for _ in 0 ..< count { loaded.append(LootRecord()) }
        var loadedByIndex = [Int32: LootRecord](
        ...
        for column in header.columns {
            ...
            case 2:
                try Tcb.checkColumn(column, "Loot.Tags", Tcb.kindArray, false, Tcb.elementString)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Loot.Tags")
                for record in loaded {
                    let elementCount = Swift.max(0, try cursor.nextLength())
                    record.tags = []
                    record.tags.reserveCapacity(elementCount)

                    for _ in 0 ..< elementCount {
                        record.tags.append(try cursor.nextString())
                    }
                }
            case 3:
                try Tcb.checkColumn(column, "Loot.Weight", Tcb.kindArray, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Loot.Weight")
                for record in loaded {
                    let elementCount = Swift.max(0, try cursor.nextLength())
                    record.weight = []
                    record.weight.reserveCapacity(elementCount)

                    for _ in 0 ..< elementCount {
                        record.weight.append(try cursor.nextI32())
                    }
                }
            default:
