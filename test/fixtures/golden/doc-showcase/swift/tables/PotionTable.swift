...
/// Every row of Potion.
public final class PotionTable: Sequence {
    ...
    public func makeIterator() -> Array<PotionRecord>.Iterator {
    ...
    public private(set) var records: [PotionRecord] = []

    private var byIndex: [Int32: PotionRecord] = [:]

    /// The row with this Index, or nil when the table has none.
    ///
    /// The lookup to reach for when a missing row is an ordinary answer - an optional
    /// reference, a key that came from user input.
    public func findByIndex(_ key: Int32) -> PotionRecord? {
        byIndex[key]
    }
    ...
    public func getByIndexOrThrow(_ key: Int32) throws -> PotionRecord {
    ...
    public subscript(_ key: Int32) -> PotionRecord? {
        findByIndex(key)
    }
    ...
    /// row with.
    public var entries: LazyMapSequence<[PotionRecord], (key: Int32, row: PotionRecord)> {
    ...
    public func read(
        ...
        var loaded = [PotionRecord]()
        loaded.reserveCapacity(count)

        for _ in 0 ..< count { loaded.append(PotionRecord()) }
        var loadedByIndex = [Int32: PotionRecord](
        ...
        for column in header.columns {
            ...
            switch column.tag {
            case 1:
                try Tcb.checkColumn(column, "Potion.Index", Tcb.kindScalar, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Potion.Index")
                var at = 0
                while at < count {
            ...
            case 2:
                try Tcb.checkColumn(column, "Potion.Name", Tcb.kindScalar, false, Tcb.elementString)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Potion.Name")
                var at = 0
                while at < count {
            ...
            case 3:
                try Tcb.checkColumn(column, "Potion.Rarity", Tcb.kindScalar, false, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Potion.Rarity")
                var at = 0
                while at < count {
                    var n = try cursor.nextSameI32(count - at)
                    while n > 0 {
                        let i = at
                        loaded[i].rarity = Rarity.of(cursor.runSameValue)
                        at += 1
                        n -= 1
                    }
                }
            case 4:
                try Tcb.checkColumn(column, "Potion.Price", Tcb.kindScalar, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Potion.Price")
                var at = 0
                while at < count {
                    var n = try cursor.nextSameI32(count - at)
                    while n > 0 {
