...
/// Every row of Access.
public final class AccessTable: Sequence {
    ...
    public func makeIterator() -> Array<AccessRecord>.Iterator {
    ...
    public private(set) var records: [AccessRecord] = []

    private var byIndex: [Int32: AccessRecord] = [:]

    /// The row with this Index, or nil when the table has none.
    ///
    /// The lookup to reach for when a missing row is an ordinary answer - an optional
    /// reference, a key that came from user input.
    public func findByIndex(_ key: Int32) -> AccessRecord? {
        byIndex[key]
    }
    ...
    public func getByIndexOrThrow(_ key: Int32) throws -> AccessRecord {
        guard let record = byIndex[key] else {
            throw RecordNotFoundError(
                "there is no record in table `Access` that corresponds to "
    ...
    public subscript(_ key: Int32) -> AccessRecord? {
        findByIndex(key)
    }
    ...
    /// row with.
    public var entries: LazyMapSequence<[AccessRecord], (key: Int32, row: AccessRecord)> {
    ...
    public func read(
        ...
        var loaded = [AccessRecord]()
        loaded.reserveCapacity(count)

        for _ in 0 ..< count { loaded.append(AccessRecord()) }
        var loadedByIndex = [Int32: AccessRecord](
        ...
        for column in header.columns {
            ...
            switch column.tag {
            case 1:
                try Tcb.checkColumn(column, "Access.Index", Tcb.kindScalar, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Access.Index")
                var at = 0
                while at < count {
            ...
            case 2:
                try Tcb.checkColumn(column, "Access.Flags", Tcb.kindScalar, false, Tcb.elementI64, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Access.Flags")
                for record in loaded {
                    record.flags = try cursor.nextI64()
                }
            case 3:
                try Tcb.checkColumn(column, "Access.Same", Tcb.kindScalar, false, Tcb.elementI64, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Access.Same")
                for record in loaded {
                    record.same = try cursor.nextI64()
                }
            default:
