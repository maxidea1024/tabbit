...
/// Every row of Line.
public final class LineTable: Sequence {
    ...
    public func makeIterator() -> Array<LineRecord>.Iterator {
    ...
    public private(set) var records: [LineRecord] = []

    private var byIndex: [Int32: LineRecord] = [:]

    /// The row with this Index, or nil when the table has none.
    ///
    /// The lookup to reach for when a missing row is an ordinary answer - an optional
    /// reference, a key that came from user input.
    public func findByIndex(_ key: Int32) -> LineRecord? {
        byIndex[key]
    }
    ...
    public func getByIndexOrThrow(_ key: Int32) throws -> LineRecord {
    ...
    public subscript(_ key: Int32) -> LineRecord? {
        findByIndex(key)
    }
    ...
    /// row with.
    public var entries: LazyMapSequence<[LineRecord], (key: Int32, row: LineRecord)> {
    ...
    public func read(
        ...
        var loaded = [LineRecord]()
        loaded.reserveCapacity(count)

        for _ in 0 ..< count { loaded.append(LineRecord()) }
        var loadedByIndex = [Int32: LineRecord](
        ...
        for column in header.columns {
            ...
            switch column.tag {
            case 1:
                try Tcb.checkColumn(column, "Line.Index", Tcb.kindScalar, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Line.Index")
                var at = 0
                while at < count {
            ...
            case 2:
                try Tcb.checkColumn(column, "Line.Body", Tcb.kindScalar, false, Tcb.elementString)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Line.Body")
                var at = 0
                while at < count {
                    var n = try cursor.nextSameString(count - at)
                    while n > 0 {
                        let i = at
                        loaded[i].body = cursor.runSameText
                        at += 1
                        n -= 1
                    }
                }
            case 3:
                try Tcb.checkColumn(column, "Line.Icon", Tcb.kindScalar, false, Tcb.elementString)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Line.Icon")
                var at = 0
                while at < count {
                    var n = try cursor.nextSameString(count - at)
                    while n > 0 {
