...
namespace Tabbit.Fixtures.DocShowcase
{
    ...
    public partial class DropTable : IEnumerable<DropRecord>
    {
        ...
        /// Field names.
        /// </summary>
        public static readonly string[] FieldNames = { "Index", "Grades", "Counts" };
        ...
        public List<object[]> BuildObjectValueMap()
        {
            ...
            foreach (var r in _records)
                result.Add(new object[] { r.Index, r.Grades, r.Counts });
        ...
        public Task ReadAsync(TcbReader reader)
        {
            ...
            foreach (var column in columns)
            {
                ...
                switch (column.Tag)
                {
                    ...
                    case 2:
                        TcbTable.CheckColumn(column, "Drop.Grades", TcbTable.KindArray, false, TcbTable.ElementVarint);
                        cursor = new TcbColumnCursor(reader, column, count, "Drop.Grades");
                        for (int i = 0; i < count; i++)
                        {
                            var record = records[i];
                            int elementCount;
                            elementCount = cursor.NextLength();
                            record._grades = new global::Tabbit.Fixtures.DocShowcase.Rarity[elementCount];
                            for (int j = 0; j < elementCount; ++j)
                            {
                                record._grades[j] = (global::Tabbit.Fixtures.DocShowcase.Rarity)cursor.NextI32();
                            }
                        }
                        break;

                    case 3:
                        TcbTable.CheckColumn(column, "Drop.Counts", TcbTable.KindArray, false, TcbTable.ElementI32, TcbTable.ElementVarint, elementNullable: true);
                        elementPresence = TcbTable.ReadElementPresence(reader, column);
                        elementAt = 0;
                        cursor = new TcbColumnCursor(reader, column, count, "Drop.Counts");
                        for (int i = 0; i < count; i++)
                        {
                            var record = records[i];
                            int elementCount;
                            elementCount = cursor.NextLength();
                            record._counts = new int[elementCount];
                            record._countsHasValueAt = new bool[elementCount];
                            for (int j = 0; j < elementCount; ++j)
                            {
                                record._countsHasValueAt[j] =
                                    TcbTable.IsPresent(elementPresence, elementAt++);
                                record._counts[j] = cursor.NextI32();
                            }
                        }
                        break;
