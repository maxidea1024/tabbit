...
namespace Tabbit.Fixtures.DocShowcase
{
    ...
    /// </summary>
    [System.Serializable]
    public partial class ShopEntryTable : IEnumerable<ShopEntryRecord>
    {
        /// <summary>
        /// Field names.
        /// </summary>
        public static readonly string[] FieldNames = { "Index", "ShopId", "PotionId", "Stock" };
        ...
        public List<object[]> BuildObjectValueMap()
        {
            ...
            foreach (var r in _records)
                result.Add(new object[] { r.Index, r.ShopId, r.PotionId, r.Stock });
        ...
        /// </remarks>
        public List<ShopEntryRecord> Records => _records;
        private List<ShopEntryRecord> _records = new List<ShopEntryRecord>();
        ...
        /// </remarks>
        public List<ShopEntryRecord>.Enumerator GetEnumerator() => _records.GetEnumerator();

        IEnumerator<ShopEntryRecord> IEnumerable<ShopEntryRecord>.GetEnumerator() => _records.GetEnumerator();

        System.Collections.IEnumerator System.Collections.IEnumerable.GetEnumerator()
            => _records.GetEnumerator();

        #region Indexing by 'Index'
        public Dictionary<int, ShopEntryRecord> RecordsByIndex => _recordsByIndex;
        private Dictionary<int, ShopEntryRecord> _recordsByIndex = new Dictionary<int, ShopEntryRecord>();
        ...
        /// </remarks>
        public ShopEntryRecord FindByIndex(int key)
            => _recordsByIndex.TryGetValue(key, out ShopEntryRecord record) ? record : null;
        ...
        /// </remarks>
        public ShopEntryRecord GetByIndexOrThrow(int key)
        {
            if (!_recordsByIndex.TryGetValue(key, out ShopEntryRecord record))
                throw new TabbitException($"There is no record in table `ShopEntry` that corresponds to field `Index` value {key}");
        ...
        {
            private readonly List<ShopEntryRecord> _rows;
            private int _at;

            internal EntryEnumerator(List<ShopEntryRecord> rows)
            {
                _rows = rows;
                _at = -1;
            }
            ...
            public (int Key, ShopEntryRecord Row) Current
        ...
        /// </remarks>
        public ShopEntryRecord this[int key] => GetByIndexOrThrow(key);
        ...
        public Task ReadAsync(TcbReader reader)
        {
            ...
            var records = new List<ShopEntryRecord>(count);

            for (int i = 0; i < count; i++)
                records.Add(new ShopEntryRecord());
            ...
            foreach (var column in columns)
            {
                ...
                {
                    case 1:
                        TcbTable.CheckColumn(column, "ShopEntry.Index", TcbTable.KindScalar, false, TcbTable.ElementI32, TcbTable.ElementVarint);
                        cursor = new TcbColumnCursor(reader, column, count, "ShopEntry.Index");
                        for (int i = 0; i < count; )
                        {
                    ...
                    case 2:
                        TcbTable.CheckColumn(column, "ShopEntry.ShopId", TcbTable.KindScalar, false, TcbTable.ElementI32);
                        cursor = new TcbColumnCursor(reader, column, count, "ShopEntry.ShopId");
                        for (int i = 0; i < count; )
                        {
                            // One call per run of equal values, not one per row - over a
                            // run-length encoded column this is most of the decode.
                    ...
                    case 3:
                        TcbTable.CheckColumn(column, "ShopEntry.PotionId", TcbTable.KindScalar, false, TcbTable.ElementI32);
                        cursor = new TcbColumnCursor(reader, column, count, "ShopEntry.PotionId");
                        for (int i = 0; i < count; )
                        {
                            // One call per run of equal values, not one per row - over a
                            // run-length encoded column this is most of the decode.
                            ...
                            {
                                var record = records[i++];
                                record._potionId_Potion_index = value;
                                record._potionId = default(PotionRecord); // will be assigned.
                            } while (--n > 0);
                        }
                        break;

                    case 4:
                        TcbTable.CheckColumn(column, "ShopEntry.Stock", TcbTable.KindScalar, false, TcbTable.ElementI32, TcbTable.ElementVarint);
                        cursor = new TcbColumnCursor(reader, column, count, "ShopEntry.Stock");
                        for (int i = 0; i < count; )
                        {
                            // One call per run of equal values, not one per row - over a
                            // run-length encoded column this is most of the decode.
            ...
            var recordsByIndex = new Dictionary<int, ShopEntryRecord>(count);
