...
// ShopRecord was generated from test/fixtures/xlsx/doc-showcase/doc-showcase.xlsx : Shop : B2.
...
func (t *ShopTable) FindByIndex(key int32) *ShopRecord {
...
func (t *ShopTable) GetByIndexOrError(key int32) (*ShopRecord, error) {
	position, found := t.byIndex[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `Shop` that corresponds to field `Index` value %v", key)
	}
...
func (t *ShopTable) ContainsIndex(key int32) bool {
...
func (t *ShopTable) Read(filename string) error {
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "Shop.Index", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Shop.Index")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "Shop.Name", tabbit.KindScalar, false, tabbit.ElementString) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Shop.Name")
				for i := int32(0); i < count; {
