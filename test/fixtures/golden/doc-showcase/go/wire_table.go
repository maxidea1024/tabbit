...
// WireTable holds every row of Wire.
...
func (t *WireTable) Records() []WireRecord { return t.records }

// Count returns how many rows the table holds.
func (t *WireTable) Count() int { return len(t.records) }
...
func (t *WireTable) All() iter.Seq[*WireRecord] {
	records := t.records

	return func(yield func(*WireRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *WireTable) FindByIndex(key int32) *WireRecord {
...
func (t *WireTable) GetByIndexOrError(key int32) (*WireRecord, error) {
	position, found := t.byIndex[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `Wire` that corresponds to field `Index` value %v", key)
	}
...
func (t *WireTable) ContainsIndex(key int32) bool {
...
func (t *WireTable) Entries() iter.Seq2[int32, *WireRecord] {
	records := t.records

	return func(yield func(int32, *WireRecord) bool) {
		for i := range records {
...
func (t *WireTable) Read(filename string) error {
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "Wire.Index", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Wire.Index")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "Wire.Name", tabbit.KindScalar, false, tabbit.ElementString) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Wire.Name")
				for i := int32(0); i < count; {
		...
		case 4:
			if tabbit.CheckColumn(reader, column, "Wire.Price", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Wire.Price")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
