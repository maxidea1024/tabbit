...
// QuestRewardEntry is one element of QuestRecord.Reward.
...
func (t *QuestTable) FindByIndex(key int32) *QuestRecord {
...
func (t *QuestTable) GetByIndexOrError(key int32) (*QuestRecord, error) {
	position, found := t.byIndex[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `Quest` that corresponds to field `Index` value %v", key)
	}
...
func (t *QuestTable) ContainsIndex(key int32) bool {
...
func (t *QuestTable) Read(filename string) error {
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "Quest.Index", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Quest.Index")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "Quest.Name", tabbit.KindScalar, false, tabbit.ElementString) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Quest.Name")
				for i := int32(0); i < count; {
		...
		case 3:
			if tabbit.CheckColumn(reader, column, "Quest.Reward.Id", tabbit.KindArray, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Quest.Reward.Id")
				for i := int32(0); i < count; i++ {
					...
					r.Reward = make([]QuestRewardEntry, elementCount)
					for j := 0; j < elementCount; j++ {
						r.Reward[j].Id = cursor.NextI32()
					}
				}
			}
		case 4:
			if tabbit.CheckColumn(reader, column, "Quest.Reward.Count", tabbit.KindArray, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Quest.Reward.Count")
				for i := int32(0); i < count; i++ {
					r := &records[i]
					elementCount := int(cursor.NextLength())
					if len(r.Reward) != elementCount {
						return fmt.Errorf(
							"Quest: the file gives one member of `Reward` a different element count than another")
					}
					for j := 0; j < elementCount; j++ {
						r.Reward[j].Count = cursor.NextI32()
					}
				}
			}
		default:
