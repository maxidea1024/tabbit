...
// LineTable holds every row of Line.
...
func (t *LineTable) Records() []LineRecord { return t.records }

// Count returns how many rows the table holds.
func (t *LineTable) Count() int { return len(t.records) }
...
func (t *LineTable) All() iter.Seq[*LineRecord] {
	records := t.records

	return func(yield func(*LineRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *LineTable) FindByIndex(key int32) *LineRecord {
...
func (t *LineTable) GetByIndexOrError(key int32) (*LineRecord, error) {
	position, found := t.byIndex[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `Line` that corresponds to field `Index` value %v", key)
	}
...
func (t *LineTable) ContainsIndex(key int32) bool {
...
func (t *LineTable) Entries() iter.Seq2[int32, *LineRecord] {
	records := t.records

	return func(yield func(int32, *LineRecord) bool) {
		for i := range records {
...
func (t *LineTable) Read(filename string) error {
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "Line.Index", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Line.Index")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "Line.Body", tabbit.KindScalar, false, tabbit.ElementString) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Line.Body")
				for i := int32(0); i < count; {
		...
		case 3:
			if tabbit.CheckColumn(reader, column, "Line.Icon", tabbit.KindScalar, false, tabbit.ElementString) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Line.Icon")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameString(count - i)
					for ; n > 0; n-- {
