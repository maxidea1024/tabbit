...
// ServerTuningTable holds every row of ServerTuning.
type ServerTuningTable struct {
	records []ServerTuningRecord
	byIndex map[int32]int
}

// Records returns every row, in the order the sheet declared them.
func (t *ServerTuningTable) Records() []ServerTuningRecord { return t.records }

// Count returns how many rows the table holds.
func (t *ServerTuningTable) Count() int { return len(t.records) }
...
func (t *ServerTuningTable) All() iter.Seq[*ServerTuningRecord] {
	records := t.records

	return func(yield func(*ServerTuningRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *ServerTuningTable) FindByIndex(key int32) *ServerTuningRecord {
...
func (t *ServerTuningTable) GetByIndexOrError(key int32) (*ServerTuningRecord, error) {
	position, found := t.byIndex[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `ServerTuning` that corresponds to field `Index` value %v", key)
	}
...
func (t *ServerTuningTable) ContainsIndex(key int32) bool {
...
func (t *ServerTuningTable) Entries() iter.Seq2[int32, *ServerTuningRecord] {
	records := t.records

	return func(yield func(int32, *ServerTuningRecord) bool) {
		for i := range records {
...
func (t *ServerTuningTable) Read(filename string) error {
	...
	records := make([]ServerTuningRecord, count)
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "ServerTuning.Index", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "ServerTuning.Index")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "ServerTuning.Knob", tabbit.KindScalar, false, tabbit.ElementString) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "ServerTuning.Knob")
				for i := int32(0); i < count; {
		...
		case 3:
			if tabbit.CheckColumn(reader, column, "ServerTuning.Value", tabbit.KindScalar, false, tabbit.ElementF32) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "ServerTuning.Value")
				for i := int32(0); i < count; i++ {
					r := &records[i]
					r.Value = cursor.NextF32()
				}
			}
		default:
	...
	if err := reader.Err(); err != nil {
		return fmt.Errorf("ServerTuning: %w", err)
	}
