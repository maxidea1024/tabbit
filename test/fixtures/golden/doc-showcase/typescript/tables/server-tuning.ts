    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          ...
          break
        case 2:
          tabbit.checkColumn(column, 'ServerTuning.Knob', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_STRING])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'ServerTuning.Knob')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameString(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
              records[i]._knob = value
          }
          break
        case 3:
          tabbit.checkColumn(column, 'ServerTuning.Value', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_F32])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'ServerTuning.Value')
          for (let i = 0; i < rowCount; ++i) {
