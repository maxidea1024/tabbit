    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          ...
          break
        case 2:
          tabbit.checkColumn(column, 'Loot.Tags', tabbit.KIND_ARRAY, false, [tabbit.ELEMENT_STRING])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Loot.Tags')
          for (let i = 0; i < rowCount; ++i) {
          ...
          break
        case 3:
          tabbit.checkColumn(column, 'Loot.Weight', tabbit.KIND_ARRAY, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Loot.Weight')
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
            const elementCount = cursor.nextLength()
            record._weight = []
            for (let j = 0; j < elementCount; ++j) {
              record._weight.push(cursor.nextI32())
            }
          }
          break
        default:
