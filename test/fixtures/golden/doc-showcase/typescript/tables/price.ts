...
export class PriceTable {
  ...
  // Indexing by 'index'
  public get recordsByIndex(): Map<number, PriceRecord> { return this._recordsByIndex }
  private _recordsByIndex: Map<number, PriceRecord> = new Map<number, PriceRecord>()
   ...
   */
  public findByIndex(key: number): PriceRecord | undefined {
   ...
   */
  public getByIndexOrThrow(key: number): PriceRecord {
    const found = this._recordsByIndex.get(key)
    if (!found)
      throw new Error(`There is no record in table "Price" that corresponds to field "index" value ${key}`)
  ...
  public readBinaryFrom(fileBytes: Uint8Array): void {
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          tabbit.checkColumn(column, 'Price.Index', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Price.Index')
          ...
          break
        case 2:
          tabbit.checkColumn(column, 'Price.Name', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_STRING])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Price.Name')
          for (let i = 0; i < rowCount; ) {
  ...
  private publish(records: PriceRecord[]): void {
    const recordsByIndex = new Map<number, PriceRecord>()
