...
export class AccessTable {
  ...
  /** All records. */
  public get records(): AccessRecord[] { return this._records }
  private _records: AccessRecord[] = []
   ...
   */
  public [Symbol.iterator](): IterableIterator<AccessRecord> {
    return this._records[Symbol.iterator]()
  }

  // Indexing by 'index'
  public get recordsByIndex(): Map<number, AccessRecord> { return this._recordsByIndex }
  private _recordsByIndex: Map<number, AccessRecord> = new Map<number, AccessRecord>()
   ...
   */
  public findByIndex(key: number): AccessRecord | undefined {
   ...
   */
  public getByIndexOrThrow(key: number): AccessRecord {
    const found = this._recordsByIndex.get(key)
    if (!found)
      throw new Error(`There is no record in table "Access" that corresponds to field "index" value ${key}`)
   ...
   */
  public *entries(): IterableIterator<[number, AccessRecord]> {
  ...
  public readBinaryFrom(fileBytes: Uint8Array): void {
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          tabbit.checkColumn(column, 'Access.Index', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Access.Index')
          ...
          break
        case 2:
          tabbit.checkColumn(column, 'Access.Flags', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I64, tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Access.Flags')
          for (let i = 0; i < rowCount; ++i) {
          ...
          break
        case 3:
          tabbit.checkColumn(column, 'Access.Same', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I64, tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Access.Same')
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
   ...
   */
  private publish(records: AccessRecord[]): void {
    const recordsByIndex = new Map<number, AccessRecord>()
