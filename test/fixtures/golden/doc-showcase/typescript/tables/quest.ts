...
/** RewardEntry as the .json export carries it. */
interface RewardEntryJson {
  id: number
  count: number
}
...
export class QuestTable {
  ...
  // Indexing by 'index'
  public get recordsByIndex(): Map<number, QuestRecord> { return this._recordsByIndex }
  private _recordsByIndex: Map<number, QuestRecord> = new Map<number, QuestRecord>()
   ...
   */
  public findByIndex(key: number): QuestRecord | undefined {
   ...
   */
  public getByIndexOrThrow(key: number): QuestRecord {
    const found = this._recordsByIndex.get(key)
    if (!found)
      throw new Error(`There is no record in table "Quest" that corresponds to field "index" value ${key}`)
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          tabbit.checkColumn(column, 'Quest.Index', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Quest.Index')
          ...
          break
        case 2:
          tabbit.checkColumn(column, 'Quest.Name', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_STRING])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Quest.Name')
          for (let i = 0; i < rowCount; ) {
          ...
          break
        case 3:
          tabbit.checkColumn(column, 'Quest.Reward.Id', tabbit.KIND_ARRAY, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Quest.Reward.Id')
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
            const elementCount = cursor.nextLength()
            record._reward = Array.from({ length: elementCount }, () => ({ id: 0, count: 0 }))
            for (let j = 0; j < elementCount; ++j)
              record._reward[j].id = cursor.nextI32()
          }
          break
        case 4:
          tabbit.checkColumn(column, 'Quest.Reward.Count', tabbit.KIND_ARRAY, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Quest.Reward.Count')
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
            const elementCount = cursor.nextLength()
            if (record._reward.length !== elementCount)
              throw new Error(`Quest.Reward: the file gives this row ${elementCount} elements for one member of the record and ${record._reward.length} for another. Every member of a record carries the same element count, so the file is damaged.`)
            for (let j = 0; j < elementCount; ++j)
              record._reward[j].count = cursor.nextI32()
          }
          break
        default:
  ...
  private publish(records: QuestRecord[]): void {
    const recordsByIndex = new Map<number, QuestRecord>()
