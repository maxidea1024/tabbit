...
export class PotionTable {
  ...
  /** All records. */
  public get records(): PotionRecord[] { return this._records }
  private _records: PotionRecord[] = []
   ...
   */
  public [Symbol.iterator](): IterableIterator<PotionRecord> {
    return this._records[Symbol.iterator]()
  }

  // Indexing by 'index'
  public get recordsByIndex(): Map<number, PotionRecord> { return this._recordsByIndex }
  private _recordsByIndex: Map<number, PotionRecord> = new Map<number, PotionRecord>()
   ...
   */
  public findByIndex(key: number): PotionRecord | undefined {
   ...
   */
  public getByIndexOrThrow(key: number): PotionRecord {
    const found = this._recordsByIndex.get(key)
    if (!found)
      throw new Error(`There is no record in table "Potion" that corresponds to field "index" value ${key}`)
   ...
   */
  public *entries(): IterableIterator<[number, PotionRecord]> {
  ...
  private readFromJson(json: string): void {
    ...
    const records: PotionRecord[] = []

    if (this.isCompactRowFormatted(dataRows)) {
      for (const dataRow of dataRows) {
        const record = new PotionRecord()
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          tabbit.checkColumn(column, 'Potion.Index', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Potion.Index')
          ...
          break
        case 2:
          tabbit.checkColumn(column, 'Potion.Name', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_STRING])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Potion.Name')
          for (let i = 0; i < rowCount; ) {
          ...
          break
        case 3:
          tabbit.checkColumn(column, 'Potion.Rarity', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Potion.Rarity')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
          ...
          break
        case 4:
          tabbit.checkColumn(column, 'Potion.Price', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Potion.Price')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
   ...
   */
  private publish(records: PotionRecord[]): void {
    const recordsByIndex = new Map<number, PotionRecord>()
