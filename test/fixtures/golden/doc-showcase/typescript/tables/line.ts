...
export class LineTable {
  ...
  /** All records. */
  public get records(): LineRecord[] { return this._records }
  private _records: LineRecord[] = []
   ...
   */
  public [Symbol.iterator](): IterableIterator<LineRecord> {
    return this._records[Symbol.iterator]()
  }

  // Indexing by 'index'
  public get recordsByIndex(): Map<number, LineRecord> { return this._recordsByIndex }
  private _recordsByIndex: Map<number, LineRecord> = new Map<number, LineRecord>()
   ...
   */
  public findByIndex(key: number): LineRecord | undefined {
   ...
   */
  public getByIndexOrThrow(key: number): LineRecord {
    const found = this._recordsByIndex.get(key)
    if (!found)
      throw new Error(`There is no record in table "Line" that corresponds to field "index" value ${key}`)
   ...
   */
  public *entries(): IterableIterator<[number, LineRecord]> {
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          tabbit.checkColumn(column, 'Line.Index', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Line.Index')
          ...
          break
        case 2:
          tabbit.checkColumn(column, 'Line.Body', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_STRING])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Line.Body')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameString(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
              records[i]._body = value
          }
          break
        case 3:
          tabbit.checkColumn(column, 'Line.Icon', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_STRING])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Line.Icon')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameString(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
   ...
   */
  private publish(records: LineRecord[]): void {
    const recordsByIndex = new Map<number, LineRecord>()
