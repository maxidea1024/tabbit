...
export class WireTable {
  ...
  /** All records. */
  public get records(): WireRecord[] { return this._records }
  private _records: WireRecord[] = []
   ...
   */
  public [Symbol.iterator](): IterableIterator<WireRecord> {
    return this._records[Symbol.iterator]()
  }

  // Indexing by 'index'
  public get recordsByIndex(): Map<number, WireRecord> { return this._recordsByIndex }
  private _recordsByIndex: Map<number, WireRecord> = new Map<number, WireRecord>()
   ...
   */
  public findByIndex(key: number): WireRecord | undefined {
   ...
   */
  public getByIndexOrThrow(key: number): WireRecord {
    const found = this._recordsByIndex.get(key)
    if (!found)
      throw new Error(`There is no record in table "Wire" that corresponds to field "index" value ${key}`)
   ...
   */
  public *entries(): IterableIterator<[number, WireRecord]> {
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          tabbit.checkColumn(column, 'Wire.Index', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Wire.Index')
          ...
          break
        case 2:
          tabbit.checkColumn(column, 'Wire.Name', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_STRING])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Wire.Name')
          for (let i = 0; i < rowCount; ) {
          ...
          break
        case 4:
          tabbit.checkColumn(column, 'Wire.Price', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Wire.Price')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
   ...
   */
  private publish(records: WireRecord[]): void {
    const recordsByIndex = new Map<number, WireRecord>()
