...
namespace Tabbit.Fixtures.DocShowcase
{
    ...
    /// </summary>
    [System.Serializable]
    public partial class WireTable : IEnumerable<WireRecord>
    {
        /// <summary>
        /// Field names.
        /// </summary>
        public static readonly string[] FieldNames = { "Index", "Name", "Price" };
        ...
        public List<object[]> BuildObjectValueMap()
        {
            ...
            foreach (var r in _records)
                result.Add(new object[] { r.Index, r.Name, r.Price });
        ...
        /// </remarks>
        public List<WireRecord> Records => _records;
        private List<WireRecord> _records = new List<WireRecord>();
        ...
        /// </remarks>
        public List<WireRecord>.Enumerator GetEnumerator() => _records.GetEnumerator();

        IEnumerator<WireRecord> IEnumerable<WireRecord>.GetEnumerator() => _records.GetEnumerator();

        System.Collections.IEnumerator System.Collections.IEnumerable.GetEnumerator()
            => _records.GetEnumerator();

        #region Indexing by 'Index'
        public Dictionary<int, WireRecord> RecordsByIndex => _recordsByIndex;
        private Dictionary<int, WireRecord> _recordsByIndex = new Dictionary<int, WireRecord>();
        ...
        /// </remarks>
        public WireRecord FindByIndex(int key)
            => _recordsByIndex.TryGetValue(key, out WireRecord record) ? record : null;
        ...
        /// </remarks>
        public WireRecord GetByIndexOrThrow(int key)
        {
            if (!_recordsByIndex.TryGetValue(key, out WireRecord record))
                throw new TabbitException($"There is no record in table `Wire` that corresponds to field `Index` value {key}");
        ...
        {
            private readonly List<WireRecord> _rows;
            private int _at;

            internal EntryEnumerator(List<WireRecord> rows)
            {
                _rows = rows;
                _at = -1;
            }
            ...
            public (int Key, WireRecord Row) Current
        ...
        /// </remarks>
        public WireRecord this[int key] => GetByIndexOrThrow(key);
        ...
        public Task ReadAsync(TcbReader reader)
        {
            ...
            var records = new List<WireRecord>(count);

            for (int i = 0; i < count; i++)
                records.Add(new WireRecord());
            ...
            foreach (var column in columns)
            {
                ...
                {
                    case 1:
                        TcbTable.CheckColumn(column, "Wire.Index", TcbTable.KindScalar, false, TcbTable.ElementI32, TcbTable.ElementVarint);
                        cursor = new TcbColumnCursor(reader, column, count, "Wire.Index");
                        for (int i = 0; i < count; )
                        {
                    ...
                    case 2:
                        TcbTable.CheckColumn(column, "Wire.Name", TcbTable.KindScalar, false, TcbTable.ElementString);
                        cursor = new TcbColumnCursor(reader, column, count, "Wire.Name");
                        for (int i = 0; i < count; )
                        {
                            // One call per run of equal values, not one per row - over a
                            // run-length encoded column this is most of the decode.
                    ...
                    case 4:
                        TcbTable.CheckColumn(column, "Wire.Price", TcbTable.KindScalar, false, TcbTable.ElementI32, TcbTable.ElementVarint);
                        cursor = new TcbColumnCursor(reader, column, count, "Wire.Price");
                        for (int i = 0; i < count; )
                        {
                            // One call per run of equal values, not one per row - over a
                            // run-length encoded column this is most of the decode.
            ...
            var recordsByIndex = new Dictionary<int, WireRecord>(count);
