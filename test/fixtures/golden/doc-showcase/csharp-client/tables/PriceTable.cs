...
namespace Tabbit.Fixtures.DocShowcase
{
    ...
    public partial class PriceTable : IEnumerable<PriceRecord>
    {
        ...
        /// Field names.
        /// </summary>
        public static readonly string[] FieldNames = { "Index", "Name", "Amount" };
        ...
        public List<object[]> BuildObjectValueMap()
        {
            ...
            foreach (var r in _records)
                result.Add(new object[] { r.Index, r.Name, r.Amount });
        ...
        public Dictionary<int, PriceRecord> RecordsByIndex => _recordsByIndex;
        private Dictionary<int, PriceRecord> _recordsByIndex = new Dictionary<int, PriceRecord>();
        ...
        /// </remarks>
        public PriceRecord FindByIndex(int key)
            => _recordsByIndex.TryGetValue(key, out PriceRecord record) ? record : null;
        ...
        /// </remarks>
        public PriceRecord GetByIndexOrThrow(int key)
        {
            if (!_recordsByIndex.TryGetValue(key, out PriceRecord record))
                throw new TabbitException($"There is no record in table `Price` that corresponds to field `Index` value {key}");
        ...
        /// </remarks>
        public PriceRecord this[int key] => GetByIndexOrThrow(key);
        ...
        public Task ReadAsync(TcbReader reader)
        {
            ...
            foreach (var column in columns)
            {
                ...
                {
                    case 1:
                        TcbTable.CheckColumn(column, "Price.Index", TcbTable.KindScalar, false, TcbTable.ElementI32, TcbTable.ElementVarint);
                        cursor = new TcbColumnCursor(reader, column, count, "Price.Index");
                        for (int i = 0; i < count; )
                        {
                    ...
                    case 2:
                        TcbTable.CheckColumn(column, "Price.Name", TcbTable.KindScalar, false, TcbTable.ElementString);
                        cursor = new TcbColumnCursor(reader, column, count, "Price.Name");
                        for (int i = 0; i < count; )
                        {
                            // One call per run of equal values, not one per row - over a
                            // run-length encoded column this is most of the decode.
            ...
            var recordsByIndex = new Dictionary<int, PriceRecord>(count);
