...
module DeclaredData
  ...
  class LoadoutRecord
    attr_accessor :index, :name, :slot, :grade

    def initialize
      @index = 0
      @name = ''
      @slot = Array.new(2) { Reward.new }
      @grade = 0
    end
    ...
    def read(filename)
      ...
      columns.each do |column|
        ...
        when 3
          ...
          cursor = Tabbit::ColumnCursor.new(reader, column, count, 'Loadout.Slot.ItemId')
          records.each do |record|
            ...
            record.slot = Array.new(element_count) { Reward.new }
        ...
        when 5
          ...
          cursor = Tabbit::ColumnCursor.new(reader, column, count, 'Loadout.Slot.Icon')
          records.each do |record|
            element_count = cursor.next_length
            if record.slot.length != element_count
              raise Tabbit::TcbError,
        ...
        when 6
          ...
          cursor = Tabbit::ColumnCursor.new(reader, column, count, 'Loadout.Grade')
          at = 0
          while at < count
            n, value = cursor.next_same_i32(count - at)
            (at...(at + n)).each do |i|
