...
public final class LoadoutRecord {
    ...
    public var slot: [Reward] = [Reward](repeating: Reward(), count: 2)

    /// an enum the schema file declares
    public var grade: Affinity = Affinity.of(0)
...
public final class LoadoutTable: Sequence {
    ...
    public func read(
        ...
        for column in header.columns {
            ...
            case 3:
                try Tcb.checkColumn(column, "Loadout.Slot.ItemId", Tcb.kindArray, false, Tcb.elementI32, Tcb.elementVarint)
                ...
                for record in loaded {
                    ...
                    record.slot = [Reward](
                        repeating: Reward(), count: elementCount)
            ...
            case 5:
                try Tcb.checkColumn(column, "Loadout.Slot.Icon", Tcb.kindArray, false, Tcb.elementString)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Loadout.Slot.Icon")
                for record in loaded {
                    let elementCount = Swift.max(0, try cursor.nextLength())
                    if record.slot.count != elementCount {
                        throw TcbError(
                    ...
                    for element in 0 ..< elementCount {
                        record.slot[element].icon = try cursor.nextString()
                    }
                }
            case 6:
                try Tcb.checkColumn(column, "Loadout.Grade", Tcb.kindScalar, false, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Loadout.Grade")
                var at = 0
                while at < count {
                    var n = try cursor.nextSameI32(count - at)
                    while n > 0 {
                        let i = at
                        loaded[i].grade = Affinity.of(cursor.runSameValue)
                        at += 1
                        n -= 1
                    }
                }
