...
public final class BonusesTable: Sequence {
    ...
    public func makeIterator() -> Array<BonusesRecord>.Iterator {
    ...
    public private(set) var records: [BonusesRecord] = []

    private var byIndex: [Int32: BonusesRecord] = [:]

    /// The row with this Index, or nil when the table has none.
    ///
    /// The lookup to reach for when a missing row is an ordinary answer - an optional
    /// reference, a key that came from user input.
    public func findByIndex(_ key: Int32) -> BonusesRecord? {
        byIndex[key]
    }
    ...
    public func getByIndexOrThrow(_ key: Int32) throws -> BonusesRecord {
        guard let record = byIndex[key] else {
            throw RecordNotFoundError(
                "there is no record in table `Bonuses` that corresponds to "
    ...
    public subscript(_ key: Int32) -> BonusesRecord? {
        findByIndex(key)
    }
    ...
    /// row with.
    public var entries: LazyMapSequence<[BonusesRecord], (key: Int32, row: BonusesRecord)> {
    ...
    /// once. DeclaredAccessor passes its own. spec/targets/accessor-instances.md.
    public func read(
        _ filename: String,
        key: [UInt8]? = nil,
        macKey: [UInt8]? = nil,
        ...
        var loaded = [BonusesRecord]()
        loaded.reserveCapacity(count)

        for _ in 0 ..< count { loaded.append(BonusesRecord()) }
        var loadedByIndex = [Int32: BonusesRecord](
        ...
        for column in header.columns {
            ...
            switch column.tag {
            case 1:
                try Tcb.checkColumn(column, "Bonuses.Index", Tcb.kindScalar, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Bonuses.Index")
                var at = 0
                while at < count {
            ...
            case 2:
                try Tcb.checkColumn(column, "Bonuses.Bonus.StatId", Tcb.kindArray, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Bonuses.Bonus.StatId")
                for record in loaded {
                    ...
                    record.bonus = [Bonus](
                        repeating: Bonus(), count: elementCount)

                    for element in 0 ..< elementCount {
                        record.bonus[element].statId = try cursor.nextI32()
                    }
                }
            case 3:
                try Tcb.checkColumn(column, "Bonuses.Bonus.Amount", Tcb.kindArray, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Bonuses.Bonus.Amount")
                for record in loaded {
                    let elementCount = Swift.max(0, try cursor.nextLength())
                    if record.bonus.count != elementCount {
                        throw TcbError(
                            "Bonuses.bonus: the file gives one "
                            + "member of this record a different element count than another")
                    }

                    for element in 0 ..< elementCount {
                        record.bonus[element].amount = try cursor.nextI32()
                    }
                }
            default:
