...
// ChestRecord was generated from test/fixtures/xlsx/declared/declared.xlsx : Declared : B24.
...
// ChestTable holds every row of Chest.
...
func (t *ChestTable) Records() []ChestRecord { return t.records }

// Count returns how many rows the table holds.
func (t *ChestTable) Count() int { return len(t.records) }
...
func (t *ChestTable) All() iter.Seq[*ChestRecord] {
	records := t.records

	return func(yield func(*ChestRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *ChestTable) FindByIndex(key int32) *ChestRecord {
...
func (t *ChestTable) GetByIndexOrError(key int32) (*ChestRecord, error) {
	position, found := t.byIndex[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `Chest` that corresponds to field `Index` value %v", key)
	}
...
func (t *ChestTable) ContainsIndex(key int32) bool {
...
func (t *ChestTable) Entries() iter.Seq2[int32, *ChestRecord] {
	records := t.records

	return func(yield func(int32, *ChestRecord) bool) {
		for i := range records {
...
func (t *ChestTable) Read(filename string) error {
	...
	records := make([]ChestRecord, count)
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "Chest.Index", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Chest.Index")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "Chest.Prize.ItemId", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Chest.Prize.ItemId")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 3:
			if tabbit.CheckColumn(reader, column, "Chest.Prize.Count", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Chest.Prize.Count")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 4:
			if tabbit.CheckColumn(reader, column, "Chest.Prize.Icon", tabbit.KindScalar, false, tabbit.ElementString) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Chest.Prize.Icon")
				for i := int32(0); i < count; {
