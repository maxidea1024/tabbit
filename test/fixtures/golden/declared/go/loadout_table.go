...
// LoadoutRecord was generated from test/fixtures/xlsx/declared/declared.xlsx : Declared : B2.
...
func (t *LoadoutTable) Read(filename string) error {
	...
	for _, column := range columns {
		...
		case 3:
			if tabbit.CheckColumn(reader, column, "Loadout.Slot.ItemId", tabbit.KindArray, false, tabbit.ElementI32, tabbit.ElementVarint) {
				...
				for i := int32(0); i < count; i++ {
					...
					r.Slot = make([]Reward, elementCount)
		...
		case 5:
			if tabbit.CheckColumn(reader, column, "Loadout.Slot.Icon", tabbit.KindArray, false, tabbit.ElementString) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Loadout.Slot.Icon")
				for i := int32(0); i < count; i++ {
					r := &records[i]
					elementCount := int(cursor.NextLength())
					if len(r.Slot) != elementCount {
					...
					for j := 0; j < elementCount; j++ {
						r.Slot[j].Icon = cursor.NextString()
					}
				}
			}
		case 6:
			if tabbit.CheckColumn(reader, column, "Loadout.Grade", tabbit.KindScalar, false, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Loadout.Grade")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
