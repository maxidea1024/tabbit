...
// BonusesRecord was generated from test/fixtures/xlsx/declared/declared.xlsx : Declared : B12.
// A multi-row group whose members a schema file declares.
type BonusesRecord struct {
	// primary index
	Index int32
...
// BonusesTable holds every row of Bonuses.
type BonusesTable struct {
	records []BonusesRecord
	byIndex map[int32]int
}

// Records returns every row, in the order the sheet declared them.
func (t *BonusesTable) Records() []BonusesRecord { return t.records }

// Count returns how many rows the table holds.
func (t *BonusesTable) Count() int { return len(t.records) }
...
func (t *BonusesTable) All() iter.Seq[*BonusesRecord] {
	records := t.records

	return func(yield func(*BonusesRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *BonusesTable) FindByIndex(key int32) *BonusesRecord {
...
func (t *BonusesTable) GetByIndexOrError(key int32) (*BonusesRecord, error) {
	position, found := t.byIndex[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `Bonuses` that corresponds to field `Index` value %v", key)
	}
...
func (t *BonusesTable) ContainsIndex(key int32) bool {
...
func (t *BonusesTable) Entries() iter.Seq2[int32, *BonusesRecord] {
	records := t.records

	return func(yield func(int32, *BonusesRecord) bool) {
		for i := range records {
...
func (t *BonusesTable) Read(filename string) error {
	...
	records := make([]BonusesRecord, count)
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "Bonuses.Index", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Bonuses.Index")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "Bonuses.Bonus.StatId", tabbit.KindArray, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Bonuses.Bonus.StatId")
				for i := int32(0); i < count; i++ {
					...
					r.Bonus = make([]Bonus, elementCount)
					for j := 0; j < elementCount; j++ {
						r.Bonus[j].StatId = cursor.NextI32()
					}
				}
			}
		case 3:
			if tabbit.CheckColumn(reader, column, "Bonuses.Bonus.Amount", tabbit.KindArray, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Bonuses.Bonus.Amount")
				for i := int32(0); i < count; i++ {
					r := &records[i]
					elementCount := int(cursor.NextLength())
					if len(r.Bonus) != elementCount {
						return fmt.Errorf(
							"Bonuses: the file gives one member of `Bonus` a different element count than another")
					}
					for j := 0; j < elementCount; j++ {
						r.Bonus[j].Amount = cursor.NextI32()
					}
				}
			}
		default:
	...
	if err := reader.Err(); err != nil {
		return fmt.Errorf("Bonuses: %w", err)
	}
