...
final class LoadoutRecord
{
    ...
    public Affinity $grade = Affinity::Fire;
        ...
        foreach ($columns as $column) {
            ...
            switch ($column['tag']) {
                ...
                case 5:
                    ...
                    $cursor = new TcbColumnCursor($reader, $column, $count, 'Loadout.Slot.Icon');
                    foreach ($records as $record) {
                        $elementCount = $cursor->nextLength();
                        if (\count($record->slot) !== $elementCount) {
                            throw new \Tabbit\TcbException(
                ...
                case 6:
                    ...
                    $cursor = new TcbColumnCursor($reader, $column, $count, 'Loadout.Grade');
                    for ($i = 0; $i < $count; ) {
                        [$n, $value] = $cursor->nextSameI32($count - $i);
                        for (; $n > 0; $n--, $i++) {
                            $records[$i]->grade = Affinity::tryFrom($value) ?? Affinity::Fire;
                        }
                    }
                    break;
