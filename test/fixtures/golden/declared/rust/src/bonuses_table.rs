...
#[derive(Clone, Debug, Default)]
pub struct BonusesTable {
    records: Vec<BonusesRecord>,
    by_index: HashMap<i32, usize>,
}

impl BonusesTable {
    /// Every row, in the order the sheet declared them.
    pub fn records(&self) -> &[BonusesRecord] {
        &self.records
    }
    ...
    pub fn iter(&self) -> std::slice::Iter<'_, BonusesRecord> {
        self.records.iter()
    }
    ...
    pub fn find_by_index(&self, key: i32) -> Option<&BonusesRecord> {
    ...
    /// error it returns.
    pub fn get_by_index_or_error(&self, key: i32) -> tabbit::Result<&BonusesRecord> {
        self.by_index
            .get(&key)
    ...
    pub fn entries(&self) -> impl Iterator<Item = (&i32, &BonusesRecord)> + '_ {
    ...
    pub fn read(&mut self, filename: &Path) -> tabbit::Result<()> {
        ...
        let mut records = vec![BonusesRecord::default(); count];
        ...
        for column in &header.columns {
            ...
            match column.tag {
                1 => {
                    tabbit::check_column(column, "Bonuses.Index", tabbit::KIND_SCALAR, false, &[tabbit::ELEMENT_I32, tabbit::ELEMENT_VARINT])?;
                    let mut cursor = tabbit::TcbColumnCursor::new(&mut reader, column, header.row_count, "Bonuses.Index")?;
                    let mut at = 0usize;
                ...
                2 => {
                    tabbit::check_column(column, "Bonuses.Bonus.StatId", tabbit::KIND_ARRAY, false, &[tabbit::ELEMENT_I32, tabbit::ELEMENT_VARINT])?;
                    let bytes_left = reader.remaining();
                    let mut cursor = tabbit::TcbColumnCursor::new(&mut reader, column, header.row_count, "Bonuses.Bonus.StatId")?;
                    for record in records.iter_mut() {
                        ...
                        record.bonus = vec![Bonus::default(); element_count];
                        for element in 0..element_count {
                            record.bonus[element].stat_id = cursor.next_i32()?;
                        }
                    }
                }
                3 => {
                    tabbit::check_column(column, "Bonuses.Bonus.Amount", tabbit::KIND_ARRAY, false, &[tabbit::ELEMENT_I32, tabbit::ELEMENT_VARINT])?;
                    let mut cursor = tabbit::TcbColumnCursor::new(&mut reader, column, header.row_count, "Bonuses.Bonus.Amount")?;
                    for record in records.iter_mut() {
                        let element_count = cursor.next_length()?.max(0) as usize;
                        if record.bonus.len() != element_count {
                            return Err(tabbit::Error::ColumnMismatch {
                                field: "Bonuses.bonus",
                        ...
                        for element in 0..element_count {
                            record.bonus[element].amount = cursor.next_i32()?;
                        }
                    }
                }
                _ => {
...
impl<'a> IntoIterator for &'a BonusesTable {
    type Item = &'a BonusesRecord;
    type IntoIter = std::slice::Iter<'a, BonusesRecord>;
