...
class BonusesTable extends Iterable<BonusesRecord> {
  ...
  List<BonusesRecord> get records => _records;
  List<BonusesRecord> _records = [];
  ...
  @override
  Iterator<BonusesRecord> get iterator => _records.iterator;

  Map<int, BonusesRecord> _byIndex = {};

  /// The row with this Index, or null when the table has none.
  ///
  /// The lookup to reach for when a missing row is an ordinary answer - an optional
  /// reference, a key that came from user input.
  BonusesRecord? findByIndex(int key) => _byIndex[key];
  ...
  BonusesRecord getByIndexOrThrow(int key) {
  ...
  BonusesRecord? operator [](int key) => findByIndex(key);
  ...
  Iterable<MapEntry<int, BonusesRecord>> get entries =>
  ...
  void read(String filename) {
    ...
    final loaded = <BonusesRecord>[];
    final loadedByIndex = <int, BonusesRecord>{};

    for (var i = 0; i < count; i++) {
      loaded.add(BonusesRecord());
    }
    ...
    for (final column in header.columns) {
      ...
      switch (column.tag) {
        case 1:
          checkColumn(column, 'Bonuses.Index', kindScalar, false, [elementI32, elementVarint]);
          cursor = TcbColumnCursor(reader, column, count, 'Bonuses.Index');
          for (var i = 0; i < count; ) {
          ...
          break;
        case 2:
          checkColumn(column, 'Bonuses.Bonus.StatId', kindArray, false, [elementI32, elementVarint]);
          cursor = TcbColumnCursor(reader, column, count, 'Bonuses.Bonus.StatId');
          for (final record in loaded) {
            ...
            record.bonus =
                List.generate(elementCount, (_) => Bonus());
          ...
          break;
        case 3:
          checkColumn(column, 'Bonuses.Bonus.Amount', kindArray, false, [elementI32, elementVarint]);
          cursor = TcbColumnCursor(reader, column, count, 'Bonuses.Bonus.Amount');
          for (final record in loaded) {
            final elementCount = cursor.nextLength();
            if (record.bonus.length != elementCount) {
              throw TcbException(
                  'Bonuses.bonus: the file gives one member of this '
