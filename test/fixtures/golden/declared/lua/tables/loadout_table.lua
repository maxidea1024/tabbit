...
---@field slot Reward[]
---@field grade integer
local LoadoutRecordMeta = tcb.strictType("a `Loadout` row", { "index", "name", "slot", "grade" })
...
local function newLoadoutRecord()
  return setmetatable({
    index = 0,
    name = "",
    slot = tcb.filledArray(2, newReward),
    grade = 1,
  }, LoadoutRecordMeta)
end
...
function LoadoutTable:readBytes(data)
  ...
  for c = 1, #columns do
    ...
    elseif column.tag == 3 then
      tcb.checkColumn(column, "Loadout.Slot.ItemId", tcb.KIND_ARRAY, false, { tcb.ELEMENT_I32, tcb.ELEMENT_VARINT })
      local cursor = tcb.newCursor(reader, column, count, "Loadout.Slot.ItemId")
      for i = 1, count do
        local record = records[i]
        ...
        record.slot = tcb.filledArray(elementCount, newReward)
    ...
    elseif column.tag == 5 then
      tcb.checkColumn(column, "Loadout.Slot.Icon", tcb.KIND_ARRAY, false, { tcb.ELEMENT_STRING })
      local cursor = tcb.newCursor(reader, column, count, "Loadout.Slot.Icon")
      for i = 1, count do
        local record = records[i]
        local elementCount = cursor:nextLength()
        if #record.slot ~= elementCount then
        ...
        for element = 1, elementCount do
          record.slot[element].icon = cursor:nextString()
        end
      end
    elseif column.tag == 6 then
      tcb.checkColumn(column, "Loadout.Grade", tcb.KIND_SCALAR, false, { tcb.ELEMENT_VARINT })
      local cursor = tcb.newCursor(reader, column, count, "Loadout.Grade")
      local at = 0
