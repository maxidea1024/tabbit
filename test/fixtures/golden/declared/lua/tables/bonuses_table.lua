...
local newBonus = require(_root .. "structs.struct_bonus")
...
---@field index integer
---@field bonus Bonus[]
local BonusesRecordMeta = tcb.strictType("a `Bonuses` row", { "index", "bonus" })

---@return BonusesRecord
local function newBonusesRecord()
  return setmetatable({
    index = 0,
    bonus = {},
  }, BonusesRecordMeta)
...
---@class BonusesTable
---@field records BonusesRecord[]
local BonusesTable = {}

local BonusesTableMeta = tcb.strictInstance("the `Bonuses` table", BonusesTable, { "records", "byIndex" })

function BonusesTable.new()
  return setmetatable({
    records = {},
    byIndex = {},
  }, BonusesTableMeta)
...
function BonusesTable:containsIndex(key)
...
function BonusesTable:readBytes(data)
  ...
  for c = 1, #columns do
    ...
    if column.tag == 1 then
      tcb.checkColumn(column, "Bonuses.Index", tcb.KIND_SCALAR, false, { tcb.ELEMENT_I32, tcb.ELEMENT_VARINT })
      local cursor = tcb.newCursor(reader, column, count, "Bonuses.Index")
      local at = 0
    ...
    elseif column.tag == 2 then
      tcb.checkColumn(column, "Bonuses.Bonus.StatId", tcb.KIND_ARRAY, false, { tcb.ELEMENT_I32, tcb.ELEMENT_VARINT })
      local cursor = tcb.newCursor(reader, column, count, "Bonuses.Bonus.StatId")
      for i = 1, count do
        local record = records[i]
        ...
        record.bonus = tcb.filledArray(elementCount, newBonus)

        for element = 1, elementCount do
          record.bonus[element].statId = cursor:nextI32()
        end
      end
    elseif column.tag == 3 then
      tcb.checkColumn(column, "Bonuses.Bonus.Amount", tcb.KIND_ARRAY, false, { tcb.ELEMENT_I32, tcb.ELEMENT_VARINT })
      local cursor = tcb.newCursor(reader, column, count, "Bonuses.Bonus.Amount")
      for i = 1, count do
        local record = records[i]
        local elementCount = cursor:nextLength()
        if #record.bonus ~= elementCount then
          tcb.fail("Bonuses: the file gives one member of this record a " ..
            "different element count than another")
        end

        for element = 1, elementCount do
          record.bonus[element].amount = cursor:nextI32()
        end
      end
    else
...
function BonusesTable:read(filename)
