...
class LoadoutRecord:
    ...
    __slots__ = ("index", "name", "slot", "grade")

    def __init__(self):
        self.index = 0
        self.name = ""
        self.slot = [Reward() for _ in range(2)]
        self.grade = Affinity(0)

    def __repr__(self):
        return "LoadoutRecord(index=%r, name=%r, slot=%r, grade=%r)" % (self.index, self.name, self.slot, self.grade)
        ...
        for column in columns:
            ...
            if column.tag == 1:
                ...
                for record in records:
                    ...
                    record.slot = [
                        Reward() for _ in range(element_count)]
                ...
                cursor = tabbit.ColumnCursor(reader, column, count, "Loadout.Slot.Icon")
                for record in records:
                    element_count = cursor.next_length()
                    if len(record.slot) != element_count:
                        raise tabbit.TcbError(
                ...
                cursor = tabbit.ColumnCursor(reader, column, count, "Loadout.Grade")
                at = 0
                while at < count:
                    n, value = cursor.next_same_i32(count - at)
                    for i in range(at, at + n):
