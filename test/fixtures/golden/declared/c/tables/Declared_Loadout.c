...
static bool Declared_LoadoutParse(Declared_LoadoutTable_t* table, tb_reader* reader) {
  int32_t row;
  int32_t at;
  ...
  if (table->count > 0) {
    table->records = (Declared_LoadoutRecord_t*)tb_arena_alloc(
  ...
  for (row = 0; row < table->count; ++row) {
    Declared_LoadoutRecord_t* record = &table->records[row];
  ...
  for (at = 0; at < column_count && !tb_failed(reader); ++at) {
    ...
    case 3:
      (void)tb_check_column(reader, column, "Loadout.Slot.ItemId", TB_KIND_ARRAY, false, TB_ELEMENT_MASK(TB_ELEMENT_I32) | TB_ELEMENT_MASK(TB_ELEMENT_VARINT));
      ...
      for (row = 0; row < table->count && !tb_failed(reader); ++row) {
        ...
        record->slot = (struct Declared_Reward*)tb_arena_alloc(
    ...
    case 5:
      (void)tb_check_column(reader, column, "Loadout.Slot.Icon", TB_KIND_ARRAY, false, TB_ELEMENT_MASK(TB_ELEMENT_STRING));

      (void)tb_cursor_init(&cursor, reader, column, table->count, "Loadout.Slot.Icon");
      ...
      for (row = 0; row < table->count && !tb_failed(reader); ++row) {
        ...
        for (element = 0; element < element_count && !tb_failed(reader); ++element)
          (void)tb_cursor_next_string(&cursor, &record->slot[element].icon);
      }
      break;

    case 6:
      (void)tb_check_column(reader, column, "Loadout.Grade", TB_KIND_SCALAR, false, TB_ELEMENT_MASK(TB_ELEMENT_VARINT));

      (void)tb_cursor_init(&cursor, reader, column, table->count, "Loadout.Grade");
      ...
      for (row = 0; row < table->count && !tb_failed(reader); ++row) {
        ...
        record->grade = (Declared_Affinity_t)scratch;
      }
      break;
...
bool Declared_LoadoutLoad(Declared_LoadoutTable_t* table, const char* filename,
  ...
  Declared_LoadoutTable_t loaded;
