...
#ifndef TABBIT_GENERATED_DECLAREDACCESSOR_LOADOUT_H
#define TABBIT_GENERATED_DECLAREDACCESSOR_LOADOUT_H
...
#include "structs/DeclaredAccessor_struct_reward.h"
...
struct LoadoutRecord {
  ...
  std::vector<Reward> slot;
  /// an enum the schema file declares
  Affinity grade = static_cast<Affinity>(0);
};
  ...
  void read(const std::string& filename) {
    ...
    for (const tabbit::Column& column : header.columns) {
      ...
      switch (column.tag) {
        ...
        case 3: {
          tabbit::check_column(column, "Loadout.Slot.ItemId", tabbit::kKindArray, false, {tabbit::kElementI32, tabbit::kElementVarint});
          ...
          for (std::size_t i = 0; i < row_count; ++i) {
            ...
            record.slot.assign(static_cast<std::size_t>(element_count), Reward());
        ...
        case 5: {
          tabbit::check_column(column, "Loadout.Slot.Icon", tabbit::kKindArray, false, {tabbit::kElementString});
          tabbit::TcbColumnCursor cursor(reader, column, header.row_count, "Loadout.Slot.Icon");
          for (std::size_t i = 0; i < row_count; ++i) {
            auto& record = records[i];
            const std::int32_t element_count = cursor.next_length();
            if (record.slot.size() != static_cast<std::size_t>(element_count)) {
            ...
            for (std::int32_t j = 0; j < element_count; ++j) {
              record.slot[static_cast<std::size_t>(j)].icon = cursor.next_string();
            }
          }
          break;
        }
        case 6: {
          tabbit::check_column(column, "Loadout.Grade", tabbit::kKindScalar, false, {tabbit::kElementVarint});
          tabbit::TcbColumnCursor cursor(reader, column, header.row_count, "Loadout.Grade");
          std::int32_t value{};
          for (std::size_t i = 0; i < row_count; ) {
            std::int32_t n = cursor.next_same_i32(
                static_cast<std::int32_t>(row_count - i), value);
            for (; n > 0; --n, ++i) {
              records[i].grade = static_cast<Affinity>(value);
            }
          }
          break;
        }
...
#endif  // TABBIT_GENERATED_DECLAREDACCESSOR_LOADOUT_H
