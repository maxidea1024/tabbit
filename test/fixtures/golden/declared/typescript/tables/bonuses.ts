...
import { Bonus } from '../structs/bonus'


/** Bonus as the .json export carries it. */
interface BonusJson {
  statId: number
  amount: number
}
...
export class BonusesRecord {
  ...
  public populateFieldValues(dataRow: IDataRow): void {
    ...
    this._bonus = dataRow.bonus.map(e => ({ statId: e.statId, amount: e.amount }))
  }
  ...
  public populateFieldValuesCompact(dataRow: any[]): void {
    ...
    const _bonus_statId = dataRow[offset++] as any[]
    const _bonus_amount = dataRow[offset++] as any[]
    this._bonus = Array.from({ length: _bonus_statId.length }, (_, k) => ({ statId: _bonus_statId[k], amount: _bonus_amount[k] }))
  }
}

// Generated from test/fixtures/xlsx/declared/declared.xlsx : Declared : B12
/** A multi-row group whose members a schema file declares. */
export class BonusesTable {
  /** Default constructor. */
  constructor() {
  }

  /** All records. */
  public get records(): BonusesRecord[] { return this._records }
  private _records: BonusesRecord[] = []
   ...
   */
  public [Symbol.iterator](): IterableIterator<BonusesRecord> {
    return this._records[Symbol.iterator]()
  }

  // Indexing by 'index'
  public get recordsByIndex(): Map<number, BonusesRecord> { return this._recordsByIndex }
  private _recordsByIndex: Map<number, BonusesRecord> = new Map<number, BonusesRecord>()
   ...
   */
  public findByIndex(key: number): BonusesRecord | undefined {
   ...
   */
  public getByIndexOrThrow(key: number): BonusesRecord {
    const found = this._recordsByIndex.get(key)
    if (!found)
      throw new Error(`There is no record in table "Bonuses" that corresponds to field "index" value ${key}`)
   ...
   */
  public *entries(): IterableIterator<[number, BonusesRecord]> {
  ...
  private readFromJson(json: string): void {
    ...
    const records: BonusesRecord[] = []

    if (this.isCompactRowFormatted(dataRows)) {
      for (const dataRow of dataRows) {
        const record = new BonusesRecord()
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (let i = 0; i < rowCount; ++i)
      records.push(new BonusesRecord())
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          tabbit.checkColumn(column, 'Bonuses.Index', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Bonuses.Index')
          ...
          break
        case 2:
          tabbit.checkColumn(column, 'Bonuses.Bonus.StatId', tabbit.KIND_ARRAY, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Bonuses.Bonus.StatId')
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
            const elementCount = cursor.nextLength()
            record._bonus = Array.from({ length: elementCount }, () => ({ statId: 0, amount: 0 }))
            for (let j = 0; j < elementCount; ++j)
              record._bonus[j].statId = cursor.nextI32()
          }
          break
        case 3:
          tabbit.checkColumn(column, 'Bonuses.Bonus.Amount', tabbit.KIND_ARRAY, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Bonuses.Bonus.Amount')
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
            const elementCount = cursor.nextLength()
            if (record._bonus.length !== elementCount)
              throw new Error(`Bonuses.Bonus: the file gives this row ${elementCount} elements for one member of the record and ${record._bonus.length} for another. Every member of a record carries the same element count, so the file is damaged.`)
            for (let j = 0; j < elementCount; ++j)
   ...
   */
  private publish(records: BonusesRecord[]): void {
    const recordsByIndex = new Map<number, BonusesRecord>()
