...
class LoadoutRecord {
    ...
    var slot: MutableList<Reward> = MutableList(2) { Reward() }
    /** an enum the schema file declares */
    var grade: Affinity = Affinity.of(0)
    ...
    fun read(filename: String) {
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                ...
                3 -> {
                    checkColumn(column, "Loadout.Slot.ItemId", KIND_ARRAY, false, ELEMENT_I32, ELEMENT_VARINT)
                    ...
                    for (record in loaded) {
                        ...
                        record.slot =
                            MutableList(elementCount.coerceAtLeast(0)) { Reward() }
                ...
                5 -> {
                    checkColumn(column, "Loadout.Slot.Icon", KIND_ARRAY, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "Loadout.Slot.Icon")
                    for (record in loaded) {
                        val elementCount = cursor.nextLength()
                        if (record.slot.size != elementCount) {
                            throw TcbException(
                ...
                6 -> {
                    checkColumn(column, "Loadout.Grade", KIND_SCALAR, false, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Loadout.Grade")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
