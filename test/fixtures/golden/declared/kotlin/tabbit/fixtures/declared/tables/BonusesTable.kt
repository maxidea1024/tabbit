...
class BonusesRecord {
    ...
    var bonus: MutableList<Bonus> = ArrayList()


}

/** Every row of Bonuses. */
class BonusesTable : Iterable<BonusesRecord> {
     ...
     */
    var records: MutableList<BonusesRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<BonusesRecord> = records.iterator()

    private var byIndex: HashMap<Int, BonusesRecord> = HashMap()
     ...
     */
    fun findByIndex(key: Int): BonusesRecord? = byIndex[key]
     ...
     */
    fun getByIndexOrThrow(key: Int): BonusesRecord =
     ...
     */
    operator fun get(key: Int): BonusesRecord? = findByIndex(key)
     ...
     */
    val entries: Sequence<Pair<Int, BonusesRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<BonusesRecord>(count)
        val loadedByIndex = HashMap<Int, BonusesRecord>(count * 2)

        repeat(count) { loaded.add(BonusesRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "Bonuses.Index", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Bonuses.Index")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "Bonuses.Bonus.StatId", KIND_ARRAY, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Bonuses.Bonus.StatId")
                    for (record in loaded) {
                        ...
                        record.bonus =
                            MutableList(elementCount.coerceAtLeast(0)) { Bonus() }
                ...
                3 -> {
                    checkColumn(column, "Bonuses.Bonus.Amount", KIND_ARRAY, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Bonuses.Bonus.Amount")
                    for (record in loaded) {
                        val elementCount = cursor.nextLength()
                        if (record.bonus.size != elementCount) {
                            throw TcbException(
