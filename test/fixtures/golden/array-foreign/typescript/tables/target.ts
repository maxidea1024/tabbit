...
// Generated from test/fixtures/xlsx/array-foreign/array-foreign.xlsx : Refs : B2
/** Reference target. */
...
export class TargetTable {
  ...
  /** All records. */
  public get records(): TargetRecord[] { return this._records }
   ...
   */
  public [Symbol.iterator](): IterableIterator<TargetRecord> {
    return this._records[Symbol.iterator]()
  }

  // Indexing by 'index'
  public get recordsByIndex(): Map<number, TargetRecord> { return this._recordsByIndex }
  private _recordsByIndex: Map<number, TargetRecord> = new Map<number, TargetRecord>()
   ...
   */
  public findByIndex(key: number): TargetRecord | undefined {
   ...
   */
  public getByIndexOrThrow(key: number): TargetRecord {
    const found = this._recordsByIndex.get(key)
    if (!found)
      throw new Error(`There is no record in table "Target" that corresponds to field "index" value ${key}`)
   ...
   */
  public *entries(): IterableIterator<[number, TargetRecord]> {
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          tabbit.checkColumn(column, 'Target.Index', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Target.Index')
          ...
          break
        case 2:
          tabbit.checkColumn(column, 'Target.Name', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_STRING])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Target.Name')
          for (let i = 0; i < rowCount; ) {
          ...
          break
        case 3:
          tabbit.checkColumn(column, 'Target.Note', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_STRING])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Target.Note')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameString(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
   ...
   */
  private publish(records: TargetRecord[]): void {
    const recordsByIndex = new Map<number, TargetRecord>()
