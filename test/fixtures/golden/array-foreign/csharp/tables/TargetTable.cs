...
namespace Tabbit.Fixtures.X
{
    ...
    /// </summary>
    [System.Serializable]
    public partial class TargetTable : IEnumerable<TargetRecord>
    {
        /// <summary>
        /// Field names.
        /// </summary>
        public static readonly string[] FieldNames = { "Index", "Name", "Note" };
        ...
        public List<object[]> BuildObjectValueMap()
        {
            ...
            foreach (var r in _records)
                result.Add(new object[] { r.Index, r.Name, r.Note });
        ...
        /// </remarks>
        public List<TargetRecord> Records => _records;
        private List<TargetRecord> _records = new List<TargetRecord>();
        ...
        /// </remarks>
        public List<TargetRecord>.Enumerator GetEnumerator() => _records.GetEnumerator();

        IEnumerator<TargetRecord> IEnumerable<TargetRecord>.GetEnumerator() => _records.GetEnumerator();

        System.Collections.IEnumerator System.Collections.IEnumerable.GetEnumerator()
            => _records.GetEnumerator();

        #region Indexing by 'Index'
        public Dictionary<int, TargetRecord> RecordsByIndex => _recordsByIndex;
        private Dictionary<int, TargetRecord> _recordsByIndex = new Dictionary<int, TargetRecord>();
        ...
        /// </remarks>
        public TargetRecord FindByIndex(int key)
            => _recordsByIndex.TryGetValue(key, out TargetRecord record) ? record : null;
        ...
        /// </remarks>
        public TargetRecord GetByIndexOrThrow(int key)
        {
            if (!_recordsByIndex.TryGetValue(key, out TargetRecord record))
                throw new TabbitException($"There is no record in table `Target` that corresponds to field `Index` value {key}");
        ...
        {
            private readonly List<TargetRecord> _rows;
            private int _at;

            internal EntryEnumerator(List<TargetRecord> rows)
            {
                _rows = rows;
                _at = -1;
            }
            ...
            public (int Key, TargetRecord Row) Current
        ...
        /// </remarks>
        public TargetRecord this[int key] => GetByIndexOrThrow(key);
        ...
        public Task ReadAsync(TcbReader reader)
        {
            ...
            var records = new List<TargetRecord>(count);
            ...
            foreach (var column in columns)
            {
                ...
                {
                    case 1:
                        TcbTable.CheckColumn(column, "Target.Index", TcbTable.KindScalar, false, TcbTable.ElementI32, TcbTable.ElementVarint);
                        cursor = new TcbColumnCursor(reader, column, count, "Target.Index");
                        for (int i = 0; i < count; )
                        {
                    ...
                    case 2:
                        TcbTable.CheckColumn(column, "Target.Name", TcbTable.KindScalar, false, TcbTable.ElementString);
                        cursor = new TcbColumnCursor(reader, column, count, "Target.Name");
                        for (int i = 0; i < count; )
                        {
                            // One call per run of equal values, not one per row - over a
                            // run-length encoded column this is most of the decode.
                    ...
                    case 3:
                        TcbTable.CheckColumn(column, "Target.Note", TcbTable.KindScalar, false, TcbTable.ElementString);
                        cursor = new TcbColumnCursor(reader, column, count, "Target.Note");
                        for (int i = 0; i < count; )
                        {
                            // One call per run of equal values, not one per row - over a
                            // run-length encoded column this is most of the decode.
            ...
            var recordsByIndex = new Dictionary<int, TargetRecord>(count);
