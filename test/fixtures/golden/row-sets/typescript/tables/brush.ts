...
// Generated from test/fixtures/xlsx/row-sets/row-sets.xlsx : Tables : J2
/** One set of rows only. */
export class BrushRecord {
  /** Default constructor */
  constructor() {
  }
  ...
  /** anything */
  public get bristles(): number { return this._bristles }

  public _index: number = 0
  public _width: number = 0
  public _bristles: number = 0

  /** Populate field values. */
  public populateFieldValues(dataRow: IDataRow): void {
    this._index = dataRow.index
    this._width = dataRow.width
    this._bristles = dataRow.bristles
  }
  ...
  public populateFieldValuesCompact(dataRow: any[]): void {
    ...
    this._width = dataRow[offset++]
    this._bristles = dataRow[offset++]
  }
}
...
export class BrushTable {
  ...
  /** All records. */
  public get records(): BrushRecord[] { return this._records }
  private _records: BrushRecord[] = []
   ...
   */
  public [Symbol.iterator](): IterableIterator<BrushRecord> {
    return this._records[Symbol.iterator]()
  }

  // Indexing by 'index'
  public get recordsByIndex(): Map<number, BrushRecord> { return this._recordsByIndex }
  private _recordsByIndex: Map<number, BrushRecord> = new Map<number, BrushRecord>()
   ...
   */
  public findByIndex(key: number): BrushRecord | undefined {
   ...
   */
  public getByIndexOrThrow(key: number): BrushRecord {
    const found = this._recordsByIndex.get(key)
    if (!found)
      throw new Error(`There is no record in table "Brush" that corresponds to field "index" value ${key}`)
   ...
   */
  public *entries(): IterableIterator<[number, BrushRecord]> {
  ...
  private readFromJson(json: string): void {
    ...
    const records: BrushRecord[] = []

    if (this.isCompactRowFormatted(dataRows)) {
      for (const dataRow of dataRows) {
        const record = new BrushRecord()
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          tabbit.checkColumn(column, 'Brush.Index', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Brush.Index')
          ...
          break
        case 2:
          tabbit.checkColumn(column, 'Brush.Width', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Brush.Width')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
              records[i]._width = value
          }
          break
        case 3:
          tabbit.checkColumn(column, 'Brush.Bristles', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Brush.Bristles')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
              records[i]._bristles = value
          }
          break
        default:
   ...
   */
  private publish(records: BrushRecord[]): void {
    const recordsByIndex = new Map<number, BrushRecord>()
