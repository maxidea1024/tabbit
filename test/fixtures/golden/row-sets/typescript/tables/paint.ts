...
import { ColourRecord } from './colour'
import { BrushRecord } from './brush'
...
export class PaintRecord {
  ...
  public populateFieldValues(dataRow: IDataRow): void {
    ...
    this._colourId_Colour_index = dataRow.colourId
    this._brushId_Brush_index = dataRow.brushId
  }
  ...
  public populateFieldValuesCompact(dataRow: any[]): void {
    ...
    this._colourId_Colour_index = dataRow[offset++]
    this._brushId_Brush_index = dataRow[offset++]
  }
}
...
export class PaintTable {
  ...
  /** All records. */
  public get records(): PaintRecord[] { return this._records }
  private _records: PaintRecord[] = []
   ...
   */
  public [Symbol.iterator](): IterableIterator<PaintRecord> {
    return this._records[Symbol.iterator]()
  }

  // Indexing by 'index'
  public get recordsByIndex(): Map<number, PaintRecord> { return this._recordsByIndex }
  private _recordsByIndex: Map<number, PaintRecord> = new Map<number, PaintRecord>()
   ...
   */
  public findByIndex(key: number): PaintRecord | undefined {
   ...
   */
  public getByIndexOrThrow(key: number): PaintRecord {
    const found = this._recordsByIndex.get(key)
    if (!found)
      throw new Error(`There is no record in table "Paint" that corresponds to field "index" value ${key}`)
   ...
   */
  public *entries(): IterableIterator<[number, PaintRecord]> {
      ...
      switch (column.tag) {
        ...
        case 2:
          ...
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
              records[i]._colourId_Colour_index = value
          }
          break
        case 3:
          ...
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
              records[i]._brushId_Brush_index = value
          }
          break
        default:
   ...
   */
  private publish(records: PaintRecord[]): void {
    const recordsByIndex = new Map<number, PaintRecord>()
