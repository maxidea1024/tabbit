...
// Generated from test/fixtures/xlsx/row-sets/row-sets.xlsx : Tables : B2
/** Given its rows twice. */
export class ColourRecord {
  /** Default constructor */
  constructor() {
  }
  ...
  /** anything */
  public get ordinal(): number { return this._ordinal }

  public _index: number = 0
  public _name: string = ''
  public _ordinal: number = 0

  /** Populate field values. */
  public populateFieldValues(dataRow: IDataRow): void {
    this._index = dataRow.index
    this._name = dataRow.name
    this._ordinal = dataRow.ordinal
  }
...
export class ColourTable {
  ...
  /** All records. */
  public get records(): ColourRecord[] { return this._records }
  private _records: ColourRecord[] = []
   ...
   */
  public [Symbol.iterator](): IterableIterator<ColourRecord> {
    return this._records[Symbol.iterator]()
  }

  // Indexing by 'index'
  public get recordsByIndex(): Map<number, ColourRecord> { return this._recordsByIndex }
  private _recordsByIndex: Map<number, ColourRecord> = new Map<number, ColourRecord>()
   ...
   */
  public findByIndex(key: number): ColourRecord | undefined {
   ...
   */
  public getByIndexOrThrow(key: number): ColourRecord {
    const found = this._recordsByIndex.get(key)
    if (!found)
      throw new Error(`There is no record in table "Colour" that corresponds to field "index" value ${key}`)
   ...
   */
  public *entries(): IterableIterator<[number, ColourRecord]> {
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          tabbit.checkColumn(column, 'Colour.Index', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Colour.Index')
          ...
          break
        case 2:
          tabbit.checkColumn(column, 'Colour.Name', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_STRING])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Colour.Name')
          for (let i = 0; i < rowCount; ) {
          ...
          break
        case 3:
          tabbit.checkColumn(column, 'Colour.Ordinal', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Colour.Ordinal')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
              records[i]._ordinal = value
          }
          break
        default:
   ...
   */
  private publish(records: ColourRecord[]): void {
    const recordsByIndex = new Map<number, ColourRecord>()
