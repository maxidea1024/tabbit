...
#include "tables/RecordTrim_Loot.h"

#include <string.h>

static bool RecordTrim_LootParse(RecordTrim_LootTable_t* table, tb_reader* reader) {
  int32_t row;
  int32_t at;
  ...
  if (table->count > 0) {
    table->records = (RecordTrim_LootRecord_t*)tb_arena_alloc(
  ...
  for (at = 0; at < column_count && !tb_failed(reader); ++at) {
    ...
    case 3:
      ...
      for (row = 0; row < table->count && !tb_failed(reader); ++row) {
        ...
        record->slot = (struct RecordTrim_LootRecord_t_slot_entry*)tb_arena_alloc(
...
bool RecordTrim_LootLoad(RecordTrim_LootTable_t* table, const char* filename,
  ...
  /* Opened unconditionally, with whatever RecordTrim_EncryptionKey and
   * RecordTrim_MacKey hold - NULL unless the project set them. A file that is
  ...
  if (!tb_open(bytes, length, RecordTrim_EncryptionKey,
        RecordTrim_EncryptionKeyLength, RecordTrim_MacKey,
        RecordTrim_MacKeyLength, RecordTrim_VerifyMac,
