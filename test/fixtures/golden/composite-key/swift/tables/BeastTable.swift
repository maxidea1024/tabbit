...
/// Every row of Beast.
public final class BeastTable: Sequence {
    ...
    public func makeIterator() -> Array<BeastRecord>.Iterator {
    ...
    public private(set) var records: [BeastRecord] = []

    private var byIndex: [String: BeastRecord] = [:]

    /// The row with this Index, or nil when the table has none.
    ///
    /// The lookup to reach for when a missing row is an ordinary answer - an optional
    /// reference, a key that came from user input.
    public func findByIndex(_ key: String) -> BeastRecord? {
        byIndex[key]
    }
    ...
    public func getByIndexOrThrow(_ key: String) throws -> BeastRecord {
    ...
    public subscript(_ key: String) -> BeastRecord? {
        findByIndex(key)
    }
    ...
    /// row with.
    public var entries: LazyMapSequence<[BeastRecord], (key: String, row: BeastRecord)> {
    ...
    public func read(
        ...
        var loaded = [BeastRecord]()
        loaded.reserveCapacity(count)

        for _ in 0 ..< count { loaded.append(BeastRecord()) }
        var loadedByIndex = [String: BeastRecord](
        ...
        for column in header.columns {
            ...
            switch column.tag {
            case 1:
                try Tcb.checkColumn(column, "Beast.Index", Tcb.kindScalar, false, Tcb.elementString)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Beast.Index")
                var at = 0
                while at < count {
            ...
            case 2:
                try Tcb.checkColumn(column, "Beast.Name", Tcb.kindScalar, false, Tcb.elementString)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Beast.Name")
                var at = 0
                while at < count {
                    var n = try cursor.nextSameString(count - at)
                    while n > 0 {
            ...
            case 3:
                try Tcb.checkColumn(column, "Beast.Pad1", Tcb.kindScalar, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Beast.Pad1")
                var at = 0
                while at < count {
