...
/// Every row of Move.
public final class MoveTable: Sequence {
    ...
    public func makeIterator() -> Array<MoveRecord>.Iterator {
    ...
    private var byIndex: [Int32: MoveRecord] = [:]

    /// The row with this Index, or nil when the table has none.
    ///
    /// The lookup to reach for when a missing row is an ordinary answer - an optional
    /// reference, a key that came from user input.
    public func findByIndex(_ key: Int32) -> MoveRecord? {
        byIndex[key]
    }
    ...
    public func getByIndexOrThrow(_ key: Int32) throws -> MoveRecord {
        guard let record = byIndex[key] else {
            throw RecordNotFoundError(
                "there is no record in table `Move` that corresponds to "
    ...
    public subscript(_ key: Int32) -> MoveRecord? {
        findByIndex(key)
    }
    ...
    /// row with.
    public var entries: LazyMapSequence<[MoveRecord], (key: Int32, row: MoveRecord)> {
    ...
    public func read(
        ...
        var loaded = [MoveRecord]()
        loaded.reserveCapacity(count)

        for _ in 0 ..< count { loaded.append(MoveRecord()) }
        var loadedByIndex = [Int32: MoveRecord](
        ...
        for column in header.columns {
            ...
            switch column.tag {
            case 1:
                try Tcb.checkColumn(column, "Move.Index", Tcb.kindScalar, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Move.Index")
                var at = 0
                while at < count {
            ...
            case 2:
                try Tcb.checkColumn(column, "Move.Name", Tcb.kindScalar, false, Tcb.elementString)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Move.Name")
                var at = 0
                while at < count {
            ...
            case 3:
                try Tcb.checkColumn(column, "Move.Pad1", Tcb.kindScalar, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Move.Pad1")
                var at = 0
                while at < count {
                    var n = try cursor.nextSameI32(count - at)
                    while n > 0 {
