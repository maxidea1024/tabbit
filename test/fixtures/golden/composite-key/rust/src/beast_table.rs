...
/// Every row of Beast.
#[derive(Clone, Debug, Default)]
pub struct BeastTable {
    records: Vec<BeastRecord>,
    by_index: HashMap<String, usize>,
}

impl BeastTable {
    /// Every row, in the order the sheet declared them.
    pub fn records(&self) -> &[BeastRecord] {
        &self.records
    }
    ...
    pub fn iter(&self) -> std::slice::Iter<'_, BeastRecord> {
        self.records.iter()
    }
    ...
    pub fn find_by_index(&self, key: &str) -> Option<&BeastRecord> {
    ...
    /// error it returns.
    pub fn get_by_index_or_error(&self, key: &str) -> tabbit::Result<&BeastRecord> {
        self.by_index
            .get(key)
    ...
    pub fn entries(&self) -> impl Iterator<Item = (&String, &BeastRecord)> + '_ {
    ...
    pub fn read(&mut self, filename: &Path) -> tabbit::Result<()> {
        ...
        let mut records = vec![BeastRecord::default(); count];
        ...
        for column in &header.columns {
            ...
            match column.tag {
                1 => {
                    tabbit::check_column(column, "Beast.Index", tabbit::KIND_SCALAR, false, &[tabbit::ELEMENT_STRING])?;
                    let mut cursor = tabbit::TcbColumnCursor::new(&mut reader, column, header.row_count, "Beast.Index")?;
                    let mut at = 0usize;
                ...
                2 => {
                    tabbit::check_column(column, "Beast.Name", tabbit::KIND_SCALAR, false, &[tabbit::ELEMENT_STRING])?;
                    let mut cursor = tabbit::TcbColumnCursor::new(&mut reader, column, header.row_count, "Beast.Name")?;
                    let mut at = 0usize;
                    while at < records.len() {
                        let (n, value) = cursor.next_same_string((records.len() - at) as i32)?;
                        for _ in 0..n {
                ...
                3 => {
                    tabbit::check_column(column, "Beast.Pad1", tabbit::KIND_SCALAR, false, &[tabbit::ELEMENT_I32, tabbit::ELEMENT_VARINT])?;
                    let mut cursor = tabbit::TcbColumnCursor::new(&mut reader, column, header.row_count, "Beast.Pad1")?;
                    let mut at = 0usize;
                    while at < records.len() {
...
impl<'a> IntoIterator for &'a BeastTable {
    type Item = &'a BeastRecord;
    type IntoIter = std::slice::Iter<'a, BeastRecord>;
