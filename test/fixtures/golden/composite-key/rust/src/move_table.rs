...
impl MoveTable {
    ...
    pub fn find_by_index(&self, key: i32) -> Option<&MoveRecord> {
    ...
    /// error it returns.
    pub fn get_by_index_or_error(&self, key: i32) -> tabbit::Result<&MoveRecord> {
        self.by_index
            .get(&key)
    ...
    pub fn entries(&self) -> impl Iterator<Item = (&i32, &MoveRecord)> + '_ {
    ...
    pub fn read(&mut self, filename: &Path) -> tabbit::Result<()> {
        ...
        let mut records = vec![MoveRecord::default(); count];
        ...
        for column in &header.columns {
            ...
            match column.tag {
                1 => {
                    tabbit::check_column(column, "Move.Index", tabbit::KIND_SCALAR, false, &[tabbit::ELEMENT_I32, tabbit::ELEMENT_VARINT])?;
                    let mut cursor = tabbit::TcbColumnCursor::new(&mut reader, column, header.row_count, "Move.Index")?;
                    let mut at = 0usize;
                ...
                2 => {
                    tabbit::check_column(column, "Move.Name", tabbit::KIND_SCALAR, false, &[tabbit::ELEMENT_STRING])?;
                    let mut cursor = tabbit::TcbColumnCursor::new(&mut reader, column, header.row_count, "Move.Name")?;
                    let mut at = 0usize;
                    while at < records.len() {
                ...
                3 => {
                    tabbit::check_column(column, "Move.Pad1", tabbit::KIND_SCALAR, false, &[tabbit::ELEMENT_I32, tabbit::ELEMENT_VARINT])?;
                    let mut cursor = tabbit::TcbColumnCursor::new(&mut reader, column, header.row_count, "Move.Pad1")?;
                    let mut at = 0usize;
                    while at < records.len() {
                        let (n, value) = cursor.next_same_i32((records.len() - at) as i32)?;
                        for _ in 0..n {
...
impl<'a> IntoIterator for &'a MoveTable {
    ...
    type IntoIter = std::slice::Iter<'a, MoveRecord>;
