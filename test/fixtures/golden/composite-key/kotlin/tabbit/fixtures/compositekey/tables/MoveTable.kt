...
/** Every row of Move. */
class MoveTable : Iterable<MoveRecord> {
     ...
     */
    var records: MutableList<MoveRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<MoveRecord> = records.iterator()

    private var byIndex: HashMap<Int, MoveRecord> = HashMap()
     ...
     */
    fun findByIndex(key: Int): MoveRecord? = byIndex[key]
     ...
     */
    fun getByIndexOrThrow(key: Int): MoveRecord =
     ...
     */
    operator fun get(key: Int): MoveRecord? = findByIndex(key)
     ...
     */
    val entries: Sequence<Pair<Int, MoveRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<MoveRecord>(count)
        val loadedByIndex = HashMap<Int, MoveRecord>(count * 2)

        repeat(count) { loaded.add(MoveRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "Move.Index", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Move.Index")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "Move.Name", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "Move.Name")
                    var at = 0
                    while (at < count) {
                ...
                3 -> {
                    checkColumn(column, "Move.Pad1", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Move.Pad1")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
