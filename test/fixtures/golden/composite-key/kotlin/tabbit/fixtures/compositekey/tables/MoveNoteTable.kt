...
class MoveNoteTable : Iterable<MoveNoteRecord> {
     ...
     */
    var records: MutableList<MoveNoteRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<MoveNoteRecord> = records.iterator()

    private var byMoveId: HashMap<Int, MoveNoteRecord> = HashMap()
     ...
     */
    fun findByMoveId(key: Int): MoveNoteRecord? = byMoveId[key]
     ...
     */
    fun getByMoveIdOrThrow(key: Int): MoveNoteRecord =
        byMoveId[key] ?: throw RecordNotFoundException(
    ...
    fun containsMoveId(key: Int): Boolean = byMoveId.containsKey(key)


    /**
     * The row with this MoveId, or null when the table has none.
     *
     * findByMoveId under the spelling Kotlin uses for a keyed collection. It
     ...
     */
    operator fun get(key: Int): MoveNoteRecord? = findByMoveId(key)
     ...
     */
    val entries: Sequence<Pair<Int, MoveNoteRecord>>
        get() = records.asSequence().map { record -> record.moveId to record }
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<MoveNoteRecord>(count)
        val loadedByMoveId = HashMap<Int, MoveNoteRecord>(count * 2)

        repeat(count) { loaded.add(MoveNoteRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "MoveNote.MoveId", KIND_SCALAR, false, ELEMENT_I32)
                    val cursor = ColumnCursor(reader, column, count, "MoveNote.MoveId")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "MoveNote.Note", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "MoveNote.Note")
                    var at = 0
                    while (at < count) {
                ...
                3 -> {
                    checkColumn(column, "MoveNote.Pad1", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "MoveNote.Pad1")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
