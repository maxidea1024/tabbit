...
/** Every row of Beast. */
class BeastTable : Iterable<BeastRecord> {
     ...
     */
    var records: MutableList<BeastRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<BeastRecord> = records.iterator()

    private var byIndex: HashMap<String, BeastRecord> = HashMap()
     ...
     */
    fun findByIndex(key: String): BeastRecord? = byIndex[key]
     ...
     */
    fun getByIndexOrThrow(key: String): BeastRecord =
     ...
     */
    operator fun get(key: String): BeastRecord? = findByIndex(key)
     ...
     */
    val entries: Sequence<Pair<String, BeastRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<BeastRecord>(count)
        val loadedByIndex = HashMap<String, BeastRecord>(count * 2)

        repeat(count) { loaded.add(BeastRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "Beast.Index", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "Beast.Index")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "Beast.Name", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "Beast.Name")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameString(count - at)
                        while (n > 0) {
                ...
                3 -> {
                    checkColumn(column, "Beast.Pad1", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Beast.Pad1")
                    var at = 0
                    while (at < count) {
