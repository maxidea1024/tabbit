...
/// Every row of Move.
class MoveTable extends Iterable<MoveRecord> {
  ...
  @override
  Iterator<MoveRecord> get iterator => _records.iterator;

  Map<int, MoveRecord> _byIndex = {};

  /// The row with this Index, or null when the table has none.
  ///
  /// The lookup to reach for when a missing row is an ordinary answer - an optional
  /// reference, a key that came from user input.
  MoveRecord? findByIndex(int key) => _byIndex[key];
  ...
  MoveRecord getByIndexOrThrow(int key) {
  ...
  MoveRecord? operator [](int key) => findByIndex(key);
  ...
  Iterable<MapEntry<int, MoveRecord>> get entries =>
  ...
  void read(String filename) {
    ...
    final loaded = <MoveRecord>[];
    final loadedByIndex = <int, MoveRecord>{};

    for (var i = 0; i < count; i++) {
      loaded.add(MoveRecord());
    }
    ...
    for (final column in header.columns) {
      ...
      switch (column.tag) {
        case 1:
          checkColumn(column, 'Move.Index', kindScalar, false, [elementI32, elementVarint]);
          cursor = TcbColumnCursor(reader, column, count, 'Move.Index');
          for (var i = 0; i < count; ) {
          ...
          break;
        case 2:
          checkColumn(column, 'Move.Name', kindScalar, false, [elementString]);
          cursor = TcbColumnCursor(reader, column, count, 'Move.Name');
          for (var i = 0; i < count; ) {
          ...
          break;
        case 3:
          checkColumn(column, 'Move.Pad1', kindScalar, false, [elementI32, elementVarint]);
          cursor = TcbColumnCursor(reader, column, count, 'Move.Pad1');
          for (var i = 0; i < count; ) {
            final (n, value) = cursor.nextSameI32(count - i);
            for (var left = n; left > 0; left--, i++) {
              loaded[i].pad1 = value;
