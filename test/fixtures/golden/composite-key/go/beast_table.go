...
func (t *BeastTable) Count() int { return len(t.records) }
...
func (t *BeastTable) All() iter.Seq[*BeastRecord] {
	records := t.records

	return func(yield func(*BeastRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *BeastTable) Entries() iter.Seq2[string, *BeastRecord] {
	records := t.records

	return func(yield func(string, *BeastRecord) bool) {
		for i := range records {
