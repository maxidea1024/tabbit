...
// BeastNoteRecord was generated from test/fixtures/xlsx/composite-key/composite-key.xlsx : Link : T2.
// Indexed by a reference to a string-keyed table, named with `key=`.
type BeastNoteRecord struct {
...
func (t *BeastNoteTable) Records() []BeastNoteRecord { return t.records }
...
func (t *BeastNoteTable) All() iter.Seq[*BeastNoteRecord] {
	records := t.records

	return func(yield func(*BeastNoteRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
// FindByBeastId returns the row with this BeastId, or nil when the table has none.
//
// The lookup to reach for when a missing row is an ordinary answer - an optional
// reference, a key that came from user input.
func (t *BeastNoteTable) FindByBeastId(key string) *BeastNoteRecord {
	position, found := t.byBeastId[key]
	if !found {
		return nil
	}
...
// GetByBeastIdOrError returns the row with this BeastId, or an error naming
// what was missing.
//
...
func (t *BeastNoteTable) GetByBeastIdOrError(key string) (*BeastNoteRecord, error) {
	position, found := t.byBeastId[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `BeastNote` that corresponds to field `BeastId` value %v", key)
	}

	return &t.records[position], nil
}

// ContainsBeastId reports whether the table holds a row with this BeastId.
func (t *BeastNoteTable) ContainsBeastId(key string) bool {
...
func (t *BeastNoteTable) Entries() iter.Seq2[string, *BeastNoteRecord] {
	records := t.records

	return func(yield func(string, *BeastNoteRecord) bool) {
		for i := range records {
			if !yield(records[i].BeastId, &records[i]) {
				return
			}
		}
	}
...
func (t *BeastNoteTable) Read(filename string) error {
	...
	records := make([]BeastNoteRecord, count)
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "BeastNote.Seq", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "BeastNote.Seq")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "BeastNote.BeastId", tabbit.KindScalar, false, tabbit.ElementString) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "BeastNote.BeastId")
				for i := int32(0); i < count; {
		...
		case 3:
			if tabbit.CheckColumn(reader, column, "BeastNote.Note", tabbit.KindScalar, false, tabbit.ElementString) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "BeastNote.Note")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameString(count - i)
					for ; n > 0; n-- {
	...
	if err := reader.Err(); err != nil {
		return fmt.Errorf("BeastNote: %w", err)
	}

	byBeastId := make(map[string]int, len(records))
	for i := range records {
		byBeastId[records[i].BeastId] = i
	}

	// Published, now that every column read and the indexes are built.
	t.records = records
	t.byBeastId = byBeastId
