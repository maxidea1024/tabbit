...
// MoveRecord was generated from test/fixtures/xlsx/composite-key/composite-key.xlsx : Link : H2.
...
// MoveTable holds every row of Move.
...
func (t *MoveTable) Records() []MoveRecord { return t.records }

// Count returns how many rows the table holds.
func (t *MoveTable) Count() int { return len(t.records) }
...
func (t *MoveTable) All() iter.Seq[*MoveRecord] {
	records := t.records
...
func (t *MoveTable) FindByIndex(key int32) *MoveRecord {
...
func (t *MoveTable) GetByIndexOrError(key int32) (*MoveRecord, error) {
	position, found := t.byIndex[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `Move` that corresponds to field `Index` value %v", key)
	}
...
func (t *MoveTable) ContainsIndex(key int32) bool {
...
func (t *MoveTable) Entries() iter.Seq2[int32, *MoveRecord] {
	records := t.records

	return func(yield func(int32, *MoveRecord) bool) {
		for i := range records {
...
func (t *MoveTable) Read(filename string) error {
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "Move.Index", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Move.Index")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "Move.Name", tabbit.KindScalar, false, tabbit.ElementString) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Move.Name")
				for i := int32(0); i < count; {
		...
		case 3:
			if tabbit.CheckColumn(reader, column, "Move.Pad1", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Move.Pad1")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
