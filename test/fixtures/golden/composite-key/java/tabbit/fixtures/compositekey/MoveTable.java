...
/** Every row of Move. */
public final class MoveTable implements Iterable<MoveRecord> {
    private List<MoveRecord> records = new ArrayList<>();
    private Map<Integer, MoveRecord> byIndex = new HashMap<>();
    ...
    public MoveRecord getByIndexOrThrow(int key) {
        MoveRecord record = byIndex.get(key);

        if (record == null) {
            throw new TcbReader.RecordNotFoundException(
                "there is no record in table `Move` that corresponds to field "
                + "`Index` value " + key);
        }
     ...
     */
    public Iterable<Map.Entry<Integer, MoveRecord>> entries() {
        final List<MoveRecord> rows = records;

        return () -> new Iterator<Map.Entry<Integer, MoveRecord>>() {
            private final Iterator<MoveRecord> source = rows.iterator();
            ...
            @Override
            public Map.Entry<Integer, MoveRecord> next() {
                final MoveRecord row = source.next();
    ...
    public void read(Path filename) {
        ...
        List<MoveRecord> loaded = new ArrayList<>(count);
        Map<Integer, MoveRecord> loadedByIndex = new HashMap<>(count * 2);

        for (int i = 0; i < count; i++) {
            loaded.add(new MoveRecord());
        }
        ...
        for (TcbReader.Column column : header.columns) {
            ...
            switch (column.tag) {
                case 1: {
                    TcbReader.checkColumn(column, "Move.Index", TcbReader.KIND_SCALAR, false, TcbReader.ELEMENT_I32, TcbReader.ELEMENT_VARINT);
                    cursor = new TcbReader.ColumnCursor(reader, column, count, "Move.Index");
                    for (int i = 0; i < count; ) {
                ...
                case 2: {
                    TcbReader.checkColumn(column, "Move.Name", TcbReader.KIND_SCALAR, false, TcbReader.ELEMENT_STRING);
                    cursor = new TcbReader.ColumnCursor(reader, column, count, "Move.Name");
                    for (int i = 0; i < count; ) {
                ...
                case 3: {
                    TcbReader.checkColumn(column, "Move.Pad1", TcbReader.KIND_SCALAR, false, TcbReader.ELEMENT_I32, TcbReader.ELEMENT_VARINT);
                    cursor = new TcbReader.ColumnCursor(reader, column, count, "Move.Pad1");
                    for (int i = 0; i < count; ) {
                        int n = cursor.nextSameI32(count - i);
                        for (; n > 0; n--, i++) {
