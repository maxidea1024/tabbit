...
---@field name string
---@field pad1 integer
local MoveRecordMeta = tcb.strictType("a `Move` row", { "index", "name", "pad1" })

---@return MoveRecord
local function newMoveRecord()
  return setmetatable({
    index = 0,
    name = "",
    pad1 = 0,
...
---@class MoveTable
---@field records MoveRecord[]
local MoveTable = {}

local MoveTableMeta = tcb.strictInstance("the `Move` table", MoveTable, { "records", "byIndex" })

function MoveTable.new()
  return setmetatable({
    records = {},
    byIndex = {},
  }, MoveTableMeta)
...
function MoveTable:containsIndex(key)
...
function MoveTable:readBytes(data)
  ...
  for c = 1, #columns do
    ...
    if column.tag == 1 then
      tcb.checkColumn(column, "Move.Index", tcb.KIND_SCALAR, false, { tcb.ELEMENT_I32, tcb.ELEMENT_VARINT })
      local cursor = tcb.newCursor(reader, column, count, "Move.Index")
      local at = 0
    ...
    elseif column.tag == 2 then
      tcb.checkColumn(column, "Move.Name", tcb.KIND_SCALAR, false, { tcb.ELEMENT_STRING })
      local cursor = tcb.newCursor(reader, column, count, "Move.Name")
      local at = 0
    ...
    elseif column.tag == 3 then
      tcb.checkColumn(column, "Move.Pad1", tcb.KIND_SCALAR, false, { tcb.ELEMENT_I32, tcb.ELEMENT_VARINT })
      local cursor = tcb.newCursor(reader, column, count, "Move.Pad1")
      local at = 0
...
function MoveTable:read(filename)
