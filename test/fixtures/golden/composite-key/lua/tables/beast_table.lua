...
---@field name string
---@field pad1 integer
local BeastRecordMeta = tcb.strictType("a `Beast` row", { "index", "name", "pad1" })

---@return BeastRecord
local function newBeastRecord()
  return setmetatable({
    index = "",
    name = "",
    pad1 = 0,
...
---@class BeastTable
---@field records BeastRecord[]
local BeastTable = {}

local BeastTableMeta = tcb.strictInstance("the `Beast` table", BeastTable, { "records", "byIndex" })

function BeastTable.new()
  return setmetatable({
    records = {},
    byIndex = {},
  }, BeastTableMeta)
...
function BeastTable:containsIndex(key)
...
function BeastTable:readBytes(data)
  ...
  for c = 1, #columns do
    ...
    if column.tag == 1 then
      tcb.checkColumn(column, "Beast.Index", tcb.KIND_SCALAR, false, { tcb.ELEMENT_STRING })
      local cursor = tcb.newCursor(reader, column, count, "Beast.Index")
      local at = 0
    ...
    elseif column.tag == 2 then
      tcb.checkColumn(column, "Beast.Name", tcb.KIND_SCALAR, false, { tcb.ELEMENT_STRING })
      local cursor = tcb.newCursor(reader, column, count, "Beast.Name")
      local at = 0
    ...
    elseif column.tag == 3 then
      tcb.checkColumn(column, "Beast.Pad1", tcb.KIND_SCALAR, false, { tcb.ELEMENT_I32, tcb.ELEMENT_VARINT })
      local cursor = tcb.newCursor(reader, column, count, "Beast.Pad1")
      local at = 0
...
function BeastTable:read(filename)
