...
// Generated from test/fixtures/xlsx/composite-key/composite-key.xlsx : Link : Z2
/** First column is the index and it is a reference to an int-keyed table. */
export class MoveNoteTable {
  /** Default constructor. */
  constructor() {
  }

  /** All records. */
  public get records(): MoveNoteRecord[] { return this._records }
  private _records: MoveNoteRecord[] = []
   ...
   */
  public [Symbol.iterator](): IterableIterator<MoveNoteRecord> {
  ...
  private readFromJson(json: string): void {
    ...
    const records: MoveNoteRecord[] = []

    if (this.isCompactRowFormatted(dataRows)) {
      for (const dataRow of dataRows) {
        const record = new MoveNoteRecord()
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (let i = 0; i < rowCount; ++i)
      records.push(new MoveNoteRecord())
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          tabbit.checkColumn(column, 'MoveNote.MoveId', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'MoveNote.MoveId')
          ...
          break
        case 2:
          tabbit.checkColumn(column, 'MoveNote.Note', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_STRING])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'MoveNote.Note')
          for (let i = 0; i < rowCount; ) {
          ...
          break
        case 3:
          tabbit.checkColumn(column, 'MoveNote.Pad1', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'MoveNote.Pad1')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
  ...
  private publish(records: MoveNoteRecord[]): void {
    ...
    this._recordsByMoveId = recordsByMoveId
  }
}
