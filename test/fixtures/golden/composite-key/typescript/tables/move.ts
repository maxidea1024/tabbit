...
export class MoveTable {
  ...
  /** All records. */
  public get records(): MoveRecord[] { return this._records }
  private _records: MoveRecord[] = []
   ...
   */
  public [Symbol.iterator](): IterableIterator<MoveRecord> {
    return this._records[Symbol.iterator]()
  }

  // Indexing by 'index'
  public get recordsByIndex(): Map<number, MoveRecord> { return this._recordsByIndex }
  private _recordsByIndex: Map<number, MoveRecord> = new Map<number, MoveRecord>()
   ...
   */
  public findByIndex(key: number): MoveRecord | undefined {
   ...
   */
  public getByIndexOrThrow(key: number): MoveRecord {
    const found = this._recordsByIndex.get(key)
    if (!found)
      throw new Error(`There is no record in table "Move" that corresponds to field "index" value ${key}`)
   ...
   */
  public *entries(): IterableIterator<[number, MoveRecord]> {
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          tabbit.checkColumn(column, 'Move.Index', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Move.Index')
          ...
          break
        case 2:
          tabbit.checkColumn(column, 'Move.Name', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_STRING])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Move.Name')
          for (let i = 0; i < rowCount; ) {
          ...
          break
        case 3:
          tabbit.checkColumn(column, 'Move.Pad1', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Move.Pad1')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
   ...
   */
  private publish(records: MoveRecord[]): void {
    const recordsByIndex = new Map<number, MoveRecord>()
