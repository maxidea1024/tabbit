...
class BeastRecord:
    ...
    def __repr__(self):
        return "BeastRecord(index=%r, name=%r, pad1=%r)" % (self.index, self.name, self.pad1)
...
class BeastTable:
    ...
    def read(self, filename):
        ...
        records = [BeastRecord() for _ in range(count)]

        for column in columns:
            block_end = reader.position + column.byte_length
            if column.tag == 1:
                tabbit.check_column(column, "Beast.Index", tabbit.KIND_SCALAR, False, (tabbit.ELEMENT_STRING,))
                cursor = tabbit.ColumnCursor(reader, column, count, "Beast.Index")
                at = 0
                while at < count:
                ...
                cursor = tabbit.ColumnCursor(reader, column, count, "Beast.Name")
                at = 0
                while at < count:
                    n, value = cursor.next_same_string(count - at)
                    for i in range(at, at + n):
                ...
                cursor = tabbit.ColumnCursor(reader, column, count, "Beast.Pad1")
                at = 0
                while at < count:
