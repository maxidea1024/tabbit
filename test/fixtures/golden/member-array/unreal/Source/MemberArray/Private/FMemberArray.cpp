...
#include "FMemberArray.h"

#include "Misc/FileHelper.h"
#include "Misc/Paths.h"


const FGuideRow* FGuideTable::FindByIndex(int32 Key) const
{
    const int32* Position = ByIndex.Find(Key);
    return Position != nullptr ? &RecordsStorage[*Position] : nullptr;
}

bool FGuideTable::ContainsIndex(int32 Key) const
{
    return ByIndex.Contains(Key);
}

bool FGuideTable::Read(const FString& Filename)
{
    TArray<uint8> Buffer;
    ...
    // Opened unconditionally, with whatever FMemberArray::EncryptionKey and
    // FMemberArray::MacKey hold - empty unless the project set them. A file that is
    ...
    if (!Tabbit::Open(Buffer, FMemberArray::EncryptionKey, Bytes, OpenError,
            FMemberArray::MacKey, FMemberArray::bVerifyMac))
    {
...
FGuideTable FMemberArray::GuideStorage;

TArray<uint8> FMemberArray::EncryptionKey;
TArray<uint8> FMemberArray::MacKey;
bool FMemberArray::bVerifyMac = true;



FGuideRow UMemberArrayLibrary::GetGuideRow(int32 Key, bool& bFound)
{
    const FGuideRow* Found = FMemberArray::Guide().FindByIndex(Key);
    ...
    return Found != nullptr ? *Found : FGuideRow();
}

int32 UMemberArrayLibrary::GetGuideRowCount()
{
    return FMemberArray::Guide().Records().Num();
}

FGuideRow UMemberArrayLibrary::GetGuideRowAt(int32 Position, bool& bFound)
{
    const TArray<FGuideRow>& Rows = FMemberArray::Guide().Records();

    bFound = Rows.IsValidIndex(Position);

    return bFound ? Rows[Position] : FGuideRow();
}


bool UMemberArrayLibrary::ReadAll(const FString& BasePath, const FString& FileExtension)
{
    return FMemberArray::ReadAll(BasePath, FileExtension);
}

bool FMemberArray::ReadAll(const FString& BasePath, const FString& FileExtension)
{
    ...
    FGuideTable LoadedGuide;

    if (!LoadedGuide.Read(FPaths::Combine(BasePath, TEXT("Guide") + FileExtension)))
    {
        return false;
    }

    GuideStorage = MoveTemp(LoadedGuide);
