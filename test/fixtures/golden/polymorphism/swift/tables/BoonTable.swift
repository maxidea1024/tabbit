...
/// Every row of Boon.
public final class BoonTable: Sequence {
    ...
    public func makeIterator() -> Array<BoonRecord>.Iterator {
    ...
    public private(set) var records: [BoonRecord] = []

    private var byIndex: [Int32: BoonRecord] = [:]

    /// The row with this Index, or nil when the table has none.
    ///
    /// The lookup to reach for when a missing row is an ordinary answer - an optional
    /// reference, a key that came from user input.
    public func findByIndex(_ key: Int32) -> BoonRecord? {
        byIndex[key]
    }
    ...
    public func getByIndexOrThrow(_ key: Int32) throws -> BoonRecord {
    ...
    public subscript(_ key: Int32) -> BoonRecord? {
        findByIndex(key)
    }
    ...
    /// row with.
    public var entries: LazyMapSequence<[BoonRecord], (key: Int32, row: BoonRecord)> {
    ...
    public func read(
        ...
        var loaded = [BoonRecord]()
        loaded.reserveCapacity(count)

        for _ in 0 ..< count { loaded.append(BoonRecord()) }
        var loadedByIndex = [Int32: BoonRecord](
        ...
        for column in header.columns {
            ...
            switch column.tag {
            case 1:
                try Tcb.checkColumn(column, "Boon.Index", Tcb.kindScalar, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Boon.Index")
                var at = 0
                while at < count {
            ...
            case 2:
                try Tcb.checkColumn(column, "Boon.Name", Tcb.kindScalar, false, Tcb.elementString)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Boon.Name")
                var at = 0
                while at < count {
            ...
            case 3:
                try Tcb.checkColumn(column, "Boon.Effect.Type", Tcb.kindScalar, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Boon.Effect.Type")
                var at = 0
                while at < count {
                    var n = try cursor.nextSameI32(count - at)
                    while n > 0 {
            ...
            case 4:
                try Tcb.checkColumn(column, "Boon.Effect.Chance", Tcb.kindScalar, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Boon.Effect.Chance")
                var at = 0
                while at < count {
                    var n = try cursor.nextSameI32(count - at)
                    while n > 0 {
            ...
            case 5:
                try Tcb.checkColumn(column, "Boon.Effect.Damage", Tcb.kindScalar, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Boon.Effect.Damage")
                var at = 0
                while at < count {
                    var n = try cursor.nextSameI32(count - at)
                    while n > 0 {
            ...
            case 6:
                try Tcb.checkColumn(column, "Boon.Effect.Pierces", Tcb.kindScalar, false, Tcb.elementBool)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Boon.Effect.Pierces")
                for record in loaded {
                    record.effect.pierces = try cursor.nextBool()
                }
            case 7:
                try Tcb.checkColumn(column, "Boon.Effect.Amount", Tcb.kindScalar, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Boon.Effect.Amount")
                var at = 0
                while at < count {
                    var n = try cursor.nextSameI32(count - at)
                    while n > 0 {
            ...
            case 8:
                try Tcb.checkColumn(column, "Boon.Effect.ElementId", Tcb.kindScalar, false, Tcb.elementI32)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Boon.Effect.ElementId")
                var at = 0
                while at < count {
                    var n = try cursor.nextSameI32(count - at)
                    while n > 0 {
            ...
            case 9:
                try Tcb.checkColumn(column, "Boon.Effect.Band", Tcb.kindScalar, false, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Boon.Effect.Band")
                var at = 0
                while at < count {
                    var n = try cursor.nextSameI32(count - at)
                    while n > 0 {
