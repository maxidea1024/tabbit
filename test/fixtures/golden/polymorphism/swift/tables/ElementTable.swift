...
public final class ElementTable: Sequence {
    ...
    public private(set) var records: [ElementRecord] = []

    private var byCode: [Int32: ElementRecord] = [:]

    /// The row with this Code, or nil when the table has none.
    ///
    /// The lookup to reach for when a missing row is an ordinary answer - an optional
    /// reference, a key that came from user input.
    public func findByCode(_ key: Int32) -> ElementRecord? {
        byCode[key]
    }
    ...
    public func getByCodeOrThrow(_ key: Int32) throws -> ElementRecord {
    ...
    public func read(
        ...
        var loaded = [ElementRecord]()
        loaded.reserveCapacity(count)

        for _ in 0 ..< count { loaded.append(ElementRecord()) }
        var loadedByCode = [Int32: ElementRecord](
        ...
        for column in header.columns {
            ...
            switch column.tag {
            case 1:
                try Tcb.checkColumn(column, "Element.Code", Tcb.kindScalar, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Element.Code")
                var at = 0
                while at < count {
                    var n = try cursor.nextSameI32(count - at)
                    while n > 0 {
                        let i = at
                        loaded[i].code = cursor.runSameValue
                        at += 1
                        n -= 1
                    }
                }
            case 2:
                try Tcb.checkColumn(column, "Element.Name", Tcb.kindScalar, false, Tcb.elementString)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Element.Name")
                var at = 0
                while at < count {
