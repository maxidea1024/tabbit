...
class ElementTable : Iterable<ElementRecord> {
     ...
     */
    var records: MutableList<ElementRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<ElementRecord> = records.iterator()

    private var byCode: HashMap<Int, ElementRecord> = HashMap()
     ...
     */
    fun findByCode(key: Int): ElementRecord? = byCode[key]
     ...
     */
    fun getByCodeOrThrow(key: Int): ElementRecord =
    ...
    fun containsCode(key: Int): Boolean = byCode.containsKey(key)


    /**
     * The row with this Code, or null when the table has none.
     *
     * findByCode under the spelling Kotlin uses for a keyed collection. It
     ...
     */
    operator fun get(key: Int): ElementRecord? = findByCode(key)
     ...
     */
    val entries: Sequence<Pair<Int, ElementRecord>>
        get() = records.asSequence().map { record -> record.code to record }
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<ElementRecord>(count)
        val loadedByCode = HashMap<Int, ElementRecord>(count * 2)

        repeat(count) { loaded.add(ElementRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "Element.Code", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Element.Code")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "Element.Name", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "Element.Name")
                    var at = 0
                    while (at < count) {
