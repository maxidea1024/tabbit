...
func (t *ElementTable) All() iter.Seq[*ElementRecord] {
	records := t.records

	return func(yield func(*ElementRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *ElementTable) Entries() iter.Seq2[int32, *ElementRecord] {
	records := t.records

	return func(yield func(int32, *ElementRecord) bool) {
		for i := range records {
			if !yield(records[i].Code, &records[i]) {
				return
			}
		}
	}
