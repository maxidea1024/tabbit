...
// CurseEffectEntry is one element of CurseRecord.Effect.
type CurseEffectEntry struct {
...
// CurseRecord was generated from test/fixtures/xlsx/polymorphism/polymorphism.xlsx : Polymorphism : P2.
// Afflictions whose effect is one of several shapes, written without the columns no row uses.
type CurseRecord struct {
	// primary index
	Index int32
	// plain column, outside the group
	Name string
	// which shape this row's effect is
	effect CurseEffectEntry
}
...
func (r *CurseRecord) Effect() Effect {
	return r.effectElement(&r.effect)
}

// effectElement builds one value from the entry the read filled.
func (r *CurseRecord) effectElement(
	entry *CurseEffectEntry) Effect {
	switch entry.Type {
	case 1:
		return DamageEffect{
...
// CurseTable holds every row of Curse.
type CurseTable struct {
	records []CurseRecord
	byIndex map[int32]int
}

// Records returns every row, in the order the sheet declared them.
func (t *CurseTable) Records() []CurseRecord { return t.records }

// Count returns how many rows the table holds.
func (t *CurseTable) Count() int { return len(t.records) }
...
func (t *CurseTable) All() iter.Seq[*CurseRecord] {
	records := t.records

	return func(yield func(*CurseRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *CurseTable) FindByIndex(key int32) *CurseRecord {
...
func (t *CurseTable) GetByIndexOrError(key int32) (*CurseRecord, error) {
	position, found := t.byIndex[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `Curse` that corresponds to field `Index` value %v", key)
	}
...
func (t *CurseTable) ContainsIndex(key int32) bool {
...
func (t *CurseTable) Entries() iter.Seq2[int32, *CurseRecord] {
	records := t.records

	return func(yield func(int32, *CurseRecord) bool) {
		for i := range records {
...
func (t *CurseTable) Read(filename string) error {
	...
	records := make([]CurseRecord, count)
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "Curse.Index", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Curse.Index")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "Curse.Name", tabbit.KindScalar, false, tabbit.ElementString) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Curse.Name")
				for i := int32(0); i < count; {
		...
		case 3:
			if tabbit.CheckColumn(reader, column, "Curse.Effect.Type", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Curse.Effect.Type")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 4:
			if tabbit.CheckColumn(reader, column, "Curse.Effect.Chance", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Curse.Effect.Chance")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 5:
			if tabbit.CheckColumn(reader, column, "Curse.Effect.Damage", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Curse.Effect.Damage")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 6:
			if tabbit.CheckColumn(reader, column, "Curse.Effect.Pierces", tabbit.KindScalar, false, tabbit.ElementBool) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Curse.Effect.Pierces")
		...
		case 7:
			if tabbit.CheckColumn(reader, column, "Curse.Effect.ElementId", tabbit.KindScalar, false, tabbit.ElementI32) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Curse.Effect.ElementId")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 8:
			if tabbit.CheckColumn(reader, column, "Curse.Effect.Band", tabbit.KindScalar, false, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Curse.Effect.Band")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 9:
			if tabbit.CheckColumn(reader, column, "Curse.Effect.Amount", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Curse.Effect.Amount")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
