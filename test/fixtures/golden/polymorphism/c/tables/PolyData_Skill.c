...
static bool PolyData_SkillParse(PolyData_SkillTable_t* table, tb_reader* reader) {
  int32_t row;
  int32_t at;
  ...
  if (table->count > 0) {
    table->records = (PolyData_SkillRecord_t*)tb_arena_alloc(
  ...
  for (at = 0; at < column_count && !tb_failed(reader); ++at) {
    ...
    case 6:
      (void)tb_check_column(reader, column, "Skill.Effect.Pierces", TB_KIND_SCALAR, false, TB_ELEMENT_MASK(TB_ELEMENT_BOOL));

      (void)tb_cursor_init(&cursor, reader, column, table->count, "Skill.Effect.Pierces");
