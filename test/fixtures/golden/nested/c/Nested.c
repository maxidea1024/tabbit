...
const uint8_t* Nested_EncryptionKey = NULL;
int32_t Nested_EncryptionKeyLength = 0;
const uint8_t* Nested_MacKey = NULL;
int32_t Nested_MacKeyLength = 0;
bool Nested_VerifyMac = true;

bool Nested_LoadAll(Nested_t* data, const char* base_path,
                 char* error, size_t error_size) {
  return Nested_LoadAllWithExtension(
...
bool Nested_LoadAllWithExtension(Nested_t* data, const char* base_path,
  ...
  if (!Nested_LoadoutLoad(&loaded.loadout, path, error, error_size)) {
...
void Nested_Free(Nested_t* data) {
  Nested_LoadoutFree(&data->loadout);
}
