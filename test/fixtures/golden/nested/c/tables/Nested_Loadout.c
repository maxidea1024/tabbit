...
static bool Nested_LoadoutParse(Nested_LoadoutTable_t* table, tb_reader* reader) {
  int32_t row;
  int32_t at;
  ...
  if (table->count > 0) {
    table->records = (Nested_LoadoutRecord_t*)tb_arena_alloc(
  ...
  for (at = 0; at < column_count && !tb_failed(reader); ++at) {
    ...
    case 8:
      (void)tb_check_column(reader, column, "Loadout.Tag", TB_KIND_ARRAY, false, TB_ELEMENT_MASK(TB_ELEMENT_STRING));

      (void)tb_cursor_init(&cursor, reader, column, table->count, "Loadout.Tag");
...
bool Nested_LoadoutLoad(Nested_LoadoutTable_t* table, const char* filename,
  ...
  /* Opened unconditionally, with whatever Nested_EncryptionKey and
   * Nested_MacKey hold - NULL unless the project set them. A file that is
  ...
  if (!tb_open(bytes, length, Nested_EncryptionKey,
        Nested_EncryptionKeyLength, Nested_MacKey,
        Nested_MacKeyLength, Nested_VerifyMac,
