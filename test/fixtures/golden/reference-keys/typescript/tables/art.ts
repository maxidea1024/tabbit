...
// Generated from test/fixtures/xlsx/reference-keys/reference-keys.xlsx : Keys : R2
/** Keyed by a uuid. */
export class ArtRecord {
  /** Default constructor */
  constructor() {
  }
...
export class ArtTable {
  ...
  /** All records. */
  public get records(): ArtRecord[] { return this._records }
  private _records: ArtRecord[] = []
   ...
   */
  public [Symbol.iterator](): IterableIterator<ArtRecord> {
    return this._records[Symbol.iterator]()
  }

  // Indexing by 'index'
  public get recordsByIndex(): Map<string, ArtRecord> { return this._recordsByIndex }
  private _recordsByIndex: Map<string, ArtRecord> = new Map<string, ArtRecord>()
   ...
   */
  public findByIndex(key: string): ArtRecord | undefined {
   ...
   */
  public getByIndexOrThrow(key: string): ArtRecord {
    const found = this._recordsByIndex.get(key)
    if (!found)
      throw new Error(`There is no record in table "Art" that corresponds to field "index" value ${key}`)
   ...
   */
  public *entries(): IterableIterator<[string, ArtRecord]> {
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          tabbit.checkColumn(column, 'Art.Index', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_UUID])
          for (let i = 0; i < rowCount; ++i) {
          ...
          break
        case 2:
          tabbit.checkColumn(column, 'Art.Path', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_STRING])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Art.Path')
          ...
          break
        case 3:
          tabbit.checkColumn(column, 'Art.Pad1', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Art.Pad1')
          for (let i = 0; i < rowCount; ) {
          ...
          break
        case 4:
          tabbit.checkColumn(column, 'Art.Pad2', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Art.Pad2')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
   ...
   */
  private publish(records: ArtRecord[]): void {
    const recordsByIndex = new Map<string, ArtRecord>()
