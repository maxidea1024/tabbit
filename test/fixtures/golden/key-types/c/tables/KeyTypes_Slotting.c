...
static bool KeyTypes_SlottingParse(KeyTypes_SlottingTable_t* table, tb_reader* reader) {
  int32_t row;
  int32_t at;
  ...
  if (table->count > 0) {
    table->records = (KeyTypes_SlottingRecord_t*)tb_arena_alloc(
    ...
    table->by_serial = (tb_index64_entry*)tb_arena_alloc(
      &table->arena, (size_t)table->count * sizeof *table->by_serial);

    if (table->by_serial == NULL)
      return tb_fail_with(reader, "out of memory allocating the Serial index");
  }
  ...
  for (at = 0; at < column_count && !tb_failed(reader); ++at) {
    ...
    case 3:
      ...
      for (row = 0; row < table->count && !tb_failed(reader); ++row) {
        ...
        (void)tb_cursor_next_i64(&cursor, &record->serial);
      }
      break;
  ...
  for (row = 0; row < table->count; ++row) {
    ...
    table->by_serial[row].key = table->records[row].serial;
    table->by_serial[row].position = row;
  }

  tb_index_sort(table->by_index, table->count);
  tb_index64_sort(table->by_serial, table->count);
...
bool KeyTypes_SlottingLoad(KeyTypes_SlottingTable_t* table, const char* filename,
  ...
  KeyTypes_SlottingTable_t loaded;
...
const KeyTypes_SlottingRecord_t* KeyTypes_SlottingFindBySerial(
  ...
  int32_t position = tb_index64_find(table->by_serial, table->count, key);
...
bool KeyTypes_SlottingContainsSerial(
  const KeyTypes_SlottingTable_t* table, int64_t key) {
  return tb_index64_find(table->by_serial, table->count, key) >= 0;
}
