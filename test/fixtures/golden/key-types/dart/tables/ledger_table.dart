...
/// Every row of Ledger.
class LedgerTable extends Iterable<LedgerRecord> {
  ...
  List<LedgerRecord> get records => _records;
  List<LedgerRecord> _records = [];
  ...
  @override
  Iterator<LedgerRecord> get iterator => _records.iterator;

  Map<BigInt, LedgerRecord> _byIndex = {};
  Map<Uuid, LedgerRecord> _byBatch = {};

  /// The row with this Index, or null when the table has none.
  ///
  /// The lookup to reach for when a missing row is an ordinary answer - an optional
  /// reference, a key that came from user input.
  LedgerRecord? findByIndex(BigInt key) => _byIndex[key];
  ...
  LedgerRecord getByIndexOrThrow(BigInt key) {
  ...
  bool containsIndex(BigInt key) => _byIndex.containsKey(key);
  ...
  LedgerRecord? operator [](BigInt key) => findByIndex(key);
  ...
  Iterable<MapEntry<BigInt, LedgerRecord>> get entries =>
  ...
  /// The lookup to reach for when a missing row is an ordinary answer - an optional
  /// reference, a key that came from user input.
  LedgerRecord? findByBatch(Uuid key) => _byBatch[key];
  ...
  /// cannot otherwise tell whether the next line is a null check or a catch.
  LedgerRecord getByBatchOrThrow(Uuid key) {
    final record = _byBatch[key];
  ...
  bool containsBatch(Uuid key) => _byBatch.containsKey(key);
  ...
  void read(String filename) {
    ...
    final loaded = <LedgerRecord>[];
    final loadedByIndex = <BigInt, LedgerRecord>{};
    final loadedByBatch = <Uuid, LedgerRecord>{};

    for (var i = 0; i < count; i++) {
      loaded.add(LedgerRecord());
    }
