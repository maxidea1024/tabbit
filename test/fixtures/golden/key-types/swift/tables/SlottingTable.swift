...
public final class SlottingRecord {
    ...
    public var index: Slot = Slot.of(0)
...
public final class SlottingTable: Sequence {
    ...
    public func makeIterator() -> Array<SlottingRecord>.Iterator {
    ...
    public private(set) var records: [SlottingRecord] = []

    private var byIndex: [Slot: SlottingRecord] = [:]

    private var bySerial: [Int64: SlottingRecord] = [:]

    /// The row with this Index, or nil when the table has none.
    ///
    /// The lookup to reach for when a missing row is an ordinary answer - an optional
    /// reference, a key that came from user input.
    public func findByIndex(_ key: Slot) -> SlottingRecord? {
        byIndex[key]
    }
    ...
    public func getByIndexOrThrow(_ key: Slot) throws -> SlottingRecord {
    ...
    public func containsIndex(_ key: Slot) -> Bool {
        byIndex[key] != nil
    }
    ...
    public subscript(_ key: Slot) -> SlottingRecord? {
        findByIndex(key)
    }
    ...
    /// row with.
    public var entries: LazyMapSequence<[SlottingRecord], (key: Slot, row: SlottingRecord)> {
    ...
    /// The lookup to reach for when a missing row is an ordinary answer - an optional
    /// reference, a key that came from user input.
    public func findBySerial(_ key: Int64) -> SlottingRecord? {
        bySerial[key]
    }

    /// The row with this Serial, or a thrown error naming what was missing.
    ///
    /// For a key that has to be there - one from another table, or a constant. The name says
    /// it throws, because a caller reading getBySerialOrThrow(key).name at a
    /// glance cannot otherwise tell whether the next line is a nil check or a catch.
    public func getBySerialOrThrow(_ key: Int64) throws -> SlottingRecord {
        guard let record = bySerial[key] else {
            throw RecordNotFoundError(
                "there is no record in table `Slotting` that corresponds to "
    ...
    public func containsSerial(_ key: Int64) -> Bool {
        bySerial[key] != nil
    }
    ...
    public func read(
        ...
        var loaded = [SlottingRecord]()
        loaded.reserveCapacity(count)

        for _ in 0 ..< count { loaded.append(SlottingRecord()) }
        var loadedByIndex = [Slot: SlottingRecord](
            minimumCapacity: count)
        var loadedBySerial = [Int64: SlottingRecord](
            minimumCapacity: count)
