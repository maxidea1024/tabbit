// ------------------------------------------------------------------------------
// Generated by Tabbit. DO NOT EDIT.
//
// Changes to this file may cause incorrect behavior and will be lost if the code is
// regenerated.
// ------------------------------------------------------------------------------

#ifndef TABBIT_GENERATED_KEYTYPESACCESSOR_LEDGER_H
#define TABBIT_GENERATED_KEYTYPESACCESSOR_LEDGER_H

#include <cstddef>
#include <cstdint>
#include <string>
#include <unordered_map>
#include <vector>

#include "tabbit/tcb_reader.h"
#include "KeyTypesAccessor_forward.h"
// Generated from test/fixtures/xlsx/key-types/key-types.xlsx : Ledger : B2
/// Keyed by a bigint, for ids that outgrew 32 bits.
struct LedgerRecord {
  /// primary index, past 32 bits
  std::int64_t index = 0;
  /// anything
  std::int32_t amount = 0;
  /// secondary index, a uuid
  tabbit::Uuid batch;
};

/// Keyed by a bigint, for ids that outgrew 32 bits.
class LedgerTable {
 public:
  const std::vector<LedgerRecord>& records() const { return records_; }

  /// How many rows the table holds.
  std::size_t size() const { return records_.size(); }

  /// Whether the table holds no rows.
  bool empty() const { return records_.empty(); }

  /// The rows, in the order the file wrote them - what a range-for binds to.
  ///
  /// A refresh replaces the vector rather than appending to it, so these are the ordinary
  /// vector invalidation rules: a loop running across a reload is reading a container that
  /// has been assigned to.
  typename std::vector<LedgerRecord>::const_iterator begin() const {
    return records_.begin();
  }

  typename std::vector<LedgerRecord>::const_iterator end() const {
    return records_.end();
  }

  /// The row with this Index, or nullptr when the table has none.
  ///
  /// The lookup to reach for when a missing row is an ordinary answer - an optional
  /// reference, a key that came from user input.
  const LedgerRecord* find_by_index(std::int64_t key) const {
    const auto it = by_index_.find(key);
    return it == by_index_.end() ? nullptr : &records_[it->second];
  }

  /// The row with this Index, or a thrown exception naming what was missing.
  ///
  /// For a key that has to be there - one from another table, or a constant. The name says
  /// it throws, because a caller reading `get_by_index(key)->name` at a glance
  /// cannot otherwise tell whether the next line is a null check or a catch.
  const LedgerRecord& get_by_index_or_throw(std::int64_t key) const {
    const LedgerRecord* found = find_by_index(key);

    if (found == nullptr) {
      throw tabbit::RecordNotFound(
        std::string("there is no record in table `Ledger` that corresponds to "
                    "field `Index` value ") + std::to_string(key));
    }

    return *found;
  }

  /// Whether the table holds a row with this Index.
  bool contains_index(std::int64_t key) const {
    return by_index_.find(key) != by_index_.end();
  }

  /// The row with this Batch, or nullptr when the table has none.
  ///
  /// The lookup to reach for when a missing row is an ordinary answer - an optional
  /// reference, a key that came from user input.
  const LedgerRecord* find_by_batch(tabbit::Uuid key) const {
    const auto it = by_batch_.find(key);
    return it == by_batch_.end() ? nullptr : &records_[it->second];
  }

  /// The row with this Batch, or a thrown exception naming what was missing.
  ///
  /// For a key that has to be there - one from another table, or a constant. The name says
  /// it throws, because a caller reading `get_by_batch(key)->name` at a glance
  /// cannot otherwise tell whether the next line is a null check or a catch.
  const LedgerRecord& get_by_batch_or_throw(tabbit::Uuid key) const {
    const LedgerRecord* found = find_by_batch(key);

    if (found == nullptr) {
      throw tabbit::RecordNotFound(
        std::string("there is no record in table `Ledger` that corresponds to "
                    "field `Batch` value ") + key.to_string());
    }

    return *found;
  }

  /// Whether the table holds a row with this Batch.
  bool contains_batch(tabbit::Uuid key) const {
    return by_batch_.find(key) != by_batch_.end();
  }

  /// Loads the table from a .tcb file written by Tabbit.
  void read(const std::string& filename) {
    // Not const, and it outlives the reader on purpose: opening the envelope decrypts in
    // place and hands back a window onto these same bytes rather than a copy of them.
    std::vector<std::uint8_t> buffer = tabbit::read_all_bytes(filename);

    // Unconditionally, and with whatever `encryption_key` and `mac_key` hold - which is
    // empty unless the project set them. A file that is neither encrypted nor signed comes
    // back from this untouched, so the load path is the same either way and there is no
    // condition here that could be the wrong way round. spec/wire/tcb-mac-and-signature.md.
    const std::pair<const std::uint8_t*, std::size_t> bytes =
      tabbit::open(buffer, encryption_key, mac_key, verify_mac);

    tabbit::TcbReader reader(bytes.first, bytes.second);

    // Column by column, matched by tag rather than position: a column this build
    // does not know is skipped by its block length, and one whose type changed
    // incompatibly fails naming the field.
    const tabbit::Header header = tabbit::read_table_header(reader);
    const std::size_t row_count = static_cast<std::size_t>(header.row_count);

    // Read into storage of its own and swapped in at the end: reading a table that is
    // already loaded is a refresh, and one that throws part way through - a truncated
    // file, a column this build cannot read - has to leave the rows already there.
    std::vector<LedgerRecord> records;
    records.resize(row_count);

    for (const tabbit::Column& column : header.columns) {
      const std::size_t block_end = reader.position() + static_cast<std::size_t>(column.byte_length);

      switch (column.tag) {
        case 1: {
          tabbit::check_column(column, "Ledger.Index", tabbit::kKindScalar, false, {tabbit::kElementI64, tabbit::kElementI32, tabbit::kElementVarint});
          tabbit::TcbColumnCursor cursor(reader, column, header.row_count, "Ledger.Index");
          for (std::size_t i = 0; i < row_count; ++i) {
            auto& record = records[i];
            record.index = cursor.next_i64();
          }
          break;
        }
        case 2: {
          tabbit::check_column(column, "Ledger.Amount", tabbit::kKindScalar, false, {tabbit::kElementI32, tabbit::kElementVarint});
          tabbit::TcbColumnCursor cursor(reader, column, header.row_count, "Ledger.Amount");
          std::int32_t value{};
          for (std::size_t i = 0; i < row_count; ) {
            std::int32_t n = cursor.next_same_i32(
                static_cast<std::int32_t>(row_count - i), value);
            for (; n > 0; --n, ++i) {
              records[i].amount = value;
            }
          }
          break;
        }
        case 3: {
          tabbit::check_column(column, "Ledger.Batch", tabbit::kKindScalar, false, {tabbit::kElementUuid});
          for (std::size_t i = 0; i < row_count; ++i) {
            auto& record = records[i];
            reader.read(record.batch);
          }
          break;
        }
        default:
          // A column added after this code was generated.
          reader.skip(column.byte_length);
          break;
      }

      tabbit::check_block_end(reader, column, block_end);
    }

    publish(std::move(records));
  }

 private:
  friend class KeyTypesAccessor;

  /// One whole load, in place of the previous one.
  ///
  /// The indexes are built here rather than by the caller, because they have to arrive
  /// with the rows: a table holding this load's rows and the last one's index would
  /// answer a lookup with a row that is no longer at that position.
  void publish(std::vector<LedgerRecord>&& records) {
    std::unordered_map<std::int64_t, std::size_t> index_map;
    index_map.reserve(records.size());
    for (std::size_t i = 0; i < records.size(); ++i) {
      index_map.emplace(records[i].index, i);
    }

    std::unordered_map<tabbit::Uuid, std::size_t> batch_map;
    batch_map.reserve(records.size());
    for (std::size_t i = 0; i < records.size(); ++i) {
      batch_map.emplace(records[i].batch, i);
    }

    records_ = std::move(records);
    by_index_ = std::move(index_map);
    by_batch_ = std::move(batch_map);
  }

  std::vector<LedgerRecord> records_;
  std::unordered_map<std::int64_t, std::size_t> by_index_;
  std::unordered_map<tabbit::Uuid, std::size_t> by_batch_;
};

#endif  // TABBIT_GENERATED_KEYTYPESACCESSOR_LEDGER_H
