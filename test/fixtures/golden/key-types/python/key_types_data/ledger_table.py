...
class LedgerRecord:
    ...
    def __repr__(self):
        return "LedgerRecord(index=%r, amount=%r, batch=%r)" % (self.index, self.amount, self.batch)


class LedgerTable:
    """Every row of Ledger."""

    __slots__ = ("records", "by_index", "by_batch")
    ...
    def get_by_batch_or_throw(self, key):
        """The row with this Batch, or a raised error naming what was missing.

        For a key that has to be there - one from another table, or a constant. The name
        says it raises, because a caller reading `get_by_batch(key).name` at a
        glance cannot otherwise tell whether the next line is a check or a try.
        """
        record = self.by_batch.get(key)

        if record is None:
            raise tabbit.RecordNotFoundError(
                "there is no record in table `Ledger` that corresponds to "
                "field `Batch` value %r" % (key,))

        return record

    def contains_batch(self, key):
        """Whether the table holds a row with this Batch."""
        return key in self.by_batch
    ...
    def read(self, filename):
        ...
        records = [LedgerRecord() for _ in range(count)]
        ...
        by_batch = {record.batch: record for record in records}
