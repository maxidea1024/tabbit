...
module KeyTypes
  ...
  class LedgerRecord
    attr_accessor :index, :amount, :batch
  ...
  class LedgerTable
    ...
    # glance cannot otherwise tell whether the next line is a check or a rescue.
    def get_by_batch_or_throw(key)
      record = @by_batch[key]

      if record.nil?
        raise Tabbit::RecordNotFoundError,
              "there is no record in table `Ledger` that corresponds to " \
              "field `Batch` value #{key.inspect}"
      end
    ...
    def read(filename)
      ...
      records = Array.new(count) { LedgerRecord.new }
      ...
      by_batch = records.to_h { |record| [record.batch, record] }
