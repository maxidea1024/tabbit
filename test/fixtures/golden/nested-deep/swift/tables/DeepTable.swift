...
public final class DeepRecord {
    ...
    public var star: [StarEntry] = [StarEntry](repeating: StarEntry(), count: 2)

    /// A record inside star.
    public struct StarEntryPosition {
    ...
    public struct StarEntry {
        ...
        /// element 1, one level further in
        public var position: StarEntryPosition = StarEntryPosition()
    }
}
...
public final class DeepTable {
    ...
    public private(set) var records: [DeepRecord] = []

    private var byIndex: [Int32: DeepRecord] = [:]

    /// The row with this Index, or nil when the table has none.
    ///
    /// The lookup to reach for when a missing row is an ordinary answer - an optional
    /// reference, a key that came from user input.
    public func findByIndex(_ key: Int32) -> DeepRecord? {
        byIndex[key]
    }
    ...
    public func getByIndexOrThrow(_ key: Int32) throws -> DeepRecord {
    ...
    /// once. NestedDeepAccessor passes its own. spec/targets/accessor-instances.md.
    public func read(
        _ filename: String,
        key: [UInt8]? = nil,
        macKey: [UInt8]? = nil,
        ...
        var loaded = [DeepRecord]()
        loaded.reserveCapacity(count)

        for _ in 0 ..< count { loaded.append(DeepRecord()) }
        var loadedByIndex = [Int32: DeepRecord](
