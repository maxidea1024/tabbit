...
func (t *DeepTable) All() iter.Seq[*DeepRecord] {
	records := t.records

	return func(yield func(*DeepRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *DeepTable) Entries() iter.Seq2[int32, *DeepRecord] {
	records := t.records

	return func(yield func(int32, *DeepRecord) bool) {
		for i := range records {
