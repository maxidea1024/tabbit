...
#ifndef TABBIT_GENERATED_NESTEDDEEPACCESSOR_DEEP_H
#define TABBIT_GENERATED_NESTEDDEEPACCESSOR_DEEP_H
...
struct DeepRecord_star_entry {
  /// element 1, a value beside a record
  std::int32_t id = 0;
  /// element 1, one level further in
  DeepRecord_star_entryPosition position;
};
...
struct DeepRecord {
  ...
  std::int32_t index = 0;
  /// element 1, a value beside a record
  std::vector<DeepRecord_star_entry> star;
};

/// A record whose member is a record.
class DeepTable {
 public:
  const std::vector<DeepRecord>& records() const { return records_; }
  ...
  typename std::vector<DeepRecord>::const_iterator begin() const {
    return records_.begin();
  }

  typename std::vector<DeepRecord>::const_iterator end() const {
    return records_.end();
  }
  ...
  const DeepRecord* find_by_index(std::int32_t key) const {
  ...
  const DeepRecord& get_by_index_or_throw(std::int32_t key) const {
    const DeepRecord* found = find_by_index(key);

    if (found == nullptr) {
      throw tabbit::RecordNotFound(
        std::string("there is no record in table `Deep` that corresponds to "
 ...
 private:
  friend class NestedDeepAccessor;
  ...
  void publish(std::vector<DeepRecord>&& records) {
  ...
  std::vector<DeepRecord> records_;
  std::unordered_map<std::int32_t, std::size_t> by_index_;
};

#endif  // TABBIT_GENERATED_NESTEDDEEPACCESSOR_DEEP_H
