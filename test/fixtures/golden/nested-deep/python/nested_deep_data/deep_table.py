...
class DeepStarEntryPosition:
    ...
    def __repr__(self):
        return "DeepStarEntryPosition(x=%r, y=%r)" % (self.x, self.y)
...
class DeepStarEntry:
    ...
    def __init__(self):
        self.id = 0
        self.position = DeepStarEntryPosition()

    def __repr__(self):
        return "DeepStarEntry(id=%r, position=%r)" % (self.id, self.position)
...
class DeepRecord:
    ...
    __slots__ = ("index", "star")

    def __init__(self):
        self.index = 0
        self.star = [DeepStarEntry() for _ in range(2)]

    def __repr__(self):
        return "DeepRecord(index=%r, star=%r)" % (self.index, self.star)
    ...
    def read(self, filename):
        ...
        for column in columns:
            ...
            if column.tag == 1:
                ...
                cursor = tabbit.ColumnCursor(reader, column, count, "Deep.Star.Id")
                for record in records:
                    ...
                    record.star = [
                        DeepStarEntry() for _ in range(element_count)]
                ...
                cursor = tabbit.ColumnCursor(reader, column, count, "Deep.Star.Position.X")
                for record in records:
                    element_count = cursor.next_length()
                    if len(record.star) != element_count:
                ...
                cursor = tabbit.ColumnCursor(reader, column, count, "Deep.Star.Position.Y")
                for record in records:
                    element_count = cursor.next_length()
                    if len(record.star) != element_count:
                        raise tabbit.TcbError(
