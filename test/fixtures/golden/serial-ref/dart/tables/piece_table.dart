...
/// Every row of Piece.
class PieceTable extends Iterable<PieceRecord> {
  ...
  List<PieceRecord> get records => _records;
  List<PieceRecord> _records = [];
  ...
  @override
  Iterator<PieceRecord> get iterator => _records.iterator;

  Map<int, PieceRecord> _byIndex = {};

  /// The row with this Index, or null when the table has none.
  ///
  /// The lookup to reach for when a missing row is an ordinary answer - an optional
  /// reference, a key that came from user input.
  PieceRecord? findByIndex(int key) => _byIndex[key];
  ...
  PieceRecord getByIndexOrThrow(int key) {
  ...
  PieceRecord? operator [](int key) => findByIndex(key);
  ...
  Iterable<MapEntry<int, PieceRecord>> get entries =>
  ...
  void read(String filename) {
    ...
    final loaded = <PieceRecord>[];
    final loadedByIndex = <int, PieceRecord>{};

    for (var i = 0; i < count; i++) {
      loaded.add(PieceRecord());
    }
    ...
    for (final column in header.columns) {
      ...
      switch (column.tag) {
        case 1:
          checkColumn(column, 'Piece.Index', kindScalar, false, [elementI32, elementVarint]);
          cursor = TcbColumnCursor(reader, column, count, 'Piece.Index');
          for (var i = 0; i < count; ) {
          ...
          break;
        case 2:
          checkColumn(column, 'Piece.Name', kindScalar, false, [elementString]);
          cursor = TcbColumnCursor(reader, column, count, 'Piece.Name');
          for (var i = 0; i < count; ) {
          ...
          break;
        case 3:
          checkColumn(column, 'Piece.Tier', kindScalar, false, [elementI32, elementVarint]);
          cursor = TcbColumnCursor(reader, column, count, 'Piece.Tier');
          for (var i = 0; i < count; ) {
            final (n, value) = cursor.nextSameI32(count - i);
            for (var left = n; left > 0; left--, i++) {
              loaded[i].tier = value;
