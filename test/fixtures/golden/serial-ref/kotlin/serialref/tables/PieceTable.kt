...
/** Every row of Piece. */
class PieceTable : Iterable<PieceRecord> {
     ...
     */
    var records: MutableList<PieceRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<PieceRecord> = records.iterator()

    private var byIndex: HashMap<Int, PieceRecord> = HashMap()
     ...
     */
    fun findByIndex(key: Int): PieceRecord? = byIndex[key]
     ...
     */
    fun getByIndexOrThrow(key: Int): PieceRecord =
     ...
     */
    operator fun get(key: Int): PieceRecord? = findByIndex(key)
     ...
     */
    val entries: Sequence<Pair<Int, PieceRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<PieceRecord>(count)
        val loadedByIndex = HashMap<Int, PieceRecord>(count * 2)

        repeat(count) { loaded.add(PieceRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "Piece.Index", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Piece.Index")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "Piece.Name", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "Piece.Name")
                    var at = 0
                    while (at < count) {
                ...
                3 -> {
                    checkColumn(column, "Piece.Tier", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Piece.Tier")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
