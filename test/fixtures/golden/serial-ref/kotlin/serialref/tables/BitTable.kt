...
/** Every row of Bit. */
class BitTable : Iterable<BitRecord> {
     ...
     */
    var records: MutableList<BitRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<BitRecord> = records.iterator()

    private var byIndex: HashMap<Int, BitRecord> = HashMap()
     ...
     */
    fun findByIndex(key: Int): BitRecord? = byIndex[key]
     ...
     */
    fun getByIndexOrThrow(key: Int): BitRecord =
     ...
     */
    operator fun get(key: Int): BitRecord? = findByIndex(key)
     ...
     */
    val entries: Sequence<Pair<Int, BitRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<BitRecord>(count)
        val loadedByIndex = HashMap<Int, BitRecord>(count * 2)

        repeat(count) { loaded.add(BitRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "Bit.Index", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Bit.Index")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "Bit.Name", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "Bit.Name")
                    var at = 0
                    while (at < count) {
                ...
                3 -> {
                    checkColumn(column, "Bit.Tier", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Bit.Tier")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
