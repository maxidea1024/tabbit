...
/** Every row of Piece. */
public final class PieceTable implements Iterable<PieceRecord> {
    private List<PieceRecord> records = new ArrayList<>();
    private Map<Integer, PieceRecord> byIndex = new HashMap<>();
     ...
     */
    public List<PieceRecord> records() {
        return records;
    }
     ...
     */
    @Override
    public Iterator<PieceRecord> iterator() {
    ...
    public PieceRecord getByIndexOrThrow(int key) {
        PieceRecord record = byIndex.get(key);

        if (record == null) {
            throw new TcbReader.RecordNotFoundException(
                "there is no record in table `Piece` that corresponds to field "
                + "`Index` value " + key);
        }
     ...
     */
    public Iterable<Map.Entry<Integer, PieceRecord>> entries() {
        final List<PieceRecord> rows = records;

        return () -> new Iterator<Map.Entry<Integer, PieceRecord>>() {
            private final Iterator<PieceRecord> source = rows.iterator();
            ...
            @Override
            public Map.Entry<Integer, PieceRecord> next() {
                final PieceRecord row = source.next();
    ...
    public void read(Path filename) {
        ...
        List<PieceRecord> loaded = new ArrayList<>(count);
        Map<Integer, PieceRecord> loadedByIndex = new HashMap<>(count * 2);

        for (int i = 0; i < count; i++) {
            loaded.add(new PieceRecord());
        }
        ...
        for (TcbReader.Column column : header.columns) {
            ...
            switch (column.tag) {
                case 1: {
                    TcbReader.checkColumn(column, "Piece.Index", TcbReader.KIND_SCALAR, false, TcbReader.ELEMENT_I32, TcbReader.ELEMENT_VARINT);
                    cursor = new TcbReader.ColumnCursor(reader, column, count, "Piece.Index");
                    for (int i = 0; i < count; ) {
                ...
                case 2: {
                    TcbReader.checkColumn(column, "Piece.Name", TcbReader.KIND_SCALAR, false, TcbReader.ELEMENT_STRING);
                    cursor = new TcbReader.ColumnCursor(reader, column, count, "Piece.Name");
                    for (int i = 0; i < count; ) {
                ...
                case 3: {
                    TcbReader.checkColumn(column, "Piece.Tier", TcbReader.KIND_SCALAR, false, TcbReader.ELEMENT_I32, TcbReader.ELEMENT_VARINT);
                    cursor = new TcbReader.ColumnCursor(reader, column, count, "Piece.Tier");
                    for (int i = 0; i < count; ) {
                        int n = cursor.nextSameI32(count - i);
                        for (; n > 0; n--, i++) {
                            loaded.get(i).tier = cursor.runSameValue;
                        }
                    }
                    break;
                }
