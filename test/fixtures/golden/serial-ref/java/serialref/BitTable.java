...
/** Every row of Bit. */
public final class BitTable implements Iterable<BitRecord> {
    private List<BitRecord> records = new ArrayList<>();
    private Map<Integer, BitRecord> byIndex = new HashMap<>();
     ...
     */
    @Override
    public Iterator<BitRecord> iterator() {
    ...
    public BitRecord getByIndexOrThrow(int key) {
        BitRecord record = byIndex.get(key);

        if (record == null) {
            throw new TcbReader.RecordNotFoundException(
                "there is no record in table `Bit` that corresponds to field "
                + "`Index` value " + key);
        }
     ...
     */
    public Iterable<Map.Entry<Integer, BitRecord>> entries() {
        final List<BitRecord> rows = records;

        return () -> new Iterator<Map.Entry<Integer, BitRecord>>() {
            private final Iterator<BitRecord> source = rows.iterator();
            ...
            @Override
            public Map.Entry<Integer, BitRecord> next() {
                final BitRecord row = source.next();
    ...
    public void read(Path filename) {
        ...
        List<BitRecord> loaded = new ArrayList<>(count);
        Map<Integer, BitRecord> loadedByIndex = new HashMap<>(count * 2);

        for (int i = 0; i < count; i++) {
            loaded.add(new BitRecord());
        }
        ...
        for (TcbReader.Column column : header.columns) {
            ...
            switch (column.tag) {
                case 1: {
                    TcbReader.checkColumn(column, "Bit.Index", TcbReader.KIND_SCALAR, false, TcbReader.ELEMENT_I32, TcbReader.ELEMENT_VARINT);
                    cursor = new TcbReader.ColumnCursor(reader, column, count, "Bit.Index");
                    for (int i = 0; i < count; ) {
                ...
                case 2: {
                    TcbReader.checkColumn(column, "Bit.Name", TcbReader.KIND_SCALAR, false, TcbReader.ELEMENT_STRING);
                    cursor = new TcbReader.ColumnCursor(reader, column, count, "Bit.Name");
                    for (int i = 0; i < count; ) {
                ...
                case 3: {
                    TcbReader.checkColumn(column, "Bit.Tier", TcbReader.KIND_SCALAR, false, TcbReader.ELEMENT_I32, TcbReader.ELEMENT_VARINT);
                    cursor = new TcbReader.ColumnCursor(reader, column, count, "Bit.Tier");
                    for (int i = 0; i < count; ) {
                        int n = cursor.nextSameI32(count - i);
                        for (; n > 0; n--, i++) {
