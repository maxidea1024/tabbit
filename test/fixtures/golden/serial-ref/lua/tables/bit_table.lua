...
---@field name string
---@field tier integer
local BitRecordMeta = tcb.strictType("a `Bit` row", { "index", "name", "tier" })

---@return BitRecord
local function newBitRecord()
  return setmetatable({
    index = 0,
    name = "",
    tier = 0,
...
---@class BitTable
---@field records BitRecord[]
local BitTable = {}

local BitTableMeta = tcb.strictInstance("the `Bit` table", BitTable, { "records", "byIndex" })

function BitTable.new()
  return setmetatable({
    records = {},
    byIndex = {},
  }, BitTableMeta)
...
function BitTable:containsIndex(key)
...
function BitTable:readBytes(data)
  ...
  for c = 1, #columns do
    ...
    if column.tag == 1 then
      tcb.checkColumn(column, "Bit.Index", tcb.KIND_SCALAR, false, { tcb.ELEMENT_I32, tcb.ELEMENT_VARINT })
      local cursor = tcb.newCursor(reader, column, count, "Bit.Index")
      local at = 0
    ...
    elseif column.tag == 2 then
      tcb.checkColumn(column, "Bit.Name", tcb.KIND_SCALAR, false, { tcb.ELEMENT_STRING })
      local cursor = tcb.newCursor(reader, column, count, "Bit.Name")
      local at = 0
    ...
    elseif column.tag == 3 then
      tcb.checkColumn(column, "Bit.Tier", tcb.KIND_SCALAR, false, { tcb.ELEMENT_I32, tcb.ELEMENT_VARINT })
      local cursor = tcb.newCursor(reader, column, count, "Bit.Tier")
      local at = 0
...
function BitTable:read(filename)
