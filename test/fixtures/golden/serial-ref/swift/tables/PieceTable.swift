...
/// Every row of Piece.
public final class PieceTable: Sequence {
    ...
    public func makeIterator() -> Array<PieceRecord>.Iterator {
    ...
    public private(set) var records: [PieceRecord] = []

    private var byIndex: [Int32: PieceRecord] = [:]

    /// The row with this Index, or nil when the table has none.
    ///
    /// The lookup to reach for when a missing row is an ordinary answer - an optional
    /// reference, a key that came from user input.
    public func findByIndex(_ key: Int32) -> PieceRecord? {
        byIndex[key]
    }
    ...
    public func getByIndexOrThrow(_ key: Int32) throws -> PieceRecord {
        guard let record = byIndex[key] else {
            throw RecordNotFoundError(
                "there is no record in table `Piece` that corresponds to "
    ...
    public subscript(_ key: Int32) -> PieceRecord? {
        findByIndex(key)
    }
    ...
    /// row with.
    public var entries: LazyMapSequence<[PieceRecord], (key: Int32, row: PieceRecord)> {
    ...
    public func read(
        ...
        var loaded = [PieceRecord]()
        loaded.reserveCapacity(count)

        for _ in 0 ..< count { loaded.append(PieceRecord()) }
        var loadedByIndex = [Int32: PieceRecord](
