...
/// Every row of Bit.
public final class BitTable: Sequence {
    ...
    public func makeIterator() -> Array<BitRecord>.Iterator {
    ...
    public private(set) var records: [BitRecord] = []

    private var byIndex: [Int32: BitRecord] = [:]

    /// The row with this Index, or nil when the table has none.
    ///
    /// The lookup to reach for when a missing row is an ordinary answer - an optional
    /// reference, a key that came from user input.
    public func findByIndex(_ key: Int32) -> BitRecord? {
        byIndex[key]
    }
    ...
    public func getByIndexOrThrow(_ key: Int32) throws -> BitRecord {
    ...
    public subscript(_ key: Int32) -> BitRecord? {
        findByIndex(key)
    }
    ...
    /// row with.
    public var entries: LazyMapSequence<[BitRecord], (key: Int32, row: BitRecord)> {
    ...
    public func read(
        ...
        var loaded = [BitRecord]()
        loaded.reserveCapacity(count)

        for _ in 0 ..< count { loaded.append(BitRecord()) }
        var loadedByIndex = [Int32: BitRecord](
        ...
        for column in header.columns {
            ...
            switch column.tag {
            case 1:
                try Tcb.checkColumn(column, "Bit.Index", Tcb.kindScalar, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Bit.Index")
                var at = 0
                while at < count {
            ...
            case 2:
                try Tcb.checkColumn(column, "Bit.Name", Tcb.kindScalar, false, Tcb.elementString)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Bit.Name")
                var at = 0
                while at < count {
            ...
            case 3:
                try Tcb.checkColumn(column, "Bit.Tier", Tcb.kindScalar, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Bit.Tier")
                var at = 0
                while at < count {
                    var n = try cursor.nextSameI32(count - at)
                    while n > 0 {
