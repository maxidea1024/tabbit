...
#ifndef TABBIT_GENERATED_SERIALREFACCESSOR_KIT_H
#define TABBIT_GENERATED_SERIALREFACCESSOR_KIT_H
...
class KitTable {
 public:
  const std::vector<KitRecord>& records() const { return records_; }
  ...
  typename std::vector<KitRecord>::const_iterator begin() const {
    return records_.begin();
  }

  typename std::vector<KitRecord>::const_iterator end() const {
    return records_.end();
  }
  ...
  const KitRecord* find_by_index(std::int32_t key) const {
  ...
  const KitRecord& get_by_index_or_throw(std::int32_t key) const {
    const KitRecord* found = find_by_index(key);

    if (found == nullptr) {
      throw tabbit::RecordNotFound(
        std::string("there is no record in table `Kit` that corresponds to "
  ...
  void read(const std::string& filename) {
    ...
    for (const tabbit::Column& column : header.columns) {
      ...
      switch (column.tag) {
        ...
        case 2: {
          tabbit::check_column(column, "Kit.Slot", tabbit::kKindArray, false, {tabbit::kElementI32});
          tabbit::TcbColumnCursor cursor(reader, column, header.row_count, "Kit.Slot");
        ...
        case 3: {
          tabbit::check_column(column, "Kit.Tier", tabbit::kKindArray, false, {tabbit::kElementI32});
          tabbit::TcbColumnCursor cursor(reader, column, header.row_count, "Kit.Tier");
          for (std::size_t i = 0; i < row_count; ++i) {
            auto& record = records[i];
            const std::int32_t element_count = cursor.next_length();
            record.tier.assign(
 ...
 private:
  ...
  void publish(std::vector<KitRecord>&& records) {
  ...
  std::vector<KitRecord> records_;
  std::unordered_map<std::int32_t, std::size_t> by_index_;
};

}  // namespace serial_ref

#endif  // TABBIT_GENERATED_SERIALREFACCESSOR_KIT_H
