...
export class BitTable {
  ...
  /** All records. */
  public get records(): BitRecord[] { return this._records }
  private _records: BitRecord[] = []
   ...
   */
  public [Symbol.iterator](): IterableIterator<BitRecord> {
    return this._records[Symbol.iterator]()
  }

  // Indexing by 'index'
  public get recordsByIndex(): Map<number, BitRecord> { return this._recordsByIndex }
  private _recordsByIndex: Map<number, BitRecord> = new Map<number, BitRecord>()
   ...
   */
  public findByIndex(key: number): BitRecord | undefined {
   ...
   */
  public getByIndexOrThrow(key: number): BitRecord {
    const found = this._recordsByIndex.get(key)
    if (!found)
      throw new Error(`There is no record in table "Bit" that corresponds to field "index" value ${key}`)
   ...
   */
  public *entries(): IterableIterator<[number, BitRecord]> {
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          tabbit.checkColumn(column, 'Bit.Index', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Bit.Index')
          ...
          break
        case 2:
          tabbit.checkColumn(column, 'Bit.Name', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_STRING])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Bit.Name')
          for (let i = 0; i < rowCount; ) {
          ...
          break
        case 3:
          tabbit.checkColumn(column, 'Bit.Tier', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Bit.Tier')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
   ...
   */
  private publish(records: BitRecord[]): void {
    const recordsByIndex = new Map<number, BitRecord>()
