...
export class PieceTable {
  ...
  /** All records. */
  public get records(): PieceRecord[] { return this._records }
  private _records: PieceRecord[] = []
   ...
   */
  public [Symbol.iterator](): IterableIterator<PieceRecord> {
    return this._records[Symbol.iterator]()
  }

  // Indexing by 'index'
  public get recordsByIndex(): Map<number, PieceRecord> { return this._recordsByIndex }
  private _recordsByIndex: Map<number, PieceRecord> = new Map<number, PieceRecord>()
   ...
   */
  public findByIndex(key: number): PieceRecord | undefined {
   ...
   */
  public getByIndexOrThrow(key: number): PieceRecord {
    const found = this._recordsByIndex.get(key)
    if (!found)
      throw new Error(`There is no record in table "Piece" that corresponds to field "index" value ${key}`)
   ...
   */
  public *entries(): IterableIterator<[number, PieceRecord]> {
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          tabbit.checkColumn(column, 'Piece.Index', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Piece.Index')
          ...
          break
        case 2:
          tabbit.checkColumn(column, 'Piece.Name', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_STRING])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Piece.Name')
          for (let i = 0; i < rowCount; ) {
          ...
          break
        case 3:
          tabbit.checkColumn(column, 'Piece.Tier', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Piece.Tier')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
   ...
   */
  private publish(records: PieceRecord[]): void {
    const recordsByIndex = new Map<number, PieceRecord>()
