...
impl PieceTable {
    ...
    pub fn records(&self) -> &[PieceRecord] {
        &self.records
    }
    ...
    pub fn iter(&self) -> std::slice::Iter<'_, PieceRecord> {
        self.records.iter()
    }
    ...
    pub fn find_by_index(&self, key: i32) -> Option<&PieceRecord> {
    ...
    /// error it returns.
    pub fn get_by_index_or_error(&self, key: i32) -> tabbit::Result<&PieceRecord> {
        self.by_index
            .get(&key)
    ...
    pub fn entries(&self) -> impl Iterator<Item = (&i32, &PieceRecord)> + '_ {
    ...
    pub fn read(&mut self, filename: &Path) -> tabbit::Result<()> {
        ...
        let mut records = vec![PieceRecord::default(); count];
        ...
        for column in &header.columns {
            ...
            match column.tag {
                1 => {
                    tabbit::check_column(column, "Piece.Index", tabbit::KIND_SCALAR, false, &[tabbit::ELEMENT_I32, tabbit::ELEMENT_VARINT])?;
                    let mut cursor = tabbit::TcbColumnCursor::new(&mut reader, column, header.row_count, "Piece.Index")?;
                    let mut at = 0usize;
                ...
                2 => {
                    tabbit::check_column(column, "Piece.Name", tabbit::KIND_SCALAR, false, &[tabbit::ELEMENT_STRING])?;
                    let mut cursor = tabbit::TcbColumnCursor::new(&mut reader, column, header.row_count, "Piece.Name")?;
                    let mut at = 0usize;
                    while at < records.len() {
                ...
                3 => {
                    tabbit::check_column(column, "Piece.Tier", tabbit::KIND_SCALAR, false, &[tabbit::ELEMENT_I32, tabbit::ELEMENT_VARINT])?;
                    let mut cursor = tabbit::TcbColumnCursor::new(&mut reader, column, header.row_count, "Piece.Tier")?;
                    let mut at = 0usize;
                    while at < records.len() {
                        let (n, value) = cursor.next_same_i32((records.len() - at) as i32)?;
                        for _ in 0..n {
...
impl<'a> IntoIterator for &'a PieceTable {
    type Item = &'a PieceRecord;
    type IntoIter = std::slice::Iter<'a, PieceRecord>;
