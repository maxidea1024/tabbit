...
{
    [System.Serializable]
    public partial class PieceRecord
    {
        #region Values
        /// <summary>
        /// primary index
    ...
    /// </summary>
    [System.Serializable]
    public partial class PieceTable : IEnumerable<PieceRecord>
    {
        /// <summary>
        /// Field names.
        /// </summary>
        public static readonly string[] FieldNames = { "Index", "Name", "Tier" };
        ...
        public List<object[]> BuildObjectValueMap()
        {
            ...
            foreach (var r in _records)
                result.Add(new object[] { r.Index, r.Name, r.Tier });
        ...
        /// </remarks>
        public List<PieceRecord> Records => _records;
        private List<PieceRecord> _records = new List<PieceRecord>();
        ...
        /// </remarks>
        public List<PieceRecord>.Enumerator GetEnumerator() => _records.GetEnumerator();

        IEnumerator<PieceRecord> IEnumerable<PieceRecord>.GetEnumerator() => _records.GetEnumerator();

        System.Collections.IEnumerator System.Collections.IEnumerable.GetEnumerator()
            => _records.GetEnumerator();

        #region Indexing by 'Index'
        public Dictionary<int, PieceRecord> RecordsByIndex => _recordsByIndex;
        private Dictionary<int, PieceRecord> _recordsByIndex = new Dictionary<int, PieceRecord>();
        ...
        /// </remarks>
        public PieceRecord FindByIndex(int key)
            => _recordsByIndex.TryGetValue(key, out PieceRecord record) ? record : null;
        ...
        /// </remarks>
        public PieceRecord GetByIndexOrThrow(int key)
        {
            if (!_recordsByIndex.TryGetValue(key, out PieceRecord record))
                throw new TabbitException($"There is no record in table `Piece` that corresponds to field `Index` value {key}");
        ...
        {
            private readonly List<PieceRecord> _rows;
            private int _at;

            internal EntryEnumerator(List<PieceRecord> rows)
            {
                _rows = rows;
                _at = -1;
            }
            ...
            public (int Key, PieceRecord Row) Current
        ...
        /// </remarks>
        public PieceRecord this[int key] => GetByIndexOrThrow(key);
        ...
        public Task ReadAsync(TcbReader reader)
        {
            ...
            var records = new List<PieceRecord>(count);

            for (int i = 0; i < count; i++)
                records.Add(new PieceRecord());
            ...
            foreach (var column in columns)
            {
                ...
                {
                    case 1:
                        TcbTable.CheckColumn(column, "Piece.Index", TcbTable.KindScalar, false, TcbTable.ElementI32, TcbTable.ElementVarint);
                        cursor = new TcbColumnCursor(reader, column, count, "Piece.Index");
                        for (int i = 0; i < count; )
                        {
                    ...
                    case 2:
                        TcbTable.CheckColumn(column, "Piece.Name", TcbTable.KindScalar, false, TcbTable.ElementString);
                        cursor = new TcbColumnCursor(reader, column, count, "Piece.Name");
                        for (int i = 0; i < count; )
                        {
                            // One call per run of equal values, not one per row - over a
                            // run-length encoded column this is most of the decode.
                    ...
                    case 3:
                        TcbTable.CheckColumn(column, "Piece.Tier", TcbTable.KindScalar, false, TcbTable.ElementI32, TcbTable.ElementVarint);
                        cursor = new TcbColumnCursor(reader, column, count, "Piece.Tier");
                        for (int i = 0; i < count; )
                        {
                            // One call per run of equal values, not one per row - over a
                            // run-length encoded column this is most of the decode.
            ...
            var recordsByIndex = new Dictionary<int, PieceRecord>(count);
