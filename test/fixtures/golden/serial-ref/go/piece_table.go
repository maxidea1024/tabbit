...
// PieceRecord was generated from test/fixtures/xlsx/serial-ref/serial-ref.xlsx : Refs : B2.
...
// PieceTable holds every row of Piece.
...
func (t *PieceTable) Records() []PieceRecord { return t.records }

// Count returns how many rows the table holds.
func (t *PieceTable) Count() int { return len(t.records) }
...
func (t *PieceTable) All() iter.Seq[*PieceRecord] {
	records := t.records

	return func(yield func(*PieceRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *PieceTable) FindByIndex(key int32) *PieceRecord {
...
func (t *PieceTable) GetByIndexOrError(key int32) (*PieceRecord, error) {
	position, found := t.byIndex[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `Piece` that corresponds to field `Index` value %v", key)
	}
...
func (t *PieceTable) ContainsIndex(key int32) bool {
...
func (t *PieceTable) Entries() iter.Seq2[int32, *PieceRecord] {
	records := t.records

	return func(yield func(int32, *PieceRecord) bool) {
		for i := range records {
...
func (t *PieceTable) Read(filename string) error {
	...
	records := make([]PieceRecord, count)
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "Piece.Index", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Piece.Index")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "Piece.Name", tabbit.KindScalar, false, tabbit.ElementString) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Piece.Name")
				for i := int32(0); i < count; {
		...
		case 3:
			if tabbit.CheckColumn(reader, column, "Piece.Tier", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Piece.Tier")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
