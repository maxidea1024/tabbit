...
func (t *TrimKitTable) All() iter.Seq[*TrimKitRecord] {
	records := t.records

	return func(yield func(*TrimKitRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *TrimKitTable) Entries() iter.Seq2[int32, *TrimKitRecord] {
	records := t.records

	return func(yield func(int32, *TrimKitRecord) bool) {
		for i := range records {
