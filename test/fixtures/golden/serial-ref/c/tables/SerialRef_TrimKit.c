...
static bool SerialRef_TrimKitParse(SerialRef_TrimKitTable_t* table, tb_reader* reader) {
  int32_t row;
  int32_t at;
  ...
  if (table->count > 0) {
    table->records = (SerialRef_TrimKitRecord_t*)tb_arena_alloc(
  ...
  for (at = 0; at < column_count && !tb_failed(reader); ++at) {
    ...
    case 2:
      ...
      for (row = 0; row < table->count && !tb_failed(reader); ++row) {
        ...
        record->bit_by_slot = (const SerialRef_BitRecord_t**)tb_arena_alloc(
          &table->arena, (size_t)element_count * sizeof *record->bit_by_slot);
        record->slot = (int32_t*)tb_arena_alloc(
          &table->arena, (size_t)element_count * sizeof *record->slot);

        if (element_count > 0
            && (record->bit_by_slot == NULL || record->slot == NULL))
...
bool SerialRef_TrimKitLoad(SerialRef_TrimKitTable_t* table, const char* filename,
  ...
  SerialRef_TrimKitTable_t loaded;
