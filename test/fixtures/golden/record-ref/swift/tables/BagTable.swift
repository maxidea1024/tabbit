...
public final class BagRecord {
    ...
    public var slots: SlotsEntry = SlotsEntry()
...
/// Every row of Bag.
public final class BagTable: Sequence {
    ...
    public func makeIterator() -> Array<BagRecord>.Iterator {
    ...
    public private(set) var records: [BagRecord] = []

    private var byIndex: [Int32: BagRecord] = [:]

    /// The row with this Index, or nil when the table has none.
    ///
    /// The lookup to reach for when a missing row is an ordinary answer - an optional
    /// reference, a key that came from user input.
    public func findByIndex(_ key: Int32) -> BagRecord? {
        byIndex[key]
    }
    ...
    public func getByIndexOrThrow(_ key: Int32) throws -> BagRecord {
    ...
    public subscript(_ key: Int32) -> BagRecord? {
        findByIndex(key)
    }
    ...
    /// row with.
    public var entries: LazyMapSequence<[BagRecord], (key: Int32, row: BagRecord)> {
    ...
    public func read(
        ...
        var loaded = [BagRecord]()
        loaded.reserveCapacity(count)

        for _ in 0 ..< count { loaded.append(BagRecord()) }
        var loadedByIndex = [Int32: BagRecord](
        ...
        for column in header.columns {
            ...
            switch column.tag {
            case 1:
                try Tcb.checkColumn(column, "Bag.Index", Tcb.kindScalar, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Bag.Index")
                var at = 0
                while at < count {
            ...
            case 2:
                try Tcb.checkColumn(column, "Bag.Slots.ItemId", Tcb.kindArray, false, Tcb.elementI32)
            ...
            case 3:
                try Tcb.checkColumn(column, "Bag.Slots.Count", Tcb.kindArray, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Bag.Slots.Count")
                for record in loaded {
                    let elementCount = Swift.max(0, try cursor.nextLength())
                    record.slots.count = []
