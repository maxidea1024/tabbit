...
/// Every row of Seal.
public final class SealTable: Sequence {
    ...
    public func makeIterator() -> Array<SealRecord>.Iterator {
    ...
    public private(set) var records: [SealRecord] = []

    private var byIndex: [Tcb.Uuid: SealRecord] = [:]

    /// The row with this Index, or nil when the table has none.
    ///
    /// The lookup to reach for when a missing row is an ordinary answer - an optional
    /// reference, a key that came from user input.
    public func findByIndex(_ key: Tcb.Uuid) -> SealRecord? {
        byIndex[key]
    }
    ...
    public func getByIndexOrThrow(_ key: Tcb.Uuid) throws -> SealRecord {
    ...
    public subscript(_ key: Tcb.Uuid) -> SealRecord? {
        findByIndex(key)
    }
    ...
    /// row with.
    public var entries: LazyMapSequence<[SealRecord], (key: Tcb.Uuid, row: SealRecord)> {
    ...
    public func read(
        ...
        var loaded = [SealRecord]()
        loaded.reserveCapacity(count)

        for _ in 0 ..< count { loaded.append(SealRecord()) }
        var loadedByIndex = [Tcb.Uuid: SealRecord](
