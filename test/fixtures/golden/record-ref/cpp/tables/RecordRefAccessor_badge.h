...
#ifndef TABBIT_GENERATED_RECORDREFACCESSOR_BADGE_H
#define TABBIT_GENERATED_RECORDREFACCESSOR_BADGE_H
...
class BadgeTable {
 public:
  const std::vector<BadgeRecord>& records() const { return records_; }
  ...
  typename std::vector<BadgeRecord>::const_iterator begin() const {
    return records_.begin();
  }

  typename std::vector<BadgeRecord>::const_iterator end() const {
    return records_.end();
  }
  ...
  const BadgeRecord* find_by_index(std::int32_t key) const {
  ...
  const BadgeRecord& get_by_index_or_throw(std::int32_t key) const {
    const BadgeRecord* found = find_by_index(key);

    if (found == nullptr) {
      throw tabbit::RecordNotFound(
        std::string("there is no record in table `Badge` that corresponds to "
  ...
  void read(const std::string& filename) {
    ...
    std::vector<BadgeRecord> records;
 ...
 private:
  ...
  void publish(std::vector<BadgeRecord>&& records) {
  ...
  std::vector<BadgeRecord> records_;
  std::unordered_map<std::int32_t, std::size_t> by_index_;
};

}  // namespace record_ref

#endif  // TABBIT_GENERATED_RECORDREFACCESSOR_BADGE_H
