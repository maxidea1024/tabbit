...
#ifndef TABBIT_GENERATED_RECORDREFACCESSOR_KIT_H
#define TABBIT_GENERATED_RECORDREFACCESSOR_KIT_H
...
struct KitRecord {
  ...
  std::int32_t index = 0;
  /// element 1, the reference
  std::vector<KitRecord_part_entry> part;
};
      ...
      switch (column.tag) {
        ...
        case 3: {
          ...
          for (std::size_t i = 0; i < row_count; ++i) {
            auto& record = records[i];
            const std::int32_t element_count = cursor.next_length();
            if (record.part.size() != static_cast<std::size_t>(element_count)) {
              throw tabbit::TcbError(
                  "Kit.part: the file gives this row a different "
            ...
            for (std::int32_t j = 0; j < element_count; ++j) {
              record.part[static_cast<std::size_t>(j)].count = cursor.next_i32();
            }
          }
          break;
        }
...
#endif  // TABBIT_GENERATED_RECORDREFACCESSOR_KIT_H
