...
#ifndef TABBIT_GENERATED_RECORDREFACCESSOR_POSE_H
#define TABBIT_GENERATED_RECORDREFACCESSOR_POSE_H
...
struct PoseRecord {
  ...
  std::int32_t index = 0;
  /// element 1, a string key
  std::vector<PoseRecord_step_entry> step;
};

/// A record member pointing at a table keyed by a string.
class PoseTable {
 public:
  const std::vector<PoseRecord>& records() const { return records_; }
  ...
  typename std::vector<PoseRecord>::const_iterator begin() const {
    return records_.begin();
  }

  typename std::vector<PoseRecord>::const_iterator end() const {
    return records_.end();
  }
  ...
  const PoseRecord* find_by_index(std::int32_t key) const {
  ...
  const PoseRecord& get_by_index_or_throw(std::int32_t key) const {
    const PoseRecord* found = find_by_index(key);

    if (found == nullptr) {
      throw tabbit::RecordNotFound(
        std::string("there is no record in table `Pose` that corresponds to "
 ...
 private:
  ...
  void publish(std::vector<PoseRecord>&& records) {
...
#endif  // TABBIT_GENERATED_RECORDREFACCESSOR_POSE_H
