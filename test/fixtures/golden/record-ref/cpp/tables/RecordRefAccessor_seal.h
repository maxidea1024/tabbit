...
#ifndef TABBIT_GENERATED_RECORDREFACCESSOR_SEAL_H
#define TABBIT_GENERATED_RECORDREFACCESSOR_SEAL_H
...
class SealTable {
 public:
  const std::vector<SealRecord>& records() const { return records_; }
  ...
  typename std::vector<SealRecord>::const_iterator begin() const {
    return records_.begin();
  }

  typename std::vector<SealRecord>::const_iterator end() const {
    return records_.end();
  }
  ...
  const SealRecord* find_by_index(tabbit::Uuid key) const {
  ...
  const SealRecord& get_by_index_or_throw(tabbit::Uuid key) const {
    const SealRecord* found = find_by_index(key);

    if (found == nullptr) {
      throw tabbit::RecordNotFound(
        std::string("there is no record in table `Seal` that corresponds to "
 ...
 private:
  ...
  void publish(std::vector<SealRecord>&& records) {
  ...
  std::vector<SealRecord> records_;
  std::unordered_map<tabbit::Uuid, std::size_t> by_index_;
};

}  // namespace record_ref

#endif  // TABBIT_GENERATED_RECORDREFACCESSOR_SEAL_H
