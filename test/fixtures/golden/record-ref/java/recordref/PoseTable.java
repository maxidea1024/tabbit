...
/** Every row of Pose. */
public final class PoseTable implements Iterable<PoseRecord> {
    private List<PoseRecord> records = new ArrayList<>();
    private Map<Integer, PoseRecord> byIndex = new HashMap<>();
     ...
     */
    public List<PoseRecord> records() {
        return records;
    }
     ...
     */
    @Override
    public Iterator<PoseRecord> iterator() {
    ...
    public PoseRecord getByIndexOrThrow(int key) {
        PoseRecord record = byIndex.get(key);

        if (record == null) {
            throw new TcbReader.RecordNotFoundException(
                "there is no record in table `Pose` that corresponds to field "
                + "`Index` value " + key);
        }
     ...
     */
    public Iterable<Map.Entry<Integer, PoseRecord>> entries() {
        final List<PoseRecord> rows = records;

        return () -> new Iterator<Map.Entry<Integer, PoseRecord>>() {
            private final Iterator<PoseRecord> source = rows.iterator();
            ...
            @Override
            public Map.Entry<Integer, PoseRecord> next() {
                final PoseRecord row = source.next();
    ...
    public void read(Path filename) {
        ...
        List<PoseRecord> loaded = new ArrayList<>(count);
        Map<Integer, PoseRecord> loadedByIndex = new HashMap<>(count * 2);

        for (int i = 0; i < count; i++) {
            loaded.add(new PoseRecord());
        }
