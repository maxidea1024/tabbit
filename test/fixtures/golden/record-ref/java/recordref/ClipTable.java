...
/** Every row of Clip. */
public final class ClipTable implements Iterable<ClipRecord> {
    private List<ClipRecord> records = new ArrayList<>();
    private Map<String, ClipRecord> byIndex = new HashMap<>();
     ...
     */
    public List<ClipRecord> records() {
        return records;
    }
     ...
     */
    @Override
    public Iterator<ClipRecord> iterator() {
     ...
     */
    public ClipRecord findByIndex(String key) {
     ...
     */
    public ClipRecord getByIndexOrThrow(String key) {
        ClipRecord record = byIndex.get(key);

        if (record == null) {
            throw new TcbReader.RecordNotFoundException(
                "there is no record in table `Clip` that corresponds to field "
                + "`Index` value " + key);
        }
     ...
     */
    public Iterable<Map.Entry<String, ClipRecord>> entries() {
        final List<ClipRecord> rows = records;

        return () -> new Iterator<Map.Entry<String, ClipRecord>>() {
            private final Iterator<ClipRecord> source = rows.iterator();
            ...
            @Override
            public Map.Entry<String, ClipRecord> next() {
                final ClipRecord row = source.next();
    ...
    public void read(Path filename) {
        ...
        List<ClipRecord> loaded = new ArrayList<>(count);
        Map<String, ClipRecord> loadedByIndex = new HashMap<>(count * 2);

        for (int i = 0; i < count; i++) {
            loaded.add(new ClipRecord());
        }
            ...
            switch (column.tag) {
                ...
                case 2: {
                    ...
                    for (int i = 0; i < count; ) {
                        ...
                        for (; n > 0; n--, i++) {
                            loaded.get(i).length = cursor.runSameValue;
                        }
                    }
                    break;
                }
