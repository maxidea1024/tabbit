...
func (t *PoseTable) All() iter.Seq[*PoseRecord] {
	records := t.records

	return func(yield func(*PoseRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *PoseTable) Entries() iter.Seq2[int32, *PoseRecord] {
	records := t.records

	return func(yield func(int32, *PoseRecord) bool) {
		for i := range records {
