...
// ClipRecord was generated from test/fixtures/xlsx/record-ref/record-ref.xlsx : Records : AP2.
...
// ClipTable holds every row of Clip.
...
func (t *ClipTable) Records() []ClipRecord { return t.records }
...
func (t *ClipTable) All() iter.Seq[*ClipRecord] {
	records := t.records

	return func(yield func(*ClipRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *ClipTable) FindByIndex(key string) *ClipRecord {
...
func (t *ClipTable) GetByIndexOrError(key string) (*ClipRecord, error) {
	position, found := t.byIndex[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `Clip` that corresponds to field `Index` value %v", key)
	}
...
func (t *ClipTable) ContainsIndex(key string) bool {
...
func (t *ClipTable) Entries() iter.Seq2[string, *ClipRecord] {
	records := t.records

	return func(yield func(string, *ClipRecord) bool) {
		for i := range records {
...
func (t *ClipTable) Read(filename string) error {
