...
func (t *SealTable) Count() int { return len(t.records) }
...
func (t *SealTable) All() iter.Seq[*SealRecord] {
	records := t.records

	return func(yield func(*SealRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *SealTable) Entries() iter.Seq2[tabbit.UUID, *SealRecord] {
	records := t.records

	return func(yield func(tabbit.UUID, *SealRecord) bool) {
		for i := range records {
