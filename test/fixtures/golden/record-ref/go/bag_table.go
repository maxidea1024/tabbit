...
func (t *BagTable) All() iter.Seq[*BagRecord] {
	records := t.records

	return func(yield func(*BagRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *BagTable) Entries() iter.Seq2[int32, *BagRecord] {
	records := t.records

	return func(yield func(int32, *BagRecord) bool) {
		for i := range records {
