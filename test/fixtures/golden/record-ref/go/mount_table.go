...
func (t *MountTable) All() iter.Seq[*MountRecord] {
	records := t.records

	return func(yield func(*MountRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *MountTable) Entries() iter.Seq2[int32, *MountRecord] {
	records := t.records

	return func(yield func(int32, *MountRecord) bool) {
		for i := range records {
