...
class ClipRecord:
    ...
    __slots__ = ("index", "length", "pad", "pad2", "pad3")
    ...
    def __repr__(self):
        return "ClipRecord(index=%r, length=%r, pad=%r, pad2=%r, pad3=%r)" % (self.index, self.length, self.pad, self.pad2, self.pad3)
...
class ClipTable:
    ...
    def read(self, filename):
        ...
        records = [ClipRecord() for _ in range(count)]

        for column in columns:
            block_end = reader.position + column.byte_length
            if column.tag == 1:
                tabbit.check_column(column, "Clip.Index", tabbit.KIND_SCALAR, False, (tabbit.ELEMENT_STRING,))
                cursor = tabbit.ColumnCursor(reader, column, count, "Clip.Index")
                at = 0
                while at < count:
                ...
                cursor = tabbit.ColumnCursor(reader, column, count, "Clip.Length")
                at = 0
                while at < count:
                ...
                cursor = tabbit.ColumnCursor(reader, column, count, "Clip.Pad")
                at = 0
                while at < count:
                    n, value = cursor.next_same_i32(count - at)
                    for i in range(at, at + n):
                ...
                cursor = tabbit.ColumnCursor(reader, column, count, "Clip.Pad2")
                at = 0
                while at < count:
                    n, value = cursor.next_same_i32(count - at)
                    for i in range(at, at + n):
                ...
                cursor = tabbit.ColumnCursor(reader, column, count, "Clip.Pad3")
                at = 0
                while at < count:
                    n, value = cursor.next_same_i32(count - at)
                    for i in range(at, at + n):
