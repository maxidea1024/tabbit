...
impl ClipTable {
    ...
    pub fn iter(&self) -> std::slice::Iter<'_, ClipRecord> {
        self.records.iter()
    }
    ...
    pub fn find_by_index(&self, key: &str) -> Option<&ClipRecord> {
    ...
    /// error it returns.
    pub fn get_by_index_or_error(&self, key: &str) -> tabbit::Result<&ClipRecord> {
        self.by_index
            .get(key)
    ...
    pub fn entries(&self) -> impl Iterator<Item = (&String, &ClipRecord)> + '_ {
    ...
    pub fn read(&mut self, filename: &Path) -> tabbit::Result<()> {
        ...
        let mut records = vec![ClipRecord::default(); count];
...
impl<'a> IntoIterator for &'a ClipTable {
    type Item = &'a ClipRecord;
    type IntoIter = std::slice::Iter<'a, ClipRecord>;
