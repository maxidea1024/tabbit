...
#[derive(Clone, Debug, Default)]
pub struct KitTable {
    records: Vec<KitRecord>,
    by_index: HashMap<i32, usize>,
}

impl KitTable {
    /// Every row, in the order the sheet declared them.
    pub fn records(&self) -> &[KitRecord] {
        &self.records
    }
    ...
    pub fn iter(&self) -> std::slice::Iter<'_, KitRecord> {
        self.records.iter()
    }
    ...
    pub fn find_by_index(&self, key: i32) -> Option<&KitRecord> {
    ...
    /// error it returns.
    pub fn get_by_index_or_error(&self, key: i32) -> tabbit::Result<&KitRecord> {
        self.by_index
            .get(&key)
    ...
    pub fn entries(&self) -> impl Iterator<Item = (&i32, &KitRecord)> + '_ {
    ...
    pub fn read(&mut self, filename: &Path) -> tabbit::Result<()> {
        ...
        let mut records = vec![KitRecord::default(); count];
        ...
        for column in &header.columns {
            ...
            match column.tag {
                ...
                3 => {
                    ...
                    for record in records.iter_mut() {
                        let element_count = cursor.next_length()?.max(0) as usize;
                        if record.part.len() != element_count {
                            return Err(tabbit::Error::ColumnMismatch {
                                field: "Kit.part",
...
impl<'a> IntoIterator for &'a KitTable {
    type Item = &'a KitRecord;
    type IntoIter = std::slice::Iter<'a, KitRecord>;
