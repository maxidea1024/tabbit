...
impl SealTable {
    ...
    pub fn iter(&self) -> std::slice::Iter<'_, SealRecord> {
        self.records.iter()
    }
    ...
    pub fn find_by_index(&self, key: tabbit::Uuid) -> Option<&SealRecord> {
    ...
    /// error it returns.
    pub fn get_by_index_or_error(&self, key: tabbit::Uuid) -> tabbit::Result<&SealRecord> {
        self.by_index
            .get(&key)
    ...
    pub fn entries(&self) -> impl Iterator<Item = (&tabbit::Uuid, &SealRecord)> + '_ {
    ...
    pub fn read(&mut self, filename: &Path) -> tabbit::Result<()> {
        ...
        let mut records = vec![SealRecord::default(); count];
        ...
        for column in &header.columns {
            ...
            match column.tag {
                ...
                2 => {
                    ...
                    let mut cursor = tabbit::TcbColumnCursor::new(&mut reader, column, header.row_count, "Seal.Label")?;
                    let mut at = 0usize;
                ...
                3 => {
                    ...
                    let mut cursor = tabbit::TcbColumnCursor::new(&mut reader, column, header.row_count, "Seal.Pad")?;
                    let mut at = 0usize;
                    while at < records.len() {
                ...
                4 => {
                    ...
                    let mut cursor = tabbit::TcbColumnCursor::new(&mut reader, column, header.row_count, "Seal.Pad2")?;
                    let mut at = 0usize;
                    while at < records.len() {
                        let (n, value) = cursor.next_same_i32((records.len() - at) as i32)?;
                        for _ in 0..n {
                ...
                5 => {
                    ...
                    let mut cursor = tabbit::TcbColumnCursor::new(&mut reader, column, header.row_count, "Seal.Pad3")?;
                    let mut at = 0usize;
                    while at < records.len() {
                        let (n, value) = cursor.next_same_i32((records.len() - at) as i32)?;
                        for _ in 0..n {
                            records[at].pad3 = value;
                            at += 1;
                        }
                    }
                }
...
impl<'a> IntoIterator for &'a SealTable {
    type Item = &'a SealRecord;
    type IntoIter = std::slice::Iter<'a, SealRecord>;
