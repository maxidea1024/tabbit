...
impl BadgeTable {
    ...
    pub fn iter(&self) -> std::slice::Iter<'_, BadgeRecord> {
        self.records.iter()
    }
    ...
    pub fn find_by_index(&self, key: i32) -> Option<&BadgeRecord> {
    ...
    /// error it returns.
    pub fn get_by_index_or_error(&self, key: i32) -> tabbit::Result<&BadgeRecord> {
        self.by_index
            .get(&key)
    ...
    pub fn entries(&self) -> impl Iterator<Item = (&i32, &BadgeRecord)> + '_ {
    ...
    pub fn read(&mut self, filename: &Path) -> tabbit::Result<()> {
        ...
        let mut records = vec![BadgeRecord::default(); count];
...
impl<'a> IntoIterator for &'a BadgeTable {
    type Item = &'a BadgeRecord;
    type IntoIter = std::slice::Iter<'a, BadgeRecord>;
