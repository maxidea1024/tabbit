...
static bool RecordRef_KitParse(RecordRef_KitTable_t* table, tb_reader* reader) {
  int32_t row;
  int32_t at;
  ...
  if (table->count > 0) {
    table->records = (RecordRef_KitRecord_t*)tb_arena_alloc(
  ...
  for (at = 0; at < column_count && !tb_failed(reader); ++at) {
    ...
    case 2:
      ...
      for (row = 0; row < table->count && !tb_failed(reader); ++row) {
        ...
        record->part_count = element_count;
        record->part = (struct RecordRef_KitRecord_t_part_entry*)tb_arena_alloc(
          &table->arena, (size_t)element_count * sizeof *record->part);

        if (element_count > 0 && record->part == NULL)
        ...
        for (element = 0; element < element_count && !tb_failed(reader); ++element)
          (void)tb_cursor_next_i32(&cursor, &record->part[element].item_id_index);
      }
      break;
