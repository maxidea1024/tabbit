...
function LoadoutTable:readBytes(data)
  ...
  for c = 1, #columns do
    ...
    elseif column.tag == 2 then
      tcb.checkColumn(column, "Loadout.Slot.ItemId", tcb.KIND_ARRAY, false, { tcb.ELEMENT_I32 })
    ...
    elseif column.tag == 3 then
      tcb.checkColumn(column, "Loadout.Slot.SwapId", tcb.KIND_ARRAY, false, { tcb.ELEMENT_I32 })
      local cursor = tcb.newCursor(reader, column, count, "Loadout.Slot.SwapId")
      for i = 1, count do
        local record = records[i]
        local elementCount = cursor:nextLength()
