...
---@field pad2 integer
---@field pad3 integer
local ItemRecordMeta = tcb.strictType("a `Item` row", { "index", "name", "pad", "pad2", "pad3" })

---@return ItemRecord
local function newItemRecord()
  return setmetatable({
    index = 0,
    name = "",
    pad = 0,
...
---@class ItemTable
---@field records ItemRecord[]
local ItemTable = {}

local ItemTableMeta = tcb.strictInstance("the `Item` table", ItemTable, { "records", "byIndex" })
...
function ItemTable:containsIndex(key)
...
function ItemTable:readBytes(data)
  ...
  for c = 1, #columns do
    ...
    if column.tag == 1 then
      tcb.checkColumn(column, "Item.Index", tcb.KIND_SCALAR, false, { tcb.ELEMENT_I32, tcb.ELEMENT_VARINT })
      local cursor = tcb.newCursor(reader, column, count, "Item.Index")
      local at = 0
    ...
    elseif column.tag == 2 then
      tcb.checkColumn(column, "Item.Name", tcb.KIND_SCALAR, false, { tcb.ELEMENT_STRING })
      local cursor = tcb.newCursor(reader, column, count, "Item.Name")
      local at = 0
    ...
    elseif column.tag == 3 then
      tcb.checkColumn(column, "Item.Pad", tcb.KIND_SCALAR, false, { tcb.ELEMENT_I32, tcb.ELEMENT_VARINT })
      local cursor = tcb.newCursor(reader, column, count, "Item.Pad")
      local at = 0
    ...
    elseif column.tag == 4 then
      tcb.checkColumn(column, "Item.Pad2", tcb.KIND_SCALAR, false, { tcb.ELEMENT_I32, tcb.ELEMENT_VARINT })
      local cursor = tcb.newCursor(reader, column, count, "Item.Pad2")
      local at = 0
    ...
    elseif column.tag == 5 then
      tcb.checkColumn(column, "Item.Pad3", tcb.KIND_SCALAR, false, { tcb.ELEMENT_I32, tcb.ELEMENT_VARINT })
      local cursor = tcb.newCursor(reader, column, count, "Item.Pad3")
      local at = 0
...
function ItemTable:read(filename)
