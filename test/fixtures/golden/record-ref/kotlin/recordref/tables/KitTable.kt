...
class KitRecord {
    ...
    var part: MutableList<PartEntry> = ArrayList()
...
/** Every row of Kit. */
class KitTable : Iterable<KitRecord> {
     ...
     */
    var records: MutableList<KitRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<KitRecord> = records.iterator()

    private var byIndex: HashMap<Int, KitRecord> = HashMap()
     ...
     */
    fun findByIndex(key: Int): KitRecord? = byIndex[key]
     ...
     */
    fun getByIndexOrThrow(key: Int): KitRecord =
     ...
     */
    operator fun get(key: Int): KitRecord? = findByIndex(key)
     ...
     */
    val entries: Sequence<Pair<Int, KitRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<KitRecord>(count)
        val loadedByIndex = HashMap<Int, KitRecord>(count * 2)

        repeat(count) { loaded.add(KitRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                ...
                3 -> {
                    ...
                    for (record in loaded) {
                        val elementCount = cursor.nextLength()
                        if (record.part.size != elementCount) {
                            throw TcbException(
