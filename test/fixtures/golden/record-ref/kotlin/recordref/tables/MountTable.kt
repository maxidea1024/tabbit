...
class MountRecord {
    ...
    var rig: MutableList<RigEntry> = MutableList(2) { RigEntry() }
    ...
    class RigEntry {
        /** element 1, two levels in */
        var core: RigEntryCore = RigEntryCore()
    }

}

/** Every row of Mount. */
class MountTable : Iterable<MountRecord> {
     ...
     */
    var records: MutableList<MountRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<MountRecord> = records.iterator()

    private var byIndex: HashMap<Int, MountRecord> = HashMap()
     ...
     */
    fun findByIndex(key: Int): MountRecord? = byIndex[key]
     ...
     */
    fun getByIndexOrThrow(key: Int): MountRecord =
     ...
     */
    operator fun get(key: Int): MountRecord? = findByIndex(key)
     ...
     */
    val entries: Sequence<Pair<Int, MountRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<MountRecord>(count)
        val loadedByIndex = HashMap<Int, MountRecord>(count * 2)

        repeat(count) { loaded.add(MountRecord()) }
