...
/** Every row of Clip. */
class ClipTable : Iterable<ClipRecord> {
     ...
     */
    var records: MutableList<ClipRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<ClipRecord> = records.iterator()

    private var byIndex: HashMap<String, ClipRecord> = HashMap()
     ...
     */
    fun findByIndex(key: String): ClipRecord? = byIndex[key]
     ...
     */
    fun getByIndexOrThrow(key: String): ClipRecord =
     ...
     */
    operator fun get(key: String): ClipRecord? = findByIndex(key)
     ...
     */
    val entries: Sequence<Pair<String, ClipRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<ClipRecord>(count)
        val loadedByIndex = HashMap<String, ClipRecord>(count * 2)

        repeat(count) { loaded.add(ClipRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "Clip.Index", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "Clip.Index")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "Clip.Length", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Clip.Length")
                    var at = 0
                    while (at < count) {
                ...
                3 -> {
                    checkColumn(column, "Clip.Pad", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Clip.Pad")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                4 -> {
                    checkColumn(column, "Clip.Pad2", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Clip.Pad2")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                5 -> {
                    checkColumn(column, "Clip.Pad3", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Clip.Pad3")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
