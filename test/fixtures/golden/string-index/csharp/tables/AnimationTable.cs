...
namespace Tabbit.Fixtures.StringIndex
{
    ...
    public partial class AnimationTable : IEnumerable<AnimationRecord>
    {
        ...
        /// Field names.
        /// </summary>
        public static readonly string[] FieldNames = { "Index", "Blend", "Slot" };
        ...
        public List<object[]> BuildObjectValueMap()
        {
            ...
            foreach (var r in _records)
                result.Add(new object[] { r.Index, r.Blend, r.Slot });
        ...
        public Dictionary<int, AnimationRecord> RecordsBySlot => _recordsBySlot;
        private Dictionary<int, AnimationRecord> _recordsBySlot = new Dictionary<int, AnimationRecord>();
        ...
        public AnimationRecord FindBySlot(int key)
            => _recordsBySlot.TryGetValue(key, out AnimationRecord record) ? record : null;
        ...
        {
            if (!_recordsBySlot.TryGetValue(key, out AnimationRecord record))
                throw new TabbitException($"There is no record in table `Animation` that corresponds to field `Slot` value {key}");

            return record;
        }

        /// <summary>Whether the table holds a row with this `Slot`.</summary>
        public bool ContainsSlot(int key) => _recordsBySlot.ContainsKey(key);
        ...
        {
            var bytes = await StringIndexAccessor.ReadAllBytesAsync(filename);

            // Unconditionally, and with whatever `StringIndexAccessor.EncryptionKey` and `StringIndexAccessor.MacKey`
            ...
            var reader = new TcbReader(TcbTable.Open(
                bytes, StringIndexAccessor.EncryptionKey, StringIndexAccessor.MacKey, StringIndexAccessor.VerifyMac));
        ...
        public Task ReadAsync(TcbReader reader)
        {
            ...
            var recordsBySlot = new Dictionary<int, AnimationRecord>(count);
            foreach (var record in records)
