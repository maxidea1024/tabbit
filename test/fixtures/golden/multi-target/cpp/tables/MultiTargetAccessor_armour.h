...
#ifndef TABBIT_GENERATED_MULTITARGETACCESSOR_ARMOUR_H
#define TABBIT_GENERATED_MULTITARGETACCESSOR_ARMOUR_H
...
class ArmourTable {
 public:
  const std::vector<ArmourRecord>& records() const { return records_; }

  /// The row with this Index, or nullptr when the table has none.
  ///
  /// The lookup to reach for when a missing row is an ordinary answer - an optional
  /// reference, a key that came from user input.
  const ArmourRecord* find_by_index(std::int32_t key) const {
  ...
  const ArmourRecord& get_by_index_or_throw(std::int32_t key) const {
    const ArmourRecord* found = find_by_index(key);

    if (found == nullptr) {
      throw tabbit::RecordNotFound(
        std::string("there is no record in table `Armour` that corresponds to "
  ...
  void read(const std::string& filename) {
    ...
    std::vector<ArmourRecord> records;
 ...
 private:
  ...
  void publish(std::vector<ArmourRecord>&& records) {
  ...
  std::vector<ArmourRecord> records_;
  std::unordered_map<std::int32_t, std::size_t> by_index_;
};

}  // namespace multi_target

#endif  // TABBIT_GENERATED_MULTITARGETACCESSOR_ARMOUR_H
