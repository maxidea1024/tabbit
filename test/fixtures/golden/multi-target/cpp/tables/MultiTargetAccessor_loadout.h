  ...
  void read(const std::string& filename) {
    ...
    for (const tabbit::Column& column : header.columns) {
      ...
      switch (column.tag) {
        ...
        case 2: {
          tabbit::check_column(column, "Loadout.Slot.Pick", tabbit::kKindFixedArray, 2, false, {tabbit::kElementI32, tabbit::kElementVarint});
          tabbit::TcbColumnCursor cursor(reader, column, header.row_count, "Loadout.Slot.Pick");
          for (std::size_t i = 0; i < row_count; ++i) {
            auto& record = records[i];
            record.slot.resize(2);
            for (std::size_t j = 0; j < 2; ++j) {
              record.slot[j].pick = cursor.next_i32();
            }
          }
          break;
        }
