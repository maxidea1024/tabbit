...
#ifndef TABBIT_GENERATED_MULTITARGETACCESSOR_LOADOUT_H
#define TABBIT_GENERATED_MULTITARGETACCESSOR_LOADOUT_H
...
struct LoadoutRecord_slot_entry {
  ...
  LoadoutSlotPickTarget pick_target = LoadoutSlotPickTarget::None;
  ...
  const WeaponRecord* weapon_by_pick() const {
    return pick_target == LoadoutSlotPickTarget::Weapon
  ...
  const ArmourRecord* armour_by_pick() const {
    return pick_target == LoadoutSlotPickTarget::Armour
  ...
  void read(const std::string& filename) {
    ...
    for (const tabbit::Column& column : header.columns) {
      ...
      switch (column.tag) {
        ...
        case 2: {
          tabbit::check_column(column, "Loadout.Slot.Pick", tabbit::kKindArray, false, {tabbit::kElementI32, tabbit::kElementVarint});
          tabbit::TcbColumnCursor cursor(reader, column, header.row_count, "Loadout.Slot.Pick");
          for (std::size_t i = 0; i < row_count; ++i) {
            ...
            for (std::int32_t j = 0; j < element_count; ++j) {
              record.slot[static_cast<std::size_t>(j)].pick = cursor.next_i32();
            }
          }
          break;
        }
...
#endif  // TABBIT_GENERATED_MULTITARGETACCESSOR_LOADOUT_H
