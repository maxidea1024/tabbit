...
#ifndef TABBIT_GENERATED_MULTITARGETACCESSOR_BANNER_H
#define TABBIT_GENERATED_MULTITARGETACCESSOR_BANNER_H
...
class BannerTable {
 public:
  const std::vector<BannerRecord>& records() const { return records_; }

  /// The row with this Index, or nullptr when the table has none.
  ///
  /// The lookup to reach for when a missing row is an ordinary answer - an optional
  /// reference, a key that came from user input.
  const BannerRecord* find_by_index(std::int32_t key) const {
  ...
  const BannerRecord& get_by_index_or_throw(std::int32_t key) const {
    const BannerRecord* found = find_by_index(key);

    if (found == nullptr) {
      throw tabbit::RecordNotFound(
        std::string("there is no record in table `Banner` that corresponds to "
  ...
  void read(const std::string& filename) {
    ...
    std::vector<BannerRecord> records;
 ...
 private:
  ...
  void publish(std::vector<BannerRecord>&& records) {
...
#endif  // TABBIT_GENERATED_MULTITARGETACCESSOR_BANNER_H
