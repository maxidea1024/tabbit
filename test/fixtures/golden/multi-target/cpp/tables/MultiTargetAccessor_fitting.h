...
#ifndef TABBIT_GENERATED_MULTITARGETACCESSOR_FITTING_H
#define TABBIT_GENERATED_MULTITARGETACCESSOR_FITTING_H
...
class FittingTable {
 public:
  const std::vector<FittingRecord>& records() const { return records_; }

  /// The row with this Index, or nullptr when the table has none.
  ///
  /// The lookup to reach for when a missing row is an ordinary answer - an optional
  /// reference, a key that came from user input.
  const FittingRecord* find_by_index(std::int32_t key) const {
  ...
  const FittingRecord& get_by_index_or_throw(std::int32_t key) const {
    const FittingRecord* found = find_by_index(key);

    if (found == nullptr) {
      throw tabbit::RecordNotFound(
        std::string("there is no record in table `Fitting` that corresponds to "
  ...
  void read(const std::string& filename) {
    ...
    std::vector<FittingRecord> records;
 ...
 private:
  ...
  void publish(std::vector<FittingRecord>&& records) {
  ...
  std::vector<FittingRecord> records_;
  std::unordered_map<std::int32_t, std::size_t> by_index_;
};

}  // namespace multi_target

#endif  // TABBIT_GENERATED_MULTITARGETACCESSOR_FITTING_H
