...
// TrinketRecord was generated from test/fixtures/xlsx/multi-target/multi-target.xlsx : Catalogues : R2.
// only the wide column names this
type TrinketRecord struct {
...
// TrinketTable holds every row of Trinket.
...
func (t *TrinketTable) Records() []TrinketRecord { return t.records }

// FindByIndex returns the row with this Index, or nil when the table has none.
//
// The lookup to reach for when a missing row is an ordinary answer - an optional
// reference, a key that came from user input.
func (t *TrinketTable) FindByIndex(key int32) *TrinketRecord {
...
func (t *TrinketTable) GetByIndexOrError(key int32) (*TrinketRecord, error) {
	position, found := t.byIndex[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `Trinket` that corresponds to field `Index` value %v", key)
	}
...
func (t *TrinketTable) ContainsIndex(key int32) bool {
	_, found := t.byIndex[key]
	return found
}

// Read loads the table from a .tcb file written by Tabbit.
func (t *TrinketTable) Read(filename string) error {
	...
	records := make([]TrinketRecord, count)
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "Trinket.Index", tabbit.KindScalar, 1, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Trinket.Index")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "Trinket.Name", tabbit.KindScalar, 1, false, tabbit.ElementString) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Trinket.Name")
				for i := int32(0); i < count; {
		...
		case 3:
			if tabbit.CheckColumn(reader, column, "Trinket.Note", tabbit.KindScalar, 1, false, tabbit.ElementString) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Trinket.Note")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameString(count - i)
					for ; n > 0; n-- {
	...
	if err := reader.Err(); err != nil {
		return fmt.Errorf("Trinket: %w", err)
	}
