...
// WeaponRecord was generated from test/fixtures/xlsx/multi-target/multi-target.xlsx : Catalogues : B2.
...
// WeaponTable holds every row of Weapon.
...
func (t *WeaponTable) Records() []WeaponRecord { return t.records }

// FindByIndex returns the row with this Index, or nil when the table has none.
//
// The lookup to reach for when a missing row is an ordinary answer - an optional
// reference, a key that came from user input.
func (t *WeaponTable) FindByIndex(key int32) *WeaponRecord {
...
func (t *WeaponTable) GetByIndexOrError(key int32) (*WeaponRecord, error) {
	position, found := t.byIndex[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `Weapon` that corresponds to field `Index` value %v", key)
	}
...
func (t *WeaponTable) ContainsIndex(key int32) bool {
	_, found := t.byIndex[key]
	return found
}

// Read loads the table from a .tcb file written by Tabbit.
func (t *WeaponTable) Read(filename string) error {
	...
	records := make([]WeaponRecord, count)
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "Weapon.Index", tabbit.KindScalar, 1, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Weapon.Index")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "Weapon.Name", tabbit.KindScalar, 1, false, tabbit.ElementString) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Weapon.Name")
				for i := int32(0); i < count; {
		...
		case 3:
			if tabbit.CheckColumn(reader, column, "Weapon.Note", tabbit.KindScalar, 1, false, tabbit.ElementString) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Weapon.Note")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameString(count - i)
					for ; n > 0; n-- {
