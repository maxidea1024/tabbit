...
// BannerRecord was generated from test/fixtures/xlsx/multi-target/multi-target.xlsx : Catalogues : AH2.
...
// BannerTable holds every row of Banner.
...
func (t *BannerTable) Records() []BannerRecord { return t.records }

// FindByIndex returns the row with this Index, or nil when the table has none.
//
// The lookup to reach for when a missing row is an ordinary answer - an optional
// reference, a key that came from user input.
func (t *BannerTable) FindByIndex(key int32) *BannerRecord {
...
func (t *BannerTable) GetByIndexOrError(key int32) (*BannerRecord, error) {
	position, found := t.byIndex[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `Banner` that corresponds to field `Index` value %v", key)
	}
...
func (t *BannerTable) ContainsIndex(key int32) bool {
	_, found := t.byIndex[key]
	return found
}

// Read loads the table from a .tcb file written by Tabbit.
func (t *BannerTable) Read(filename string) error {
	...
	records := make([]BannerRecord, count)
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "Banner.Index", tabbit.KindScalar, 1, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Banner.Index")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "Banner.Name", tabbit.KindScalar, 1, false, tabbit.ElementString) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Banner.Name")
				for i := int32(0); i < count; {
		...
		case 3:
			if tabbit.CheckColumn(reader, column, "Banner.Note", tabbit.KindScalar, 1, false, tabbit.ElementString) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Banner.Note")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameString(count - i)
					for ; n > 0; n-- {
