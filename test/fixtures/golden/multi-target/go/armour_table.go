...
// ArmourRecord was generated from test/fixtures/xlsx/multi-target/multi-target.xlsx : Catalogues : J2.
// second target
type ArmourRecord struct {
...
// ArmourTable holds every row of Armour.
type ArmourTable struct {
	records []ArmourRecord
	byIndex map[int32]int
}

// Records returns every row, in the order the sheet declared them.
func (t *ArmourTable) Records() []ArmourRecord { return t.records }

// FindByIndex returns the row with this Index, or nil when the table has none.
//
// The lookup to reach for when a missing row is an ordinary answer - an optional
// reference, a key that came from user input.
func (t *ArmourTable) FindByIndex(key int32) *ArmourRecord {
...
func (t *ArmourTable) GetByIndexOrError(key int32) (*ArmourRecord, error) {
	position, found := t.byIndex[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `Armour` that corresponds to field `Index` value %v", key)
	}
...
func (t *ArmourTable) ContainsIndex(key int32) bool {
	_, found := t.byIndex[key]
	return found
}

// Read loads the table from a .tcb file written by Tabbit.
func (t *ArmourTable) Read(filename string) error {
	...
	records := make([]ArmourRecord, count)
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "Armour.Index", tabbit.KindScalar, 1, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Armour.Index")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "Armour.Name", tabbit.KindScalar, 1, false, tabbit.ElementString) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Armour.Name")
				for i := int32(0); i < count; {
		...
		case 3:
			if tabbit.CheckColumn(reader, column, "Armour.Note", tabbit.KindScalar, 1, false, tabbit.ElementString) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Armour.Note")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameString(count - i)
					for ; n > 0; n-- {
	...
	if err := reader.Err(); err != nil {
		return fmt.Errorf("Armour: %w", err)
	}
