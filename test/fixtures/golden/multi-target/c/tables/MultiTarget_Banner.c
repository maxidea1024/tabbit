...
static bool MultiTarget_BannerParse(MultiTarget_BannerTable_t* table, tb_reader* reader) {
  int32_t row;
  int32_t at;
  ...
  if (table->count > 0) {
    table->records = (MultiTarget_BannerRecord_t*)tb_arena_alloc(
  ...
  for (row = 0; row < table->count; ++row) {
    MultiTarget_BannerRecord_t* record = &table->records[row];
  ...
  for (at = 0; at < column_count && !tb_failed(reader); ++at) {
    ...
    case 1:
      (void)tb_check_column(reader, column, "Banner.Index", TB_KIND_SCALAR, 1, false, TB_ELEMENT_MASK(TB_ELEMENT_I32) | TB_ELEMENT_MASK(TB_ELEMENT_VARINT));

      (void)tb_cursor_init(&cursor, reader, column, table->count, "Banner.Index");
    ...
    case 2:
      (void)tb_check_column(reader, column, "Banner.Name", TB_KIND_SCALAR, 1, false, TB_ELEMENT_MASK(TB_ELEMENT_STRING));

      (void)tb_cursor_init(&cursor, reader, column, table->count, "Banner.Name");
    ...
    case 3:
      (void)tb_check_column(reader, column, "Banner.Note", TB_KIND_SCALAR, 1, false, TB_ELEMENT_MASK(TB_ELEMENT_STRING));

      (void)tb_cursor_init(&cursor, reader, column, table->count, "Banner.Note");
...
bool MultiTarget_BannerLoad(MultiTarget_BannerTable_t* table, const char* filename,
  ...
  MultiTarget_BannerTable_t loaded;
...
void MultiTarget_BannerFree(MultiTarget_BannerTable_t* table) {
  tb_arena_free(&table->arena);

  memset(table, 0, sizeof *table);
}

const MultiTarget_BannerRecord_t* MultiTarget_BannerFindByIndex(
  const MultiTarget_BannerTable_t* table, int32_t key) {
  int32_t position = tb_index_find(table->by_index, table->count, key);

  return position < 0 ? NULL : &table->records[position];
}

bool MultiTarget_BannerContainsIndex(
  const MultiTarget_BannerTable_t* table, int32_t key) {
