...
static bool MultiTarget_ArmourParse(MultiTarget_ArmourTable_t* table, tb_reader* reader) {
  int32_t row;
  int32_t at;
  ...
  if (table->count > 0) {
    table->records = (MultiTarget_ArmourRecord_t*)tb_arena_alloc(
  ...
  for (row = 0; row < table->count; ++row) {
    MultiTarget_ArmourRecord_t* record = &table->records[row];
  ...
  for (at = 0; at < column_count && !tb_failed(reader); ++at) {
    ...
    case 1:
      (void)tb_check_column(reader, column, "Armour.Index", TB_KIND_SCALAR, 1, false, TB_ELEMENT_MASK(TB_ELEMENT_I32) | TB_ELEMENT_MASK(TB_ELEMENT_VARINT));

      (void)tb_cursor_init(&cursor, reader, column, table->count, "Armour.Index");
    ...
    case 2:
      (void)tb_check_column(reader, column, "Armour.Name", TB_KIND_SCALAR, 1, false, TB_ELEMENT_MASK(TB_ELEMENT_STRING));

      (void)tb_cursor_init(&cursor, reader, column, table->count, "Armour.Name");
    ...
    case 3:
      (void)tb_check_column(reader, column, "Armour.Note", TB_KIND_SCALAR, 1, false, TB_ELEMENT_MASK(TB_ELEMENT_STRING));

      (void)tb_cursor_init(&cursor, reader, column, table->count, "Armour.Note");
...
bool MultiTarget_ArmourLoad(MultiTarget_ArmourTable_t* table, const char* filename,
  ...
  MultiTarget_ArmourTable_t loaded;
