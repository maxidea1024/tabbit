...
static bool MultiTarget_LoadoutParse(MultiTarget_LoadoutTable_t* table, tb_reader* reader) {
  int32_t row;
  int32_t at;
  ...
  if (table->count > 0) {
    table->records = (MultiTarget_LoadoutRecord_t*)tb_arena_alloc(
  ...
  for (at = 0; at < column_count && !tb_failed(reader); ++at) {
    ...
    case 1:
      ...
      for (row = 0; row < table->count && !tb_failed(reader); ++row) {
        MultiTarget_LoadoutRecord_t* record = &table->records[row];
    ...
    case 2:
      (void)tb_check_column(reader, column, "Loadout.Slot.Pick", TB_KIND_ARRAY, false, TB_ELEMENT_MASK(TB_ELEMENT_I32) | TB_ELEMENT_MASK(TB_ELEMENT_VARINT));

      (void)tb_cursor_init(&cursor, reader, column, table->count, "Loadout.Slot.Pick");
      ...
      for (row = 0; row < table->count && !tb_failed(reader); ++row) {
        ...
        record->slot = (struct MultiTarget_LoadoutRecord_t_slot_entry*)tb_arena_alloc(
        ...
        for (element = 0; element < element_count && !tb_failed(reader); ++element)
          (void)tb_cursor_next_i32(&cursor, &record->slot[element].pick);
      }
      break;
...
bool MultiTarget_LoadoutLoad(MultiTarget_LoadoutTable_t* table, const char* filename,
  ...
  MultiTarget_LoadoutTable_t loaded;
...
const MultiTarget_WeaponRecord_t* MultiTarget_LoadoutSlotWeaponByPick(
  ...
  if (element->pick_target != MULTI_TARGET_LOADOUT_SLOT_PICK_TARGET_WEAPON)
    return NULL;
...
const MultiTarget_ArmourRecord_t* MultiTarget_LoadoutSlotArmourByPick(
  const struct MultiTarget_LoadoutRecord_t_slot_entry* element) {
  if (element->pick_target != MULTI_TARGET_LOADOUT_SLOT_PICK_TARGET_ARMOUR)
    return NULL;
