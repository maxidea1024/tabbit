...
public final class FittingTable {
    private List<FittingRecord> records = new ArrayList<>();
    private Map<Integer, FittingRecord> byIndex = new HashMap<>();
     ...
     */
    public List<FittingRecord> records() {
        return records;
    }
     ...
     */
    public FittingRecord findByIndex(int key) {
     ...
     */
    public FittingRecord getByIndexOrThrow(int key) {
        FittingRecord record = byIndex.get(key);

        if (record == null) {
            throw new TcbReader.RecordNotFoundException(
                "there is no record in table `Fitting` that corresponds to field "
                + "`Index` value " + key);
        }
    ...
    public void read(Path filename) {
        ...
        List<FittingRecord> loaded = new ArrayList<>(count);
        Map<Integer, FittingRecord> loadedByIndex = new HashMap<>(count * 2);

        for (int i = 0; i < count; i++) {
            loaded.add(new FittingRecord());
        }
            ...
            switch (column.tag) {
                ...
                case 2: {
                    ...
                    for (int i = 0; i < count; ) {
                        int n = cursor.nextSameI32(count - i);
                        for (; n > 0; n--, i++) {
                            loaded.get(i).main.pick = cursor.runSameValue;
                        }
                    }
                    break;
                }
        ...
        for (FittingRecord record : loaded) {
