...
public final class BannerTable {
    private List<BannerRecord> records = new ArrayList<>();
    private Map<Integer, BannerRecord> byIndex = new HashMap<>();
     ...
     */
    public List<BannerRecord> records() {
        return records;
    }
     ...
     */
    public BannerRecord findByIndex(int key) {
     ...
     */
    public BannerRecord getByIndexOrThrow(int key) {
        BannerRecord record = byIndex.get(key);

        if (record == null) {
            throw new TcbReader.RecordNotFoundException(
                "there is no record in table `Banner` that corresponds to field "
                + "`Index` value " + key);
        }
    ...
    public void read(Path filename) {
        ...
        List<BannerRecord> loaded = new ArrayList<>(count);
        Map<Integer, BannerRecord> loadedByIndex = new HashMap<>(count * 2);

        for (int i = 0; i < count; i++) {
            loaded.add(new BannerRecord());
        }
        ...
        for (BannerRecord record : loaded) {
