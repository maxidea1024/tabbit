...
public final class RackTable {
    private List<RackRecord> records = new ArrayList<>();
    private Map<Integer, RackRecord> byIndex = new HashMap<>();
     ...
     */
    public List<RackRecord> records() {
        return records;
    }
     ...
     */
    public RackRecord findByIndex(int key) {
     ...
     */
    public RackRecord getByIndexOrThrow(int key) {
        RackRecord record = byIndex.get(key);

        if (record == null) {
            throw new TcbReader.RecordNotFoundException(
                "there is no record in table `Rack` that corresponds to field "
                + "`Index` value " + key);
        }
    ...
    public void read(Path filename) {
        ...
        List<RackRecord> loaded = new ArrayList<>(count);
        Map<Integer, RackRecord> loadedByIndex = new HashMap<>(count * 2);

        for (int i = 0; i < count; i++) {
            loaded.add(new RackRecord());
        }
