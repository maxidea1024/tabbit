...
class WeaponTable {
  ...
  List<WeaponRecord> get records => _records;
  List<WeaponRecord> _records = [];

  Map<int, WeaponRecord> _byIndex = {};

  /// The row with this Index, or null when the table has none.
  ///
  /// The lookup to reach for when a missing row is an ordinary answer - an optional
  /// reference, a key that came from user input.
  WeaponRecord? findByIndex(int key) => _byIndex[key];
  ...
  WeaponRecord getByIndexOrThrow(int key) {
  ...
  void read(String filename) {
    ...
    final loaded = <WeaponRecord>[];
    final loadedByIndex = <int, WeaponRecord>{};

    for (var i = 0; i < count; i++) {
      loaded.add(WeaponRecord());
    }
    ...
    for (final column in header.columns) {
      ...
      switch (column.tag) {
        case 1:
          checkColumn(column, 'Weapon.Index', kindScalar, 1, false, [elementI32, elementVarint]);
          cursor = TcbColumnCursor(reader, column, count, 'Weapon.Index');
          for (var i = 0; i < count; ) {
          ...
          break;
        case 2:
          checkColumn(column, 'Weapon.Name', kindScalar, 1, false, [elementString]);
          cursor = TcbColumnCursor(reader, column, count, 'Weapon.Name');
          for (var i = 0; i < count; ) {
          ...
          break;
        case 3:
          checkColumn(column, 'Weapon.Note', kindScalar, 1, false, [elementString]);
          cursor = TcbColumnCursor(reader, column, count, 'Weapon.Note');
          for (var i = 0; i < count; ) {
            final (n, value) = cursor.nextSameString(count - i);
            for (var left = n; left > 0; left--, i++) {
              loaded[i].note = value;
