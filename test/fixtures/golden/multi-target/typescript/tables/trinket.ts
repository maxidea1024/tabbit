...
export class TrinketTable {
  ...
  /** All records. */
  public get records(): TrinketRecord[] { return this._records }
  private _records: TrinketRecord[] = []

  // Indexing by 'index'
  public get recordsByIndex(): Map<number, TrinketRecord> { return this._recordsByIndex }
  private _recordsByIndex: Map<number, TrinketRecord> = new Map<number, TrinketRecord>()
   ...
   */
  public findByIndex(key: number): TrinketRecord | undefined {
   ...
   */
  public getByIndexOrThrow(key: number): TrinketRecord {
    const found = this._recordsByIndex.get(key)
    if (!found)
      throw new Error(`There is no record in table "Trinket" that corresponds to field "index" value ${key}`)
  ...
  private readFromJson(json: string): void {
    ...
    const records: TrinketRecord[] = []

    if (this.isCompactRowFormatted(dataRows)) {
      for (const dataRow of dataRows) {
        const record = new TrinketRecord()
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (let i = 0; i < rowCount; ++i)
      records.push(new TrinketRecord())
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          tabbit.checkColumn(column, 'Trinket.Index', tabbit.KIND_SCALAR, 1, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Trinket.Index')
          ...
          break
        case 2:
          tabbit.checkColumn(column, 'Trinket.Name', tabbit.KIND_SCALAR, 1, false, [tabbit.ELEMENT_STRING])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Trinket.Name')
          for (let i = 0; i < rowCount; ) {
          ...
          break
        case 3:
          tabbit.checkColumn(column, 'Trinket.Note', tabbit.KIND_SCALAR, 1, false, [tabbit.ELEMENT_STRING])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Trinket.Note')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameString(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
   ...
   */
  private publish(records: TrinketRecord[]): void {
    const recordsByIndex = new Map<number, TrinketRecord>()
