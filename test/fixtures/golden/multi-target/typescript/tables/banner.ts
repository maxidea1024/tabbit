...
// Generated from test/fixtures/xlsx/multi-target/multi-target.xlsx : Catalogues : AH2
/** the wide column's last target */
export class BannerTable {
  /** Default constructor. */
  constructor() {
  }

  /** All records. */
  public get records(): BannerRecord[] { return this._records }
  private _records: BannerRecord[] = []

  // Indexing by 'index'
  public get recordsByIndex(): Map<number, BannerRecord> { return this._recordsByIndex }
  private _recordsByIndex: Map<number, BannerRecord> = new Map<number, BannerRecord>()
   ...
   */
  public findByIndex(key: number): BannerRecord | undefined {
   ...
   */
  public getByIndexOrThrow(key: number): BannerRecord {
    const found = this._recordsByIndex.get(key)
    if (!found)
      throw new Error(`There is no record in table "Banner" that corresponds to field "index" value ${key}`)
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          tabbit.checkColumn(column, 'Banner.Index', tabbit.KIND_SCALAR, 1, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Banner.Index')
          ...
          break
        case 2:
          tabbit.checkColumn(column, 'Banner.Name', tabbit.KIND_SCALAR, 1, false, [tabbit.ELEMENT_STRING])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Banner.Name')
          for (let i = 0; i < rowCount; ) {
          ...
          break
        case 3:
          tabbit.checkColumn(column, 'Banner.Note', tabbit.KIND_SCALAR, 1, false, [tabbit.ELEMENT_STRING])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Banner.Note')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameString(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
   ...
   */
  private publish(records: BannerRecord[]): void {
    const recordsByIndex = new Map<number, BannerRecord>()
