...
// Generated from test/fixtures/xlsx/multi-target/multi-target.xlsx : Catalogues : J2
/** second target */
export class ArmourTable {
  /** Default constructor. */
  constructor() {
  }

  /** All records. */
  public get records(): ArmourRecord[] { return this._records }
  private _records: ArmourRecord[] = []

  // Indexing by 'index'
  public get recordsByIndex(): Map<number, ArmourRecord> { return this._recordsByIndex }
  private _recordsByIndex: Map<number, ArmourRecord> = new Map<number, ArmourRecord>()
   ...
   */
  public findByIndex(key: number): ArmourRecord | undefined {
   ...
   */
  public getByIndexOrThrow(key: number): ArmourRecord {
    const found = this._recordsByIndex.get(key)
    if (!found)
      throw new Error(`There is no record in table "Armour" that corresponds to field "index" value ${key}`)
  ...
  private readFromJson(json: string): void {
    ...
    const records: ArmourRecord[] = []

    if (this.isCompactRowFormatted(dataRows)) {
      for (const dataRow of dataRows) {
        const record = new ArmourRecord()
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          tabbit.checkColumn(column, 'Armour.Index', tabbit.KIND_SCALAR, 1, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Armour.Index')
          ...
          break
        case 2:
          tabbit.checkColumn(column, 'Armour.Name', tabbit.KIND_SCALAR, 1, false, [tabbit.ELEMENT_STRING])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Armour.Name')
          for (let i = 0; i < rowCount; ) {
          ...
          break
        case 3:
          tabbit.checkColumn(column, 'Armour.Note', tabbit.KIND_SCALAR, 1, false, [tabbit.ELEMENT_STRING])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Armour.Note')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameString(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
   ...
   */
  private publish(records: ArmourRecord[]): void {
    const recordsByIndex = new Map<number, ArmourRecord>()
