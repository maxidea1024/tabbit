    ...
    public partial class BannerTable
    {
        ...
        public Record GetByIndexOrThrow(int key)
        {
            if (!_recordsByIndex.TryGetValue(key, out Record record))
                throw new TabbitException($"There is no record in table `Banner` that corresponds to field `Index` value {key}");
        ...
        public Task ReadAsync(TcbReader reader)
        {
            ...
            foreach (var column in columns)
            {
                ...
                {
                    case 1:
                        TcbTable.CheckColumn(column, "Banner.Index", TcbTable.KindScalar, 1, false, TcbTable.ElementI32, TcbTable.ElementVarint);
                        cursor = new TcbColumnCursor(reader, column, count, "Banner.Index");
                        for (int i = 0; i < count; )
                        {
                    ...
                    case 2:
                        TcbTable.CheckColumn(column, "Banner.Name", TcbTable.KindScalar, 1, false, TcbTable.ElementString);
                        cursor = new TcbColumnCursor(reader, column, count, "Banner.Name");
                        for (int i = 0; i < count; )
                        {
                            // One call per run of equal values, not one per row - over a
                            // run-length encoded column this is most of the decode.
                    ...
                    case 3:
                        TcbTable.CheckColumn(column, "Banner.Note", TcbTable.KindScalar, 1, false, TcbTable.ElementString);
                        cursor = new TcbColumnCursor(reader, column, count, "Banner.Note");
                        for (int i = 0; i < count; )
                        {
                            // One call per run of equal values, not one per row - over a
                            // run-length encoded column this is most of the decode.
