...
class ArmourTable {
     ...
     */
    var records: MutableList<ArmourRecord> = ArrayList()
        private set

    private var byIndex: HashMap<Int, ArmourRecord> = HashMap()
     ...
     */
    fun findByIndex(key: Int): ArmourRecord? = byIndex[key]
     ...
     */
    fun getByIndexOrThrow(key: Int): ArmourRecord =
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<ArmourRecord>(count)
        val loadedByIndex = HashMap<Int, ArmourRecord>(count * 2)

        repeat(count) { loaded.add(ArmourRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "Armour.Index", KIND_SCALAR, 1, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Armour.Index")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "Armour.Name", KIND_SCALAR, 1, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "Armour.Name")
                    var at = 0
                    while (at < count) {
                ...
                3 -> {
                    checkColumn(column, "Armour.Note", KIND_SCALAR, 1, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "Armour.Note")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameString(count - at)
                        while (n > 0) {
