...
{
    [System.Serializable]
    public partial class TownPriceColumnRecord
    {
        #region Values
        public int Goods => _goods_Goods_index;
        public GoodsRecord GoodsByGoods => _goods;

        public int At => _at;
        #endregion

        #region Reference wiring
        public void SetReference_Goods_INTERNAL(GoodsRecord value) => _goods = value;
        #endregion

        #region Storage
        internal GoodsRecord _goods;
        internal int _goods_Goods_index;
        internal int _at;
        #endregion
        ...
        public override string ToString()
        {
            ...
            sb.Append("\"Goods\":"); ToStringHelper.ToString(Goods, sb);
    ...
    /// </summary>
    [System.Serializable]
    public partial class TownPriceColumnTable : IEnumerable<TownPriceColumnRecord>
    {
        /// <summary>
        /// Field names.
        /// </summary>
        public static readonly string[] FieldNames = { "Goods", "At" };
        ...
        public List<object[]> BuildObjectValueMap()
        {
            ...
            foreach (var r in _records)
                result.Add(new object[] { r.Goods, r.At });
        ...
        /// </remarks>
        public List<TownPriceColumnRecord> Records => _records;
        private List<TownPriceColumnRecord> _records = new List<TownPriceColumnRecord>();
        ...
        /// </remarks>
        public List<TownPriceColumnRecord>.Enumerator GetEnumerator() => _records.GetEnumerator();

        IEnumerator<TownPriceColumnRecord> IEnumerable<TownPriceColumnRecord>.GetEnumerator() => _records.GetEnumerator();

        System.Collections.IEnumerator System.Collections.IEnumerable.GetEnumerator()
            => _records.GetEnumerator();

        #region Indexing by 'Goods'
        public Dictionary<int, TownPriceColumnRecord> RecordsByGoods => _recordsByGoods;
        private Dictionary<int, TownPriceColumnRecord> _recordsByGoods = new Dictionary<int, TownPriceColumnRecord>();
        ...
        /// </remarks>
        public TownPriceColumnRecord FindByGoods(int key)
            => _recordsByGoods.TryGetValue(key, out TownPriceColumnRecord record) ? record : null;

        /// <summary>
        /// The row with this `Goods`, or a thrown exception naming what was
        /// missing.
        /// </summary>
        /// <remarks>
        /// For a key that has to be there - one from another table, or a constant. The name
        /// says it throws, because a caller reading `GetByGoods(id).Name` at
        /// a glance cannot otherwise tell whether the next line is a null check or a catch.
        /// </remarks>
        public TownPriceColumnRecord GetByGoodsOrThrow(int key)
        {
            if (!_recordsByGoods.TryGetValue(key, out TownPriceColumnRecord record))
                throw new TabbitException($"There is no record in table `TownPriceColumn` that corresponds to field `Goods` value {key}");

            return record;
        }

        /// <summary>Whether the table holds a row with this `Goods`.</summary>
        public bool ContainsGoods(int key) => _recordsByGoods.ContainsKey(key);
        #endregion // Indexing by `Goods`

        /// <summary>Each row with the `Goods` it is keyed by.</summary>
        /// <remarks>
        ...
        /// `Goods` here, something else in the next table.
        ///
        ...
        {
            private readonly List<TownPriceColumnRecord> _rows;
            private int _at;

            internal EntryEnumerator(List<TownPriceColumnRecord> rows)
            {
                _rows = rows;
                _at = -1;
            }
            ...
            public (int Key, TownPriceColumnRecord Row) Current
                => (_rows[_at].Goods, _rows[_at]);
        }
        ...
        /// </summary>
        /// <remarks>
        /// `GetByGoodsOrThrow` under the spelling the language uses for a
        ...
        /// `Goods` is 0, not the first row - the rows in order are
        /// `Records`. Nothing in the type says which, which is why the table itself is not
        /// subscriptable by position anywhere.
        ///
        /// It does not replace `FindByGoods`: a key that may be absent
        /// wants the one whose name says a miss is an ordinary answer.
        /// </remarks>
        public TownPriceColumnRecord this[int key] => GetByGoodsOrThrow(key);
        ...
        public Task ReadAsync(TcbReader reader)
        {
            ...
            var records = new List<TownPriceColumnRecord>(count);

            for (int i = 0; i < count; i++)
                records.Add(new TownPriceColumnRecord());
            ...
            foreach (var column in columns)
            {
                ...
                {
                    case 1:
                        TcbTable.CheckColumn(column, "TownPriceColumn.Goods", TcbTable.KindScalar, false, TcbTable.ElementI32);
                        ...
                        for (int i = 0; i < count; )
                        {
                            ...
                            do
                            {
                                ...
                                record._goods_Goods_index = value;
                                record._goods = default(GoodsRecord); // will be assigned.
                            } while (--n > 0);
                        }
                        break;

                    case 2:
                        TcbTable.CheckColumn(column, "TownPriceColumn.At", TcbTable.KindScalar, false, TcbTable.ElementI32, TcbTable.ElementVarint);
            ...
            var recordsByGoods = new Dictionary<int, TownPriceColumnRecord>(count);
            foreach (var record in records)
                recordsByGoods.Add(record.Goods, record);
            ...
            _records = records;
            _recordsByGoods = recordsByGoods;
