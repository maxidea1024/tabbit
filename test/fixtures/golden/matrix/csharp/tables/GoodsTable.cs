...
namespace Tabbit.Fixtures.Matrix
{
    ...
    /// </summary>
    [System.Serializable]
    public partial class GoodsTable : IEnumerable<GoodsRecord>
    {
        /// <summary>
        /// Field names.
        /// </summary>
        public static readonly string[] FieldNames = { "Id", "Name" };
        ...
        public List<object[]> BuildObjectValueMap()
        {
            ...
            foreach (var r in _records)
                result.Add(new object[] { r.Id, r.Name });
        ...
        /// </remarks>
        public List<GoodsRecord> Records => _records;
        private List<GoodsRecord> _records = new List<GoodsRecord>();
        ...
        /// </remarks>
        public List<GoodsRecord>.Enumerator GetEnumerator() => _records.GetEnumerator();

        IEnumerator<GoodsRecord> IEnumerable<GoodsRecord>.GetEnumerator() => _records.GetEnumerator();

        System.Collections.IEnumerator System.Collections.IEnumerable.GetEnumerator()
            => _records.GetEnumerator();

        #region Indexing by 'Id'
        public Dictionary<int, GoodsRecord> RecordsById => _recordsById;
        private Dictionary<int, GoodsRecord> _recordsById = new Dictionary<int, GoodsRecord>();
        ...
        /// </remarks>
        public GoodsRecord FindById(int key)
            => _recordsById.TryGetValue(key, out GoodsRecord record) ? record : null;
        ...
        /// </remarks>
        public GoodsRecord GetByIdOrThrow(int key)
        {
            if (!_recordsById.TryGetValue(key, out GoodsRecord record))
                throw new TabbitException($"There is no record in table `Goods` that corresponds to field `Id` value {key}");
        ...
        {
            private readonly List<GoodsRecord> _rows;
            private int _at;

            internal EntryEnumerator(List<GoodsRecord> rows)
            {
                _rows = rows;
                _at = -1;
            }
            ...
            public (int Key, GoodsRecord Row) Current
        ...
        /// </remarks>
        public GoodsRecord this[int key] => GetByIdOrThrow(key);
        ...
        public Task ReadAsync(TcbReader reader)
        {
            ...
            var records = new List<GoodsRecord>(count);

            for (int i = 0; i < count; i++)
                records.Add(new GoodsRecord());
            ...
            foreach (var column in columns)
            {
                ...
                {
                    case 1:
                        TcbTable.CheckColumn(column, "Goods.Id", TcbTable.KindScalar, false, TcbTable.ElementI32, TcbTable.ElementVarint);
                        cursor = new TcbColumnCursor(reader, column, count, "Goods.Id");
                        for (int i = 0; i < count; )
                        {
                    ...
                    case 2:
                        TcbTable.CheckColumn(column, "Goods.Name", TcbTable.KindScalar, false, TcbTable.ElementString);
                        cursor = new TcbColumnCursor(reader, column, count, "Goods.Name");
                        for (int i = 0; i < count; )
                        {
                            // One call per run of equal values, not one per row - over a
                            // run-length encoded column this is most of the decode.
            ...
            var recordsById = new Dictionary<int, GoodsRecord>(count);
