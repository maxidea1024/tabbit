...
export class GoodsTable {
  ...
  /** All records. */
  public get records(): GoodsRecord[] { return this._records }
  private _records: GoodsRecord[] = []
   ...
   */
  public [Symbol.iterator](): IterableIterator<GoodsRecord> {
    return this._records[Symbol.iterator]()
  }

  // Indexing by 'id'
  public get recordsById(): Map<number, GoodsRecord> { return this._recordsById }
  private _recordsById: Map<number, GoodsRecord> = new Map<number, GoodsRecord>()
   ...
   */
  public findById(key: number): GoodsRecord | undefined {
   ...
   */
  public getByIdOrThrow(key: number): GoodsRecord {
    const found = this._recordsById.get(key)
    if (!found)
      throw new Error(`There is no record in table "Goods" that corresponds to field "id" value ${key}`)
   ...
   */
  public *entries(): IterableIterator<[number, GoodsRecord]> {
  ...
  private readFromJson(json: string): void {
    ...
    const records: GoodsRecord[] = []

    if (this.isCompactRowFormatted(dataRows)) {
      for (const dataRow of dataRows) {
        const record = new GoodsRecord()
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          tabbit.checkColumn(column, 'Goods.Id', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Goods.Id')
          ...
          break
        case 2:
          tabbit.checkColumn(column, 'Goods.Name', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_STRING])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Goods.Name')
          for (let i = 0; i < rowCount; ) {
   ...
   */
  private publish(records: GoodsRecord[]): void {
    const recordsById = new Map<number, GoodsRecord>()
