...
export class TownPriceColumnRecord {
  /** Default constructor */
  constructor() {
  }

  public get goods(): number { return this._goods_Goods_index }
  public get goodsByGoods(): GoodsRecord { return this._goods }

  public get at(): number { return this._at }

  public setReference_goods_INTERNAL(value: GoodsRecord) { this._goods = value; }

  public _goods: GoodsRecord
  public _goods_Goods_index: number = 0
  public _at: number = 0

  /** Populate field values. */
  public populateFieldValues(dataRow: IDataRow): void {
    this._goods_Goods_index = dataRow.goods
    this._at = dataRow.at
  }
  ...
  public populateFieldValuesCompact(dataRow: any[]): void {
    let offset = 0
    this._goods_Goods_index = dataRow[offset++]
...
// Generated from test/fixtures/xlsx/matrix/matrix.xlsx : Grids : B2
/** Price modifier by town and goods. */
export class TownPriceColumnTable {
  /** Default constructor. */
  constructor() {
  }

  /** All records. */
  public get records(): TownPriceColumnRecord[] { return this._records }
  private _records: TownPriceColumnRecord[] = []
   ...
   */
  public [Symbol.iterator](): IterableIterator<TownPriceColumnRecord> {
    return this._records[Symbol.iterator]()
  }

  // Indexing by 'goods'
  public get recordsByGoods(): Map<number, TownPriceColumnRecord> { return this._recordsByGoods }
  private _recordsByGoods: Map<number, TownPriceColumnRecord> = new Map<number, TownPriceColumnRecord>()

  /**
   * The row with this goods, or undefined when the table has none.
   *
   * The lookup to reach for when a missing row is an ordinary answer.
   */
  public findByGoods(key: number): TownPriceColumnRecord | undefined {
    return this._recordsByGoods.get(key)
  }
   ...
   * `getByGoods(id).name` at a glance cannot otherwise tell whether the
   * next line is a check or a catch.
   */
  public getByGoodsOrThrow(key: number): TownPriceColumnRecord {
    const found = this._recordsByGoods.get(key)
    if (!found)
      throw new Error(`There is no record in table "TownPriceColumn" that corresponds to field "goods" value ${key}`)

    return found
  }

  /** Whether the table holds a row with this goods. */
  public containsGoods(key: number): boolean {
    return this._recordsByGoods.has(key)
  }
   ...
   */
  public *entries(): IterableIterator<[number, TownPriceColumnRecord]> {
  ...
  private readFromJson(json: string): void {
    ...
    const records: TownPriceColumnRecord[] = []

    if (this.isCompactRowFormatted(dataRows)) {
      for (const dataRow of dataRows) {
        const record = new TownPriceColumnRecord()
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (let i = 0; i < rowCount; ++i)
      records.push(new TownPriceColumnRecord())
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          tabbit.checkColumn(column, 'TownPriceColumn.Goods', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'TownPriceColumn.Goods')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
              records[i]._goods_Goods_index = value
          }
          break
        case 2:
          tabbit.checkColumn(column, 'TownPriceColumn.At', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'TownPriceColumn.At')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
              records[i]._at = value
   ...
   */
  private publish(records: TownPriceColumnRecord[]): void {
    const recordsByGoods = new Map<number, TownPriceColumnRecord>()

    for (const record of records) {
      recordsByGoods.set(record.goods, record)
    }

    this._records = records
    this._recordsByGoods = recordsByGoods
  }
}
