...
module MatrixData
  ...
  class ElementChartColumnTable
    ...
    # The row with this Defender, or nil when the table has none.
    #
    # find_by_defender under the spelling Ruby uses for a keyed collection. It
    ...
    def each_pair
      ...
      @records.each { |record| yield record.defender, record }
      self
    end
    ...
    def read(filename)
      ...
      records = Array.new(count) { ElementChartColumnRecord.new }
      ...
      columns.each do |column|
        ...
        when 1
          ...
          cursor = Tabbit::ColumnCursor.new(reader, column, count, 'ElementChartColumn.Defender')
          at = 0
          while at < count
        ...
        when 2
          ...
          cursor = Tabbit::ColumnCursor.new(reader, column, count, 'ElementChartColumn.At')
          at = 0
          while at < count
            n, value = cursor.next_same_i32(count - at)
            (at...(at + n)).each do |i|
      ...
      by_defender = records.to_h { |record| [record.defender, record] }

      # Published, now that every column read.
      @records = records
      @by_defender = by_defender
    end
  end
end
