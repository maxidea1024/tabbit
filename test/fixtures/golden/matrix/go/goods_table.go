...
// GoodsRecord was generated from test/fixtures/xlsx/matrix/matrix.xlsx : Goods : B2.
...
// GoodsTable holds every row of Goods.
...
func (t *GoodsTable) Records() []GoodsRecord { return t.records }

// Count returns how many rows the table holds.
func (t *GoodsTable) Count() int { return len(t.records) }
...
func (t *GoodsTable) All() iter.Seq[*GoodsRecord] {
	records := t.records

	return func(yield func(*GoodsRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *GoodsTable) FindById(key int32) *GoodsRecord {
...
func (t *GoodsTable) GetByIdOrError(key int32) (*GoodsRecord, error) {
	position, found := t.byId[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `Goods` that corresponds to field `Id` value %v", key)
	}
...
func (t *GoodsTable) ContainsId(key int32) bool {
	_, found := t.byId[key]
	return found
}
...
func (t *GoodsTable) Entries() iter.Seq2[int32, *GoodsRecord] {
	records := t.records

	return func(yield func(int32, *GoodsRecord) bool) {
		for i := range records {
...
func (t *GoodsTable) Read(filename string) error {
	...
	records := make([]GoodsRecord, count)
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "Goods.Id", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Goods.Id")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "Goods.Name", tabbit.KindScalar, false, tabbit.ElementString) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Goods.Name")
				for i := int32(0); i < count; {
