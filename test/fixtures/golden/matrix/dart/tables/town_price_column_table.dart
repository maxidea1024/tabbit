...
class TownPriceColumnRecord {
  int goods = 0;
  GoodsRecord? goodsByGoods;
  int at = 0;

}

/// Every row of TownPriceColumn.
class TownPriceColumnTable extends Iterable<TownPriceColumnRecord> {
  ...
  List<TownPriceColumnRecord> get records => _records;
  List<TownPriceColumnRecord> _records = [];
  ...
  @override
  Iterator<TownPriceColumnRecord> get iterator => _records.iterator;

  Map<int, TownPriceColumnRecord> _byGoods = {};

  /// The row with this Goods, or null when the table has none.
  ///
  /// The lookup to reach for when a missing row is an ordinary answer - an optional
  /// reference, a key that came from user input.
  TownPriceColumnRecord? findByGoods(int key) => _byGoods[key];
  ...
  TownPriceColumnRecord getByGoodsOrThrow(int key) {
    final record = _byGoods[key];
  ...
  bool containsGoods(int key) => _byGoods.containsKey(key);


  /// The row with this Goods, or null when the table has none.
  ///
  /// `findByGoods` under the spelling Dart uses for a keyed collection. It
  ...
  TownPriceColumnRecord? operator [](int key) => findByGoods(key);
  ...
  Iterable<MapEntry<int, TownPriceColumnRecord>> get entries =>
      _records.map((record) => MapEntry(record.goods, record));
  ...
  void read(String filename) {
    ...
    final loaded = <TownPriceColumnRecord>[];
    final loadedByGoods = <int, TownPriceColumnRecord>{};

    for (var i = 0; i < count; i++) {
      loaded.add(TownPriceColumnRecord());
    }
    ...
    for (final column in header.columns) {
      ...
      switch (column.tag) {
        case 1:
          checkColumn(column, 'TownPriceColumn.Goods', kindScalar, false, [elementI32]);
          cursor = TcbColumnCursor(reader, column, count, 'TownPriceColumn.Goods');
          for (var i = 0; i < count; ) {
            final (n, value) = cursor.nextSameI32(count - i);
            for (var left = n; left > 0; left--, i++) {
              loaded[i].goods = value;
            }
          }
          break;
        case 2:
          checkColumn(column, 'TownPriceColumn.At', kindScalar, false, [elementI32, elementVarint]);
          cursor = TcbColumnCursor(reader, column, count, 'TownPriceColumn.At');
          for (var i = 0; i < count; ) {
            final (n, value) = cursor.nextSameI32(count - i);
            for (var left = n; left > 0; left--, i++) {
              loaded[i].at = value;
