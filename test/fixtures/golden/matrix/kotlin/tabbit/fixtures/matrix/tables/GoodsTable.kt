...
/** Every row of Goods. */
class GoodsTable : Iterable<GoodsRecord> {
     ...
     */
    var records: MutableList<GoodsRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<GoodsRecord> = records.iterator()

    private var byId: HashMap<Int, GoodsRecord> = HashMap()
     ...
     */
    fun findById(key: Int): GoodsRecord? = byId[key]
     ...
     */
    fun getByIdOrThrow(key: Int): GoodsRecord =
     ...
     */
    operator fun get(key: Int): GoodsRecord? = findById(key)
     ...
     */
    val entries: Sequence<Pair<Int, GoodsRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<GoodsRecord>(count)
        val loadedById = HashMap<Int, GoodsRecord>(count * 2)

        repeat(count) { loaded.add(GoodsRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "Goods.Id", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Goods.Id")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "Goods.Name", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "Goods.Name")
                    var at = 0
                    while (at < count) {
