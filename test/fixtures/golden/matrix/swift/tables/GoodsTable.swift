...
/// Every row of Goods.
public final class GoodsTable: Sequence {
    ...
    public func makeIterator() -> Array<GoodsRecord>.Iterator {
    ...
    public private(set) var records: [GoodsRecord] = []

    private var byId: [Int32: GoodsRecord] = [:]

    /// The row with this Id, or nil when the table has none.
    ///
    /// The lookup to reach for when a missing row is an ordinary answer - an optional
    /// reference, a key that came from user input.
    public func findById(_ key: Int32) -> GoodsRecord? {
        byId[key]
    }
    ...
    public func getByIdOrThrow(_ key: Int32) throws -> GoodsRecord {
        guard let record = byId[key] else {
            throw RecordNotFoundError(
                "there is no record in table `Goods` that corresponds to "
                + "field `Id` value \(key)")
        }
    ...
    public subscript(_ key: Int32) -> GoodsRecord? {
        findById(key)
    }
    ...
    /// row with.
    public var entries: LazyMapSequence<[GoodsRecord], (key: Int32, row: GoodsRecord)> {
    ...
    public func read(
        ...
        var loaded = [GoodsRecord]()
        loaded.reserveCapacity(count)

        for _ in 0 ..< count { loaded.append(GoodsRecord()) }
        var loadedById = [Int32: GoodsRecord](
        ...
        for column in header.columns {
            ...
            switch column.tag {
            case 1:
                try Tcb.checkColumn(column, "Goods.Id", Tcb.kindScalar, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Goods.Id")
                var at = 0
                while at < count {
            ...
            case 2:
                try Tcb.checkColumn(column, "Goods.Name", Tcb.kindScalar, false, Tcb.elementString)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Goods.Name")
                var at = 0
                while at < count {
