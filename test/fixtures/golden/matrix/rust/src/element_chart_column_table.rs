...
impl ElementChartColumnTable {
    ...
    pub fn records(&self) -> &[ElementChartColumnRecord] {
        &self.records
    }
    ...
    pub fn iter(&self) -> std::slice::Iter<'_, ElementChartColumnRecord> {
        self.records.iter()
    }
    ...
    /// The row with this Defender, or an error naming what was missing.
    ///
    ...
    pub fn read(&mut self, filename: &Path) -> tabbit::Result<()> {
        ...
        let mut records = vec![ElementChartColumnRecord::default(); count];
        ...
        for column in &header.columns {
            ...
            match column.tag {
                1 => {
                    ...
                    let mut cursor = tabbit::TcbColumnCursor::new(&mut reader, column, header.row_count, "ElementChartColumn.Defender")?;
                    let mut at = 0usize;
                ...
                2 => {
                    ...
                    let mut cursor = tabbit::TcbColumnCursor::new(&mut reader, column, header.row_count, "ElementChartColumn.At")?;
                    let mut at = 0usize;
                    while at < records.len() {
                        let (n, value) = cursor.next_same_i32((records.len() - at) as i32)?;
                        for _ in 0..n {
        ...
        let by_defender = records
            .iter()
            .enumerate()
            .map(|(position, record)| (record.defender.clone(), position))
            .collect();
...
impl<'a> IntoIterator for &'a ElementChartColumnTable {
    type Item = &'a ElementChartColumnRecord;
    type IntoIter = std::slice::Iter<'a, ElementChartColumnRecord>;
