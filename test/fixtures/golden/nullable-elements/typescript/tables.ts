...
export class Tables {
  ...
  public async readAll(load: (fileName: string) => Promise<string>, fileExtension: string = '.json'): Promise<void> {
    ...
    listing.readJsonFrom(await load(`Listing${fileExtension}`))
    const folded = new FoldedTable()
    folded.readJsonFrom(await load(`Folded${fileExtension}`))
  ...
  public readAllSync(load: (fileName: string) => string, fileExtension: string = '.json'): void {
    const listing = new ListingTable()
    listing.readJsonFrom(load(`Listing${fileExtension}`))
    const folded = new FoldedTable()
    folded.readJsonFrom(load(`Folded${fileExtension}`))
  ...
  public async readAllBinary(load: (fileName: string) => Promise<Uint8Array>, fileExtension: string = '.tcb'): Promise<void> {
    const listing = new ListingTable()
    listing.readBinaryFrom(await load(`Listing${fileExtension}`))
    const folded = new FoldedTable()
    folded.readBinaryFrom(await load(`Folded${fileExtension}`))
  ...
  public readAllBinarySync(load: (fileName: string) => Uint8Array, fileExtension: string = '.tcb'): void {
    const listing = new ListingTable()
    listing.readBinaryFrom(load(`Listing${fileExtension}`))
    const folded = new FoldedTable()
    folded.readBinaryFrom(load(`Folded${fileExtension}`))
