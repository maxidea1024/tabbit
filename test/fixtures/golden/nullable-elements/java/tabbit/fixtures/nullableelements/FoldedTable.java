...
/** Every row of Folded. */
public final class FoldedTable implements Iterable<FoldedRecord> {
    private List<FoldedRecord> records = new ArrayList<>();
    private Map<Integer, FoldedRecord> byIndex = new HashMap<>();
     ...
     */
    public List<FoldedRecord> records() {
        return records;
    }
     ...
     */
    @Override
    public Iterator<FoldedRecord> iterator() {
    ...
    public FoldedRecord getByIndexOrThrow(int key) {
        FoldedRecord record = byIndex.get(key);

        if (record == null) {
            throw new TcbReader.RecordNotFoundException(
                "there is no record in table `Folded` that corresponds to field "
                + "`Index` value " + key);
        }
     ...
     */
    public Iterable<Map.Entry<Integer, FoldedRecord>> entries() {
        final List<FoldedRecord> rows = records;

        return () -> new Iterator<Map.Entry<Integer, FoldedRecord>>() {
            private final Iterator<FoldedRecord> source = rows.iterator();
            ...
            @Override
            public Map.Entry<Integer, FoldedRecord> next() {
                final FoldedRecord row = source.next();
    ...
    public void read(Path filename) {
        ...
        List<FoldedRecord> loaded = new ArrayList<>(count);
        Map<Integer, FoldedRecord> loadedByIndex = new HashMap<>(count * 2);

        for (int i = 0; i < count; i++) {
            loaded.add(new FoldedRecord());
        }
