...
**고친 방법은 같은 실행 파일을 쓰게 한 것입니다.** 부수 효과로 서버 테스트 13개가 전부 빨라졌고
...
주는 설계입니다. 상세 설계·미결·게이트는 [접근자 객체화](accessor-instances.md)로
분리했습니다.
...
|작업|내용|
|--|--|
|레시피 옵션|`CSharpRecipe`에 출력 형식(배타: 소스 / 어셈블리)과 어셈블리 이름 추가. 옵션 모양(bool 대 enum 문자열)은 `WriteCargoToml`·`WriteGoMod`·`WriteUpdater` 선례와의 일관성 기준으로 확정. `--new-recipe` 스켈레톤은 리플렉션 기반이라 자동 반영|
...
액세서 타입의 이름을 정하게 합니다 — 지금은 13개 언어 중 5개에서만 그렇게 동작하고, 7개는
...
[생성 코드의 이름 체계](generated-naming.md)로 분리했습니다.
...
|접근자 객체화|인스턴스 + 전역 헬퍼 병행, 검증은 `context.Tables` — 상세는 [접근자 객체화](accessor-instances.md)|
|규칙 우선순위|순차 스테이지 한정 · 티어 + 차단점 · 어트리뷰트 지정 + 조회 명령 — 상세는 [규칙 우선순위](rule-priority.md)|
|이름 체계|정본 하나에서 언어별 표기로 파생 · `AccessorName`이 타입 이름을 정하게 — 상세는 [생성 코드의 이름 체계](generated-naming.md)|

|미결|선택지|권고|
|--|--|--|
|API 분리 방식 (5a)|물리 분리 / ref asm 동봉|물리 분리 — 단 `Context` 의존 실측 후 확정|
|계약 어셈블리 이름|—|[생성 코드의 이름 체계](generated-naming.md) §3에서 함께 확정|
|`.generated` 중간 정책|소스 커밋 전환 / 비커밋 유지|7단계까지의 기간에 따라|
|옵션 모양 (7단계)|bool / enum 문자열|선례 일관성 기준|
|접근자 객체화 세부 (6단계)|시임 귀속 등 4건. 이름은 9단계에서 함께 정합니다|[접근자 객체화](accessor-instances.md) §5에서 확정|
|스테이지별 컨텍스트|`pre/`가 쓸 수 없는 멤버를 타입에서 빼는 안|[접근자 객체화 §3.1](accessor-instances.md)에 기록. 6단계와 같은 주기에 할지 미결|
|이름 체계 세부 (9단계)|Rust·Python의 옵션 신설 여부 등 5건|[생성 코드의 이름 체계](generated-naming.md) §5에서 확정|
