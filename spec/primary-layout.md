...
- **셀 병합은 읽지 않고, 엔티티 범위 안의 병합은 경고합니다.** 병합 범위의 좌상단 셀만
  값을 갖고 나머지는 빈 칸의 일반 규칙(§3.7)으로 읽힙니다 — 병합에 의존하던 헤더가
  조용히 빈 칸이 되는 경로를 경고가 드러냅니다. 메모 컬럼과 엔티티 밖의 병합은 자유입니다.
...
- **키의 사전과 「배열에 붙는가, 원소에 붙는가」는 DSL §6.1의 표가 정본**입니다. 이
  ...
  `text` · `namespace` · `asset`처럼 아직 DSL에 없는 role 키도 DSL §6.1에 등재해 양쪽
  표기를 통일합니다(§16의 0).
...
|순서|무엇|게이트|
|--|--|--|
|0|**이 스펙의 확정** — **됨.** 레이아웃 id는 `tabbit`(기존 레이아웃 폐기 · 승계, 6번), role 메타 키는 통일(DSL §6.1 등재가 1단계의 선행 작업입니다), 셀 병합은 경고(§3.2)|—|
|1|**파서 뼈대** — 선언 셀 · 행 키 · 경계 · 메모 컬럼 · 경로(`[n]` · `.`) · 접힌 타입 표현 · enum · const. 새 파서는 **임시 id로 등록**합니다 — `tabbit` 승계는 6번입니다|게이트 1 · 3|
...
|6|**교체와 이관** — 기존 `tabbit` 파서의 삭제, 새 파서의 id를 임시 id에서 `tabbit`으로, 옛 표기 픽스처 · 쇼케이스 시트의 헤더 이관. 바뀌는 것은 헤더뿐이므로(§9) 데이터 칸은 그대로입니다|이관한 픽스처 전체의 골든 **바이트 무변경** — 게이트 1의 전면 확장입니다|
