...
|4′|**`key` 메타** — 기본 인덱스를 첫 컬럼에서 옮기는 것(§3.5)|아래의 「드러난 것」 2번|
...
**픽스처의 헤더를 규칙으로 옮길 수는 없습니다.** 처음에는 `FixtureGen`의 `SheetBuilder`가
새 표기로 내게 하면 74개 호출 지점이 한 번에 옮겨진다고 보았는데, 옮길 수 없는 자리가
있습니다.

|무엇|왜 규칙으로 안 되는가|
|--|--|
|**이름의 숫자 접기** — `Text1` · `Text2`|새 레이아웃은 `FoldSerialFields`를 읽지 않으므로(§5) 이 이름들은 `text[0]` · `text[1]`로 적어야 배열이 됩니다. 그런데 **접기가 켜져 있었는지는 recipe가 아는 것**이고 픽스처 생성기는 모릅니다. 무조건 옮기면 접기가 꺼져 있던 테이블의 필드 2개가 배열 1개가 되고, 그것이 이 도구가 없애려는 조용한 형태 변경입니다. **94개 레시피 중 33개가 접기를 켭니다**|
|**옛 표기의 오류 픽스처**|`~~` 마커·최소 크기·`**이름`처럼 옛 표기 자체를 시험하는 픽스처는 옮기는 것이 아니라 다시 정해야 합니다|

그래서 6단계는 **픽스처마다 판단이 필요한 이관**입니다 — 「이 숫자가 배열이었나」를 recipe와
함께 읽어야 하고, 그 판단을 틀리면 테이블의 형태가 조용히 바뀝니다. 게이트는 전체 골든이고
검증 한 바퀴가 **13분 21초**입니다.

**그래서 이 단계는 착수 전에 목록을 만들어야 합니다** — 접기를 켠 33개 레시피가 어느 워크북을
읽는지, 그 워크북의 어느 컬럼이 숫자 이름인지. 그 목록 없이 시작하면 규칙으로 옮기는 것과
같아집니다.
