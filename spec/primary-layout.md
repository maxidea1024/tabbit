...
|`$`|예약 컬럼 — `$type`(다형 레코드의 변종) · `$key` · `$value`|[다형과 참조 배열 §5.2](polymorphism.md)|
|`\|`|**되돌렸습니다.** `foreign Weapon\|Armour`는 거부되고, 여러 테이블 중 하나여도 되는 값은 괄호 메타 `refs=Weapon;Armour`의 검사입니다|[참조가 내는 이름 §6](reference-surface-naming.md)|
...
- **성분이 `foreign`인 것은 됩니다.** 연결 테이블 — 한 행이 두 카탈로그의 짝을 뜻하는
  ...
  이름](reference-surface-naming.md) §4·§5), 둘을 바꿔 쓰면 **부를 수 없는 조회**가
  나옵니다.
...
- **대괄호가 이어지면 레벨이 하나 더 생기고, 그 레벨은 이름이 없습니다.** `grid[0][1]`은
  ...
  적을 낱말이 없으므로 번호로 가리킵니다([다중 중첩](nested-multi-level.md)의 `Grid1.2`가
...
|예약 컬럼|장래의 역할|선행 스펙|
|--|--|--|
|`$type`|**다형 레코드**의 변종 판별 — 행(원소)마다 변종 이름을 적는 컬럼|**[다형과 참조 배열](polymorphism.md)이 그 설계입니다** — 와이어는 형식 무변경이고(§6), 이 컬럼의 표기는 그 문서 §5.2입니다|
|`$key`|**map**의 키 컬럼 — `prices[]` 그룹에서 키가 들어가는 자리|DSL 5′ (`set` · `map`). 셀 표기(`1:sword;2:shield`)와 컬럼 표기의 확정이 거기입니다|
|`$value`|다형 변종의 값을 한 셀로 압축(Luban `$value`)|**거절 유지** — [다형과 참조 배열 §10](polymorphism.md)이 그 판정을 그대로 두었습니다. (다) `sep` 표기와 겹치는 두 번째 방법이 됩니다|
...
|tabbit의 현재|**검사만 있습니다.** 참조로 올리는 것은 되돌렸습니다 — 셀렉터에는 돌려줄 타입이 없습니다 ([설계](reference-surface-naming.md))|**설계까지** — [다형과 참조 배열](polymorphism.md). 구현은 없습니다|
...
|5|**수식 오류 지연** — **이미 됨**([수식 오류](formula-errors.md)). 이 순서에 남길 것이 없습니다(§14.2)|—|
