...
|6|**교체와 이관** — 기존 `tabbit` 파서 · 메시지 · 카탈로그의 삭제, 새 파서의 id를 `primary`에서 `tabbit`으로(클래스 이름도), 옛 표기 픽스처 · 쇼케이스 시트의 헤더 이관. 바뀌는 것은 헤더뿐이므로(§9) 데이터 칸은 그대로입니다|이관한 픽스처 전체의 골든 **바이트 무변경** — 게이트 1의 전면 확장입니다|
