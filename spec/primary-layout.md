...
|6|**교체와 이관** — **막혀 있습니다**(§16.5): 새 표기에 「배열의 배열」의 자리가 없고, 그 형태를 쓰는 픽스처가 손으로 만든 워크북입니다. 교체 자체는 깨끗합니다. 기존 `tabbit` 파서 · 메시지 · 카탈로그의 삭제, 새 파서의 id를 `primary`에서 `tabbit`으로(클래스 이름도), 옛 표기 픽스처 · 쇼케이스 시트의 헤더 이관. 바뀌는 것은 헤더뿐이므로(§9) 데이터 칸은 그대로입니다|이관한 픽스처 전체의 골든 **바이트 무변경** — 게이트 1의 전면 확장입니다|
|7|**작성 문서** — **됨.** [시트 작성 — 주 레이아웃](../doc/sheets-primary.md). §12의 표와 §3.7의 빈 칸 총정리를 수록했습니다. 기존 `sheets.md`의 대체는 6단계에서 함께 합니다|—|
