...
Tabbit의 **주 저작 레이아웃**의 설계입니다. STRUCT DSL이 들어오면서 기존 `tabbit`
...
|`:field` · `:type` · `:desc` · `:target`|헤더 행 키(§3.3)|
|빈 칸|데이터 행|
...
|표기|뜻|
|--|--|
|(생략)|**첫 필드 컬럼이 기본 인덱스** — 지금 규칙 그대로|
|`key=code`|그 이름의 컬럼이 기본 인덱스. 첫 컬럼 강제가 풀리고, 첫 컬럼은 평범한 필드가 됩니다|
|`key="stage,slot"`|**복합 키.** 조합이 유일해야 하고, 조회가 다인자가 됩니다 — `FindByKey(stage, slot)`|

- **복합 키의 표기는 따옴표 쉼표 목록입니다.** 쉼표가 든 메타 값의 따옴표 규칙(§3.4)
  그대로이고, Luban의 `+`를 들여오지 않습니다 — 새 기호를 만들지 않기 위해서입니다(§2).
- **독립 다중 키는 이 메타가 아니라 보조 인덱스입니다.** Luban의 `index=a,b`(키마다 따로
  유일, 맵도 따로)는 기본 인덱스 하나 + `*` 컬럼으로 적습니다 — 이미 있는 형태입니다.
- 성분의 규칙 — `key`에 적힌 이름은 **실재하는 최상위 스칼라 필드**여야 하고(경로 ·
  `[]` · `[n]` 불가), 성분마다 기본 인덱스의 기존 규칙(인덱스 가능 타입 · 옵셔널 불가 ·
  양쪽 대상)이 적용됩니다. 같은 이름을 두 번 적으면 오류입니다.
- 복합 키의 성분에 `*`를 따로 붙이는 것은 됩니다 — 「조합도 유일하고 이 성분 혼자서도
  유일하다」는 서로 다른 두 선언입니다. **복합 보조 인덱스**(여러 컬럼의 `*` 조합)는
  담지 않습니다.
- **복합 키 테이블은 `foreign`의 대상이 될 수 없습니다** — 참조는 키 값 하나를 담는
  구조이므로([설계](reference-key-types.md)) 거부로 시작하고, 요구가 실측되면 그때
  형태를 정합니다.
- 와이어는 움직이지 않습니다 — 성분 컬럼이 그대로 실립니다. 움직이는 것은 **생성 코드의
  조회 표면**(다인자 사전)이고, 모든 언어에 파급되므로 별도 단계입니다(§16의 7).
  그때까지 복합 `key`는 문법 · 모델 · 조합 유일성 검사까지 받고, 조회 생성만 이름과 함께
  거절합니다. 단일 `key=` 지정은 조회 형태가 지금과 같으므로 1단계에 포함됩니다.
...
> 레이아웃의 시트는 그대로 유효합니다(레이아웃이 다르므로).
...
|2|**새 레코드의 시작은 키 칸에 값이 있는 행**입니다(§3.5의 지정 포함 — 복합 키면 성분 중 하나라도). 키 칸이 전부 빈 행은 직전 레코드의 **연장 행**이고, 복합 키의 성분 일부만 채운 행은 그 빈 성분을 가리켜 오류입니다|
...
시트 저작 문서에 그대로 옮길 수 있는 형태로 적습니다. 모든 엔티티가 같은 뼈대입니다 —
...
기본형 하나로는 드러나지 않는 조합들입니다. 저작 문서의 「이런 것도 됩니다」 절의
씨앗입니다.
...
- Luban의 `index=a,b`(독립 키 2개)와 뜻이 다릅니다 — 독립 키는 `*` 보조 인덱스로
  적습니다(§12의 12).
...
기존 `tabbit` · `rescue` · `named-range` 레이아웃은 각자의 파서로 그대로 유효합니다. 이 레이아웃은
`[TabbitLayout]` 어트리뷰트를 단 파서 파일 하나로 추가되고, **그 파일을 지우면 흔적이
남지 않아야 합니다** — 코어 규칙 그대로입니다.
...
|`index=a,b` 독립 다중 키|**다른 방식**|기본 인덱스 + 보조 인덱스 `*` — 이미 있는 형태입니다|
...
|variant(`필드@변형` 열)|**다른 방식**|[행 벌](table-row-sets.md)이 그 자리|
...
저작 문서에 눈에 띄게 적어야 하고, 오류 메시지가 이 표의 오른쪽을 안내해야 합니다.

|#|헷갈리는 것|안내|
|--|--|--|
|1|`[]`의 두 자리 — `costs` + `int[]` vs `costs[]` + `int`|「**이름의 `[]`는 행, 타입의 `[]`는 셀 안**」 한 줄. §5.1의 표를 저작 문서에 그대로|
...
|4|인덱스 빈 칸의 뜻이 테이블에 따라 다름 — 멀티 로우 없는 표에서는 오류, 있는 표에서는 연장 신호|`[]` 컬럼의 존재가 갈림길임을 저작 문서에 명시. 오류 메시지도 두 경우를 구분해야 합니다|
...
|10|`-`(값 없음)와 빈 칸의 구분 — 기존 규칙이지만 새 문서로 유입되는 사용자에게는 처음|[빈 칸과 없음](blank-and-null-cells.md)을 저작 문서 앞부분에|
|11|`#`의 세 자리 — 마커 열(행 제외) · `:field` 단독(메모 컬럼) · `:field` 이름 앞(묘비)|뜻은 「모델에 넣지 않음」 하나이고, 위치가 대상(행 · 컬럼 · 필드)을 정합니다. 저작 문서에 한 표로|
|12|Luban의 `index=a,b`는 **독립 키 2개**, 이 레이아웃의 `key="a,b"`는 **복합 키 하나**|독립 키는 `*` 보조 인덱스로 적습니다(§3.5). Luban에서 오는 사용자용 주의|
...
|항목|성격|
|--|--|
|**레이아웃 id**|명명. 이 문서는 「주 레이아웃」으로 지칭합니다. 후보: `tabbit2`, 또는 기존 id의 세대 교체 계획과 함께 결정|
...
|**role 메타 키의 DSL 통일**|시트는 `text (group=Common)`, DSL은 `(text)` 플래그입니다. 그룹 · 네임스페이스 키를 DSL §6.1에 등재하면서 양쪽 표기를 맞춰야 합니다|
|**셀 병합의 처리**|이 레이아웃은 병합을 읽지 않지만, 병합된 헤더가 있으면 좌상단 외 셀이 빈 칸으로 읽힙니다. 금지(오류) · 경고 · 무시 중 선택이 필요합니다|
...
|**기존 `tabbit` 레이아웃의 장래**|병존 기간과 이관 안내. 샘플 · 픽스처의 이관 순서도 여기에 걸립니다|
...
**수식 오류 보고의 지연.** [행 키 레이아웃 §8.2](keyed-layout.md)가 적어 둔 그대로입니다 —
현재는 임포터가 수식 오류를 검사하므로, 아무도 읽지 않는 메모 컬럼의 `#N/A` 하나가 변환을
멈춥니다. 임포터는 `RawCell`에 기록만 하고 레이아웃이 필드로 읽는 셀에서 보고하도록
옮깁니다. 메모 컬럼을 자유 공간으로 선언한 이 설계의 전제 조건이고, 레이아웃과 무관한 일반
기능이므로 다른 레이아웃에도 그대로 유효합니다.
...
|#|무엇|기준|
|--|--|--|
|1|**표기 동치** — 같은 데이터를 기존 `tabbit` 레이아웃과 이 레이아웃으로 적은 픽스처 2벌|모델이 동일하고 **골든 산출물이 한 바이트도 다르지 않아야** 합니다. [행 키 레이아웃 §10](keyed-layout.md)의 방식이고, 파서 결함이 곧 산출물 차이로 드러나므로 픽스처 신설보다 강한 검증입니다|
...
|순서|무엇|게이트|
|--|--|--|
|0|**이 스펙의 확정** — 특히 착수 전에 정해야 하는 레이아웃 id와 role 메타 키(§13)|—|
|1|**파서 뼈대** — 선언 셀 · 행 키 · 경계 · 메모 컬럼 · 경로(`[n]` · `.`) · 접힌 타입 표현 · enum · const|게이트 1 · 3|
|2|**멀티 로우** — `[]` 경로, 레코드 분할, §6의 규칙 전부|게이트 2와 §11의 3 · 4번 사례|
|3|**괄호 메타** — 제약 키의 `Field.Constraints` 대입, DSL 제약과의 교집합 확인|게이트 3 (형식 · 생성 코드 무변경)|
|4|**예약 컬럼의 거절 자리** — `:type` · `:key` · `:value`|오류 픽스처|
|5|**수식 오류 지연**(코어, §14.2)|기존 스위트 + 메모 컬럼 수식 픽스처|
|6|**저작 문서** — §8을 씨앗으로 `doc/`에. §12의 표가 필수 수록 항목입니다|—|
|7|**복합 키 조회** — `key="a,b"`의 다인자 조회를 모든 언어에(§3.5). 그때까지 복합 키는 문법 · 유일성 검사까지 받고 조회 생성만 거절합니다|적합성 코퍼스 — 모든 언어의 컴파일 · 되읽기. **골든이 움직입니다**|

1~5는 형식도 생성 코드도 건드리지 않습니다 — 골든이 판정 기준인 변경들입니다.
7은 모든 언어의 조회 표면이 움직이는 별도 등급이고, 와이어는 그때도 무변경입니다.
