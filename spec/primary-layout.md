...
- 와이어는 움직이지 않습니다 — 성분 컬럼이 그대로 실립니다. 움직이는 것은 **생성 코드의
  ...
  단계입니다(§16의 7). 그때까지 복합 키는 문법 · 모델 · 유일성 검사까지 받고, 조회
...
둘 다 「여러 벌 중 하나를 고른다」이지만 **고르는 단위가 다릅니다.** 겹치는 기능이
아니므로 어느 쪽도 다른 쪽을 대신하지 않습니다.

|  |`:variant`|[행 벌](table-row-sets.md)|
|--|--|--|
...
|variant(`필드@변형` 열, `--variant`)|**채택 · 변형**|`:variant` 행(§3.6). 이름 접미 `@변형`은 `@N` 와이어 태그와 충돌해 들여오지 않습니다. 행 벌과는 단위가 다릅니다 — §3.6의 대비표|
...
|13|`:variant`(필드의 값 컬럼 여러 벌)와 행 벌(행 집합 여러 벌)|§3.6의 대비표를 저작 문서에. 「지역별」이라는 말이 양쪽 모두에 걸리므로 예시로 갈라야 합니다|
