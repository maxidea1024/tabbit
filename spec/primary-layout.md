...
|#|막는 것|
|--|--|
|1|**새 표기에 「배열의 배열」의 자리가 없습니다.** `member-array` 픽스처가 그 형태를 갖고 있고(생성 코드가 `int[][] grid`를 냅니다), 옛 표기는 `Grid1.2`로 적었습니다 — **두 레벨 중 어느 쪽에도 이름이 없습니다**([다중 중첩](nested-multi-level.md)). §5는 경로의 모든 레벨이 이름을 갖는다고 정하므로, 이 형태를 적을 방법이 없습니다. 자연스러운 표기는 `grid[0][1]`이고 이미 있는 `[]` 어휘 안이지만, **한 마디에 대괄호를 둘 두는 것은 표기의 확장**이므로 §5가 정해야 합니다|
...
**착수 순서.** ① §5에 배열의 배열의 표기를 정하고, ② `member-array`를 생성기로 옮기고,
③ 그다음에 교체와 이관을 한 번에 합니다. ①이 없으면 ③에서 표현할 수 없는 픽스처가 남습니다.
