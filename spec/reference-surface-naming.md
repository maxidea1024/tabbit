...
> [다형과 참조 배열 §3](polymorphism.md)이 함께 개정 대상입니다
...
|**거절**|생성될 이름을 **컬럼이 이미 쓰고 있으면 거부**합니다. 되돌리지 않습니다|
...
([이름 표기 규약](naming-conventions.md)).
...
![이름이 부딪히는 유일한 거절](reference-surface-clash.svg)
...
|이웃 컬럼이 바뀌면|**안 바뀝니다**|
|실패하는 경우|**없습니다.** 부딪히는 컬럼이 있으면 거절이고(3.3), 그것은 이름 규칙의 실패가 아니라 그 컬럼의 문제입니다|
...
|와이어|대상 키 타입|**그대로**|
|대상 개수|하나|**하나 이상**|
|이 빌드에 대상이 없으면|거부|판정하지 않고 경고 ([컬럼 제약](column-constraints.md))|
...
|struct 다형성|값 임베딩이고 참조가 아닙니다. [다형성 §5](polymorphism.md)|
|점 표기(`Table.Field`)의 이름|그 컬럼은 행이 아니라 **값**을 돌려줍니다. 지금 그대로입니다|
|레코드 타입 이름(`ItemRecord` · `ItemTable.Record`)|[이름 체계](generated-naming.md)가 현행 유지로 정한 축입니다|
...
- **문서 개정** — [다중 대상 참조](multi-target-references.md) ·
  [다중 대상 접근자](multi-target-accessors.md) · [다형과 참조 배열](polymorphism.md) ·
...
|거절|`mailByMailId` 컬럼이 **그 셀을 가리켜 거부**되는지|
|배열|`foreign Mail[]`|
