...
- **named-range 표기가 이 모델로 번역됩니다.** `character[0]["Id"]`는 `Character1.Id`와 같은 것을 가리키므로,
  named-range 레이아웃 파서는 **두 번째 모델이 아니라 번역**입니다. 모델이 하나면 두 레이아웃이
...
|대안|왜 아닌가|
|--|--|
|`character[0]["Id"]` (named-range 그대로)|엑셀 헤더에 따옴표를 넣는 것이 편집에 불편하고, 인덱스 표기가 연번 규칙과 이중이 됩니다. 우리 레이아웃에는 이미 연번이 있습니다|
...
|8|`named-range` 레이아웃 파서에서 이 모델로 번역|—|
