...
>
> 상태: **전 단계 구현 완료** (2026-08-25) — `$type` 표기 · 모델 · 결합 · 검증 · 판별자
> 정렬 · 모든 언어의 생성 표면(4단계), 멀티 로우 `effects[].$type`(5단계), 그리고 지운
> 변종의 Tombstone `(removed)`(6단계). **형식은 움직이지 않았습니다** — 기존 24개 시나리오의
> 골든이 한 파일도 안 바뀌었습니다.
>
> **참조 쪽은 되돌렸습니다 — [참조가 내는 이름](../references/reference-surface-naming.md) §6.**
...
> `$type`은 예약 컬럼으로만 있습니다 ([주 레이아웃 §7](../layout/primary-layout.md))

「행마다 형태가 다른 데이터」를 적는 방법이 이 도구에 하나뿐입니다 — [다중 대상
참조](../references/multi-target-accessors.md)이고, 그것은 **키 하나가 어느 카탈로그의 행인가**에만
...
겉모습이 비슷하므로 먼저 표로 고정합니다. [주 레이아웃 §7.1](../layout/primary-layout.md)이 앞의 둘을
...
조회해 산출합니다([다중 대상 접근자 §7](../references/multi-target-accessors.md)). 분기하는 것은 **선언과
...
|무엇|상태|
|--|--|
|~~`foreign A\|B` — 검사 · 승격 · 대상별 접근자~~|**되돌렸습니다.** 검사만 남고 표기는 `refs=`입니다 ([참조가 내는 이름 §6](../references/reference-surface-naming.md))|
...
|`abstract` · `extends` · 변종의 `@N` · Tombstone `(removed)`|**됨**(§11의 1과 6). 문법 · 변종 집합 · 판별자 번호 · 지운 변종의 번호 예약. 소비하는 표기가 `$type`입니다|
|`$type`|**됨**(§11의 4와 5). 스칼라 그룹과 멀티 로우 배열 둘 다입니다|
...
([주 레이아웃 §3.4](../layout/primary-layout.md)).
...
[다중 대상 접근자 §5](../references/multi-target-accessors.md)가 남긴 자리입니다 — 「목록이 같은 선언들을
...
|**판별자 값**|**`@N`.** 생략하면 선언 순서로 부여합니다. Tombstone은 `(removed)` — 5.1.1절|[와이어 태그](../../doc/sheets.md#와이어-태그-n)와 같은 문제입니다 — 번호가 선언 순서에 고정되면 변종을 재배열·삭제할 때 배포된 리더가 다른 변종으로 읽습니다|
...
|무엇|근거|
|--|--|
|`$value` — 변종의 값을 한 셀로 압축|[주 레이아웃 §7](../layout/primary-layout.md)의 1차 판정 그대로 거절합니다. `sep` 표기와 중복되는 두 번째 방법이 됩니다|
|다단 상속 · 변종의 변종|5.1|
