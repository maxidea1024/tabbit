...
타입이 아니라 변종의 것**이기 때문입니다. `extends Effect`가 「어느 집합에 속하는가」를 정하고
`@1`이 「그 집합 안에서 몇 번인가」를 정합니다 — 시트에서 `price@3`이 「Price 컬럼의 번호는
...
|--|
|**컬럼** — [와이어 태그 `@N`](../../../doc/sheets/naming.md#와이어-태그-n). 컬럼을 가리키는 것을 위치에서 번호로 옮깁니다|
|**변종** — 이 절. 변종을 가리키는 것을 선언 순서에서 번호로 옮깁니다|
...
|--|--|--|
|컬럼의 `@N`|**와이어 태그** (`Field.WireTag`)|파일 안에서 그 **컬럼**을 가리키는 것|
|변종의 `@N`|**판별자** (`PolymorphicVariant.Discriminator`)|파일 안에서 그 **변종**을 가리키는 것|
...
|**어느 행도 쓰지 않는 변종 멤버의 컬럼은 적지 않아도 됩니다.** 쿠킹이 그 컬럼을 그룹의 끝에 비어 있는 채로 더합니다 — 이름·옵셔널 여부·자리가 저자가 적고 비워 둔 컬럼과 같습니다. 추상 타입 자신의 필드는 예외이고, 컬럼이 없으면 struct 그룹과 같은 이유로 거부합니다. 와이어 태그를 직접 적은 표에서는 더할 컬럼에 줄 태그가 없으므로 거부합니다|

**컬럼을 더하는 이유는 타입이 선언마다 하나이기 때문입니다.** 같은 추상 타입을 쓰는 표가
여럿이면 생성 코드의 변종 타입은 그중 하나가 아니라 선언의 것이어야 하고, 각 표의 평면
원소(§7.2)에서 변종 객체를 만드는 코드는 선언된 멤버 전부를 읽습니다. 표마다 컬럼 집합이
다르면 그 코드가 어느 표에서는 없는 필드를 읽게 되고, 어느 표가 먼저 읽혔는지에 따라
컴파일 오류가 나거나 다른 표가 적은 값이 타입에서 빠집니다. 컬럼을 더하면 표마다 같은
집합이 되고, 생성기는 이 규칙을 모릅니다.
