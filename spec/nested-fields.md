...
[실측](../samples/named-range/doc/레이아웃-분석-20260808.md#실제-사용되는-중첩-모양-셋)이었습니다 —
615개 테이블에서 깊이 1의 세 모양이 리프의 99.5%였기 때문입니다. 그 판단이 뒤집힌 이유는
...
|깊은 그룹의 원소 번호|**최상위 레벨에만.** `Pos.Sub.X1`처럼 안쪽 레벨에 번호가 붙으면서 중첩하는 것은 거부합니다|와이어는 담을 수 있지만 배열이 레코드 아래로 들어가 생성기 13개의 경우가 곱해집니다. 실측에 없는 모양이라 **일부 언어에서만 되는 것보다 그 이름과 함께 거부하는 편이** 낫습니다|
...
모든 생성기가 전부 이 모양입니다. C#의 뷰를 읽고 확정하였고, 나머지 12개가 그것을 따랐습니다.
...
|6b|나머지 언어. 하나씩, 적합성 코퍼스에 모양 추가|**됨** — 전부|
