...
|코드 생성|**13개 언어 전부 + `json`·`binary`.** 깊이 제한이 없습니다|타깃마다 `SupportsDeepNestedFields`로 나타냅니다. 14번째 타깃은 이 플래그를 먼저 만나고, 지원하기 전에는 **자기 이름과 함께 거부**합니다|

## 6. 생성기 — 선언은 그룹 단위, 읽기는 와이어 컬럼 단위

13개 생성기가 전부 이 모양입니다. C#의 뷰를 읽고 확정하였고, 나머지 12개가 그것을 따랐습니다.
...
**13개가 전부 지원하므로 이 오류를 내는 타깃은 이제 없습니다.** 검사는 `ITarget`에 남아 있고,
...
|6b|나머지 11개 언어. 하나씩, 적합성 코퍼스에 모양 추가|**됨** — 13개 전부|
