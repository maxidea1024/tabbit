...
> 그 파일을 고치고 다시 실행합니다.
>
> 상태: **설계안.** 도입 여부는 [행렬 선언 — 1급 엔티티 검토](../types/matrix-entity.md)에서
> 검토 중이고, 이 문서는 **도입하기로 하면 쓸 표기**를 정합니다.
...
|`:col` 칸이 빈 컬럼|**오류**입니다 — 키라면 값을, 메모라면 `#`를 안내합니다|
...
|이름|무엇|
|--|--|
|`At(rowKey, colKey)`|칸 하나. 축의 키가 없으면 그 언어의 조회 실패 규약을 따릅니다|
|`Row(rowKey)`|한 행. 열 축의 키 순서와 같은 순서입니다|
...
|무엇|선택지|
|--|--|
|`:field`의 C칸 생략|생략하면 격자의 이름을 `value`로 둘 것인지, 이름을 필수로 할 것인지|
|축의 키가 참조일 때 검사 시점|다른 참조와 같게 할 것 — 대상 테이블에 없는 키를 오류로|
