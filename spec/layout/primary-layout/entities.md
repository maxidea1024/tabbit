...
|Kotlin|**`List<T>`**|`listOf( … )` — 컬럼 멤버의 `MutableList`가 아닙니다. 더할 수 없는 상수는 타입으로 그렇게 나타냅니다|
...
|(enum 원소)|그 enum|**어느 언어도 숫자로 캐스팅하지 않습니다** — 라벨을 이름으로 적으므로 상수 파일이 enum에 의존하게 되고, 그 의존이 타입 그래프에 없으면 크레이트가 컴파일되지 않습니다. 적합성 코퍼스의 `Limits`가 그 자리를 지키는 이유입니다|
...
- **검사입니다.** 컬럼은 타입을 그대로 지키고, 값이 목록의 어느 테이블에도 없으면 그 셀을
  가리켜 보고합니다. 어느 쪽에서 왔는지는 따지지 않으므로 **대상끼리 id가 겹쳐도 됩니다** —
