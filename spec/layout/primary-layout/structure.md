...
시작하는 미지의 값은 오류**입니다 — Luban이 미지의 헤더 태그를 로그로만 남기는 것과 갈리는
...
|그 밖|**그 행의 태그** — `key` 또는 `key=value`, 쉼표로 여러 개. 레시피가 부를 때만 그 행이 빠집니다([태그](../tags.md)). `:enum`과 `:const`에서는 담기기만 하고 행은 빠지지 않습니다 — 그 행들은 생성 코드이기 때문입니다|
...
- **첫 키가 기본 인덱스입니다** — 행을 가리키는 것 · 참조 대상 · 멀티 로우의 레코드 경계(§6.1)가
  전부 첫 키의 것입니다.
...
- **키 컬럼(§3.5의 성분 포함)에는 변형을 둘 수 없습니다.** 행을 가리키는 값이 빌드마다 달라지면
