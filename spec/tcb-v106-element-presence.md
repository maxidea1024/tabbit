...
>
> 상태: **구현 완료** — 변환기와 13개 런타임 전부 · 대상 버전 **106** (105를 대체)
...
13개 언어가 내는 원소별 답은 [원소가 없을 수 있는 배열](nullable-array-elements.md)에 있습니다.
...
모양을 위한 것입니다. 이제 이 바이트에 예약 비트가 없습니다.
...
|값의 압축|없는 원소도 빈 값을 차지합니다. 압축하면 인코딩 13종 × kind 3종의 디코드를 「present인 원소만 세면서」 다시 쓰게 되고, 그것을 13개 언어에서 반복합니다. 원소당 1비트를 내고 그 작업을 사지 않았습니다|
...
|나머지 8개 언어|생성 코드의 컴파일. 언리얼은 UHT가 헤더를 받는지까지|
|`nullable-elements` 골든|13개 언어 전부와 `json`·`binary`. 표기 4종과 `string?[]`이 한 테이블에 있습니다|
...
|파일 전체|버전 4바이트|
|13개 런타임|컬럼 구조체에 `bool` 한 칸, `ReadElementPresence` 하나, `CheckColumn`의 비교 한 줄|
