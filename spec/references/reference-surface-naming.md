...
> 있습니다.
>
> 상태: **구현 완료** (2026-08-25) — 세 단계 전부. `refs=`가 들어갔고, 다중 대상 참조를 걷어냈고,
...
**컬럼 이름에는 「id」가 적혀 있고 타입 칸에는 「행」이 적혀 있습니다.** 소비하는 쪽이 받는
...
**키가 공개 표면에 없습니다.** 밑줄로 시작하는 이름은 「읽지 마십시오」를 뜻하는 이름이고,
문서에도 없습니다.
...
한 도구가 같은 것을 세 가지로 내고 있고, **저자는 어느 것을 받을지 시트만 보고 알 수
...
**흔한 경우에 깁니다.** `postRow.MailByMailId.Title`이고, 승격된 단일 대상 선언이 이전 대규모 코퍼스에서
...
- **셀렉터는 공통 분모가 없습니다.** A와 B가 무엇을 공유하는지 아무 데도 적혀 있지 않으므로
...
- **컬럼을 나누면 간단합니다.** `itemId foreign Item`과 `mountId foreign Mount` 두 컬럼이면
  각각이 규칙 1·2를 그대로 받고, 어느 쪽인지는 값이 있는 쪽이 정합니다. 판별자도 슬롯도
  캐스트도 없습니다.
...
|`CheckKeyBandsDoNotOverlap`|대상이 하나면 겹칠 것이 없습니다. 검사 쪽은 「어느 하나에 있으면 통과」이므로 겹침을 보지 않습니다|
...
- **승격되었던 다중 대상 컬럼이 검사로 돌아갑니다.** 그 컬럼의 와이어가 대상 키 타입에서
  원래 폭으로 돌아가므로 **샘플의 커밋된 출력이 움직입니다.** 미리 재고 diff를 봅니다.
...
|샘플 재생성|이름이 바뀌고 **값이 안 바뀌는지.** 승격이 풀린 컬럼은 폭이 되돌아갑니다|
...
표시를 달았고, 대상 하나짜리 참조 배열(§11의 3)은 그대로 유효합니다. struct 다형성(§5 · §6)도
그대로입니다.
