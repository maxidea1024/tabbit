...
|미결|위치|이 체계에서|
|--|--|--|
|계약 어셈블리 이름|[검증 사용성 §5a](validation-usability-and-assembly-output.md)|규칙 작성자에게 열리는 표면의 이름. 생성 액세서의 네임스페이스와 충돌하지 않는 자리|
|인스턴스 타입 이름|[접근자 객체화 §5](accessor-instances.md)|정본에서 파생. 정적 파사드와 다른 타입이어야 하므로 파생 규칙이 필요합니다|
|코드 생성 dll의 어셈블리 이름|[검증 사용성 §7](validation-usability-and-assembly-output.md)|네임스페이스류 정본에서 파생|
