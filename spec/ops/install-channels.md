...
**선결 사항이 1개 남아 있습니다.** Windows 아카이브에 실행 파일 외의 파일이 들어 있습니다
(4절). 릴리즈는 `v0.1.0`으로 해소되었습니다(2절).
...
**릴리즈는 게시되어 있습니다.** `v0.1.0`이 2026-08-26에 올라갔고, 아카이브 6개와
`SHA256SUMS`가 함께 있습니다. 문서가 가리키는 내려받기 주소가 실재하므로, 이 절의 4단계는
이제 실제로 밟을 수 있는 절차입니다.
...
지금은 그렇지 않습니다. `v0.1.0`의 게시본 실측입니다.

```
tabbit-0.1.0-win-x64.zip
  tabbit.exe                    66,646,914 바이트
  aspnetcorev2_inprocess.dll       392,488 바이트
  LICENSE                            1,468 바이트
  readme.md                          4,032 바이트

tabbit-0.1.0-linux-arm64.tar.gz
  tabbit                        65,350,079 바이트
  LICENSE · readme.md
```
...
> **해당하는 것은 Windows뿐입니다.** `aspnetcorev2_inprocess.dll`은 IIS 인프로세스 모듈이라
> Windows 게시에만 나오고, Linux·macOS 아카이브는 실행 파일 하나에 `LICENSE`와 `readme.md`가
> 붙은 것이 전부입니다. 매니페스트가 실행 파일을 지목해야 하는 채널에서 판정이 갈리는 곳도
> Windows 쪽(Scoop·winget)뿐입니다.
