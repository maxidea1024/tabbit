...
>
> 상태: **설계 — 구현 없음.** `abstract` · `extends`는 예약어로만 있고
> ([`SchemaParser.cs:190`](../src/Schema/SchemaParser.cs#L190)), `:type`은 예약 컬럼으로만
> 있습니다 ([주 레이아웃 §7](primary-layout.md))
...
|`abstract` · `extends`|**예약어.** `schema.polymorphism-reserved`로 거부합니다|
...
|#|무엇|형식|생성 코드|
|--|--|--|--|
|1|**`abstract struct`** — 문법 · 모델 · 변종 목록. 필드가 없는 것까지|무변경|무변경|
|2|**다형 참조** — `foreign Reward` 스칼라, `extends=` 메타 키, 컬럼 계약 검증|무변경|추상 타입 + 판별 enum|
...
|게이트|확인하는 것|
|--|--|
|기존 골든|**무변경.** 1~3은 새 표기를 추가할 뿐입니다|
|새 시나리오 `polymorphism`|전 타깃. 언어 컴파일과 되읽기까지 — [복합 키](primary-layout.md)가 세운 형태 그대로입니다|
