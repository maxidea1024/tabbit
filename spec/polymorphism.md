...
**컬럼의 `@N`과 다른 점이 하나 있습니다** — 컬럼 태그는 테이블 단위로 전부 또는 전무이고,
변종 번호는 **추상 타입 단위**입니다. 컬럼 태그를 안 쓰는 테이블에서도 변종 번호는 쓸 수 있고,
그 반대도 됩니다. 두 번호가 서로를 모르기 때문입니다.
...
|판별자 컬럼의 프로퍼티|`<경로>` 그대로 — `effect.$type` 컬럼은 `Effect`를 냅니다|
