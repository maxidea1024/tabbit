...
>
> 상태: **1단계 구현됨** — `abstract struct` · `extends` · `@N`이 STRUCT DSL의 문법이
> 되었고 변종 집합이 해석됩니다(§11). 나머지 넷은 설계입니다. `:type`은 예약 컬럼으로만
...
|게이트|확인하는 것|
|--|--|
|기존 골든|**무변경.** 1~3은 새 표기를 추가할 뿐입니다 — 1단계에서 24개가 실측으로 무변경입니다|
