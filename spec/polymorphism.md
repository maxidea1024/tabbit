...
> 형식 개정은 필요 없고, 비용을 가르는 것은 변종의 개수가 아니라 행의 순서였습니다.
> `foreign A\|B[]`은 아직 거부합니다(§4). `:type`은 예약 컬럼으로만 있습니다
> ([주 레이아웃 §7](primary-layout.md))
...
|`foreign X[]` — 셀 안의 참조 배열|**됨**(§11의 3). 대상 하나짜리는 원소마다 행 하나로 해석됩니다. 대상이 여럿인 배열은 아직 거부합니다|
...
같은 데이터 2,000행, 변종당 멤버 6개입니다.

|형태|`.tcb`|참조 대비|
|--|--|--|
|**참조 경로** — 변종 5개가 각자 카탈로그, 그것을 가리키는 테이블 하나|**10,850 B**|1.00배|
|값 임베딩 5 — **변종별로 행이 묶여 있음**|**14,888 B**|**1.37배**|
|값 임베딩 16 — 변종별로 묶여 있음|21,501 B|1.98배|
|값 임베딩 5 — **행마다 변종이 번갈아 나옴**|**65,730 B**|**6.06배**|
|값 임베딩 16 — 행마다 번갈아|83,602 B|7.71배|

**가르는 것은 변종의 개수가 아니라 행의 순서입니다.** 변종을 5개에서 16개로 넓히면 묶인
쪽이 1.4배가 되는데, 같은 5개를 섞어 놓으면 4.4배가 됩니다.

이유는 RLE입니다. 변종 멤버 컬럼은 대부분이 빈 값이고, **빈 값이 이어져 있을 때만** 런이
됩니다. 변종이 행마다 번갈아 나오면 5행마다 런이 끊기므로 런 하나가 4행짜리가 되고, 그
컬럼 하나가 2,000행에서 2,420바이트입니다 — 인코딩 보고서가 그 수를 그대로 적습니다.
변종별로 묶으면 같은 컬럼이 런 두어 개로 접힙니다.
...
|**변종의 폭**|**16개까지 견딥니다.** 상위 문서가 판정 자리로 지목한 곳이고, 묶인 데이터에서 1.98배입니다|
|**행 순서**|**설계가 답해야 하는 자리가 새로 생겼습니다.** 섞인 데이터의 6~8배는 큰 테이블에서 받아들일 수 없습니다 — 내보낼 때 판별자로 정렬할지, 시트 규약으로 둘지, 그대로 두고 문서로 안내할지를 4단계가 정합니다|
|**범용 압축**|이 차이를 크게 줄입니다 — 섞인 5개가 Deflate 뒤 15,398B이고 묶인 것이 2,609B입니다. 다만 그것은 [모든 런타임에 압축 해제기를 들이는](../doc/roadmap.md) 별건입니다|

**값 임베딩이 확보하는 것은 크기가 아닙니다.** 묶인 데이터에서도 참조 경로보다 크고, 그것이
사는 것은 **id를 부여하지 않는 것**입니다(9절).
...
|3′|**대상이 여럿인 배열** — `foreign A\|B[]`의 원소별 판별자와 접근자|무변경|모든 언어|
|4|**다형 레코드** — `:type`, 컬럼의 합집합. **행 순서를 어떻게 다룰지가 이 단계의 첫 결정입니다**(§6.2)|**무변경** — 측정으로 확인(§6.1)|변종 타입|
...
|와이어 측정|**됨**(§6.1). 형식 개정 없음, 변종 16개까지 견딤, 그리고 행 순서가 6~8배를 가름|
