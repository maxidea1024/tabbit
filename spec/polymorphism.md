...
> 변종의 묘비 `(removed)`(6단계). **형식은 움직이지 않았습니다** — 기존 24개 시나리오의
> 골든이 한 파일도 안 바뀌었습니다.
>
...
|`abstract` · `extends` · 변종의 `@N` · 묘비 `(removed)`|**됨**(§11의 1과 6). 문법 · 변종 집합 · 판별자 번호 · 지운 변종의 번호 예약. 소비하는 표기가 `$type`입니다|
...
|**판별자 값**|**`@N`.** 생략하면 선언 순서로 부여합니다. 묘비는 `(removed)` — 5.1.1절|[와이어 태그](../doc/sheets.md#와이어-태그-n)와 같은 문제입니다 — 번호가 선언 순서에 고정되면 변종을 재배열·삭제할 때 배포된 리더가 다른 변종으로 읽습니다|
...
|**묘비**|`struct BurnEffect extends Effect @4 (removed)`. 지운 변종을 그 줄에 `(removed)`만 달아 남겨 두면 그 번호는 다른 변종에 가지 않습니다 — **멤버의 묘비와 같은 표기**이고(`field x int (removed)`), 같은 것이 한 단 위에 있는 것이기 때문입니다. 묘비는 타입이 아닙니다: 어느 컬럼도 담을 수 없고, 생성되는 코드가 없고, 집합의 원소도 아니고, **남은 `field` 줄은 해석조차 되지 않습니다** — 그래서 지운 변종이 쓰던 타입이 함께 없어져도 그 줄을 지울 필요가 없습니다. 잡아 두는 것은 번호 하나입니다|
...
|변종의 `@N`이 겹침|오류|
|변종이 묘비가 잡고 있는 `@N`을 씀|오류. **묘비를 이름으로** 가리킵니다|
...
|6|**변종의 묘비** — `struct X extends Y @N (removed)`. 멤버의 묘비와 같은 표기입니다(5.1.1)|**됨 — 무변경**|**됨 — 무변경.** 묘비는 어느 언어에도 나오지 않습니다|
...
|**묘비**|**됨**(6단계). 단위 테스트가 번호 예약을 판정하고 — 묘비가 먼저 쓰였을 때와 나중에 쓰였을 때 둘 다 — 시나리오의 `BurnEffect`가 **어느 언어의 어느 파일에도 나오지 않는지**를 산출 트리 전수로 확인합니다. 「`(removed)`를 장식으로 읽는 빌드」는 모든 타깃에 이름을 남기면서 전부 컴파일되기 때문입니다|
