...
>
> 상태: **1~3단계 구현됨** — `abstract struct`가 문법이 되었고, 테이블이 `extends=`로
> 집합에 속하며, `foreign Reward`가 그 집합으로 펼쳐지고, `foreign Item[]`이 셀 배열로
> 받아집니다(§11). 남은 둘은 설계이고, `foreign A\|B[]`은 아직 거부합니다(§4).
...
|**적합성 코퍼스**|**아직입니다.** 15개 리더가 실제로 돌아가는 코퍼스에 `foreign T[]` 컬럼이 없습니다 — 원소별 할당을 그 15개에서 확인하는 자리가 그것이고, 지금 그 몫을 하는 것은 접힌 연번 그룹(`serial-ref`)입니다|
