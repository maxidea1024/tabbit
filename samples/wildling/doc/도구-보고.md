...
**11건이 전부 닫혔습니다.** 확인은 `기획데이터/tools/verify.py` 가 하고,
지금 **통과 9 · 건너뜀 1**입니다 — 건너뛴 하나는 캐시된 실행이라 검증을 다시 돌지 않은 것입니다. 확인은 `기획데이터/tools/verify.py` 가 하고, 지금 **통과 10 · 실패 0**입니다.
---
...
**산출물의 자리.** `html` 은 유니티 애셋이 아니므로 `기획데이터/out/html` 입니다. 게임이
로드하지 않고 사람이 읽습니다.
