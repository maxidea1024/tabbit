...
|[3](#3-합성-값-타입과-와이어-태그--메시지가-가리키는-곳)|합성 값 타입과 와이어 태그의 메시지|남음 — 진단의 순서|
|[4](#4-string-상수의-c-생성-미지원)|`string[]` 상수의 C# 생성|**닫힘** — 상수 6개가 배열로|
|[5](#5-멀티-로우-그룹의-dsl-struct-이름)|멀티 로우 그룹의 DSL struct 이름|남음|
|[6](#6-매트릭스-표의-레이아웃-한정)|매트릭스 표의 레이아웃 한정|남음 — 문서 문제|
...
**10건 중 7건이 닫혔습니다.** 남은 셋은 진단의 순서(3), 표기의 미지원(5), 문서의 범위(6)이고
전부 막지 않습니다. 확인은 `기획데이터/tools/verify.py` 가 하고, 지금 **통과 10 · 실패 0**입니다.
---
