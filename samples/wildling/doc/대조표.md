...
인용은 전부 실물입니다 — `기획데이터/data/*.tsv` 와 `unity/Assets/Tabbit/Generated/` 에서
그대로 옮겼습니다.
...
|`MonsterSkill` — `:table MonsterSkill(key="monster_id,skill_id")`|**성분이 둘 다 참조**인 연결 테이블입니다. 그 조합이 [도구 보고](도구-보고.md) §7이 나온 자리입니다|
...
[도구 보고](도구-보고.md) §1 · §8이 나온 자리입니다 — **가장 자연스러운 형태가 가장 덜
다녀진 길이었습니다.**
...
|`html`|`기획데이터/out/html/` — **유니티 애셋이 아니므로 프로젝트 밖입니다**|
|인코딩 보고서|`기획데이터/out/encoding-report.txt` — 컬럼마다 이긴 인코딩과 후보별 실측 크기|
|스키마 기준선|`기획데이터/out/schema-baseline.json` — `Monster.Tags` 가 문자열에서 배열이 되었을 때 그것을 잡은 것이 이 파일입니다|
...
[unity-check.txt](../기획데이터/out/unity-check.txt) 의 내용입니다. **행 수만 보면 「읽었다」
