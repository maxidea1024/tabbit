...
namespace Wildling.Data
{
    ...
    public partial class BattleSpeedTable : IEnumerable<BattleSpeedTable.Record>
    {
        ...
        /// Field names.
        /// </summary>
        public static readonly string[] FieldNames = { "At", "Multiplier", "Label" };
        ...
        public List<object[]> BuildObjectValueMap()
        {
            ...
            foreach (var r in _records)
                result.Add(new object[] { r.At, r.Multiplier, r.Label });
        ...
        /// <summary>Whether the table holds a row with this `At`.</summary>
        public bool ContainsAt(int key) => _recordsByAt.ContainsKey(key);
        #endregion // Indexing by `At`

        /// <summary>Each row with the `At` it is keyed by.</summary>
        /// <remarks>
        ...
        /// `At` here, something else in the next table.
        ///
        ...
        /// </summary>
        /// <remarks>
        /// `GetByAtOrThrow` under the spelling the language uses for a
        ...
        /// `At` is 0, not the first row - the rows in order are
        ...
        public Task ReadAsync(TcbReader reader)
        {
            ...
            foreach (var column in columns)
            {
                ...
                {
                    case 1:
                        TcbTable.CheckColumn(column, "BattleSpeed.At", TcbTable.KindScalar, false, TcbTable.ElementI32, TcbTable.ElementVarint);
                        cursor = new TcbColumnCursor(reader, column, count, "BattleSpeed.At");
                        for (int i = 0; i < count; )
                        {
                    ...
                    case 2:
                        TcbTable.CheckColumn(column, "BattleSpeed.Multiplier", TcbTable.KindScalar, false, TcbTable.ElementI32, TcbTable.ElementVarint);
                        cursor = new TcbColumnCursor(reader, column, count, "BattleSpeed.Multiplier");
                        for (int i = 0; i < count; )
                        {
                            // One call per run of equal values, not one per row - over a
                            // run-length encoded column this is most of the decode.
                            ...
                            {
                                var record = records[i++];
                                record._multiplier = value;
                            } while (--n > 0);
                        }
                        break;

                    case 3:
                        TcbTable.CheckColumn(column, "BattleSpeed.Label", TcbTable.KindScalar, false, TcbTable.ElementString);
                        cursor = new TcbColumnCursor(reader, column, count, "BattleSpeed.Label");
                        for (int i = 0; i < count; )
                        {
                            // One call per run of equal values, not one per row - over a
                            // run-length encoded column this is most of the decode.
