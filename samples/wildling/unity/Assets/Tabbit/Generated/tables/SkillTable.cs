...
namespace Wildling.Data
{
    ...
    public partial class SkillTable : IEnumerable<SkillTable.Record>
    {
        ...
        /// Field names.
        /// </summary>
        public static readonly string[] FieldNames = { "SkillId", "Name", "Description", "Element", "TargetScope", "Cooldown", "Icon" };
        ...
        public List<object[]> BuildObjectValueMap()
        {
            ...
            foreach (var r in _records)
                result.Add(new object[] { r.SkillId, r.Name, r.Description, r.Element, r.TargetScope, r.Cooldown, r.Icon });
        ...
        /// <summary>Whether the table holds a row with this `SkillId`.</summary>
        public bool ContainsSkillId(string key) => _recordsBySkillId.ContainsKey(key);
        #endregion // Indexing by `SkillId`

        /// <summary>Each row with the `SkillId` it is keyed by.</summary>
        /// <remarks>
        ...
        /// `SkillId` here, something else in the next table.
        ///
        ...
        /// </summary>
        /// <remarks>
        /// `GetBySkillIdOrThrow` under the spelling the language uses for a
        ...
        /// `SkillId` is 0, not the first row - the rows in order are
        ...
        public Task ReadAsync(TcbReader reader)
        {
            ...
            foreach (var column in columns)
            {
                ...
                {
                    case 1:
                        TcbTable.CheckColumn(column, "Skill.SkillId", TcbTable.KindScalar, false, TcbTable.ElementString);
                        cursor = new TcbColumnCursor(reader, column, count, "Skill.SkillId");
                        for (int i = 0; i < count; )
                        {
                    ...
                    case 3:
                        TcbTable.CheckColumn(column, "Skill.Description", TcbTable.KindScalar, false, TcbTable.ElementString);
                        cursor = new TcbColumnCursor(reader, column, count, "Skill.Description");
                        for (int i = 0; i < count; )
                        {
                            // One call per run of equal values, not one per row - over a
                            // run-length encoded column this is most of the decode.
                    ...
                    case 4:
                        TcbTable.CheckColumn(column, "Skill.Element", TcbTable.KindScalar, true, TcbTable.ElementVarint);
                        presence = TcbTable.ReadPresence(reader, column, count);
                        cursor = new TcbColumnCursor(reader, column, count, "Skill.Element");
                        for (int i = 0; i < count; )
                        {
                            // One call per run of equal values, not one per row - over a
                            // run-length encoded column this is most of the decode.
                        ...
                        for (int i = 0; i < count; i++)
                        {
                            records[i]._elementHasValue = TcbTable.IsPresent(presence, i);
                            ...
                            if (!records[i]._elementHasValue)
                                records[i]._element = default(global::Wildling.Data.Element);
                        }
                        break;

                    case 5:
                        TcbTable.CheckColumn(column, "Skill.TargetScope", TcbTable.KindScalar, false, TcbTable.ElementVarint);
                        cursor = new TcbColumnCursor(reader, column, count, "Skill.TargetScope");
                        for (int i = 0; i < count; )
                        {
                            // One call per run of equal values, not one per row - over a
                            // run-length encoded column this is most of the decode.
                            ...
                            {
                                var record = records[i++];
                                record._targetScope = (global::Wildling.Data.TargetScope)value;
                            } while (--n > 0);
                        }
                        break;
            ...
            _records = records;
            _recordsBySkillId = recordsBySkillId;
