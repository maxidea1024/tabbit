...
namespace Clover.Data
{
    ...
    public partial class TagTable : IEnumerable<TagTable.Record>
    {
        ...
        /// Field names.
        /// </summary>
        public static readonly string[] FieldNames = { "TagId", "Name", "MinAnte", "SortOrder" };
        ...
        public List<object[]> BuildObjectValueMap()
        {
            ...
            foreach (var r in _records)
                result.Add(new object[] { r.TagId, r.Name, r.MinAnte, r.SortOrder });
        ...
        /// <summary>Whether the table holds a row with this `TagId`.</summary>
        public bool ContainsTagId(string key) => _recordsByTagId.ContainsKey(key);
        #endregion // Indexing by `TagId`

        /// <summary>Each row with the `TagId` it is keyed by.</summary>
        /// <remarks>
        ...
        /// `TagId` here, something else in the next table.
        ///
        ...
        /// </summary>
        /// <remarks>
        /// `GetByTagIdOrThrow` under the spelling the language uses for a
        ...
        /// `TagId` is 0, not the first row - the rows in order are
        ...
        public Task ReadAsync(TcbReader reader)
        {
            ...
            foreach (var column in columns)
            {
                ...
                {
                    case 1:
                        TcbTable.CheckColumn(column, "Tag.TagId", TcbTable.KindScalar, false, TcbTable.ElementString);
                        cursor = new TcbColumnCursor(reader, column, count, "Tag.TagId");
                        for (int i = 0; i < count; )
                        {
                    ...
                    case 2:
                        TcbTable.CheckColumn(column, "Tag.Name", TcbTable.KindScalar, false, TcbTable.ElementString);
                        cursor = new TcbColumnCursor(reader, column, count, "Tag.Name");
                        for (int i = 0; i < count; )
                        {
                            // One call per run of equal values, not one per row - over a
                            // run-length encoded column this is most of the decode.
                    ...
                    case 3:
                        TcbTable.CheckColumn(column, "Tag.MinAnte", TcbTable.KindScalar, false, TcbTable.ElementI32, TcbTable.ElementVarint);
                        cursor = new TcbColumnCursor(reader, column, count, "Tag.MinAnte");
                        for (int i = 0; i < count; )
                        {
                            // One call per run of equal values, not one per row - over a
                            // run-length encoded column this is most of the decode.
                    ...
                    case 4:
                        TcbTable.CheckColumn(column, "Tag.SortOrder", TcbTable.KindScalar, false, TcbTable.ElementI32, TcbTable.ElementVarint);
                        cursor = new TcbColumnCursor(reader, column, count, "Tag.SortOrder");
                        for (int i = 0; i < count; )
                        {
                            // One call per run of equal values, not one per row - over a
                            // run-length encoded column this is most of the decode.
            ...
            _records = records;
            _recordsByTagId = recordsByTagId;
