...
namespace Clover.Data
{
    ...
    public partial class DeckTable : IEnumerable<DeckRecord>
    {
        ...
        /// <summary>Whether the table holds a row with this `DeckId`.</summary>
        public bool ContainsDeckId(string key) => _recordsByDeckId.ContainsKey(key);
        #endregion // Indexing by `DeckId`

        /// <summary>Each row with the `DeckId` it is keyed by.</summary>
        /// <remarks>
        ...
        /// `DeckId` here, something else in the next table.
        ///
        ...
        /// </summary>
        /// <remarks>
        /// `GetByDeckIdOrThrow` under the spelling the language uses for a
        ...
        /// `DeckId` is 0, not the first row - the rows in order are
        ...
        public Task ReadAsync(TcbReader reader)
        {
            ...
            foreach (var column in columns)
            {
                ...
                {
                    case 1:
                        TcbTable.CheckColumn(column, "Deck.DeckId", TcbTable.KindScalar, false, TcbTable.ElementString);
                        cursor = new TcbColumnCursor(reader, column, count, "Deck.DeckId");
                        for (int i = 0; i < count; )
                        {
                            ...
                            do
                            {
                                ...
                                record._deckId = value;
                            } while (--n > 0);
                        }
                        break;

                    case 2:
                        TcbTable.CheckColumn(column, "Deck.Name", TcbTable.KindScalar, false, TcbTable.ElementString);
                        cursor = new TcbColumnCursor(reader, column, count, "Deck.Name");
                        for (int i = 0; i < count; )
                        {
                            // One call per run of equal values, not one per row - over a
                            // run-length encoded column this is most of the decode.
                    ...
                    case 3:
                        TcbTable.CheckColumn(column, "Deck.Unlock", TcbTable.KindScalar, false, TcbTable.ElementString);
                        cursor = new TcbColumnCursor(reader, column, count, "Deck.Unlock");
                        for (int i = 0; i < count; )
                        {
                            // One call per run of equal values, not one per row - over a
                            // run-length encoded column this is most of the decode.
                    ...
                    case 4:
                        TcbTable.CheckColumn(column, "Deck.SortOrder", TcbTable.KindScalar, false, TcbTable.ElementI32, TcbTable.ElementVarint);
                        cursor = new TcbColumnCursor(reader, column, count, "Deck.SortOrder");
                        for (int i = 0; i < count; )
                        {
                            // One call per run of equal values, not one per row - over a
                            // run-length encoded column this is most of the decode.
            ...
            _records = records;
            _recordsByDeckId = recordsByDeckId;
