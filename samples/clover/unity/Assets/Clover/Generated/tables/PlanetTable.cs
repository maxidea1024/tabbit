...
{
    [System.Serializable]
    public partial class PlanetRecord
    {
        #region Values
        /// <summary>
        /// 식별자
        ...
        public override string ToString()
        {
            ...
            sb.Append("\"PlanetId\":"); ToStringHelper.ToString(PlanetId, sb);
    ...
    /// </summary>
    [System.Serializable]
    public partial class PlanetTable : IEnumerable<PlanetRecord>
    {
        /// <summary>
        /// Field names.
        /// </summary>
        public static readonly string[] FieldNames = { "PlanetId", "Name", "Hand", "SortOrder" };
        ...
        public List<object[]> BuildObjectValueMap()
        {
            ...
            foreach (var r in _records)
                result.Add(new object[] { r.PlanetId, r.Name, r.Hand, r.SortOrder });
        ...
        /// </remarks>
        public List<PlanetRecord> Records => _records;
        private List<PlanetRecord> _records = new List<PlanetRecord>();
        ...
        /// </remarks>
        public List<PlanetRecord>.Enumerator GetEnumerator() => _records.GetEnumerator();

        IEnumerator<PlanetRecord> IEnumerable<PlanetRecord>.GetEnumerator() => _records.GetEnumerator();

        System.Collections.IEnumerator System.Collections.IEnumerable.GetEnumerator()
            => _records.GetEnumerator();

        #region Indexing by 'PlanetId'
        public Dictionary<string, PlanetRecord> RecordsByPlanetId => _recordsByPlanetId;
        private Dictionary<string, PlanetRecord> _recordsByPlanetId = new Dictionary<string, PlanetRecord>();

        /// <summary>
        /// The row with this `PlanetId`, or null when the table has none.
        /// </summary>
        /// <remarks>
        ...
        /// </remarks>
        public PlanetRecord FindByPlanetId(string key)
            => _recordsByPlanetId.TryGetValue(key, out PlanetRecord record) ? record : null;

        /// <summary>
        /// The row with this `PlanetId`, or a thrown exception naming what was
        /// missing.
        /// </summary>
        /// <remarks>
        /// For a key that has to be there - one from another table, or a constant. The name
        /// says it throws, because a caller reading `GetByPlanetId(id).Name` at
        /// a glance cannot otherwise tell whether the next line is a null check or a catch.
        /// </remarks>
        public PlanetRecord GetByPlanetIdOrThrow(string key)
        {
            if (!_recordsByPlanetId.TryGetValue(key, out PlanetRecord record))
                throw new TabbitException($"There is no record in table `Planet` that corresponds to field `PlanetId` value {key}");

            return record;
        }

        /// <summary>Whether the table holds a row with this `PlanetId`.</summary>
        public bool ContainsPlanetId(string key) => _recordsByPlanetId.ContainsKey(key);
        #endregion // Indexing by `PlanetId`

        /// <summary>Each row with the `PlanetId` it is keyed by.</summary>
        /// <remarks>
        ...
        /// `PlanetId` here, something else in the next table.
        ///
        ...
        {
            private readonly List<PlanetRecord> _rows;
            private int _at;

            internal EntryEnumerator(List<PlanetRecord> rows)
            {
                _rows = rows;
                _at = -1;
            }
            ...
            public (string Key, PlanetRecord Row) Current
                => (_rows[_at].PlanetId, _rows[_at]);
        }
        ...
        /// </summary>
        /// <remarks>
        /// `GetByPlanetIdOrThrow` under the spelling the language uses for a
        ...
        /// `PlanetId` is 0, not the first row - the rows in order are
        /// `Records`. Nothing in the type says which, which is why the table itself is not
        /// subscriptable by position anywhere.
        ///
        /// It does not replace `FindByPlanetId`: a key that may be absent
        /// wants the one whose name says a miss is an ordinary answer.
        /// </remarks>
        public PlanetRecord this[string key] => GetByPlanetIdOrThrow(key);
        ...
        public Task ReadAsync(TcbReader reader)
        {
            ...
            var records = new List<PlanetRecord>(count);

            for (int i = 0; i < count; i++)
                records.Add(new PlanetRecord());
            ...
            foreach (var column in columns)
            {
                ...
                {
                    case 1:
                        TcbTable.CheckColumn(column, "Planet.PlanetId", TcbTable.KindScalar, false, TcbTable.ElementString);
                        cursor = new TcbColumnCursor(reader, column, count, "Planet.PlanetId");
                        for (int i = 0; i < count; )
                        {
                            ...
                            do
                            {
                                ...
                                record._planetId = value;
                            } while (--n > 0);
                        }
                        break;

                    case 2:
                        TcbTable.CheckColumn(column, "Planet.Name", TcbTable.KindScalar, false, TcbTable.ElementString);
                        cursor = new TcbColumnCursor(reader, column, count, "Planet.Name");
                        for (int i = 0; i < count; )
                        {
                            // One call per run of equal values, not one per row - over a
                            // run-length encoded column this is most of the decode.
                    ...
                    case 3:
                        TcbTable.CheckColumn(column, "Planet.Hand", TcbTable.KindScalar, false, TcbTable.ElementVarint);
                        cursor = new TcbColumnCursor(reader, column, count, "Planet.Hand");
                        for (int i = 0; i < count; )
                        {
                            // One call per run of equal values, not one per row - over a
                            // run-length encoded column this is most of the decode.
                            ...
                            {
                                var record = records[i++];
                                record._hand = (global::Clover.Data.PokerHandKind)value;
                            } while (--n > 0);
                        }
                        break;

                    case 4:
                        TcbTable.CheckColumn(column, "Planet.SortOrder", TcbTable.KindScalar, false, TcbTable.ElementI32, TcbTable.ElementVarint);
                        cursor = new TcbColumnCursor(reader, column, count, "Planet.SortOrder");
                        for (int i = 0; i < count; )
                        {
                            // One call per run of equal values, not one per row - over a
                            // run-length encoded column this is most of the decode.
            ...
            var recordsByPlanetId = new Dictionary<string, PlanetRecord>(count);
            foreach (var record in records)
                recordsByPlanetId.Add(record.PlanetId, record);
            ...
            _records = records;
            _recordsByPlanetId = recordsByPlanetId;
