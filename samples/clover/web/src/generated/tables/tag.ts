...
export class TagTable {
   ...
   */
  public [Symbol.iterator](): IterableIterator<TagRecord> {
  ...
  /**
   * Each row with the tagId it is keyed by -
   * `for (const [key, row] of table.entries())`.
   *
   * What this saves a caller is not the key value - the row carries it - but having to know
   * which column the key is: tagId here, something else in the next table.
   *
   ...
   */
  public *entries(): IterableIterator<[string, TagRecord]> {
    for (const record of this._records) {
      yield [record.tagId, record]
    }
  }
