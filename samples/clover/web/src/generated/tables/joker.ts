...
export class JokerTable {
   ...
   */
  public [Symbol.iterator](): IterableIterator<JokerRecord> {
  ...
  /**
   * Each row with the jokerId it is keyed by -
   * `for (const [key, row] of table.entries())`.
   *
   * What this saves a caller is not the key value - the row carries it - but having to know
   * which column the key is: jokerId here, something else in the next table.
   *
   ...
   */
  public *entries(): IterableIterator<[string, JokerRecord]> {
    for (const record of this._records) {
      yield [record.jokerId, record]
    }
  }
