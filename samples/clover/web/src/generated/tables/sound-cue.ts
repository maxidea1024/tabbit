...
export class SoundCueTable {
   ...
   */
  public [Symbol.iterator](): IterableIterator<SoundCueRecord> {
  ...
  /**
   * Each row with the cueId it is keyed by -
   * `for (const [key, row] of table.entries())`.
   *
   * What this saves a caller is not the key value - the row carries it - but having to know
   * which column the key is: cueId here, something else in the next table.
   *
   ...
   */
  public *entries(): IterableIterator<[string, SoundCueRecord]> {
    for (const record of this._records) {
      yield [record.cueId, record]
    }
  }
