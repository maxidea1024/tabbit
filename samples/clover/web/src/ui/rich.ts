 ...
 * 강조할 자리를 찾는 규칙.
 *
 * **셋입니다** — 수(`8` · `1/4` · `2.5` · `1,200`), 값에 붙는 기호가 있는 수(`$3` · `×2` ·
 * `+40` · `Lv.3` · `8회` · `10%`), 그리고 「」 로 묶은 이름. 그 밖의 것은 강조하지 않습니다 —
 * 전부 강조하면 아무것도 강조되지 않습니다.
 *
 * **세 자리마다의 쉼표가 수의 안쪽입니다.** 빼면 `1,200` 이 `1` 과 `200` 둘로 갈려 그
 * 사이의 쉼표만 옅게 남습니다 — 요구 점수가 네 자리를 넘는 자리가 그렇게 보였습니다.
 */
const MARK = /(「[^」]*」)|([$×+\-]?\d+(?:,\d{3})*(?:[./]\d+)*\s*(?:회|장|개|%|배)?)|(Lv\.\d+)/g
