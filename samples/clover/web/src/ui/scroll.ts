...
export class Fling {
  ...
  over = 0
  /** 이 누름이 얼마나 움직였는가. 고르는 자리마다 이것을 확인합니다. */
  moved = 0
...
export class ScrollView extends Container {
  ...
  constructor(private readonly width_: number, private readonly height_: number) {
    ...
    // **렌더 그룹으로 두지 않습니다.** 굴리는 것이 이 통의 y 하나를 옮기는 일이므로 렌더
    // 그룹이 맞는 자리이고 실제로 그렇게 두었는데, **되돌렸습니다.**
    //
    // 이 통은 마스크 안에 있고 그 안의 조커 딱지가 저마다 또 마스크를 겁니다 — 스텐실이
    // 세 겹이고, 그 가운데 한 겹이 렌더 그룹의 경계를 지납니다. 데스크탑에서는 멀쩡했으나
    // **안드로이드의 WebView 에서 도감을 굴리다 화면이 까매졌다는 보고가 있었고**, 그
    // 기계의 드라이버에서 스텐실이 경계를 넘어 살아 있는지를 여기서 확인할 길이 없습니다.
    //
    // **확인할 길이 없는 이득은 두지 않습니다.** [스텐실 마스크를 걷는
    // 것](../../doc/performance/structure.md)이 먼저이고, 마스크가 한 겹으로 줄면 이것을
    // 다시 봅니다.
   ...
   * 방금 끈 것인가.
   *
   * **줄을 누르는 쪽이 이것을 확인합니다.** 끌고 손을 떼는 자리가 어느 줄 위이므로, 보지
   ...
   * 지금 끌고 있는가.
   *
   * **가리킨 것을 알리는 쪽이 이것을 확인합니다.** `dragged` 는 손을 뗀 뒤에도 다음 누름까지
