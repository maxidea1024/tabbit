 ...
 * 2×2 칸을 가르는 줄.
 *
 * **칸마다 상자를 두르는 것과 다릅니다.** 네 값이 테 없이 놓이면 두 줄 네 칸이 아니라
 * 여덟 조각이 흩어진 것으로 읽혀서, 어느 라벨이 어느 값의 것인지가 사이의 넓이에만
 * 남습니다 — 가르는 줄 둘이면 상자 넷보다 적은 선으로 같은 것을 알립니다.
 */
export const CELL_RULE_INSET = 6
/** 판 아래 단추 둘의 사이. */
 ...
 * 낸 카드가 놓이는 줄.
 *
 * **위에 남는 띠는 값이 떠오르는 자리입니다.** 족보 이름(`headline`, 214)과 이 줄 사이가
 * 비어 보이지만 득점하는 카드마다 값이 그 띠로 올라옵니다 — 다 올릴 수는 없고, 값의 윗변이
 * 족보 이름에 닿지 않는 데까지입니다.
 *
 * 값의 윗변은 `PLAY_Y - RISER_ON_CARD - RISER_LIFT - 글의 반높이` 이고, 족보 이름의
 * 아랫변이 233 입니다.
 */
export const PLAY_Y = 350
 ...
 * 손패 한 장이 다음 장에 덮이는 폭.
 *
 * **손패는 언제나 겹칩니다.** 여덟 장이 2픽셀을 띄우고 나란히 놓이던 동안 그 줄은 쥔
 * 패가 아니라 진열된 카드 여덟 장이었습니다 — 겹쳐야 한 벌로 읽히고, 덮이는 것은 오른쪽
 * 변이므로 왼쪽 위의 끗과 무늬는 장마다 남습니다.
 *
 * **높이와 기울기는 건드리지 않습니다.** 가운데를 높이고 양끝을 기울인 부채꼴은 한 번
 * 걷었습니다(`cards.ts` 의 `syncCards`) — 여덟 장이 저마다 다른 높이와 기울기가 되어
 * 줄이 고르지 않게 보입니다. 겹침은 그것과 다른 것입니다.
 */
export const HAND_OVERLAP = 12

/**
 * 손패 줄이 쓸 수 있는 가로.
 *
 * 장수가 늘면 이쪽이 겹침의 폭을 정합니다 — 바우처로 열두 장을 쥐는 판에서 겹침만으로
 * 자리를 잡으면 줄이 화면의 변을 넘어갑니다.
 */
export const HAND_SPAN = 720

/** 그 장수의 손패가 한 장마다 나아가는 거리. */
export function handSpacing(count: number): number {
  return Math.min(SIZE.cardWidth - HAND_OVERLAP, HAND_SPAN / Math.max(1, count))
}

/**
 * 손패 바로 위에서 지시문이 읽히는 기준선입니다.
 *
 * **족보 이름은 여기가 아닙니다.** 그것은 판 안의 칩 × 배수 위이고(`PANEL_ROWS.handLabel`),
 * 여기 놓이는 것은 무엇을 하라는 한 줄뿐입니다.
 */
export const HAND_INFO_Y = HAND_Y - SIZE.cardHeight / 2 - 40

/**
 * 판이 도는 동안 손패 줄이 물러나는 거리.
 *
 * **자리가 애초에 모자랐습니다.** 낸 카드의 아랫변(428)과 손패의 윗변(546) 사이가
 * 118픽셀인데 카드 하나가 124픽셀입니다 — 바뀌는 카드가 나와 서는 줄을 어디에 두어도
 * 위나 아래로 걸쳤고, 들어오며 1.16배로 부푸는 그 순간이 가장 심했습니다.
 *
 * **낸 카드는 제자리이고 손패가 물러납니다.** 줄 둘을 함께 움직이면 낸 순간에 화면이
 * 크게 흔들리고, 물러나는 것이 무엇인지도 읽히지 않습니다.
 *
 * **34픽셀이 상한입니다.** 손패의 아랫변이 670이고 단추 줄의 윗변이 712이므로 그 사이가
 * 42픽셀이고, 8픽셀을 남깁니다.
 */
 ...
 *
 * **낸 카드가 판으로 올라가는 동안입니다.** 그보다 빠르면 손패가 먼저 내려가 있고, 그보다
 * 느리면 바뀌는 카드가 아직 내려가는 중인 줄 위로 들어옵니다.
 */
export const HAND_DROP_SEC = 0.28

/**
 * 바뀌는 카드가 나와 서는 줄.
 *
 * **낸 카드의 아랫변과 손패의 윗변 한가운데입니다.** 카드가 바뀌는 것은 득점 도중에도
 * 일어나므로(`OnCardScored` · `OnHandPlayed`) 판 가운데에 두면 낸 카드와 겹칩니다.
 *
 * **고정값이 아닙니다.** 손패가 물러난 만큼 이 줄도 따라 내려가야 하고, 그러지 않으면
 * 물러나서 생긴 자리가 아래쪽에만 남습니다 — `drop` 은 그때의 물러난 거리입니다.
 *
 * 34픽셀 물러났을 때 위아래로 14픽셀씩 뜹니다. **들어오며 부푸는 것까지 넣으면 그 여유가
 * 4픽셀입니다** — 그래서 이 줄로 들어오는 카드는 덜 부풉니다(`SHOW_SLAM`).
 */
export const SHOW_SLAM = 1.08

export function showRowY(drop: number): number {
  const above = PLAY_Y + SIZE.cardHeight / 2
  const below = HAND_Y + drop - SIZE.cardHeight / 2
 ...
 * 런 정보와 메뉴의 폭.
 *
 * **둘이 같습니다.** 런 정보를 넓게(128 대 96) 두었던 것은 판 안에서 더 자주 여는 쪽이라는
 * 까닭이었는데, 나란히 선 단추 둘의 폭이 다르면 그 차이가 먼저 읽히고 판의 밑단이 한쪽으로
 * 기울어 보입니다 — 무엇을 더 자주 누르는가는 폭이 아니라 자리가 알립니다.
 *
 * **글이 넘치면 단추가 글자를 한 계단 줄입니다**(`ui/widgets.ts` 의 `set text`). 말마다
 * 길이가 다르므로 어느 말에서 줄어드는지를 여기서 미리 셀 수 없고, 셀 필요도 없습니다.
 */
...
/**
 * 득점이 끝나고 딜러가 낸 카드를 걷기까지. 밀리초.
 *
 * **결과를 읽는 시간입니다.** 무엇을 냈고 얼마가 되었는지가 한 화면에 남아 있어야 합니다 —
 * 걷는 것과 다음 패가 이어져 있으므로, 이 몫이 없으면 점수가 멎는 그 프레임에 판이
 * 비워지기 시작합니다.
 */
export const HAND_READ_MS = 900

/**
 * 보스의 이름이 뜨고 카드가 걸리기 시작하기까지. 밀리초.
 *
 * **누가인지를 읽고 나서 무엇이 일어나는지를 봅니다.** 둘이 같은 순간에 일어나면 눈은
 * 움직이는 쪽(카드 줄)만 보고, 그것을 한 것은 화면 왼쪽에 그대로 남습니다.
 */
export const BOSS_SAY_MS = 620

/**
 * 값의 사슬에 속한 글이 다음 값에 밀려 물러나는 데. 밀리초.
 *
 * **한 번에 하나만 또렷합니다.** 값이 0.42초마다 오르는데 글 하나가 1.05초를 살면 판 위에
 * 숫자가 늘 두셋 겹쳐 있고, 그중 어느 것이 방금 뜬 것인지는 크기로만 갈립니다 — 다음 값이
 * 뜨는 자리에서 앞의 것이 물러나면 또렷한 것이 언제나 하나입니다.
 *
 * **곧바로 지우지는 않습니다.** 그러면 값이 깜빡이는 것으로 보이고, 방금 지나간 것이 무엇
 * 이었는지가 화면에서 끊깁니다.
 */
export const RISER_YIELD = 200
 ...
 *
 * **값이 겹치지 않게 되면서 짧아졌습니다.** 46픽셀이던 것은 다음 값이 뜨기 전에 앞의 것이
 * 자리를 비켜야 했기 때문이고, 지금은 사슬의 앞 글이 물러나므로(`Riser.chained`) 그만큼
 * 올라갈 이유가 없습니다 — 낸 카드의 줄을 올린 만큼 이 글도 위로 가고, 그 끝이 족보
 * 이름에 닿습니다.
 */
export const RISER_LIFT = 38
...
/**
 * 칩 × 배수 상자의 윗변. **족보 이름(348~372)에서 8픽셀입니다.**
 *
 * 그 글이 설명하는 것이 이 두 수이므로 이 무리의 머리이고, 사이는 무리 안의 값입니다.
 */
export const CHIPS_Y = 380

/**
 * 왼쪽 판의 줄들이 서는 자리.
 *
 * **네 무리이고 무리 사이가 24픽셀입니다.** 낱개로 적어 두었더니 사이가 12·30·12로
 * 제각각이었고, 그러면 여섯 칸이 한 덩어리로 보입니다 — 어느 둘이 한 벌인지는 사이의
 * 넓이가 가르는 것이고, 그 넓이가 고르지 않으면 아무것도 가르지 않습니다.
 *
 * |무리|담기는 것|
 * |--|--|
 * |블라인드|딱지 32–236|
 * |이번 손|라운드 점수 260–316 · 게이지 324–336 · 족보 이름 348–372 · 칩 × 배수 380–440|
 * |자원|핸드 · 버리기 464–512 · 금액 · 안테 512–560|
 * |적용 중|584부터|
 *
 * **라운드 점수는 칩 × 배수에 붙습니다.** 그 셋이 한 식이고 무리 안의 사이는 무리 사이보다
 * 좁아야 하므로 절반인 12픽셀입니다 — 딱지에 붙여 두던 동안에는 판이 시작할 때 한 번 보는
 * 요구 점수와 매 수마다 보는 라운드 점수가 한 무리였습니다.
 *
 * **족보 이름이 그 식의 한가운데에 있습니다.** 위에서 아래로 라운드 점수 · 족보 이름 ·
 * 칩 × 배수이고, 눈이 한 번 내려오면서 「이 판은 무슨 족보이고 값은 이것이다」로 읽힙니다.
 * 손패 위로 옮겨 두었던 동안에는 칩과 배수가 어디서 나온 값인지를 잇는 줄이 판에서
 * 없어졌습니다.
 */
export const PANEL_ROWS = {
  /** 라운드 점수 줄의 윗변. 딱지(32~236) 아래 24 입니다. */
  score: 260,
  /** 족보 이름이 앉는 띠의 윗변. 높이는 24 이고, 칩 × 배수 상자와 8픽셀입니다. */
  handLabel: 348,
  /** 2×2 칸의 첫 줄. */
  hands: 464,
  money: 512,
 ...
 * 라운드 점수의 글 줄.
 *
 * **칸 안이 아닙니다.** 눌린 칸에 담아 두었더니 게이지가 그 칸의 아랫변 안쪽에 들어가
 * 그 칸의 장식으로 읽혔고, 판에서 유일하게 길이로 읽히는 것이 테 안에 갇혀 있었습니다.
 */
export const SCORE_ROW_H = 56

/** 라운드 점수 줄과 그 아래 게이지까지. */
export const SCORE_H = SCORE_ROW_H + 8 + 12

/**
 * 무리를 가르는 줄들. **각 사이의 한가운데입니다.**
 *
 * 셋 다 무리 사이(24)의 한가운데입니다 — 딱지와 라운드 점수 사이, 칩 × 배수와 자원 넷
 * 사이, 자원 넷과 적용 중 사이입니다. 첫 줄이 278 이던 동안에는 그 자리가 라운드 점수
 * 칸의 안쪽이라 칸의 그림에 덮여 아무 데도 보이지 않았습니다.
 */
export const PANEL_GROOVES = [248, 452, 572] as const
 ...
 * 덱 더미로 쌓는 뒷면의 최대 장 수.
 *
 * **여덟입니다.** 밑장은 한 장마다 오른쪽으로 2픽셀씩 비켜서므로, 아홉째부터는 그 끝이
 * 화면의 오른쪽 변을 넘습니다 — 덱은 화면의 오른쪽 끝에 붙어 있습니다.
 */
export const DECK_SHEETS = 8

/**
 * 남은 장수에 맞는 더미의 두께.
 *
 * **두 장 아래로는 얇아지지 않습니다.** 한 장이 되면 더미가 아니라 낱장이 되어 손패의
 * 카드와 같은 물건으로 보입니다 — 다 뽑아 비었을 때만 한 장입니다.
 */
export function deckSheets(left: number, all: number): number {
  if (left <= 0) return 1
  const full = Math.max(1, all)
  return Math.max(2, Math.min(DECK_SHEETS, Math.round(left / full * DECK_SHEETS)))
}
