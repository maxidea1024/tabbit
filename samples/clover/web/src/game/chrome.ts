...
export class ChromePart {
  ...
  /**
   * 두 칸의 값을 도구가 고정해 두었는가.
   *
   * **파형을 확인하는 도구가 쓰는 자리입니다**(`forceScore`). 그 도구는 배당을 지목해 놓고
   * 파형이 어느 쪽으로 흐르는지를 재는데, 두 칸의 값은 화면이 늘 다시 씁니다 — 고른 카드가
   * 없으면 0 이고(`Input.showHandPreview`), 판 밖에서도 0 입니다. 그래서 지목한 값이 그다음
   * 프레임에 0 이 되었고, 수가 0 인 칸은 파형을 아예 그리지 않으므로(`uAlive`) 그 도구가
   * 재던 것이 빈 상자였습니다.
   *
   * **`ScoreWave.hold` 과 같은 자리입니다** — 한 번 고정하면 이 판에서는 풀지 않습니다.
   */
  scoreHeld = false
