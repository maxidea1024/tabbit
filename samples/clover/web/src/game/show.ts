...
import {
  ACTIVE_GLOW, BLIND_MUSIC_DIM, BLUR_BACK_PX, BLUR_PX, BOARD_X, BUTTON_Y, CHIPS_GAP, CHIPS_H, CHIPS_Y, CONSUMABLE_TRAY, DEALER, DECK_X, DECK_Y, DELTA_LIFE, DELTA_POOL, EMBER, JOKER_TRAY, JOKER_Y, LAND_AT, LEFT, PACK_TITLE_Y, PACK_X, PANEL_ROWS, PANEL_W, PLAY_H, PLAY_W, PLAY_Y, RIGHT_COL, BOSS_SAY_MS, HAND_READ_MS, RISER_HOLD, RISER_LIFT, RISER_ON_CARD, RISER_SPAN, RISER_YIELD, SELL_WAIT, TRAY_PAD_X, within, IN_X, IN_W, SCORE_H,
} from './metrics'
...
export class ShowPart {
  ...
  buildPanel(): void {
    ...
    //
    // **손패 위로 옮겨 두었던 것을 되돌렸습니다.** 고른 카드 바로 위에 두면 그 자리에서
    // 읽히기는 하지만, 칩과 배수가 어디서 나온 값인지를 잇는 줄이 판에서 없어집니다 —
    // 판을 개편하며 이 띠가 게이지에 먹혔고, 그때 옮겨 간 자리입니다.
    putText(this.game.chrome.handLabel,
            box(IN_X, PANEL_ROWS.handLabel, IN_W, 24), CENTER)
    ...
    this.game.input.hint.position.set(BOARD_X, this.game.cards.handInfoY)
  ...
  private placeRuleBanner(): void {
    this.ruleBanner.place(SIZE.width / 2, this.game.cards.handY - SIZE.cardHeight / 2 - 14)
  }
