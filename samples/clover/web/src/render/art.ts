 ...
 *
 * **1 입니다. 쉬고 있는 크기 그대로 풉니다.**
 *
 * 1.25 이던 동안 카드 그림(220 × 310)은 밀도 2에서 푸는 크기가 310이라 한 번도 줄지
 * 않았고, 그리는 자리에서 176 × 248 로 GPU 가 줄였습니다 — 그 줄이기는 쌍선형이라
 * 가장자리의 대비를 잃습니다. 실제로 화면의 카드는 원화를 쌍선형으로 줄인 것과 같았고
 * (가장 센 기울기 165 대 161), 제대로 줄인 것은 206 이었습니다. **판이 도는 내내 보이는
 * 것이 그 흐려진 쪽이었습니다.**
 *
 * 지금은 푸는 자리에서 `resizeQuality: 'high'` 로 줄이고 그린 자리에서는 1대1입니다.
 *
 * **값은 가리켰을 때입니다.** 가리키면 1.1배로 커지므로(`TIP_GROW`) 그 동안만 1.1배로
 * 늘여 그립니다 — 쉬는 크기를 흐리게 두고 가리킨 동안을 또렷하게 하는 것보다, 늘 보이는
 * 쪽이 또렷한 편이 낫습니다. 여유를 되살리려면 1.1 입니다.
 */
const HEADROOM = 1.0
