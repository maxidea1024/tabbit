...
export class JokerView extends Container {
  ...
  /**
   * 딱지 전체가 한 번 번쩍입니다. **울렁이지 않고 자리도 그대로입니다.**
   *
   * **에디션이 낸 값이 이것을 씁니다.** 조커 하나가 한 자리에서 셋을 잇달아 냅니다 —
   * 누적값과 효과와 에디션입니다. 셋이 같은 몸짓으로 뜨던 동안에는 그 딱지가 세 번
   * 발동한 것으로 보였습니다. 에디션이 낸 것은 딱지의 효과가 아니라 딱지에 붙은 무늬가
   * 낸 것이므로, 흔들리는 것이 아니라 그 무늬가 번쩍이는 것이 맞습니다.
   */
