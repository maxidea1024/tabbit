...
import { COLOR, DEAD, ENHANCEMENT_PAPER, PAINT, SEAL_INK } from './ink'
import {
...
import { describe, rankDisplay, valueText } from '../core/describe'
...
import {
  ...
  TURN_BACK_MS, TURN_STEP_MS, type Beat, type Feel,
} from './juice'
...
import { UI, SIZE, SIDE_PANEL, setUiTheme, popupLeft, popupCenter, rarityColor, TEXT, WEIGHT } from './theme'
import { box, type Box, CENTER, pointOf, putText, splitX } from '../ui/layout'
import { Button, Panel, restyleButtons } from '../ui/widgets'
...
import { attachTip, TipHold, TIP_GROW, TIP_RISE } from '../ui/tip'
 ...
 */
const PACK_CARDS_Y = HAND_Y
const PACK_X = BOARD_X
/**
 ...
 */
const PACK_TITLE_Y = PACK_CARDS_Y - PACK_CARD_H / 2 - 108
 ...
 * 칸 수를 적은 글(193) 아래입니다.
 */
const FOCUS_Y = JOKER_TRAY.y + JOKER_TRAY.height + 36
/**
 * 자리를 비우는 글의 판 크기.
 *
...
export class Game {
   ...
   */
  private held?: { kind: 'joker' | 'consumable' | 'shop' | 'pack' | 'pack_slot' | 'voucher'; uid: number }
  /**
   ...
   */
  private heldNode?: Container
  /** 누르기 시작할 때 고른 것이 무엇이었는가. 손을 뗄 때 그대로인지 봅니다. */
  private heldAtPress?: { kind: string; uid: number }
  /** 그 누름이 고른 것과 그 단추 줄의 밖이었는가. */
  private pressOutsideHeld = false
  /** 고른 것 밑에 서는 버튼들. */
   ...
   */
  private readonly pendingJokers = new Map<number, JokerInstance>()
   ...
   */
  private readonly consumableSlide = new Map<number, Spring>()
  /**
   ...
   * 있었습니다.
   */
  private readonly consumableGrow = new Map<number, Spring>()
  ...
  private readonly payoutRows: { reason: string; amount: number }[] = []
  /**
   * 정산 판을 세울 차례인가.
   *
  ...
  private readonly score = new Slot(t('ui.slot.round_score'), PANEL_W, 52, UI.ink)
  ...
  private readonly hands = new Slot(t('ui.slot.hands'), 124, 52, UI.good)
  private readonly discards = new Slot(t('ui.slot.discards'), 124, 52, UI.discard)
  private readonly money = new Slot(t('ui.slot.money'), 124, 52, UI.money)
  private readonly anteSlot = new Slot(t('ui.slot.ante'), 124, 52, UI.ink)
  /**
   * 왼쪽 판의 값 칸 전부.
   *
   ...
   */
  private readonly sortSlide = new Spring(0, 200, 22)
  /** 산 직후 강조되는 칸 수 글. 어느 글이고 언제까지인가. */
  private countPulse?: { node: Text; base: number; until: number }
  private readonly countScale = new Spring(1, 300, 18)
  /** 「적용 중」에서 밝게 남아 있는 줄. 바우처를 산 직후입니다. */
  private activeGlow?: { label: string; until: number; plate?: Graphics }
  /**
   * 리롤한 직후.
   *
  ...
  private readonly focusLayer = new Container()
  /**
   * 덱이 이 시각까지 나와 있습니다.
   *
   ...
   */
  private deckPeekUntil = 0
  /** 덱으로 날아가는 카드. 팩에서 집은 그 카드 자체입니다. */
  private deckFlight?: { node: Container; motion: Motion; at: number }
  /** 덱이 카드를 받을 때 한 번 눌리는 것. 덱 더미의 세로 어긋남입니다. */
  private readonly deckBump = new Spring(0, 300, 18)
   ...
   */
  private readonly hold = new TipHold(event => this.world.toLocal(event.global))
  /**
   * 마지막으로 쓴 것이 손가락인가.
   *
  ...
  constructor(private readonly app: Application, private readonly data: Data, seed: string,
    ...
    this.coins.onLand = (index, gain, share) => {
      ...
      this.flashPanel(gain ? UI.money : UI.bad, 0.5)
    }
    ...
    this.playButton = new Button(t('ui.button.play'), PLAY_W, PLAY_H, 'primary', () => this.play())
    this.discardButton = new Button(t('ui.button.discard'), PLAY_W, PLAY_H, 'danger',
      () => this.discard())
    // **가운데 버튼이 곧 몇 장 골랐는가입니다.** 점 다섯을 따로 두면 같은 것을 두 곳에서
    // 세게 되고, 그 둘 사이를 눈이 오갑니다.
    this.clearButton = new Button('-', CLEAR_W, PLAY_H, 'neutral', () => this.clearSelection())
    this.primaryButton = new Button(t('ui.button.select_blind'), 210, 50, 'primary', () => this.primary())
    this.skipButton = new Button(t('ui.button.skip'), 150, 38, 'dare',
      () => {
        this.audio.play('blind_skip')
        this.act({ t: 'skip_blind' })
      })
    this.rerollButton = new Button(t('ui.button.reroll'), 128, 44, 'select', () => this.reroll())
    this.sortRankButton = new Button(t('ui.button.sort_rank'), SORT_W, SORT_H, 'neutral', () => this.sortHand('rank'))
    this.sortSuitButton = new Button(t('ui.button.sort_suit'), SORT_W, SORT_H, 'neutral', () => this.sortHand('suit'))
    // 위의 칸들과 같은 격자입니다 — 너비도 자리도.
    // **「족보 목록」 이 아니라 「런 정보」 입니다.** 족보는 그 안의 한 갈래가 되었습니다.
    this.infoButton = new Button(t('ui.run_info.title'), PANEL_BTN_W, FOOT_BTN_H, 'neutral',
      () => this.toggleHandList())
    this.menuButton = new Button(t('ui.button.menu'), PANEL_BTN_W, FOOT_BTN_H, 'neutral',
      () => this.openMenu())
    ...
    app.stage.addEventListener('pointerdown', event => {
      ...
      this.heldAtPress = this.held
      this.pressOutsideHeld = this.held !== undefined && !this.pressedHeld(event.target)
    }, { capture: true })
    ...
    app.stage.on('globalpointermove', event => {
      ...
      this.advanceDrag()
      this.hold.moved(this.pointerAt)
    })
    ...
    app.stage.on('pointertap', () => this.dismissAfterTap())
  ...
  private relabel(): void {
    ...
    this.handLabel.style.stroke = outline(TEXT.big, UI.outline)
    this.headline.style.stroke = outline(TEXT.hero, UI.outline)
  ...
  private continueRun(): void {
    ...
    this.toasts.push(t('ui.run.resumeFailed'), t('ui.run.resumeFailedBody'), UI.bad, 3.4)
  }
   ...
   */
  private pressedHeld(target: unknown): boolean {
    const node = target as Container | null
    if (!node) return false
    for (let at: Container | null = node; at !== null; at = at.parent) {
      if (at === this.heldBar) return true
      if (this.heldNode && !this.heldNode.destroyed && at === this.heldNode) return true
    }
    return false
  }
   ...
   */
  private dismissAfterTap(): void {
    const was = this.heldAtPress
    const outside = this.pressOutsideHeld
    this.heldAtPress = undefined
    this.pressOutsideHeld = false
  ...
  private askQuit(): void {
    ...
    if (!canQuit()) {
      this.toasts.push(t('ui.quit.browser'), t('ui.quit.browserBody'), UI.inkDim, 3.6)
      return
    }
  ...
  private async startRanked(): Promise<void> {
    ...
    this.toasts.push(t('ui.lb.ranked'), t('ui.lb.ranked.on'), UI.good, 2.6)
  }
  ...
  private dropRun(): void {
    ...
    this.deckPeekUntil = 0
    this.deckFlight?.node.destroy({ children: true })
    this.deckFlight = undefined
    this.deckBump.snap(0)
    ...
    this.consumableSlide.clear()
    this.consumableGrow.clear()
  ...
  private act(action: Action): void {
    ...
    for (const event of step.events) {
      if (event.t === 'JokerModified') {
        ...
        if (one) this.pendingJokers.set(event.uid, one)
      }
    ...
    if (this.arriveHold === undefined) {
      ...
      if (step.events.some(event => event.t === 'JokerAdded')) {
        this.arriveHold = { kind: 'joker', until: this.clock + 4, byBeat: true }
      } else if (!this.itemFlyOwned
  ...
  private announce(events: readonly GameEvent[]): void {
    ...
    if (destroyed > 0) {
      this.toasts.push(tf('ui.toast.cards_destroyed', { n: destroyed }), t('ui.deck.removed'), UI.bad, 2.4)
    }
    if (added > 0) {
      this.toasts.push(tf('ui.toast.cards_added', { n: added }), t('ui.deck.added'), UI.good, 2.4)
    }
  }
  ...
  private reroll(): void {
    ...
    for (const slot of [...this.shopTiles.keys()]) this.lingerTile(slot, 0)
    this.rerolled = true
    this.act({ t: 'reroll' })
  }
  ...
  private advanceShopLift(seconds: number): void {
    ...
    // **딱지가 없으면 여기서 끝입니다.**
    if (this.shopTiles.size === 0 && this.packSlotTiles.size === 0 && !this.voucherTile) {
    ...
    const up = this.held?.kind === 'shop' ? `card:${this.held.uid}`
      : this.held?.kind === 'pack_slot' ? `pack:${this.held.uid}`
      : this.held?.kind === 'voucher' ? 'voucher'
      : this.focus?.slot !== undefined ? `card:${this.focus.slot}` : undefined
  ...
  private showBeat(beat: Beat): void {
    ...
    switch (event.t) {
      ...
      case 'HandEvaluated':
        ...
        this.say(tf('ui.hand.level', { name: this.handName(event.hand), level: event.level }), UI.ink, 3, 0.35)
      ...
      case 'MoneyChanged': {
        ...
        const voiced = hosted !== undefined || sold !== undefined || bought !== undefined
          || event.reason === 'reroll'
        if (!voiced) this.audio.play(event.delta > 0 ? 'joker_money' : 'shop_reroll')
        ...
        if (why) {
          ...
          if (sold && this.packLayer.visible) {
            ...
            this.later.push({ at: this.clock + SELL_WAIT, run: () => this.popAt(at, line, tint, 0.5) })
          }
      ...
      case 'JokerAdded': {
        if (this.arriveHold?.kind !== 'joker' || this.arriveHold.byBeat !== true) break
        this.arriveHold = undefined
        this.arriveFrom = this.actorAt ?? { x: BOARD_X, y: JOKER_Y }
        this.refresh()
        this.audio.play('pack_pick')
        this.later.push({
          at: this.clock + LAND_AT,
      ...
      case 'JokerDestroyed': {
        const view = this.burning.find(one => one.uid === event.uid)
      ...
      case 'JokerModified': {
        const view = this.jokers.get(event.uid)
        const joker = this.state.jokers.find(one => one.uid === event.uid)
        this.pendingJokers.delete(event.uid)
        if (!view) break
        // **이름은 이벤트가 들고 옵니다.** 상태에서 찾으면 같은 액션에서 부서진 딱지의
        // 이름을 찾을 수 없습니다.
        const name = this.editionName(event.edition as EditionKind) || t('ui.note.applied')
        ...
        this.audio.play('card_flip')
        view.turnOver(joker, this.jokerLookOf(joker), TURN_BACK_MS / 1000)
        break
      }
  ...
  private showCardChange(beat: Beat): void {
    ...
    wanted.forEach((one, index) => {
      ...
      // 통째로 갈리는 것으로 보였습니다.
      const at = this.clock + this.feel.drawLandMs / 1000 + index * (TURN_STEP_MS / 1000)
  ...
  private turnShownCard(uid: number, kind: 'modify' | 'destroy' | 'add'): void {
    ...
    if (kind === 'destroy') {
      ...
      // 「무엇을」입니다.
      if (was) {
        this.popAt({ x: view.x, y: view.y - RISER_ON_CARD },
          this.cardLabel(was), UI.bad, 0.5)
      }
      this.audio.play('card_destroy')
    ...
    if (kind === 'add') {
      ...
      view.onFlipped = () => {
        ...
        if (now) {
          this.popAt({ x: view.x, y: view.y - RISER_ON_CARD },
            this.cardLabel(now), UI.good, 0.5)
        }
    ...
    const text = was ? this.changeText(was, now) : this.cardLabel(now)
    ...
    view.onFlipped = () => {
      ...
      this.popAt({ x: view.x, y: view.y - RISER_ON_CARD }, text, UI.legendary, 0.5)
      this.audio.play('card_flip')
  ...
  private tellDeck(count: number, tint: number, cue: string): void {
    ...
    this.popAt({ x: DECK_X - 36, y: DECK_Y - RISER_ON_CARD }, `${count}`, tint, 0.5)
  ...
  private tick(deltaMs: number): void {
    ...
    this.hold.advance(seconds, () => this.audio.play('button'))
    this.advanceReveals()
    this.advancePack(seconds)
    this.advanceFocus(seconds)
    this.advanceDeckFlight(seconds)
    this.advanceLater()
    ...
    const away = this.shown.phase !== 'round' && this.clock >= this.deckHold
      && this.clock >= this.deckPeekUntil
      && !(this.packHoldsCards && this.clock >= this.shopStayUntil)
    this.deckBump.advance(seconds)
    this.deckPile.y = this.deckBump.value
    ...
    this.advanceCountPulse(seconds)
    this.advanceActiveGlow()
    this.advanceHandControls(seconds)
    this.badge.advance(seconds)
  ...
  private peek(): unknown {
    ...
    return {
      ...
      focusEnter: Math.round(this.focusEnter * 100) / 100,
      ...
      jokersBack: [...this.jokers.values()].filter(view => view.facingBack).length,
      ...
      countPulse: this.countPulse !== undefined,
  ...
  private jokerLookOf(joker: JokerInstance): JokerLook {
   ...
   */
  private changeText(was: CardInstance, now: CardInstance): string {
    const parts: string[] = []
    if (was.suit !== now.suit) {
      parts.push(`${SUIT_PIP[was.suit] ?? ''} → ${SUIT_PIP[now.suit] ?? ''}`)
    }
    if (was.rank !== now.rank) {
      parts.push(`${rankDisplay(this.data, was.rank)} → ${rankDisplay(this.data, now.rank)}`)
    }
  ...
  private cardLabel(card: CardInstance): string {
    return `${SUIT_PIP[card.suit] ?? ''} ${rankDisplay(this.data, card.rank)}`
  }
  ...
  private refresh(): void {
    ...
    this.slotDelta(this.hands, this.panelShown.hands, state.handsLeft, UI.good)
    this.slotDelta(this.discards, this.panelShown.discards, state.discardsLeft, UI.discard)
    this.slotDelta(this.anteSlot, this.panelShown.ante, state.ante, UI.ink)
  ...
  private drawHandList(): void {
    ...
    rows.forEach((row, index) => {
      ...
      const name = new Text({
        ...
        style: { fontSize: TEXT.base, fill: seen ? UI.ink : UI.inkDim, fontWeight: WEIGHT.normal },
      })
      ...
      const value = new Text({
        ...
        style: { fontSize: TEXT.base, fill: seen ? UI.chips : UI.inkDim, fontWeight: WEIGHT.normal },
      })
  ...
  private drawRunInfoRows(layer: Container, width: number, top: number): void {
    ...
    rows.forEach((row, index) => {
      ...
      const name = new Text({
        text: row.name,
        style: { fontSize: TEXT.base, fill: row.here ? UI.ink : UI.inkDim, fontWeight: WEIGHT.bold },
      })
  ...
  private drawBlindPick(): void {
    ...
    order.forEach((blind, index) => {
      ...
      const reward = label(tf('ui.blind.reward',
        { n: rewardOf(this.data, this.state, row.blind) }), 13, UI.money, '800')
  ...
  private drawGameOver(): void {
    ...
    const cells: [string, string, number][] = [
      [t('ui.slot.ante'), `${state.ante} / ${this.data.run.winAnte}`, UI.ink],
    ...
    const seed = new Text({
      text: state.seed,
      style: { fontSize: TEXT.body, fill: UI.mark, fontWeight: WEIGHT.normal, fontFamily: NUMERALS, letterSpacing: 1 },
    })
    ...
    this.flashScreen(won ? UI.money : UI.bad, won ? 0.5 : 0.34)
    if (won) this.particles.bills(POPUP_X, SIZE.height / 2, 44, UI.money, 1.3, 1.1)
  }
  ...
  private async judgeRun(): Promise<void> {
    ...
    if (line.moved !== undefined && line.moved > 0) {
      ...
      this.jolt(4, 2.2, 1)
      if (line.moved >= 25) {
        this.particles.bills(POPUP_X, SIZE.height / 2, 22, UI.money, 1.1, 1)
      }
    }
    if (line.tier !== undefined) {
      this.audio.play('run_win')
      this.flashScreen(UI.money, 0.3)
  ...
  private syncBadge(): void {
    ...
    if (state.pack) {
      const row = this.data.tables.boosterPack.findByPackId(state.pack.packId)
    ...
    if (state.phase === 'shop') {
      ...
      this.badge.setInfo(t('ui.guide.shop.head'),
        ...
        [tf('ui.badge.next_target', { n: targetOf(this.data, ahead, next).toLocaleString('en-US') })
          + '   ' + tf('ui.blind.reward', { n: rewardOf(this.data, ahead, next) }),
          t('ui.shop.note')],
  ...
  private syncButtons(): void {
    ...
    this.playButton.visible = this.scene === 'run'
    this.discardButton.visible = this.scene === 'run'
    this.clearButton.visible = this.scene === 'run'
    ...
    this.sortRankButton.visible = this.scene === 'run'
    this.sortSuitButton.visible = this.scene === 'run'
  ...
  private beginDrag(kind: 'hand' | 'joker', uid: number, view: Container,
    ...
    if (kind === 'joker') {
      const view = this.jokers.get(uid)
      if (view) this.hold.arm(event, () => this.showTooltip(view))
      return
    }
    const card = this.state.deck.find(one => one.uid === uid)
    // 손에 있는 것이므로 「덱에 남았다」가 아니라 「손에 있다」입니다.
    if (card) this.hold.arm(event, () => this.showCardTip(card, false, true, view))
  }
  ...
  private pick(kind: 'joker' | 'consumable' | 'shop' | 'pack' | 'pack_slot' | 'voucher',
               uid: number): void {
    if (this.player.busy) return
  ...
  private focusEligible(kind: 'joker' | 'consumable', uid: number): boolean {
    if (kind === 'joker') {
      const held = this.state.jokers.find(one => one.uid === uid)
      // `Eternal` 은 팔리지 않습니다.
      return held !== undefined && held.sticker !== 1
    }
    return this.state.consumables.some(one => one.uid === uid)
  }
  ...
  private syncHeldBar(): void {
    ...
    if (held.kind === 'shop') {
      ...
      this.heldNode = this.jokers.get(held.uid)
      ...
      } else if (this.handReady) buttons.push(new Button(t('ui.button.use'), 68, HELD_H, 'primary', () => {
        this.held = undefined
      ...
      if (!this.focus) {
        buttons.push(new Button(tf('ui.button.sell', { n: this.data.economy.sellMin }), 92, HELD_H, 'danger', () => {
          this.held = undefined
          this.audio.play('joker_sell')
  ...
  private itemFlying(from: { x: number; y: number }): void {
    ...
    motion.drift()
    this.itemArrive = { uid: last.uid, warp: 1, glow: 0, filter: new ArriveFilter(), from, motion }
  ...
  private advanceConsumableLift(seconds: number): void {
    for (const one of this.consumableTiles) {
      ...
      if (face) {
        face.rotation = sway(this.clock, phase, 1.1, 1.1) * (Math.PI / 180)
        // **얼굴만 키웁니다.** 칸의 자리는 줄 밖의 여럿이 읽으므로(`spotOf` · `itemSpot` ·
        // 태우기) 칸을 키우면 그 값들의 뜻이 함께 바뀝니다.
        face.scale.set(grow.value)
      }
      one.tile.y = one.baseY - spring.value + sway(this.clock, phase * 1.3, 1.8, 0.7)
  ...
  private syncActive(): void {
    ...
    const glow = this.activeGlow && this.clock < this.activeGlow.until ? this.activeGlow : undefined
    if (glow) {
  ...
  private drawPayout(): void {
    ...
    const sumText = new Text({
      text: '$0',
      style: { fontSize: TEXT.giant, fill: UI.yellow, fontWeight: WEIGHT.bold, fontFamily: NUMERALS },
    })
    ...
    for (let i = 0; i < many; i++) {
      const one = new Text({
        text: '$',
        style: { fontSize: TEXT.head, fill: UI.yellow, fontWeight: WEIGHT.bold, fontFamily: NUMERALS },
      })
  ...
  private advanceBurningItems(seconds: number): void {
    for (let i = this.burningItems.length - 1; i >= 0; i--) {
      ...
      if (travel >= 1 && !one.flashed) {
        ...
        this.audio.play('card_slam')
        this.jolt(7, 1.4, 0.3)
        this.flashPanel(UI.legendary, 0.6)
      }
  ...
  private syncShop(): void {
    ...
    const stock: { key: keyof Game['shopRows']; put: () => void }[] = []
    ...
    for (const one of stock) {
      if (rerolled && one.key === 'items') this.beginReveal(inner, this.clock + 0.1, true)
      one.put()
  ...
  private gateShopFoot(): void {
    ...
    const ready = this.clock >= foot.readyAt && !this.focus
  ...
  private get packHoldsCards(): boolean {
    const open = this.state.pack
   ...
   */
  private get shopParked(): boolean {
    const packBusy = (this.state.pack !== null || this.packPending)
      && this.clock >= this.shopStayUntil
    return packBusy || this.clock < this.shopHoldUntil
  }
   ...
   */
  private get handReady(): boolean {
    const dealt = this.deals.length === 0 && this.clock >= this.dealtUntil
   ...
   */
  private advanceHandControls(seconds: number): void {
    const usable = this.handReady
    this.sortSlide.target = usable ? 0 : SORT_HIDE
    this.sortSlide.advance(seconds)
    const off = this.sortSlide.value
    const sortY = BUTTON_Y + (PLAY_H - SORT_H) / 2 + off
    this.sortRankButton.y = sortY
    this.sortSuitButton.y = sortY
    this.sortRankButton.enabled = usable && this.shown.hand.length > 1
    this.sortSuitButton.enabled = usable && this.shown.hand.length > 1
    this.playButton.y = BUTTON_Y + off
    this.clearButton.y = BUTTON_Y + off
    this.discardButton.y = BUTTON_Y + off
    this.hint.y = BUTTON_Y - 30 + off
  }

  /** 상점을 이만큼 더 내려가 있게 합니다. 산 것이 닿는 것을 보는 동안입니다. */
  private holdShop(seconds: number): void {
    this.shopHoldUntil = Math.max(this.shopHoldUntil, this.clock + seconds)
  }
  ...
  private buyFrom(slot: number, item: ShopItem): void {
    ...
    if (!this.roomFor(item.kind)) {
      this.enterFocus(item, held => {
        // 내놓는 것이 먼저이고, 그다음은 그냥 사는 것입니다.
        this.giveUp(item, held)
   ...
   */
  private lingerTile(slot: number, hold = BUY_LINGER): void {
    const one = this.shopTiles.get(slot)
   ...
   */
  private lingerNode(tile: Container, hold: number): void {
    ...
    tile.zIndex = 1
    this.overlay.addChild(tile)
    this.leavingTiles.push({ node: tile, at: this.clock + hold })
  }
   ...
   * 조커가 줄 위에서 떨어졌습니다.
   */
  private holdArrival(item: ShopItem, from: { x: number; y: number },
    ...
    this.arriveHold = { kind, until: this.clock + wait + LAND_AT }
    this.later.push({
      at: this.clock + wait,
      run: () => {
        this.arriveHold = undefined
  ...
  private landed(item: ShopItem): void {
    ...
    this.popAt({ x: spot.x, y: spot.y - RISER_ON_CARD },
      shopLabel(item.kind, item.id, this.data), UI.money, 0.5)
    // **칸 수도 알립니다.** 자리가 몇 남았는지는 물건이 닿는 순간에 눈이 가지 않습니다.
    this.pulseCount(joker ? this.jokerCount : this.consumableCount)
  }
   ...
   */
  private pulseCount(node: Text): void {
    if (this.countPulse && this.countPulse.node !== node) this.endCountPulse()
    if (!this.countPulse) {
      this.countPulse = { node, base: node.style.fill as number, until: this.clock + COUNT_PULSE }
    } else this.countPulse.until = this.clock + COUNT_PULSE
  ...
  private advanceCountPulse(seconds: number): void {
    const one = this.countPulse
    if (!one) return
    // 마지막 0.3초에 제 크기로 돌아옵니다. 색은 그때까지 값의 색입니다.
    if (this.clock > one.until - 0.3) this.countScale.target = 1
    this.countScale.advance(seconds)
    one.node.scale.set(this.countScale.value)
    if (this.clock >= one.until && this.countScale.settled) this.endCountPulse()
  }

  /** 「적용 중」의 밝은 줄이 숨 쉬듯 오르내립니다. 때가 지나면 다음 그리기에 걷힙니다. */
  private advanceActiveGlow(): void {
    const one = this.activeGlow
    if (!one) return
    if (this.clock >= one.until) {
      this.activeGlow = undefined
      this.syncActive()
      return
    }
    if (one.plate && !one.plate.destroyed) {
      one.plate.alpha = 0.55 + 0.45 * Math.sin(this.clock * 4.2)
    }
  }
   ...
   */
  private giveUp(item: ShopItem, held: number): void {
    this.audio.play('joker_sell')
    this.sellFrom = item.kind === ShopItemKind.Joker ? this.jokerSpot(held) : this.itemSpot(held)
  }
   ...
   */
  private enterFocus(item: ShopItem, commit: (held: number) => void, slot?: number): void {
    ...
    this.spotNodes.set('focus:cancel', { node: cancel, cx: 50, cy: 17 })
  ...
  private leaveFocus(): void {
    ...
    this.focus = undefined
    this.held = undefined
    this.spotNodes.delete('focus:cancel')
    this.refresh()
  }
  ...
  private flyToDeck(node: Container, from: { x: number; y: number },
    ...
    this.deckPeekUntil = this.clock + DECK_PEEK
    this.deckFlight = { node, motion, at: this.clock + LAND_AT }
  }

  /** 덱으로 날아가는 카드를 한 단계 옮기고, 닿으면 덱이 받습니다. */
  private advanceDeckFlight(seconds: number): void {
    const one = this.deckFlight
    if (!one) return
    one.motion.advance(seconds)
    one.node.position.set(one.motion.x.value, one.motion.y.value)
    one.node.scale.set(one.motion.scale.value)
    one.node.rotation = one.motion.rotation.value * (Math.PI / 180)
    if (this.clock < one.at) return

    // 닿았습니다. **덱이 한 번 눌리고 그 자리에 이름이 뜹니다.** 카드는 덱 더미에 들어간
    // 것이므로 더 그리지 않습니다.
    one.node.destroy({ children: true })
    this.deckFlight = undefined
    this.deckBump.kick(240)
    this.audio.play('card_place')
    this.particles.burst(DECK_X, DECK_Y, 14, COLOR.cardEdge, 0.9, 0.7)
    this.popAt({ x: DECK_X - 36, y: DECK_Y - RISER_ON_CARD }, t('ui.deck.added'), UI.good, 0.5)
  }
  ...
  private buyVoucher(): void {
    ...
    this.popAt(at, one.title, UI.money, 0.5)
    this.boughtFrom = at
    // 딱지가 남아 값과 이름이 그 위에 뜹니다. 다 보고 난 뒤에 빈 칸이 됩니다.
    this.voucherTile = undefined
    this.lingerNode(one.tile, PAY_BEAT)
  ...
  private openPackSlot(slot: number): void {
    ...
    if (spot) {
      ...
      this.lingerNode(spot.tile, PAY_BEAT)
    }
    this.shopStayUntil = this.clock + PAY_BEAT
  ...
  private shopVoucherCell(cx: number, cy: number, fit: number): () => void {
    ...
    // 값 칩도 카드 칸과 같이 붙습니다.
    this.tipOn(tile, at => {
      this.tooltip.show(title, t('ui.kind.voucher'), 0, lines, at, SIZE, cost)
    })
    this.reveal(tile)
    return () => {
  ...
  private buildPack(): void {
    ...
    this.spotNodes.set('packSkip', { node: skip, cx: PLAY_W / 2, cy: PLAY_H / 2 })
    ...
    open.options.forEach((item, index) => {
      ...
      this.later.push({
        ...
        run: () => {
          ...
          if (one) this.particles.burst(one.motion.x.target, one.motion.y.target, 8, ink, 0.7, 0.6)
        },
      })
    })
  }
  ...
  private packCard(item: ShopItem, index: number): PackFace {
    ...
    // 중인지가 한 가지로 읽힙니다.
    attachTip(node, this.hold, tip, () => this.tooltip.hide())
  ...
  private takeFromPack(index: number): void {
    ...
    if (!this.roomFor(item.kind)) {
      if (!this.canSwap(item)) {
        ...
        this.toasts.push(t('ui.swap.title'), this.fullNote(item.kind), UI.bad, 3)
        return
      }
      ...
      this.enterFocus(item, held => {
        ...
        this.pickFromPack(index, item, node, { t: 'swap_pack', index, held }, BUY_LINGER)
      })
      return
    }
    this.pickFromPack(index, item, node, { t: 'pick_pack', index }, 0)
  }
   ...
   */
  private pickFromPack(index: number, item: ShopItem, node: Container, action: Action,
                       wait: number): void {
  ...
  private advancePack(seconds: number): void {
    ...
    if (this.packPending && open && this.shopAway) this.syncPack()
    ...
    if (!open && this.packEnter < 0.03) {
      ...
      this.packViews.clear()
      this.packGone.length = 0
      this.spotNodes.delete('packSkip')
      this.packNote = undefined
      this.packSkip = undefined
      this.packTitle = undefined
    ...
    const shown = this.packEnter * (1 - 0.55 * this.focusEnter)
    if (this.packNote) this.packNote.alpha = shown
    if (this.packSkip) {
      this.packSkip.alpha = shown
      this.packSkip.enabled = !this.focus
    }
    ...
    for (const one of this.packViews.values()) {
      ...
      node.eventMode = this.focus ? 'none' : 'static'
      ...
      else if (node.alpha > 0) node.alpha = 1 - 0.6 * this.focusEnter
    }
