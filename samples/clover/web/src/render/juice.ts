...
export class TimelinePlayer {
  ...
  /**
   * 지금 연출이 흐르는 빠르기. **옵션의 배속에 빠르게 넘기기가 얹힌 값입니다.**
   *
   * **박자가 아닌 것도 이 값을 씁니다.** 히트스톱과 왼쪽 판의 숫자와 떠오르는 글은
   * 실제 시계로 돌고 있었는데, 박자만 배속을 타므로 배속을 올리면 그 셋이 박자보다
   * 느려졌습니다 — 4배에서 곱하기 한 번의 히트스톱 0.12초가 박자 한 개분(0.48초)을
   * 통째로 먹었고, 두 상자는 굴러가는 시정수 0.13초로 0.12초 간격의 값을 받아
   * 끝까지 따라잡지 못했습니다.
   */
  get rate(): number {
    return this.speed
