...
uniform vec2  uSurge;    // 두 칸의 세기. x 가 칩, y 가 배수입니다
...
void main(void) {
  ...
  float w = sin(u * TAU * 1.0 + uPhase * 1.00)
          + sin(u * TAU * 2.3 - uPhase * 0.68 + 1.7) * 0.647
          + sin(u * TAU * 4.7 + uPhase * 1.42 + 4.1) * 0.324;
  w /= 1.971;
  ...
  float mask = clamp(0.5 - sd / max(1e-4, fwidth(sd)), 0.0, 1.0);
...
export class ScoreWave {
   ...
   */
  get surge(): { chips: number; mult: number; level: number; phase: number } {
