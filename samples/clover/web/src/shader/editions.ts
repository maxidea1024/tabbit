...
void main(void) {
  ...
  vec3 sheet = src.rgb * 0.55 + tint * 0.8;
  finalColor = vec4(mix(src.rgb, sheet, amount) + grain * src.a, src.a);
}
`
...
void main(void) {
  ...
  vec3 tint = rainbow(t);
  float mask = smoothstep(0.5, 0.95, luma) * clamp(uStrength, 0.0, 1.0) * 0.62;
  finalColor = vec4(mix(src.rgb, tint, clamp(mask, 0.0, 1.0) * src.a), src.a);
}
`
...
void main(void) {
  ...
  finalColor = vec4(mix(src.rgb, flipped, clamp(uStrength, 0.0, 0.85)) + glow * src.a, src.a);
}
`
