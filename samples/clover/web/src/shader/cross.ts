...
// 값만 거꾸로 넣습니다.
//
// **재는 여기 없습니다.** 다섯은 화면을 처리하는 식 하나씩인데 재는 바람을 거슬러 오르는
// 얼개이고 셰이더가 둘입니다 — `shader/ash.ts` 입니다.
//
...
/** 부드러운 결의 노이즈 그림. 태우는 문턱이 이것입니다. */
uniform sampler2D uSoft;
/** 모래알의 노이즈 그림. 타는 가장자리의 톱니입니다. */
...
/**
 * 타는 문턱. **그림 둘을 읽습니다** — 해시 열두 번이던 자리입니다.
 *
 * 큰 결이 구멍의 자리를 정하고 잔 결이 그 가장자리를 톱니로 만듭니다. 종이가 탄 가장자리는
 * 톱니입니다. 큰 결은 값이 0.17~0.61 에 몰려 있으므로 펴서 씁니다.
 */
float layers(vec2 here) {
  float big = (texture(uSoft, here * 0.9 + 0.2).r - 0.38) * 3.4 + 0.5;
  float fine = texture(uGrain, here * 1.4 + 0.5).r;
  return clamp(big * 0.78 + fine * 0.22, 0.0, 1.0);
}
...
void main(void) {
  ...
  if (uKind < 0.5) {
    ...
    // 탐. 아래에서 위로 번집니다.
    color = plain(grab(uv));
    float grain = layers(vec2(uv.x * uAspect, uv.y));
    float rise = 1.0 - uv.y;
