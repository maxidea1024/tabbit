...
void main(void) {
  ...
  // 흐르는 빠르기.
  //
  // **조용할 때도 흐릅니다.** 0.05 는 한 화면을 보고 있는 동안 무늬가 거의 제자리라
  // 배경이 그림 한 장으로 보였습니다 — 판이 도는 내내 깔려 있는 것이므로, 아무 일이
  // 없을 때 살아 있는 쪽이 맞습니다.
  //
  // **열기가 올려 주는 몫은 그 절반입니다.** 0.09 이던 동안 점수가 목표에 닿을 즈음의
  // 배경은 조용할 때의 세 배로 흘렀고, 그 구간에서 배경이 딴 화면이 되었습니다.
  float t = uTime * (0.085 + uHeat * 0.045);
  ...
  // **밝기도 같습니다** — 조용할 때를 올리고 열기의 몫을 절반으로 줄입니다. 무늬가 보이는
  // 것과 무늬가 요동치는 것은 다른 일이고, 늘 보이는 쪽이 배경이 하는 일입니다.
  vec3 color = uInk;
  color += uGlow * (0.22 + 0.25 * uHeat) * bands;
  color += uGlow * 0.08 * f;
  ...
  if (uPulse > 0.002) {
    ...
    // **세기는 절반입니다.** 한 방이 올 때마다 배경이 통째로 흰빛에 가까워져서, 그 순간
    // 판 위의 카드와 값이 함께 씻겼습니다 — 알릴 것은 이 한 겹 말고도 화면 번쩍임과
    // 터지는 것과 소리가 함께 하고 있습니다.
    color += uGlow * (uPulse * (0.22 + 0.55 * bands) + ring * uPulse * 0.75);
  }
