...
export interface Vm {
  ...
  /**
   * 이 효과가 「누가 했는가」를 이미 냈는가. `report` 가 세웁니다.
   *
   * **값을 바꾸는 연산만 `report` 를 부릅니다.** 카드를 만들고 부수고 규칙을 거는 것은
   * 부르지 않으므로, 그것들에도 「누가 했는가」를 붙이려면 이미 붙었는지를 알아야 합니다 —
   * 붙었는지 보지 않고 붙이면 값을 낸 조커가 두 번 발동한 것으로 보입니다.
   */
  reported?: boolean

  /**
   * 이 효과가 상태를 바꾸었는가. **이벤트를 내지 않는 연산이 세웁니다.**
   *
   * 무엇이 바뀌었는지는 연산이 낸 이벤트로 알 수 있지만, 이벤트를 하나도 내지 않는 연산이
   * 있습니다 — 보스가 카드를 무력화하고 조커를 섞고 하나를 끄는 것들입니다. 그것들이
   * 「나는 무언가를 했다」를 남기는 자리입니다.
   *
   * **대상이 없어 아무 일도 하지 않은 것과 가릅니다.** 부술 조커가 없는데 부수는 효과가
   * 돌면 상태는 그대로이고, 그때 발동한 것으로 알리면 화면에 이유 없는 몸짓이 하나 납니다.
   */
  changed?: boolean
