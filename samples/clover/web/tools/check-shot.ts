...
// 읽으므로, 데스크탑에서 멀쩡한 것이 그쪽에서 뚫릴 수 있습니다 — 실제로 그랬습니다. 그
// 창은 픽셀 밀도가 2이고 비율이 2.14 이므로, **판을 옮기며 굽는 길도 그쪽이 지납니다.**
//
...
async function measure(browser: Browser, lite: boolean, url: string): Promise<void> {
  ...
  await crossed(page)
  await openRun(page)
  // **판이 걸린 것을 세웁니다.** 맨 딱지는 셰이더가 걸리지 않아 필터의 사각형을 재는
  // 길을 지나지 않습니다 — 여기 두 창이 둘 다 기준 비율이 아니므로, 에디션을 걸어야 그
  // 자리가 이 게이트에 들어옵니다.
  await grantJoker(page, 3, 1)
