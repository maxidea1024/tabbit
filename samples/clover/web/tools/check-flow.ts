...
//
// 둘을 확인합니다. **덱이 카드를 받자마자 빠지지 않는가**와 **상점의 빈자리가 산 물건이 사라진
...
interface Shot {
  ...
  deckX: number
  leaving: number
  /** 그중 아직 온전히 남아 있는 것. 사라지기 시작한 딱지는 빠집니다. */
  lingering: number
  tiles: [number, number, number][]
  /** 조커 줄과 소모품 칸에 실제로 놓여 있는 것의 수. 상태의 수가 아닙니다. */
  drawn: number
}
...
async function main(): Promise<number> {
  ...
  if (before.tiles.length < 2) {
    ...
    for (let frame = 0; frame < 60 * 4; frame++) {
      const now = await shot(page)
      // **산 물건이 두 곳에 보이면 안 됩니다.** 딱지가 온전히 그 자리에 놓여 있는 동안 제
