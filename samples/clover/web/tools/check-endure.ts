...
import {
  chooseFive, clickPrimary, closeGuide, discardHand, pass, peek, playHand, rate,
  settle, shopSlot, skipLogin, spare, startNewRun, takePayout
} from './harness'
  ...
  for (let turn = 0; turn < rounds; turn++) {
    ...
    if (state.phase === 'round') {
      ...
      if (state.phase === 'shop' && state.payout) {
        // 정산 판이 아직 떠 있습니다. 「받는다」 를 눌러야 상점이 뜹니다.
      ...
      //
      // **상점 판이 아직 뜨지 않았으면 그 차례는 넘깁니다.** 국면이 상점이 되는 것과 판이
      // 뜨는 것은 다른 순간이고, 뜨지 않은 판의 단추를 누를 수는 없습니다.
