...
ECONOMY = [
    ...
    ('LegendaryBaseCost', 'int', 10, '전설 조커는 값이 없으므로 판매가의 기준이 이것입니다'),
    ('TarotCost', 'int', 3, '상점의 타로 값'),
    ('PlanetCost', 'int', 3, '상점의 행성 값'),
    ('SpectralCost', 'int', 4, '상점의 유령 값'),
    ('PlayingCardCost', 'int', 1, '상점의 플레잉 카드 값'),
...
FEEL = [
    ...
    ('DrawStaggerMs', 'int', 40, '카드를 뽑을 때 장마다의 간격'),
]
...
ACHIEVEMENT = [
    ...
    # **`collector` 는 기본 150종입니다.** 500종으로 올리지 않습니다 — 한 런에서 만나는
    # 조커가 20종 남짓이므로 500종은 도달하지 못하는 조건이 되고, 도달하지 못하는
    # 도전과제는 목록에서 빠진 것과 같습니다. 확장은 계열 하나를 채우는 것으로 봅니다.
    ('collector', '수집가', '기본 조커 150종을 모두 발견합니다'),
    ('greenhouse', '온실', '확장 조커의 한 계열 25종을 모두 발견합니다'),
]
