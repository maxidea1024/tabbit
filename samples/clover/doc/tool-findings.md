...
**결함이었고, 코어에서 고쳤습니다(2026-09-12).** 자작 조커 350종을 걷고 조커 격자를 다시
만들다 났습니다. 아래의 우회는 그대로 남아 있고, 고친 코어에서는 필요하지 않습니다.
...
**닿는 자리는 쿠킹입니다.** 공유 변종 타입(`OpGrant`)의 멤버 목록이 선언이 아니라 **그
추상 타입을 처음 만난 표의 열**에서 만들어졌고, 표의 평면 레코드(`OperationEntry`)는 그 표
자신의 열로만 만들어집니다. 변종을 풀어 주는 코드는 공유 타입의 멤버 전부를
`_operation.Sticker` 로 읽으므로, 두 목록이 어긋난 표에서 없는 필드를 읽습니다. 도구 자신이
「데이터가 아니라 tabbit 의 결함」이라고 적습니다.

**순서가 반대였다면 컴파일은 되고 값이 사라졌습니다.** 열이 없는 표가 먼저 오면 `OpGrant`
타입 자체가 `Sticker` 없이 생성되고, `DeckEffect` 가 적은 스티커는 읽히기만 하고 변종
객체에 실리지 않습니다. TypeScript 를 포함한 모든 정적 타입 언어가 같은 자리를 지납니다 —
변환이 C# 검증 접근자에서 먼저 멈추어 그쪽까지 가지 않았을 뿐입니다.

**고친 방법은 열의 합성입니다.** 선언된 변종 멤버에 열이 없으면 쿠킹이 그 열을 그룹 끝에
비어 있는 채로 더합니다 — 아래 우회가 손으로 한 것과 같은 열입니다. 그래서 같은 추상 타입을
쓰는 표 10개가 어느 시트가 어떻게 적었든 같은 멤버 집합을 갖습니다. 규칙은
[스펙 §5.2](../../../spec/types/polymorphism/notation.md#52-시트--컬럼의-합집합)에 있고,
픽스처의 `Boon`·`Curse` 표가 두 순서를 모두 확인합니다.
...
**우회가 지워지면 이 항목이 닫힙니다.** 다음 격자 재생성에서 `OP_FIELDS` 의 `sticker` 를
걷으면 됩니다 — 고친 코어가 같은 열을 만들어 내므로 산출물은 바뀌지 않습니다.
