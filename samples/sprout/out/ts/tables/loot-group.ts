...
export class LootGroupRecord {
  ...
  /** draws per open */
  public get draws(): number { return this._draws }

  /** duplicates allowed */
  public get allowDuplicate(): boolean { return this._allowDuplicate }

  public _id: number = 0
  public _nameKey: string = ''
  public _draws: number = 0
  public _allowDuplicate: boolean = false

  /** Populate field values. */
  public populateFieldValues(dataRow: IDataRow): void {
    this._id = dataRow.id
    this._nameKey = dataRow.nameKey
    this._draws = dataRow.draws
    this._allowDuplicate = dataRow.allowDuplicate
  }
  ...
  public populateFieldValuesCompact(dataRow: any[]): void {
    ...
    this._draws = dataRow[offset++]
    this._allowDuplicate = dataRow[offset++]
  }
}

// Generated from samples/sprout/xlsx/Reward.xlsx : LootGroupTable : A1
export class LootGroupTable {
  /** Default constructor. */
  constructor() {
  }

  /** All records. */
  public get records(): LootGroupRecord[] { return this._records }
  private _records: LootGroupRecord[] = []
   ...
   */
  public [Symbol.iterator](): IterableIterator<LootGroupRecord> {
    return this._records[Symbol.iterator]()
  }

  // Indexing by 'id'
  public get recordsById(): Map<number, LootGroupRecord> { return this._recordsById }
  private _recordsById: Map<number, LootGroupRecord> = new Map<number, LootGroupRecord>()
   ...
   */
  public findById(key: number): LootGroupRecord | undefined {
   ...
   */
  public getByIdOrThrow(key: number): LootGroupRecord {
    const found = this._recordsById.get(key)
    if (!found)
      throw new Error(`There is no record in table "LootGroup" that corresponds to field "id" value ${key}`)
   ...
   */
  public *entries(): IterableIterator<[number, LootGroupRecord]> {
  ...
  public readJsonFrom(json: string): void {
    ...
    const records: LootGroupRecord[] = []

    if (this.isCompactRowFormatted(dataRows)) {
      for (const dataRow of dataRows) {
        const record = new LootGroupRecord()
  ...
  public readBinaryFrom(fileBytes: Uint8Array): void {
    ...
    for (let i = 0; i < rowCount; ++i)
      records.push(new LootGroupRecord())
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          tabbit.checkColumn(column, 'LootGroup.Id', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'LootGroup.Id')
          ...
          break
        case 2:
          tabbit.checkColumn(column, 'LootGroup.NameKey', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_STRING])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'LootGroup.NameKey')
          for (let i = 0; i < rowCount; ) {
          ...
          break
        case 3:
          tabbit.checkColumn(column, 'LootGroup.Draws', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'LootGroup.Draws')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
              records[i]._draws = value
          }
          break
        case 4:
          tabbit.checkColumn(column, 'LootGroup.AllowDuplicate', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_BOOL])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'LootGroup.AllowDuplicate')
          for (let i = 0; i < rowCount; ++i) {
   ...
   */
  private publish(records: LootGroupRecord[]): void {
    const recordsById = new Map<number, LootGroupRecord>()
