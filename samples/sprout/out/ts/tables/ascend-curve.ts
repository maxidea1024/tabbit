...
export class AscendCurveTable {
  ...
  /** All records. */
  public get records(): AscendCurveRecord[] { return this._records }
  private _records: AscendCurveRecord[] = []
   ...
   */
  public [Symbol.iterator](): IterableIterator<AscendCurveRecord> {
    return this._records[Symbol.iterator]()
  }

  // Indexing by 'id'
  public get recordsById(): Map<number, AscendCurveRecord> { return this._recordsById }
  private _recordsById: Map<number, AscendCurveRecord> = new Map<number, AscendCurveRecord>()
   ...
   */
  public findById(key: number): AscendCurveRecord | undefined {
   ...
   */
  public getByIdOrThrow(key: number): AscendCurveRecord {
    const found = this._recordsById.get(key)
    if (!found)
      throw new Error(`There is no record in table "AscendCurve" that corresponds to field "id" value ${key}`)
   ...
   */
  public *entries(): IterableIterator<[number, AscendCurveRecord]> {
  ...
  private readFromJson(json: string): void {
    ...
    const records: AscendCurveRecord[] = []

    if (this.isCompactRowFormatted(dataRows)) {
      for (const dataRow of dataRows) {
        const record = new AscendCurveRecord()
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (let i = 0; i < rowCount; ++i)
      records.push(new AscendCurveRecord())
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          tabbit.checkColumn(column, 'AscendCurve.Id', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'AscendCurve.Id')
          ...
          break
        case 2:
          tabbit.checkColumn(column, 'AscendCurve.Grade', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'AscendCurve.Grade')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
          ...
          break
        case 3:
          tabbit.checkColumn(column, 'AscendCurve.Step', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'AscendCurve.Step')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
          ...
          break
        case 4:
          tabbit.checkColumn(column, 'AscendCurve.Scale', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'AscendCurve.Scale')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
              records[i]._scale = value
          }
          break
        default:
   ...
   */
  private publish(records: AscendCurveRecord[]): void {
    const recordsById = new Map<number, AscendCurveRecord>()
