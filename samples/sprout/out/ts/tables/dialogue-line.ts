...
import { DialogueSpeaker } from '../enums/dialogue-speaker'
...
export class DialogueLineRecord {
  ...
  /** owning dialogue */
  public get dialogueId(): number { return this._dialogueId }

  /** line order */
  public get lineOrder(): number { return this._lineOrder }

  /** speaker */
  public get speaker(): DialogueSpeaker { return this._speaker }

  /** portrait asset */
  public get portrait(): string { return this._portrait }

  /** line key */
  public get lineKey(): string { return this._lineKey }

  public _id: number = 0
  public _dialogueId: number = 0
  public _lineOrder: number = 0
  public _speaker: DialogueSpeaker = 0 as DialogueSpeaker
  public _portrait: string = ''
  public _lineKey: string = ''

  /** Populate field values. */
  public populateFieldValues(dataRow: IDataRow): void {
    this._id = dataRow.id
    this._dialogueId = dataRow.dialogueId
    this._lineOrder = dataRow.lineOrder
    this._speaker = dataRow.speaker
    this._portrait = dataRow.portrait
    this._lineKey = dataRow.lineKey
  }
  ...
  public populateFieldValuesCompact(dataRow: any[]): void {
    ...
    this._dialogueId = dataRow[offset++]
    this._lineOrder = dataRow[offset++]
    this._speaker = dataRow[offset++]
    this._portrait = dataRow[offset++]
    this._lineKey = dataRow[offset++]
  }
}
...
export class DialogueLineTable {
  ...
  /** All records. */
  public get records(): DialogueLineRecord[] { return this._records }
  private _records: DialogueLineRecord[] = []
   ...
   */
  public [Symbol.iterator](): IterableIterator<DialogueLineRecord> {
    return this._records[Symbol.iterator]()
  }

  // Indexing by 'id'
  public get recordsById(): Map<number, DialogueLineRecord> { return this._recordsById }
  private _recordsById: Map<number, DialogueLineRecord> = new Map<number, DialogueLineRecord>()
   ...
   */
  public findById(key: number): DialogueLineRecord | undefined {
   ...
   */
  public getByIdOrThrow(key: number): DialogueLineRecord {
    const found = this._recordsById.get(key)
    if (!found)
      throw new Error(`There is no record in table "DialogueLine" that corresponds to field "id" value ${key}`)
   ...
   */
  public *entries(): IterableIterator<[number, DialogueLineRecord]> {
  ...
  private readFromJson(json: string): void {
    ...
    const records: DialogueLineRecord[] = []

    if (this.isCompactRowFormatted(dataRows)) {
      for (const dataRow of dataRows) {
        const record = new DialogueLineRecord()
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (let i = 0; i < rowCount; ++i)
      records.push(new DialogueLineRecord())
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          tabbit.checkColumn(column, 'DialogueLine.Id', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'DialogueLine.Id')
          ...
          break
        case 2:
          tabbit.checkColumn(column, 'DialogueLine.DialogueId', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'DialogueLine.DialogueId')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
              records[i]._dialogueId = value
          }
          break
        case 3:
          tabbit.checkColumn(column, 'DialogueLine.LineOrder', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'DialogueLine.LineOrder')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
              records[i]._lineOrder = value
          }
          break
        case 4:
          tabbit.checkColumn(column, 'DialogueLine.Speaker', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'DialogueLine.Speaker')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
              records[i]._speaker = value as DialogueSpeaker
          }
          break
        case 5:
          tabbit.checkColumn(column, 'DialogueLine.Portrait', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_STRING])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'DialogueLine.Portrait')
          for (let i = 0; i < rowCount; ) {
          ...
          break
        case 6:
          tabbit.checkColumn(column, 'DialogueLine.LineKey', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_STRING])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'DialogueLine.LineKey')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameString(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
              records[i]._lineKey = value
          }
          break
        default:
   ...
   */
  private publish(records: DialogueLineRecord[]): void {
    const recordsById = new Map<number, DialogueLineRecord>()
