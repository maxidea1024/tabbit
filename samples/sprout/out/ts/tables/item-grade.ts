...
export class ItemGradeRecord {
  ...
  public get sellScale(): number { return this._sellScale }

  public _id: number = 0
  public _grade: GradeType = 0 as GradeType
  public _frame: string = ''
  public _sellScale: number = 0
  ...
  public populateFieldValues(dataRow: IDataRow): void {
    ...
    this._sellScale = dataRow.sellScale
  }
  ...
  public populateFieldValuesCompact(dataRow: any[]): void {
    ...
    this._sellScale = dataRow[offset++]
  }
}
...
export class ItemGradeTable {
  ...
  /** All records. */
  public get records(): ItemGradeRecord[] { return this._records }
  private _records: ItemGradeRecord[] = []
   ...
   */
  public [Symbol.iterator](): IterableIterator<ItemGradeRecord> {
    return this._records[Symbol.iterator]()
  }

  // Indexing by 'id'
  public get recordsById(): Map<number, ItemGradeRecord> { return this._recordsById }
  private _recordsById: Map<number, ItemGradeRecord> = new Map<number, ItemGradeRecord>()
   ...
   */
  public findById(key: number): ItemGradeRecord | undefined {
   ...
   */
  public getByIdOrThrow(key: number): ItemGradeRecord {
    const found = this._recordsById.get(key)
    if (!found)
      throw new Error(`There is no record in table "ItemGrade" that corresponds to field "id" value ${key}`)
   ...
   */
  public *entries(): IterableIterator<[number, ItemGradeRecord]> {
  ...
  public readJsonFrom(json: string): void {
    ...
    const records: ItemGradeRecord[] = []

    if (this.isCompactRowFormatted(dataRows)) {
      for (const dataRow of dataRows) {
        const record = new ItemGradeRecord()
  ...
  public readBinaryFrom(fileBytes: Uint8Array): void {
    ...
    for (let i = 0; i < rowCount; ++i)
      records.push(new ItemGradeRecord())
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          tabbit.checkColumn(column, 'ItemGrade.Id', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'ItemGrade.Id')
          ...
          break
        case 2:
          tabbit.checkColumn(column, 'ItemGrade.Grade', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'ItemGrade.Grade')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
          ...
          break
        case 3:
          tabbit.checkColumn(column, 'ItemGrade.Frame', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_STRING])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'ItemGrade.Frame')
          for (let i = 0; i < rowCount; ) {
          ...
          break
        case 4:
          tabbit.checkColumn(column, 'ItemGrade.SellScale', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'ItemGrade.SellScale')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
              records[i]._sellScale = value
          }
          break
        default:
   ...
   */
  private publish(records: ItemGradeRecord[]): void {
    const recordsById = new Map<number, ItemGradeRecord>()
