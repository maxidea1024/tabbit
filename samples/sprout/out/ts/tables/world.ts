...
export class WorldRecord {
  ...
  /** map asset */
  public get map(): string { return this._map }

  /** theme colour */
  public get themeColour(): string { return this._themeColour }

  public _id: number = 0
  public _nameKey: string = ''
  public _map: string = ''
  public _themeColour: string = ''

  /** Populate field values. */
  public populateFieldValues(dataRow: IDataRow): void {
    this._id = dataRow.id
    this._nameKey = dataRow.nameKey
    this._map = dataRow.map
    this._themeColour = dataRow.themeColour
  }
  ...
  public populateFieldValuesCompact(dataRow: any[]): void {
    ...
    this._map = dataRow[offset++]
    this._themeColour = dataRow[offset++]
  }
}
...
export class WorldTable {
  ...
  /** All records. */
  public get records(): WorldRecord[] { return this._records }
  private _records: WorldRecord[] = []
   ...
   */
  public [Symbol.iterator](): IterableIterator<WorldRecord> {
    return this._records[Symbol.iterator]()
  }

  // Indexing by 'id'
  public get recordsById(): Map<number, WorldRecord> { return this._recordsById }
  private _recordsById: Map<number, WorldRecord> = new Map<number, WorldRecord>()
   ...
   */
  public findById(key: number): WorldRecord | undefined {
   ...
   */
  public getByIdOrThrow(key: number): WorldRecord {
    const found = this._recordsById.get(key)
    if (!found)
      throw new Error(`There is no record in table "World" that corresponds to field "id" value ${key}`)
   ...
   */
  public *entries(): IterableIterator<[number, WorldRecord]> {
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          tabbit.checkColumn(column, 'World.Id', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'World.Id')
          ...
          break
        case 2:
          tabbit.checkColumn(column, 'World.NameKey', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_STRING])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'World.NameKey')
          for (let i = 0; i < rowCount; ) {
          ...
          break
        case 3:
          tabbit.checkColumn(column, 'World.Map', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_STRING])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'World.Map')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameString(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
              records[i]._map = value
          }
          break
        case 4:
          tabbit.checkColumn(column, 'World.ThemeColour', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_STRING])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'World.ThemeColour')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameString(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
              records[i]._themeColour = value
          }
          break
        default:
   ...
   */
  private publish(records: WorldRecord[]): void {
    const recordsById = new Map<number, WorldRecord>()
