...
export class RoleBonusTable {
  ...
  /** All records. */
  public get records(): RoleBonusRecord[] { return this._records }
  private _records: RoleBonusRecord[] = []
   ...
   */
  public [Symbol.iterator](): IterableIterator<RoleBonusRecord> {
    return this._records[Symbol.iterator]()
  }

  // Indexing by 'id'
  public get recordsById(): Map<number, RoleBonusRecord> { return this._recordsById }
  private _recordsById: Map<number, RoleBonusRecord> = new Map<number, RoleBonusRecord>()
   ...
   */
  public findById(key: number): RoleBonusRecord | undefined {
   ...
   */
  public getByIdOrThrow(key: number): RoleBonusRecord {
    const found = this._recordsById.get(key)
    if (!found)
      throw new Error(`There is no record in table "RoleBonus" that corresponds to field "id" value ${key}`)
   ...
   */
  public *entries(): IterableIterator<[number, RoleBonusRecord]> {
  ...
  private readFromJson(json: string): void {
    ...
    const records: RoleBonusRecord[] = []

    if (this.isCompactRowFormatted(dataRows)) {
      for (const dataRow of dataRows) {
        const record = new RoleBonusRecord()
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (let i = 0; i < rowCount; ++i)
      records.push(new RoleBonusRecord())
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          tabbit.checkColumn(column, 'RoleBonus.Id', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'RoleBonus.Id')
          ...
          break
        case 2:
          tabbit.checkColumn(column, 'RoleBonus.Role', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'RoleBonus.Role')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
          ...
          break
        case 3:
          tabbit.checkColumn(column, 'RoleBonus.Stat', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'RoleBonus.Stat')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
          ...
          break
        case 4:
          tabbit.checkColumn(column, 'RoleBonus.Bonus', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'RoleBonus.Bonus')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
   ...
   */
  private publish(records: RoleBonusRecord[]): void {
    const recordsById = new Map<number, RoleBonusRecord>()
