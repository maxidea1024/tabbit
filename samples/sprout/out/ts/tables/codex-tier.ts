...
export class CodexTierRecord {
  ...
  /** entries required */
  public get required(): number { return this._required }
  ...
  public populateFieldValues(dataRow: IDataRow): void {
    ...
    this._required = dataRow.required
  ...
  public populateFieldValuesCompact(dataRow: any[]): void {
    ...
    this._required = dataRow[offset++]
    this._rewardId = dataRow[offset++]
  }
}

// Generated from samples/sprout/xlsx/Codex.xlsx : CodexTierTable : A1
export class CodexTierTable {
  /** Default constructor. */
  constructor() {
  }

  /** All records. */
  public get records(): CodexTierRecord[] { return this._records }
  private _records: CodexTierRecord[] = []
   ...
   */
  public [Symbol.iterator](): IterableIterator<CodexTierRecord> {
    return this._records[Symbol.iterator]()
  }

  // Indexing by 'id'
  public get recordsById(): Map<number, CodexTierRecord> { return this._recordsById }
  private _recordsById: Map<number, CodexTierRecord> = new Map<number, CodexTierRecord>()
   ...
   */
  public findById(key: number): CodexTierRecord | undefined {
   ...
   */
  public getByIdOrThrow(key: number): CodexTierRecord {
    const found = this._recordsById.get(key)
    if (!found)
      throw new Error(`There is no record in table "CodexTier" that corresponds to field "id" value ${key}`)
   ...
   */
  public *entries(): IterableIterator<[number, CodexTierRecord]> {
  ...
  public readJsonFrom(json: string): void {
    ...
    const records: CodexTierRecord[] = []

    if (this.isCompactRowFormatted(dataRows)) {
      for (const dataRow of dataRows) {
        const record = new CodexTierRecord()
  ...
  public readBinaryFrom(fileBytes: Uint8Array): void {
    ...
    for (let i = 0; i < rowCount; ++i)
      records.push(new CodexTierRecord())
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          tabbit.checkColumn(column, 'CodexTier.Id', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'CodexTier.Id')
          ...
          break
        case 2:
          tabbit.checkColumn(column, 'CodexTier.CodexId', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'CodexTier.CodexId')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
          ...
          break
        case 3:
          tabbit.checkColumn(column, 'CodexTier.Required', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'CodexTier.Required')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
              records[i]._required = value
          }
          break
        case 4:
          tabbit.checkColumn(column, 'CodexTier.RewardId', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'CodexTier.RewardId')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
   ...
   */
  private publish(records: CodexTierRecord[]): void {
    const recordsById = new Map<number, CodexTierRecord>()
