...
export class IdleRateRecord {
  ...
  public get capHours(): number { return this._capHours }

  public _id: number = 0
  public _stageId: number = 0
  public _rate: number = 0
  public _capHours: number = 0
  ...
  public populateFieldValues(dataRow: IDataRow): void {
    ...
    this._capHours = dataRow.capHours
  }
...
export class IdleRateTable {
  ...
  /** All records. */
  public get records(): IdleRateRecord[] { return this._records }
  private _records: IdleRateRecord[] = []
   ...
   */
  public [Symbol.iterator](): IterableIterator<IdleRateRecord> {
    return this._records[Symbol.iterator]()
  }

  // Indexing by 'id'
  public get recordsById(): Map<number, IdleRateRecord> { return this._recordsById }
  private _recordsById: Map<number, IdleRateRecord> = new Map<number, IdleRateRecord>()
   ...
   */
  public findById(key: number): IdleRateRecord | undefined {
   ...
   */
  public getByIdOrThrow(key: number): IdleRateRecord {
    const found = this._recordsById.get(key)
    if (!found)
      throw new Error(`There is no record in table "IdleRate" that corresponds to field "id" value ${key}`)
   ...
   */
  public *entries(): IterableIterator<[number, IdleRateRecord]> {
  ...
  private readFromJson(json: string): void {
    ...
    const records: IdleRateRecord[] = []

    if (this.isCompactRowFormatted(dataRows)) {
      for (const dataRow of dataRows) {
        const record = new IdleRateRecord()
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (let i = 0; i < rowCount; ++i)
      records.push(new IdleRateRecord())
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          tabbit.checkColumn(column, 'IdleRate.Id', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'IdleRate.Id')
          ...
          break
        case 2:
          tabbit.checkColumn(column, 'IdleRate.StageId', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'IdleRate.StageId')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
          ...
          break
        case 3:
          tabbit.checkColumn(column, 'IdleRate.Rate', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'IdleRate.Rate')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
          ...
          break
        case 4:
          tabbit.checkColumn(column, 'IdleRate.CapHours', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'IdleRate.CapHours')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
              records[i]._capHours = value
          }
          break
        default:
   ...
   */
  private publish(records: IdleRateRecord[]): void {
    const recordsById = new Map<number, IdleRateRecord>()
