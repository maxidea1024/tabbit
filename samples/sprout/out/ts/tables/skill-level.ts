...
export class SkillLevelTable {
  ...
  /** All records. */
  public get records(): SkillLevelRecord[] { return this._records }
  private _records: SkillLevelRecord[] = []
   ...
   */
  public [Symbol.iterator](): IterableIterator<SkillLevelRecord> {
    return this._records[Symbol.iterator]()
  }

  // Indexing by 'id'
  public get recordsById(): Map<number, SkillLevelRecord> { return this._recordsById }
  private _recordsById: Map<number, SkillLevelRecord> = new Map<number, SkillLevelRecord>()
   ...
   */
  public findById(key: number): SkillLevelRecord | undefined {
   ...
   */
  public getByIdOrThrow(key: number): SkillLevelRecord {
    const found = this._recordsById.get(key)
    if (!found)
      throw new Error(`There is no record in table "SkillLevel" that corresponds to field "id" value ${key}`)
   ...
   */
  public *entries(): IterableIterator<[number, SkillLevelRecord]> {
  ...
  private readFromJson(json: string): void {
    ...
    const records: SkillLevelRecord[] = []

    if (this.isCompactRowFormatted(dataRows)) {
      for (const dataRow of dataRows) {
        const record = new SkillLevelRecord()
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (let i = 0; i < rowCount; ++i)
      records.push(new SkillLevelRecord())
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          tabbit.checkColumn(column, 'SkillLevel.Id', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'SkillLevel.Id')
          ...
          break
        case 2:
          tabbit.checkColumn(column, 'SkillLevel.SkillId', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'SkillLevel.SkillId')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
              records[i]._skillId = value
          }
          break
        case 3:
          tabbit.checkColumn(column, 'SkillLevel.Level', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'SkillLevel.Level')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
          ...
          break
        case 4:
          tabbit.checkColumn(column, 'SkillLevel.Power', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'SkillLevel.Power')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
          ...
          break
        case 5:
          tabbit.checkColumn(column, 'SkillLevel.Cost', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I64, tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'SkillLevel.Cost')
          for (let i = 0; i < rowCount; ++i) {
   ...
   */
  private publish(records: SkillLevelRecord[]): void {
    const recordsById = new Map<number, SkillLevelRecord>()
