...
export class ConsumableTable {
  ...
  /** All records. */
  public get records(): ConsumableRecord[] { return this._records }
  private _records: ConsumableRecord[] = []
   ...
   */
  public [Symbol.iterator](): IterableIterator<ConsumableRecord> {
    return this._records[Symbol.iterator]()
  }

  // Indexing by 'id'
  public get recordsById(): Map<number, ConsumableRecord> { return this._recordsById }
  private _recordsById: Map<number, ConsumableRecord> = new Map<number, ConsumableRecord>()
   ...
   */
  public findById(key: number): ConsumableRecord | undefined {
   ...
   */
  public getByIdOrThrow(key: number): ConsumableRecord {
    const found = this._recordsById.get(key)
    if (!found)
      throw new Error(`There is no record in table "Consumable" that corresponds to field "id" value ${key}`)
   ...
   */
  public *entries(): IterableIterator<[number, ConsumableRecord]> {
  ...
  private readFromJson(json: string): void {
    ...
    if (this.isCompactRowFormatted(dataRows)) {
      for (const dataRow of dataRows) {
        const record = new ConsumableRecord()
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (let i = 0; i < rowCount; ++i)
      records.push(new ConsumableRecord())
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          tabbit.checkColumn(column, 'Consumable.Id', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Consumable.Id')
          ...
          break
        case 2:
          tabbit.checkColumn(column, 'Consumable.ItemId', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Consumable.ItemId')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
          ...
          break
        case 3:
          tabbit.checkColumn(column, 'Consumable.BuffId', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Consumable.BuffId')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
              records[i]._buffId = value
          }
          break
        case 4:
          tabbit.checkColumn(column, 'Consumable.Cooldown', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Consumable.Cooldown')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
   ...
   */
  private publish(records: ConsumableRecord[]): void {
    const recordsById = new Map<number, ConsumableRecord>()
