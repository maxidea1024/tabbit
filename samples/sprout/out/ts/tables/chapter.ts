...
export class ChapterRecord {
  ...
  /** owning world */
  public get worldId(): number { return this._worldId }
...
export class ChapterTable {
  ...
  /** All records. */
  public get records(): ChapterRecord[] { return this._records }
   ...
   */
  public [Symbol.iterator](): IterableIterator<ChapterRecord> {
    return this._records[Symbol.iterator]()
  }

  // Indexing by 'id'
  public get recordsById(): Map<number, ChapterRecord> { return this._recordsById }
  private _recordsById: Map<number, ChapterRecord> = new Map<number, ChapterRecord>()
   ...
   */
  public findById(key: number): ChapterRecord | undefined {
   ...
   */
  public getByIdOrThrow(key: number): ChapterRecord {
    const found = this._recordsById.get(key)
    if (!found)
      throw new Error(`There is no record in table "Chapter" that corresponds to field "id" value ${key}`)
   ...
   */
  public *entries(): IterableIterator<[number, ChapterRecord]> {
  ...
  public readBinaryFrom(fileBytes: Uint8Array): void {
    ...
    for (let i = 0; i < rowCount; ++i)
      records.push(new ChapterRecord())
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          tabbit.checkColumn(column, 'Chapter.Id', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Chapter.Id')
          ...
          break
        case 2:
          tabbit.checkColumn(column, 'Chapter.WorldId', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Chapter.WorldId')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
              records[i]._worldId = value
          }
          break
        case 3:
          tabbit.checkColumn(column, 'Chapter.NameKey', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_STRING])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Chapter.NameKey')
          for (let i = 0; i < rowCount; ) {
          ...
          break
        case 4:
          tabbit.checkColumn(column, 'Chapter.SortOrder', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Chapter.SortOrder')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
          ...
          break
        case 5:
          tabbit.checkColumn(column, 'Chapter.UnlockId', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Chapter.UnlockId')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
   ...
   */
  private publish(records: ChapterRecord[]): void {
    const recordsById = new Map<number, ChapterRecord>()
