...
export class GachaOddsTable {
  ...
  /** All records. */
  public get records(): GachaOddsRecord[] { return this._records }
  private _records: GachaOddsRecord[] = []
   ...
   */
  public [Symbol.iterator](): IterableIterator<GachaOddsRecord> {
    return this._records[Symbol.iterator]()
  }

  // Indexing by 'id'
  public get recordsById(): Map<number, GachaOddsRecord> { return this._recordsById }
  private _recordsById: Map<number, GachaOddsRecord> = new Map<number, GachaOddsRecord>()
   ...
   */
  public findById(key: number): GachaOddsRecord | undefined {
   ...
   */
  public getByIdOrThrow(key: number): GachaOddsRecord {
    const found = this._recordsById.get(key)
    if (!found)
      throw new Error(`There is no record in table "GachaOdds" that corresponds to field "id" value ${key}`)
   ...
   */
  public *entries(): IterableIterator<[number, GachaOddsRecord]> {
  ...
  private readFromJson(json: string): void {
    ...
    const records: GachaOddsRecord[] = []

    if (this.isCompactRowFormatted(dataRows)) {
      for (const dataRow of dataRows) {
        const record = new GachaOddsRecord()
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (let i = 0; i < rowCount; ++i)
      records.push(new GachaOddsRecord())
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          tabbit.checkColumn(column, 'GachaOdds.Id', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'GachaOdds.Id')
          ...
          break
        case 2:
          tabbit.checkColumn(column, 'GachaOdds.GachaId', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'GachaOdds.GachaId')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
          ...
          break
        case 3:
          tabbit.checkColumn(column, 'GachaOdds.Grade', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'GachaOdds.Grade')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
          ...
          break
        case 4:
          tabbit.checkColumn(column, 'GachaOdds.Rate', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'GachaOdds.Rate')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
   ...
   */
  private publish(records: GachaOddsRecord[]): void {
    const recordsById = new Map<number, GachaOddsRecord>()
