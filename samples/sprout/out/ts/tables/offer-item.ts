...
export class OfferItemTable {
  ...
  /** All records. */
  public get records(): OfferItemRecord[] { return this._records }
  private _records: OfferItemRecord[] = []
   ...
   */
  public [Symbol.iterator](): IterableIterator<OfferItemRecord> {
    return this._records[Symbol.iterator]()
  }

  // Indexing by 'id'
  public get recordsById(): Map<number, OfferItemRecord> { return this._recordsById }
  private _recordsById: Map<number, OfferItemRecord> = new Map<number, OfferItemRecord>()
   ...
   */
  public findById(key: number): OfferItemRecord | undefined {
   ...
   */
  public getByIdOrThrow(key: number): OfferItemRecord {
    const found = this._recordsById.get(key)
    if (!found)
      throw new Error(`There is no record in table "OfferItem" that corresponds to field "id" value ${key}`)
   ...
   */
  public *entries(): IterableIterator<[number, OfferItemRecord]> {
  ...
  public readJsonFrom(json: string): void {
    ...
    const records: OfferItemRecord[] = []

    if (this.isCompactRowFormatted(dataRows)) {
      for (const dataRow of dataRows) {
        const record = new OfferItemRecord()
  ...
  public readBinaryFrom(fileBytes: Uint8Array): void {
    ...
    for (let i = 0; i < rowCount; ++i)
      records.push(new OfferItemRecord())
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          tabbit.checkColumn(column, 'OfferItem.Id', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'OfferItem.Id')
          ...
          break
        case 2:
          tabbit.checkColumn(column, 'OfferItem.OfferId', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'OfferItem.OfferId')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
          ...
          break
        case 3:
          tabbit.checkColumn(column, 'OfferItem.ItemId', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'OfferItem.ItemId')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
          ...
          break
        case 4:
          tabbit.checkColumn(column, 'OfferItem.Count', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'OfferItem.Count')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
   ...
   */
  private publish(records: OfferItemRecord[]): void {
    const recordsById = new Map<number, OfferItemRecord>()
