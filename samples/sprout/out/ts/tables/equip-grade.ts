...
export class EquipGradeRecord {
  ...
  /** option slots */
  public get optionSlots(): number { return this._optionSlots }
  ...
  public _optionSlots: number = 0
  public _frame: string = ''

  /** Populate field values. */
  public populateFieldValues(dataRow: IDataRow): void {
    this._id = dataRow.id
    this._grade = dataRow.grade
    this._optionSlots = dataRow.optionSlots
    this._frame = dataRow.frame
  }
  ...
  public populateFieldValuesCompact(dataRow: any[]): void {
    ...
    this._optionSlots = dataRow[offset++]
    this._frame = dataRow[offset++]
  }
}
...
export class EquipGradeTable {
  ...
  /** All records. */
  public get records(): EquipGradeRecord[] { return this._records }
  private _records: EquipGradeRecord[] = []
   ...
   */
  public [Symbol.iterator](): IterableIterator<EquipGradeRecord> {
    return this._records[Symbol.iterator]()
  }

  // Indexing by 'id'
  public get recordsById(): Map<number, EquipGradeRecord> { return this._recordsById }
  private _recordsById: Map<number, EquipGradeRecord> = new Map<number, EquipGradeRecord>()
   ...
   */
  public findById(key: number): EquipGradeRecord | undefined {
   ...
   */
  public getByIdOrThrow(key: number): EquipGradeRecord {
    const found = this._recordsById.get(key)
    if (!found)
      throw new Error(`There is no record in table "EquipGrade" that corresponds to field "id" value ${key}`)
   ...
   */
  public *entries(): IterableIterator<[number, EquipGradeRecord]> {
  ...
  public readJsonFrom(json: string): void {
    ...
    const records: EquipGradeRecord[] = []

    if (this.isCompactRowFormatted(dataRows)) {
      for (const dataRow of dataRows) {
        const record = new EquipGradeRecord()
  ...
  public readBinaryFrom(fileBytes: Uint8Array): void {
    ...
    for (let i = 0; i < rowCount; ++i)
      records.push(new EquipGradeRecord())
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          tabbit.checkColumn(column, 'EquipGrade.Id', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'EquipGrade.Id')
          ...
          break
        case 2:
          tabbit.checkColumn(column, 'EquipGrade.Grade', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'EquipGrade.Grade')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
          ...
          break
        case 3:
          tabbit.checkColumn(column, 'EquipGrade.OptionSlots', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'EquipGrade.OptionSlots')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
              records[i]._optionSlots = value
          }
          break
        case 4:
          tabbit.checkColumn(column, 'EquipGrade.Frame', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_STRING])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'EquipGrade.Frame')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameString(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
              records[i]._frame = value
          }
          break
        default:
   ...
   */
  private publish(records: EquipGradeRecord[]): void {
    const recordsById = new Map<number, EquipGradeRecord>()
