...
export class DailyQuestRecord {
  ...
  public get resetHour(): number { return this._resetHour }

  public _id: number = 0
  public _questId: number = 0
  public _points: number = 0
  public _resetHour: number = 0
  ...
  public populateFieldValues(dataRow: IDataRow): void {
    ...
    this._resetHour = dataRow.resetHour
  }
  ...
  public populateFieldValuesCompact(dataRow: any[]): void {
    ...
    this._resetHour = dataRow[offset++]
  }
}
...
export class DailyQuestTable {
  ...
  /** All records. */
  public get records(): DailyQuestRecord[] { return this._records }
  private _records: DailyQuestRecord[] = []
   ...
   */
  public [Symbol.iterator](): IterableIterator<DailyQuestRecord> {
    return this._records[Symbol.iterator]()
  }

  // Indexing by 'id'
  public get recordsById(): Map<number, DailyQuestRecord> { return this._recordsById }
  private _recordsById: Map<number, DailyQuestRecord> = new Map<number, DailyQuestRecord>()
   ...
   */
  public findById(key: number): DailyQuestRecord | undefined {
   ...
   */
  public getByIdOrThrow(key: number): DailyQuestRecord {
    const found = this._recordsById.get(key)
    if (!found)
      throw new Error(`There is no record in table "DailyQuest" that corresponds to field "id" value ${key}`)
   ...
   */
  public *entries(): IterableIterator<[number, DailyQuestRecord]> {
  ...
  public readJsonFrom(json: string): void {
    ...
    const records: DailyQuestRecord[] = []

    if (this.isCompactRowFormatted(dataRows)) {
      for (const dataRow of dataRows) {
        const record = new DailyQuestRecord()
  ...
  public readBinaryFrom(fileBytes: Uint8Array): void {
    ...
    for (let i = 0; i < rowCount; ++i)
      records.push(new DailyQuestRecord())
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          tabbit.checkColumn(column, 'DailyQuest.Id', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'DailyQuest.Id')
          ...
          break
        case 2:
          tabbit.checkColumn(column, 'DailyQuest.QuestId', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'DailyQuest.QuestId')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
          ...
          break
        case 3:
          tabbit.checkColumn(column, 'DailyQuest.Points', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'DailyQuest.Points')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
          ...
          break
        case 4:
          tabbit.checkColumn(column, 'DailyQuest.ResetHour', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'DailyQuest.ResetHour')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
              records[i]._resetHour = value
          }
          break
        default:
   ...
   */
  private publish(records: DailyQuestRecord[]): void {
    const recordsById = new Map<number, DailyQuestRecord>()
