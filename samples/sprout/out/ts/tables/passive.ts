...
export class PassiveRecord {
  ...
  public get unlockStep(): number { return this._unlockStep }
  ...
  public populateFieldValues(dataRow: IDataRow): void {
    ...
    this._unlockStep = dataRow.unlockStep
  ...
  public populateFieldValuesCompact(dataRow: any[]): void {
    ...
    this._unlockStep = dataRow[offset++]
...
export class PassiveTable {
  ...
  /** All records. */
  public get records(): PassiveRecord[] { return this._records }
  private _records: PassiveRecord[] = []
   ...
   */
  public [Symbol.iterator](): IterableIterator<PassiveRecord> {
    return this._records[Symbol.iterator]()
  }

  // Indexing by 'id'
  public get recordsById(): Map<number, PassiveRecord> { return this._recordsById }
  private _recordsById: Map<number, PassiveRecord> = new Map<number, PassiveRecord>()
   ...
   */
  public findById(key: number): PassiveRecord | undefined {
   ...
   */
  public getByIdOrThrow(key: number): PassiveRecord {
    const found = this._recordsById.get(key)
    if (!found)
      throw new Error(`There is no record in table "Passive" that corresponds to field "id" value ${key}`)
   ...
   */
  public *entries(): IterableIterator<[number, PassiveRecord]> {
  ...
  private readFromJson(json: string): void {
    ...
    if (this.isCompactRowFormatted(dataRows)) {
      for (const dataRow of dataRows) {
        const record = new PassiveRecord()
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (let i = 0; i < rowCount; ++i)
      records.push(new PassiveRecord())
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          tabbit.checkColumn(column, 'Passive.Id', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Passive.Id')
          ...
          break
        case 2:
          tabbit.checkColumn(column, 'Passive.CharacterId', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Passive.CharacterId')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
          ...
          break
        case 3:
          tabbit.checkColumn(column, 'Passive.UnlockStep', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Passive.UnlockStep')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
              records[i]._unlockStep = value
          }
          break
        case 4:
          tabbit.checkColumn(column, 'Passive.Stat', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Passive.Stat')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
          ...
          break
        case 5:
          tabbit.checkColumn(column, 'Passive.Bonus', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Passive.Bonus')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
   ...
   */
  private publish(records: PassiveRecord[]): void {
    const recordsById = new Map<number, PassiveRecord>()
