...
export class CharacterVoiceRecord {
  ...
  /** audio asset */
  public get clip(): string { return this._clip }

  /** subtitle key */
  public get subtitleKey(): string { return this._subtitleKey }

  public _id: number = 0
  public _characterId: number = 0
  public _cue: string = ''
  public _clip: string = ''
  public _subtitleKey: string = ''

  /** Populate field values. */
  public populateFieldValues(dataRow: IDataRow): void {
    this._id = dataRow.id
    this._characterId = dataRow.characterId
    this._cue = dataRow.cue
    this._clip = dataRow.clip
    this._subtitleKey = dataRow.subtitleKey
  }
  ...
  public populateFieldValuesCompact(dataRow: any[]): void {
    ...
    this._cue = dataRow[offset++]
    this._clip = dataRow[offset++]
    this._subtitleKey = dataRow[offset++]
  }
}
...
export class CharacterVoiceTable {
  ...
  /** All records. */
  public get records(): CharacterVoiceRecord[] { return this._records }
  private _records: CharacterVoiceRecord[] = []
   ...
   */
  public [Symbol.iterator](): IterableIterator<CharacterVoiceRecord> {
    return this._records[Symbol.iterator]()
  }

  // Indexing by 'id'
  public get recordsById(): Map<number, CharacterVoiceRecord> { return this._recordsById }
  private _recordsById: Map<number, CharacterVoiceRecord> = new Map<number, CharacterVoiceRecord>()
   ...
   */
  public findById(key: number): CharacterVoiceRecord | undefined {
   ...
   */
  public getByIdOrThrow(key: number): CharacterVoiceRecord {
    const found = this._recordsById.get(key)
    if (!found)
      throw new Error(`There is no record in table "CharacterVoice" that corresponds to field "id" value ${key}`)
   ...
   */
  public *entries(): IterableIterator<[number, CharacterVoiceRecord]> {
  ...
  public readJsonFrom(json: string): void {
    ...
    const records: CharacterVoiceRecord[] = []

    if (this.isCompactRowFormatted(dataRows)) {
      for (const dataRow of dataRows) {
        const record = new CharacterVoiceRecord()
  ...
  public readBinaryFrom(fileBytes: Uint8Array): void {
    ...
    for (let i = 0; i < rowCount; ++i)
      records.push(new CharacterVoiceRecord())
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          tabbit.checkColumn(column, 'CharacterVoice.Id', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'CharacterVoice.Id')
          ...
          break
        case 2:
          tabbit.checkColumn(column, 'CharacterVoice.CharacterId', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'CharacterVoice.CharacterId')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
          ...
          break
        case 3:
          tabbit.checkColumn(column, 'CharacterVoice.Cue', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_STRING])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'CharacterVoice.Cue')
          for (let i = 0; i < rowCount; ) {
          ...
          break
        case 4:
          tabbit.checkColumn(column, 'CharacterVoice.Clip', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_STRING])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'CharacterVoice.Clip')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameString(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
              records[i]._clip = value
          }
          break
        case 5:
          tabbit.checkColumn(column, 'CharacterVoice.SubtitleKey', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_STRING])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'CharacterVoice.SubtitleKey')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameString(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
              records[i]._subtitleKey = value
          }
          break
        default:
   ...
   */
  private publish(records: CharacterVoiceRecord[]): void {
    const recordsById = new Map<number, CharacterVoiceRecord>()
