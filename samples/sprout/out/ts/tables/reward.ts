...
export class RewardRecord {
  ...
  /** fixed item counts */
  public get itemCounts(): number[] { return this._itemCounts }

  public _id: number = 0
  public _type: RewardType = 0 as RewardType
  public _lootGroupId: number = 0
  public _itemIds: number[] = []
  public _itemCounts: number[] = []
  ...
  public populateFieldValues(dataRow: IDataRow): void {
    ...
    this._itemIds = dataRow.itemIds
    this._itemCounts = dataRow.itemCounts
  }
  ...
  public populateFieldValuesCompact(dataRow: any[]): void {
    ...
    this._itemIds = dataRow[offset++]
    this._itemCounts = dataRow[offset++]
  }
}
...
export class RewardTable {
  ...
  /** All records. */
  public get records(): RewardRecord[] { return this._records }
  private _records: RewardRecord[] = []
   ...
   */
  public [Symbol.iterator](): IterableIterator<RewardRecord> {
    return this._records[Symbol.iterator]()
  }

  // Indexing by 'id'
  public get recordsById(): Map<number, RewardRecord> { return this._recordsById }
  private _recordsById: Map<number, RewardRecord> = new Map<number, RewardRecord>()
   ...
   */
  public findById(key: number): RewardRecord | undefined {
   ...
   */
  public getByIdOrThrow(key: number): RewardRecord {
    const found = this._recordsById.get(key)
    if (!found)
      throw new Error(`There is no record in table "Reward" that corresponds to field "id" value ${key}`)
   ...
   */
  public *entries(): IterableIterator<[number, RewardRecord]> {
  ...
  public readBinaryFrom(data: Uint8Array): void {
    ...
    for (const column of columns) {
      ...
      switch (column.tag) {
        case 1:
          tabbit.checkColumn(column, 'Reward.Id', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Reward.Id')
          ...
          break
        case 2:
          tabbit.checkColumn(column, 'Reward.Type', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Reward.Type')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
          ...
          break
        case 3:
          tabbit.checkColumn(column, 'Reward.LootGroupId', tabbit.KIND_SCALAR, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Reward.LootGroupId')
          for (let i = 0; i < rowCount; ) {
            const { n, value } = cursor.nextSameI32(rowCount - i)
            for (let left = n; left > 0; --left, ++i)
          ...
          break
        case 4:
          tabbit.checkColumn(column, 'Reward.ItemIds', tabbit.KIND_ARRAY, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Reward.ItemIds')
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
            const elementCount = cursor.nextLength()
            record._itemIds = []
            for (let j = 0; j < elementCount; ++j) {
              record._itemIds.push(cursor.nextI32())
            }
          }
          break
        case 5:
          tabbit.checkColumn(column, 'Reward.ItemCounts', tabbit.KIND_ARRAY, false, [tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT])
          cursor = new tabbit.TcbColumnCursor(reader, column, rowCount, 'Reward.ItemCounts')
          for (let i = 0; i < rowCount; ++i) {
            const record = records[i]
            const elementCount = cursor.nextLength()
            record._itemCounts = []
            for (let j = 0; j < elementCount; ++j) {
              record._itemCounts.push(cursor.nextI32())
            }
          }
          break
        default:
   ...
   */
  private publish(records: RewardRecord[]): void {
    const recordsById = new Map<number, RewardRecord>()
