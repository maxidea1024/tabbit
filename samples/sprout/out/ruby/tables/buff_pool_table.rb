...
module Sprout
  ...
  class BuffPoolTable
    ...
    def read(filename)
      ...
      records = Array.new(count) { BuffPoolRecord.new }
      ...
      columns.each do |column|
        ...
        when 1
          ...
          cursor = Tabbit::ColumnCursor.new(reader, column, count, 'BuffPool.Id')
          at = 0
          while at < count
        ...
        when 2
          ...
          cursor = Tabbit::ColumnCursor.new(reader, column, count, 'BuffPool.BuffIds')
          records.each do |record|
            element_count = cursor.next_length
            record.buff_ids = Array.new(element_count) { cursor.next_i32 }
          end
        when 3
          Tabbit.check_column(column, 'BuffPool.Weights', Tabbit::KIND_ARRAY, false, [Tabbit::ELEMENT_I32, Tabbit::ELEMENT_VARINT])
          cursor = Tabbit::ColumnCursor.new(reader, column, count, 'BuffPool.Weights')
          records.each do |record|
            element_count = cursor.next_length
            record.weights = Array.new(element_count) { cursor.next_i32 }
          end
        else
