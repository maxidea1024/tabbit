...
module Sprout
  ...
  class StageWaveTable
    ...
    def read(filename)
      ...
      records = Array.new(count) { StageWaveRecord.new }
      ...
      columns.each do |column|
        ...
        when 1
          ...
          cursor = Tabbit::ColumnCursor.new(reader, column, count, 'StageWave.Id')
          at = 0
          while at < count
        ...
        when 2
          ...
          cursor = Tabbit::ColumnCursor.new(reader, column, count, 'StageWave.StageId')
          at = 0
          while at < count
            n, value = cursor.next_same_i32(count - at)
            (at...(at + n)).each do |i|
        ...
        when 3
          ...
          cursor = Tabbit::ColumnCursor.new(reader, column, count, 'StageWave.WaveOrder')
          at = 0
          while at < count
            n, value = cursor.next_same_i32(count - at)
            (at...(at + n)).each do |i|
        ...
        when 4
          ...
          cursor = Tabbit::ColumnCursor.new(reader, column, count, 'StageWave.MonsterIds')
          records.each do |record|
            element_count = cursor.next_length
            record.monster_ids = Array.new(element_count) { cursor.next_i32 }
          end
        when 5
          Tabbit.check_column(column, 'StageWave.MonsterLevels', Tabbit::KIND_ARRAY, false, [Tabbit::ELEMENT_I32, Tabbit::ELEMENT_VARINT])
          cursor = Tabbit::ColumnCursor.new(reader, column, count, 'StageWave.MonsterLevels')
          records.each do |record|
            element_count = cursor.next_length
            record.monster_levels = Array.new(element_count) { cursor.next_i32 }
          end
        else
