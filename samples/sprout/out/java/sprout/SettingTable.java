...
public final class SettingTable implements Iterable<SettingRecord> {
    ...
    public SettingRecord getBySettingKeyOrThrow(String key) {
        SettingRecord record = bySettingKey.get(key);

        if (record == null) {
            throw new TcbReader.RecordNotFoundException(
                "there is no record in table `Setting` that corresponds to field "
    ...
    public void read(Path filename) {
        ...
        List<SettingRecord> loaded = new ArrayList<>(count);
        Map<String, SettingRecord> loadedBySettingKey = new HashMap<>(count * 2);

        for (int i = 0; i < count; i++) {
            loaded.add(new SettingRecord());
        }
        ...
        for (TcbReader.Column column : header.columns) {
            ...
            switch (column.tag) {
                case 1: {
                    ...
                    cursor = new TcbReader.ColumnCursor(reader, column, count, "Setting.SettingKey");
                    for (int i = 0; i < count; ) {
                        int n = cursor.nextSameString(count - i);
                        for (; n > 0; n--, i++) {
                            loaded.get(i).settingKey = cursor.runSameText;
                        }
                    }
                    break;
                }
                case 2: {
                    ...
                    cursor = new TcbReader.ColumnCursor(reader, column, count, "Setting.Value");
                    for (int i = 0; i < count; ) {
                        int n = cursor.nextSameString(count - i);
                        for (; n > 0; n--, i++) {
                            loaded.get(i).value = cursor.runSameText;
                        }
                    }
                    break;
                }
                case 3: {
                    ...
                    cursor = new TcbReader.ColumnCursor(reader, column, count, "Setting.DescKey");
                    for (int i = 0; i < count; ) {
                        int n = cursor.nextSameString(count - i);
                        for (; n > 0; n--, i++) {
        ...
        for (SettingRecord record : loaded) {
            loadedBySettingKey.put(record.settingKey, record);
        }
