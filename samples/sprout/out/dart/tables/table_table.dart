...
/// Every row of Table.
class TableTable extends Iterable<TableRecord> {
  ...
  List<TableRecord> get records => _records;
  List<TableRecord> _records = [];
  ...
  @override
  Iterator<TableRecord> get iterator => _records.iterator;

  Map<int, TableRecord> _byId = {};

  /// The row with this Id, or null when the table has none.
  ///
  /// The lookup to reach for when a missing row is an ordinary answer - an optional
  /// reference, a key that came from user input.
  TableRecord? findById(int key) => _byId[key];
  ...
  TableRecord getByIdOrThrow(int key) {
  ...
  TableRecord? operator [](int key) => findById(key);
  ...
  Iterable<MapEntry<int, TableRecord>> get entries =>
  ...
  void read(String filename) {
    ...
    final loaded = <TableRecord>[];
    final loadedById = <int, TableRecord>{};

    for (var i = 0; i < count; i++) {
      loaded.add(TableRecord());
    }
    ...
    for (final column in header.columns) {
      ...
      switch (column.tag) {
        case 1:
          checkColumn(column, 'Table.Id', kindScalar, false, [elementI32, elementVarint]);
          cursor = TcbColumnCursor(reader, column, count, 'Table.Id');
          for (var i = 0; i < count; ) {
          ...
          break;
        case 2:
          checkColumn(column, 'Table.NameKey', kindScalar, false, [elementString]);
          cursor = TcbColumnCursor(reader, column, count, 'Table.NameKey');
          for (var i = 0; i < count; ) {
          ...
          break;
        case 3:
          checkColumn(column, 'Table.Value', kindScalar, false, [elementI32, elementVarint]);
          cursor = TcbColumnCursor(reader, column, count, 'Table.Value');
          for (var i = 0; i < count; ) {
            final (n, value) = cursor.nextSameI32(count - i);
            for (var left = n; left > 0; left--, i++) {
