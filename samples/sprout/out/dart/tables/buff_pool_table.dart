...
class BuffPoolTable extends Iterable<BuffPoolRecord> {
  ...
  List<BuffPoolRecord> get records => _records;
  List<BuffPoolRecord> _records = [];
  ...
  @override
  Iterator<BuffPoolRecord> get iterator => _records.iterator;

  Map<int, BuffPoolRecord> _byId = {};

  /// The row with this Id, or null when the table has none.
  ///
  /// The lookup to reach for when a missing row is an ordinary answer - an optional
  /// reference, a key that came from user input.
  BuffPoolRecord? findById(int key) => _byId[key];
  ...
  BuffPoolRecord getByIdOrThrow(int key) {
  ...
  BuffPoolRecord? operator [](int key) => findById(key);
  ...
  Iterable<MapEntry<int, BuffPoolRecord>> get entries =>
  ...
  void read(String filename) {
    ...
    final loaded = <BuffPoolRecord>[];
    final loadedById = <int, BuffPoolRecord>{};

    for (var i = 0; i < count; i++) {
      loaded.add(BuffPoolRecord());
    }
    ...
    for (final column in header.columns) {
      ...
      switch (column.tag) {
        case 1:
          checkColumn(column, 'BuffPool.Id', kindScalar, false, [elementI32, elementVarint]);
          cursor = TcbColumnCursor(reader, column, count, 'BuffPool.Id');
          for (var i = 0; i < count; ) {
          ...
          break;
        case 2:
          checkColumn(column, 'BuffPool.BuffIds', kindArray, false, [elementI32, elementVarint]);
          cursor = TcbColumnCursor(reader, column, count, 'BuffPool.BuffIds');
          for (final record in loaded) {
            final elementCount = cursor.nextLength();
            record.buffIds = List.generate(elementCount, (_) => cursor.nextI32());
          }
          break;
        case 3:
          checkColumn(column, 'BuffPool.Weights', kindArray, false, [elementI32, elementVarint]);
          cursor = TcbColumnCursor(reader, column, count, 'BuffPool.Weights');
          for (final record in loaded) {
            final elementCount = cursor.nextLength();
            record.weights = List.generate(elementCount, (_) => cursor.nextI32());
          }
          break;
        default:
