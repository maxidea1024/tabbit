...
/// Every row of Pity.
class PityTable extends Iterable<PityRecord> {
  ...
  List<PityRecord> get records => _records;
  List<PityRecord> _records = [];
  ...
  @override
  Iterator<PityRecord> get iterator => _records.iterator;

  Map<int, PityRecord> _byId = {};

  /// The row with this Id, or null when the table has none.
  ///
  /// The lookup to reach for when a missing row is an ordinary answer - an optional
  /// reference, a key that came from user input.
  PityRecord? findById(int key) => _byId[key];
  ...
  PityRecord getByIdOrThrow(int key) {
  ...
  PityRecord? operator [](int key) => findById(key);
  ...
  Iterable<MapEntry<int, PityRecord>> get entries =>
  ...
  void read(String filename) {
    ...
    final loaded = <PityRecord>[];
    final loadedById = <int, PityRecord>{};

    for (var i = 0; i < count; i++) {
      loaded.add(PityRecord());
    }
    ...
    for (final column in header.columns) {
      ...
      switch (column.tag) {
        case 1:
          checkColumn(column, 'Pity.Id', kindScalar, false, [elementI32, elementVarint]);
          cursor = TcbColumnCursor(reader, column, count, 'Pity.Id');
          for (var i = 0; i < count; ) {
          ...
          break;
        case 2:
          checkColumn(column, 'Pity.Threshold', kindScalar, false, [elementI32, elementVarint]);
          cursor = TcbColumnCursor(reader, column, count, 'Pity.Threshold');
          for (var i = 0; i < count; ) {
            final (n, value) = cursor.nextSameI32(count - i);
            for (var left = n; left > 0; left--, i++) {
              loaded[i].threshold = value;
            }
          }
          break;
        case 3:
          checkColumn(column, 'Pity.Grade', kindScalar, false, [elementVarint]);
          cursor = TcbColumnCursor(reader, column, count, 'Pity.Grade');
          for (var i = 0; i < count; ) {
            final (n, value) = cursor.nextSameI32(count - i);
            for (var left = n; left > 0; left--, i++) {
          ...
          break;
        case 4:
          checkColumn(column, 'Pity.ResetOnHit', kindScalar, false, [elementBool]);
          cursor = TcbColumnCursor(reader, column, count, 'Pity.ResetOnHit');
