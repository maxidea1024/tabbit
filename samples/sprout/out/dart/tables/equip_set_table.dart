...
class EquipSetTable extends Iterable<EquipSetRecord> {
  ...
  List<EquipSetRecord> get records => _records;
  List<EquipSetRecord> _records = [];
  ...
  @override
  Iterator<EquipSetRecord> get iterator => _records.iterator;

  Map<int, EquipSetRecord> _byId = {};

  /// The row with this Id, or null when the table has none.
  ///
  /// The lookup to reach for when a missing row is an ordinary answer - an optional
  /// reference, a key that came from user input.
  EquipSetRecord? findById(int key) => _byId[key];
  ...
  EquipSetRecord getByIdOrThrow(int key) {
  ...
  EquipSetRecord? operator [](int key) => findById(key);
  ...
  Iterable<MapEntry<int, EquipSetRecord>> get entries =>
  ...
  void read(String filename) {
    ...
    final loaded = <EquipSetRecord>[];
    final loadedById = <int, EquipSetRecord>{};

    for (var i = 0; i < count; i++) {
      loaded.add(EquipSetRecord());
    }
    ...
    for (final column in header.columns) {
      ...
      switch (column.tag) {
        case 1:
          checkColumn(column, 'EquipSet.Id', kindScalar, false, [elementI32, elementVarint]);
          cursor = TcbColumnCursor(reader, column, count, 'EquipSet.Id');
          for (var i = 0; i < count; ) {
          ...
          break;
        case 2:
          checkColumn(column, 'EquipSet.NameKey', kindScalar, false, [elementString]);
          cursor = TcbColumnCursor(reader, column, count, 'EquipSet.NameKey');
          for (var i = 0; i < count; ) {
          ...
          break;
        case 3:
          checkColumn(column, 'EquipSet.MemberIds', kindArray, false, [elementI32, elementVarint]);
          cursor = TcbColumnCursor(reader, column, count, 'EquipSet.MemberIds');
          for (final record in loaded) {
            final elementCount = cursor.nextLength();
            record.memberIds = List.generate(elementCount, (_) => cursor.nextI32());
          }
          break;
        default:
