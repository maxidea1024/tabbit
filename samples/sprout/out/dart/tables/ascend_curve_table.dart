...
class AscendCurveTable extends Iterable<AscendCurveRecord> {
  ...
  List<AscendCurveRecord> get records => _records;
  List<AscendCurveRecord> _records = [];
  ...
  @override
  Iterator<AscendCurveRecord> get iterator => _records.iterator;

  Map<int, AscendCurveRecord> _byId = {};

  /// The row with this Id, or null when the table has none.
  ///
  /// The lookup to reach for when a missing row is an ordinary answer - an optional
  /// reference, a key that came from user input.
  AscendCurveRecord? findById(int key) => _byId[key];
  ...
  AscendCurveRecord getByIdOrThrow(int key) {
  ...
  AscendCurveRecord? operator [](int key) => findById(key);
  ...
  Iterable<MapEntry<int, AscendCurveRecord>> get entries =>
  ...
  void read(String filename) {
    ...
    final loaded = <AscendCurveRecord>[];
    final loadedById = <int, AscendCurveRecord>{};

    for (var i = 0; i < count; i++) {
      loaded.add(AscendCurveRecord());
    }
    ...
    for (final column in header.columns) {
      ...
      switch (column.tag) {
        case 1:
          checkColumn(column, 'AscendCurve.Id', kindScalar, false, [elementI32, elementVarint]);
          cursor = TcbColumnCursor(reader, column, count, 'AscendCurve.Id');
          for (var i = 0; i < count; ) {
          ...
          break;
        case 2:
          checkColumn(column, 'AscendCurve.Grade', kindScalar, false, [elementVarint]);
          cursor = TcbColumnCursor(reader, column, count, 'AscendCurve.Grade');
          for (var i = 0; i < count; ) {
            final (n, value) = cursor.nextSameI32(count - i);
            for (var left = n; left > 0; left--, i++) {
          ...
          break;
        case 3:
          checkColumn(column, 'AscendCurve.Step', kindScalar, false, [elementI32, elementVarint]);
          cursor = TcbColumnCursor(reader, column, count, 'AscendCurve.Step');
          for (var i = 0; i < count; ) {
            final (n, value) = cursor.nextSameI32(count - i);
            for (var left = n; left > 0; left--, i++) {
              loaded[i].step = value;
          ...
          break;
        case 4:
          checkColumn(column, 'AscendCurve.Scale', kindScalar, false, [elementI32, elementVarint]);
          cursor = TcbColumnCursor(reader, column, count, 'AscendCurve.Scale');
          for (var i = 0; i < count; ) {
            final (n, value) = cursor.nextSameI32(count - i);
            for (var left = n; left > 0; left--, i++) {
              loaded[i].scale = value;
            }
          }
          break;
        default:
