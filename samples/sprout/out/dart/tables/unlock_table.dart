...
class UnlockRecord {
  ...
  int id = 0;
  /// unlock kind
  UnlockType type = UnlockType.of(0);
  /// condition
  int conditionId = 0;
  /// locked message key
  String messageKey = '';
...
/// Every row of Unlock.
class UnlockTable extends Iterable<UnlockRecord> {
  ...
  List<UnlockRecord> get records => _records;
  ...
  @override
  Iterator<UnlockRecord> get iterator => _records.iterator;

  Map<int, UnlockRecord> _byId = {};

  /// The row with this Id, or null when the table has none.
  ///
  /// The lookup to reach for when a missing row is an ordinary answer - an optional
  /// reference, a key that came from user input.
  UnlockRecord? findById(int key) => _byId[key];
  ...
  UnlockRecord getByIdOrThrow(int key) {
  ...
  UnlockRecord? operator [](int key) => findById(key);
  ...
  Iterable<MapEntry<int, UnlockRecord>> get entries =>
  ...
  void read(String filename) {
    ...
    final loaded = <UnlockRecord>[];
    final loadedById = <int, UnlockRecord>{};

    for (var i = 0; i < count; i++) {
      loaded.add(UnlockRecord());
    }
    ...
    for (final column in header.columns) {
      ...
      switch (column.tag) {
        case 1:
          checkColumn(column, 'Unlock.Id', kindScalar, false, [elementI32, elementVarint]);
          cursor = TcbColumnCursor(reader, column, count, 'Unlock.Id');
          for (var i = 0; i < count; ) {
          ...
          break;
        case 2:
          checkColumn(column, 'Unlock.Type', kindScalar, false, [elementVarint]);
          cursor = TcbColumnCursor(reader, column, count, 'Unlock.Type');
          for (var i = 0; i < count; ) {
            final (n, value) = cursor.nextSameI32(count - i);
            for (var left = n; left > 0; left--, i++) {
              loaded[i].type = UnlockType.of(value);
            }
          }
          break;
        case 3:
          checkColumn(column, 'Unlock.ConditionId', kindScalar, false, [elementI32, elementVarint]);
          cursor = TcbColumnCursor(reader, column, count, 'Unlock.ConditionId');
          for (var i = 0; i < count; ) {
            final (n, value) = cursor.nextSameI32(count - i);
            for (var left = n; left > 0; left--, i++) {
          ...
          break;
        case 4:
          checkColumn(column, 'Unlock.MessageKey', kindScalar, false, [elementString]);
          cursor = TcbColumnCursor(reader, column, count, 'Unlock.MessageKey');
          for (var i = 0; i < count; ) {
            final (n, value) = cursor.nextSameString(count - i);
            for (var left = n; left > 0; left--, i++) {
              loaded[i].messageKey = value;
            }
          }
          break;
        default:
