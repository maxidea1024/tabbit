...
class PriceRecord {
  ...
  int id = 0;
  /// currency
  CurrencyType currency = CurrencyType.of(0);
  /// amount
  int amount = 0;
...
/// Every row of Price.
class PriceTable extends Iterable<PriceRecord> {
  ...
  List<PriceRecord> get records => _records;
  ...
  @override
  Iterator<PriceRecord> get iterator => _records.iterator;

  Map<int, PriceRecord> _byId = {};

  /// The row with this Id, or null when the table has none.
  ///
  /// The lookup to reach for when a missing row is an ordinary answer - an optional
  /// reference, a key that came from user input.
  PriceRecord? findById(int key) => _byId[key];
  ...
  PriceRecord getByIdOrThrow(int key) {
  ...
  PriceRecord? operator [](int key) => findById(key);
  ...
  Iterable<MapEntry<int, PriceRecord>> get entries =>
  ...
  void read(String filename) {
    ...
    final loaded = <PriceRecord>[];
    final loadedById = <int, PriceRecord>{};

    for (var i = 0; i < count; i++) {
      loaded.add(PriceRecord());
    }
    ...
    for (final column in header.columns) {
      ...
      switch (column.tag) {
        case 1:
          checkColumn(column, 'Price.Id', kindScalar, false, [elementI32, elementVarint]);
          cursor = TcbColumnCursor(reader, column, count, 'Price.Id');
          for (var i = 0; i < count; ) {
          ...
          break;
        case 2:
          checkColumn(column, 'Price.Currency', kindScalar, false, [elementVarint]);
          cursor = TcbColumnCursor(reader, column, count, 'Price.Currency');
          for (var i = 0; i < count; ) {
            final (n, value) = cursor.nextSameI32(count - i);
            for (var left = n; left > 0; left--, i++) {
              loaded[i].currency = CurrencyType.of(value);
            }
          }
          break;
        case 3:
          checkColumn(column, 'Price.Amount', kindScalar, false, [elementI32, elementVarint]);
          cursor = TcbColumnCursor(reader, column, count, 'Price.Amount');
          for (var i = 0; i < count; ) {
            final (n, value) = cursor.nextSameI32(count - i);
            for (var left = n; left > 0; left--, i++) {
              loaded[i].amount = value;
            }
          }
          break;
        case 4:
          checkColumn(column, 'Price.Discount', kindScalar, false, [elementI32, elementVarint]);
          cursor = TcbColumnCursor(reader, column, count, 'Price.Discount');
          for (var i = 0; i < count; ) {
            final (n, value) = cursor.nextSameI32(count - i);
            for (var left = n; left > 0; left--, i++) {
              loaded[i].discount = value;
            }
          }
          break;
        default:
