...
namespace Sprout.Tables
{
    ...
    [System.Serializable]
    public partial class TableTable : IEnumerable<TableRecord>
    {
        /// <summary>
        /// Field names.
        /// </summary>
        public static readonly string[] FieldNames = { "Id", "NameKey", "Value" };
        ...
        public List<object[]> BuildObjectValueMap()
        {
            ...
            foreach (var r in _records)
                result.Add(new object[] { r.Id, r.NameKey, r.Value });
        ...
        /// </remarks>
        public List<TableRecord> Records => _records;
        private List<TableRecord> _records = new List<TableRecord>();
        ...
        /// </remarks>
        public List<TableRecord>.Enumerator GetEnumerator() => _records.GetEnumerator();

        IEnumerator<TableRecord> IEnumerable<TableRecord>.GetEnumerator() => _records.GetEnumerator();

        System.Collections.IEnumerator System.Collections.IEnumerable.GetEnumerator()
            => _records.GetEnumerator();

        #region Indexing by 'Id'
        public Dictionary<int, TableRecord> RecordsById => _recordsById;
        private Dictionary<int, TableRecord> _recordsById = new Dictionary<int, TableRecord>();
        ...
        public TableRecord FindById(int key)
            => _recordsById.TryGetValue(key, out TableRecord record) ? record : null;
        ...
        {
            if (!_recordsById.TryGetValue(key, out TableRecord record))
                throw new TabbitException($"There is no record in table `Table` that corresponds to field `Id` value {key}");
        ...
        {
            private readonly List<TableRecord> _rows;
            private int _at;

            internal EntryEnumerator(List<TableRecord> rows)
            {
                _rows = rows;
                _at = -1;
            }
            ...
            public (int Key, TableRecord Row) Current
        ...
        /// </remarks>
        public TableRecord this[int key] => GetByIdOrThrow(key);
        ...
        public Task ReadAsync(TcbReader reader)
        {
            ...
            foreach (var column in columns)
            {
                ...
                {
                    case 1:
                        TcbTable.CheckColumn(column, "Table.Id", TcbTable.KindScalar, false, TcbTable.ElementI32, TcbTable.ElementVarint);
                        cursor = new TcbColumnCursor(reader, column, count, "Table.Id");
                        for (int i = 0; i < count; )
                        {
                    ...
                    case 2:
                        TcbTable.CheckColumn(column, "Table.NameKey", TcbTable.KindScalar, false, TcbTable.ElementString);
                        cursor = new TcbColumnCursor(reader, column, count, "Table.NameKey");
                        for (int i = 0; i < count; )
                        {
                            // One call per run of equal values, not one per row - over a
                            // run-length encoded column this is most of the decode.
                    ...
                    case 3:
                        TcbTable.CheckColumn(column, "Table.Value", TcbTable.KindScalar, false, TcbTable.ElementI32, TcbTable.ElementVarint);
                        cursor = new TcbColumnCursor(reader, column, count, "Table.Value");
                        for (int i = 0; i < count; )
                        {
                            // One call per run of equal values, not one per row - over a
                            // run-length encoded column this is most of the decode.
            ...
            var recordsById = new Dictionary<int, TableRecord>(count);
