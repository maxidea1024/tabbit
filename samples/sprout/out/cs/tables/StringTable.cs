...
namespace Sprout.Tables
{
    ...
    [System.Serializable]
    public partial class StringTable : IEnumerable<StringRecord>
    {
        /// <summary>
        /// Field names.
        /// </summary>
        ...
        /// </remarks>
        public List<StringRecord> Records => _records;
        private List<StringRecord> _records = new List<StringRecord>();
        ...
        /// </remarks>
        public List<StringRecord>.Enumerator GetEnumerator() => _records.GetEnumerator();

        IEnumerator<StringRecord> IEnumerable<StringRecord>.GetEnumerator() => _records.GetEnumerator();

        System.Collections.IEnumerator System.Collections.IEnumerable.GetEnumerator()
            => _records.GetEnumerator();

        #region Indexing by 'StringKey'
        public Dictionary<string, StringRecord> RecordsByStringKey => _recordsByStringKey;
        private Dictionary<string, StringRecord> _recordsByStringKey = new Dictionary<string, StringRecord>();
        ...
        public StringRecord FindByStringKey(string key)
            => _recordsByStringKey.TryGetValue(key, out StringRecord record) ? record : null;
        ...
        {
            if (!_recordsByStringKey.TryGetValue(key, out StringRecord record))
        ...
        /// <summary>Each row with the `StringKey` it is keyed by.</summary>
        /// <remarks>
        ...
        /// `StringKey` here, something else in the next table.
        ///
        ...
        {
            private readonly List<StringRecord> _rows;
            private int _at;

            internal EntryEnumerator(List<StringRecord> rows)
            {
                _rows = rows;
                _at = -1;
            }
            ...
            public (string Key, StringRecord Row) Current
                => (_rows[_at].StringKey, _rows[_at]);
        }
        ...
        /// </summary>
        /// <remarks>
        /// `GetByStringKeyOrThrow` under the spelling the language uses for a
        ...
        /// `StringKey` is 0, not the first row - the rows in order are
        /// `Records`. Nothing in the type says which, which is why the table itself is not
        /// subscriptable by position anywhere.
        ///
        /// It does not replace `FindByStringKey`: a key that may be absent
        /// wants the one whose name says a miss is an ordinary answer.
        /// </remarks>
        public StringRecord this[string key] => GetByStringKeyOrThrow(key);
        ...
        public Task ReadAsync(TcbReader reader)
        {
            ...
            var records = new List<StringRecord>(count);
            ...
            var recordsByStringKey = new Dictionary<string, StringRecord>(count);
