...
/** Every row of World. */
class WorldTable : Iterable<WorldRecord> {
     ...
     */
    var records: MutableList<WorldRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<WorldRecord> = records.iterator()

    private var byId: HashMap<Int, WorldRecord> = HashMap()
     ...
     */
    fun findById(key: Int): WorldRecord? = byId[key]
     ...
     */
    fun getByIdOrThrow(key: Int): WorldRecord =
     ...
     */
    operator fun get(key: Int): WorldRecord? = findById(key)
     ...
     */
    val entries: Sequence<Pair<Int, WorldRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<WorldRecord>(count)
        val loadedById = HashMap<Int, WorldRecord>(count * 2)

        repeat(count) { loaded.add(WorldRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "World.Id", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "World.Id")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "World.NameKey", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "World.NameKey")
                    var at = 0
                    while (at < count) {
                ...
                3 -> {
                    checkColumn(column, "World.Map", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "World.Map")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameString(count - at)
                        while (n > 0) {
                ...
                4 -> {
                    checkColumn(column, "World.ThemeColour", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "World.ThemeColour")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameString(count - at)
                        while (n > 0) {
