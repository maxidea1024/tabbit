...
class OfferRecord {
    ...
    var nameKey: String = ""
    /** shop kind */
    var shop: ShopType = ShopType.of(0)
...
/** Every row of Offer. */
class OfferTable : Iterable<OfferRecord> {
     ...
     */
    var records: MutableList<OfferRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<OfferRecord> = records.iterator()

    private var byId: HashMap<Int, OfferRecord> = HashMap()
     ...
     */
    fun findById(key: Int): OfferRecord? = byId[key]
     ...
     */
    fun getByIdOrThrow(key: Int): OfferRecord =
     ...
     */
    operator fun get(key: Int): OfferRecord? = findById(key)
     ...
     */
    val entries: Sequence<Pair<Int, OfferRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<OfferRecord>(count)
        val loadedById = HashMap<Int, OfferRecord>(count * 2)

        repeat(count) { loaded.add(OfferRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "Offer.Id", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Offer.Id")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "Offer.NameKey", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "Offer.NameKey")
                    var at = 0
                    while (at < count) {
                ...
                3 -> {
                    checkColumn(column, "Offer.Shop", KIND_SCALAR, false, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Offer.Shop")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                4 -> {
                    checkColumn(column, "Offer.Price", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Offer.Price")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                5 -> {
                    checkColumn(column, "Offer.Limit", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Offer.Limit")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
