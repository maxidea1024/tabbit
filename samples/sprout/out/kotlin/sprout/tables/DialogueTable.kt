...
class DialogueTable : Iterable<DialogueRecord> {
     ...
     */
    var records: MutableList<DialogueRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<DialogueRecord> = records.iterator()

    private var byId: HashMap<Int, DialogueRecord> = HashMap()
     ...
     */
    fun findById(key: Int): DialogueRecord? = byId[key]
     ...
     */
    fun getByIdOrThrow(key: Int): DialogueRecord =
     ...
     */
    operator fun get(key: Int): DialogueRecord? = findById(key)
     ...
     */
    val entries: Sequence<Pair<Int, DialogueRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<DialogueRecord>(count)
        val loadedById = HashMap<Int, DialogueRecord>(count * 2)

        repeat(count) { loaded.add(DialogueRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "Dialogue.Id", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Dialogue.Id")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "Dialogue.SceneKey", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "Dialogue.SceneKey")
                    var at = 0
                    while (at < count) {
                ...
                3 -> {
                    checkColumn(column, "Dialogue.CutsceneId", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Dialogue.CutsceneId")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                4 -> {
                    checkColumn(column, "Dialogue.Skippable", KIND_SCALAR, false, ELEMENT_BOOL)
                    val cursor = ColumnCursor(reader, column, count, "Dialogue.Skippable")
