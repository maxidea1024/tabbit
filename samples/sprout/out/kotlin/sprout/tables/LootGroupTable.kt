...
class LootGroupTable : Iterable<LootGroupRecord> {
     ...
     */
    var records: MutableList<LootGroupRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<LootGroupRecord> = records.iterator()

    private var byId: HashMap<Int, LootGroupRecord> = HashMap()
     ...
     */
    fun findById(key: Int): LootGroupRecord? = byId[key]
     ...
     */
    fun getByIdOrThrow(key: Int): LootGroupRecord =
     ...
     */
    operator fun get(key: Int): LootGroupRecord? = findById(key)
     ...
     */
    val entries: Sequence<Pair<Int, LootGroupRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<LootGroupRecord>(count)
        val loadedById = HashMap<Int, LootGroupRecord>(count * 2)

        repeat(count) { loaded.add(LootGroupRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "LootGroup.Id", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "LootGroup.Id")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "LootGroup.NameKey", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "LootGroup.NameKey")
                    var at = 0
                    while (at < count) {
                ...
                3 -> {
                    checkColumn(column, "LootGroup.Draws", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "LootGroup.Draws")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                4 -> {
                    checkColumn(column, "LootGroup.AllowDuplicate", KIND_SCALAR, false, ELEMENT_BOOL)
                    val cursor = ColumnCursor(reader, column, count, "LootGroup.AllowDuplicate")
