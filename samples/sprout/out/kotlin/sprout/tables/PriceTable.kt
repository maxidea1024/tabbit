...
class PriceRecord {
    ...
    var id: Int = 0
    /** currency */
    var currency: CurrencyType = CurrencyType.of(0)
    /** amount */
    var amount: Int = 0
...
class PriceTable : Iterable<PriceRecord> {
    ...
    private var byId: HashMap<Int, PriceRecord> = HashMap()
     ...
     */
    fun findById(key: Int): PriceRecord? = byId[key]
     ...
     */
    fun getByIdOrThrow(key: Int): PriceRecord =
     ...
     */
    operator fun get(key: Int): PriceRecord? = findById(key)
    ...
    fun read(filename: String) {
        ...
        val loadedById = HashMap<Int, PriceRecord>(count * 2)
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "Price.Id", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Price.Id")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "Price.Currency", KIND_SCALAR, false, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Price.Currency")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                4 -> {
                    checkColumn(column, "Price.Discount", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Price.Discount")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
