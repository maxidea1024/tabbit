...
class EquipGradeTable : Iterable<EquipGradeRecord> {
     ...
     */
    var records: MutableList<EquipGradeRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<EquipGradeRecord> = records.iterator()

    private var byId: HashMap<Int, EquipGradeRecord> = HashMap()
     ...
     */
    fun findById(key: Int): EquipGradeRecord? = byId[key]
     ...
     */
    fun getByIdOrThrow(key: Int): EquipGradeRecord =
     ...
     */
    operator fun get(key: Int): EquipGradeRecord? = findById(key)
     ...
     */
    val entries: Sequence<Pair<Int, EquipGradeRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<EquipGradeRecord>(count)
        val loadedById = HashMap<Int, EquipGradeRecord>(count * 2)

        repeat(count) { loaded.add(EquipGradeRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "EquipGrade.Id", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "EquipGrade.Id")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "EquipGrade.Grade", KIND_SCALAR, false, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "EquipGrade.Grade")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                3 -> {
                    checkColumn(column, "EquipGrade.OptionSlots", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "EquipGrade.OptionSlots")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                4 -> {
                    checkColumn(column, "EquipGrade.Frame", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "EquipGrade.Frame")
                    var at = 0
                    while (at < count) {
