...
/** Every row of String. */
class StringTable : Iterable<StringRecord> {
     ...
     */
    var records: MutableList<StringRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<StringRecord> = records.iterator()

    private var byStringKey: HashMap<String, StringRecord> = HashMap()
     ...
     */
    fun findByStringKey(key: String): StringRecord? = byStringKey[key]
     ...
     */
    fun getByStringKeyOrThrow(key: String): StringRecord =
        byStringKey[key] ?: throw RecordNotFoundException(
    ...
    fun containsStringKey(key: String): Boolean = byStringKey.containsKey(key)


    /**
     * The row with this StringKey, or null when the table has none.
     *
     * findByStringKey under the spelling Kotlin uses for a keyed collection. It
     ...
     */
    operator fun get(key: String): StringRecord? = findByStringKey(key)
     ...
     */
    val entries: Sequence<Pair<String, StringRecord>>
        get() = records.asSequence().map { record -> record.stringKey to record }
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<StringRecord>(count)
        val loadedByStringKey = HashMap<String, StringRecord>(count * 2)

        repeat(count) { loaded.add(StringRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "String.StringKey", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "String.StringKey")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "String.En", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "String.En")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameString(count - at)
                        while (n > 0) {
                ...
                3 -> {
                    checkColumn(column, "String.Ko", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "String.Ko")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameString(count - at)
                        while (n > 0) {
                ...
                4 -> {
                    checkColumn(column, "String.Ja", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "String.Ja")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameString(count - at)
                        while (n > 0) {
