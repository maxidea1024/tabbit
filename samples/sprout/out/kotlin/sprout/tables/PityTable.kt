...
class PityRecord {
    ...
    /** resets on hit */
    var resetOnHit: Boolean = false

}

/** Every row of Pity. */
class PityTable : Iterable<PityRecord> {
     ...
     */
    var records: MutableList<PityRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<PityRecord> = records.iterator()

    private var byId: HashMap<Int, PityRecord> = HashMap()
     ...
     */
    fun findById(key: Int): PityRecord? = byId[key]
     ...
     */
    fun getByIdOrThrow(key: Int): PityRecord =
     ...
     */
    operator fun get(key: Int): PityRecord? = findById(key)
     ...
     */
    val entries: Sequence<Pair<Int, PityRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<PityRecord>(count)
        val loadedById = HashMap<Int, PityRecord>(count * 2)

        repeat(count) { loaded.add(PityRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "Pity.Id", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Pity.Id")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "Pity.Threshold", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Pity.Threshold")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                3 -> {
                    checkColumn(column, "Pity.Grade", KIND_SCALAR, false, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Pity.Grade")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                4 -> {
                    checkColumn(column, "Pity.ResetOnHit", KIND_SCALAR, false, ELEMENT_BOOL)
                    val cursor = ColumnCursor(reader, column, count, "Pity.ResetOnHit")
