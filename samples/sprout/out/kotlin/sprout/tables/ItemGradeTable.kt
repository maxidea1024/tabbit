...
class ItemGradeTable : Iterable<ItemGradeRecord> {
     ...
     */
    var records: MutableList<ItemGradeRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<ItemGradeRecord> = records.iterator()

    private var byId: HashMap<Int, ItemGradeRecord> = HashMap()
     ...
     */
    fun findById(key: Int): ItemGradeRecord? = byId[key]
     ...
     */
    fun getByIdOrThrow(key: Int): ItemGradeRecord =
     ...
     */
    operator fun get(key: Int): ItemGradeRecord? = findById(key)
     ...
     */
    val entries: Sequence<Pair<Int, ItemGradeRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<ItemGradeRecord>(count)
        val loadedById = HashMap<Int, ItemGradeRecord>(count * 2)

        repeat(count) { loaded.add(ItemGradeRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "ItemGrade.Id", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "ItemGrade.Id")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "ItemGrade.Grade", KIND_SCALAR, false, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "ItemGrade.Grade")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                3 -> {
                    checkColumn(column, "ItemGrade.Frame", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "ItemGrade.Frame")
                    var at = 0
                    while (at < count) {
                ...
                4 -> {
                    checkColumn(column, "ItemGrade.SellScale", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "ItemGrade.SellScale")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
