...
class EquipSetRecord {
    ...
    var memberIds: MutableList<Int> = ArrayList()

}

/** Every row of EquipSet. */
class EquipSetTable : Iterable<EquipSetRecord> {
     ...
     */
    var records: MutableList<EquipSetRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<EquipSetRecord> = records.iterator()

    private var byId: HashMap<Int, EquipSetRecord> = HashMap()
     ...
     */
    fun findById(key: Int): EquipSetRecord? = byId[key]
     ...
     */
    fun getByIdOrThrow(key: Int): EquipSetRecord =
     ...
     */
    operator fun get(key: Int): EquipSetRecord? = findById(key)
     ...
     */
    val entries: Sequence<Pair<Int, EquipSetRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<EquipSetRecord>(count)
        val loadedById = HashMap<Int, EquipSetRecord>(count * 2)

        repeat(count) { loaded.add(EquipSetRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "EquipSet.Id", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "EquipSet.Id")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "EquipSet.NameKey", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "EquipSet.NameKey")
                    var at = 0
                    while (at < count) {
                ...
                3 -> {
                    checkColumn(column, "EquipSet.MemberIds", KIND_ARRAY, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "EquipSet.MemberIds")
                    for (record in loaded) {
                        val elementCount = cursor.nextLength()
                        record.memberIds = ArrayList(elementCount.coerceAtLeast(0))
                        repeat(elementCount) { record.memberIds.add(cursor.nextI32()) }
                    }
                }
                else ->
