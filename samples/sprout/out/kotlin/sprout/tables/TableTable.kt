...
/** Every row of Table. */
class TableTable : Iterable<TableRecord> {
     ...
     */
    var records: MutableList<TableRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<TableRecord> = records.iterator()

    private var byId: HashMap<Int, TableRecord> = HashMap()
     ...
     */
    fun findById(key: Int): TableRecord? = byId[key]
     ...
     */
    fun getByIdOrThrow(key: Int): TableRecord =
     ...
     */
    operator fun get(key: Int): TableRecord? = findById(key)
     ...
     */
    val entries: Sequence<Pair<Int, TableRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<TableRecord>(count)
        val loadedById = HashMap<Int, TableRecord>(count * 2)

        repeat(count) { loaded.add(TableRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "Table.Id", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Table.Id")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "Table.NameKey", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "Table.NameKey")
                    var at = 0
                    while (at < count) {
                ...
                3 -> {
                    checkColumn(column, "Table.Value", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Table.Value")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
