...
class CodexTierTable : Iterable<CodexTierRecord> {
     ...
     */
    var records: MutableList<CodexTierRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<CodexTierRecord> = records.iterator()

    private var byId: HashMap<Int, CodexTierRecord> = HashMap()
     ...
     */
    fun findById(key: Int): CodexTierRecord? = byId[key]
     ...
     */
    fun getByIdOrThrow(key: Int): CodexTierRecord =
     ...
     */
    operator fun get(key: Int): CodexTierRecord? = findById(key)
     ...
     */
    val entries: Sequence<Pair<Int, CodexTierRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<CodexTierRecord>(count)
        val loadedById = HashMap<Int, CodexTierRecord>(count * 2)

        repeat(count) { loaded.add(CodexTierRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "CodexTier.Id", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "CodexTier.Id")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "CodexTier.CodexId", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "CodexTier.CodexId")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                3 -> {
                    checkColumn(column, "CodexTier.Required", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "CodexTier.Required")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                4 -> {
                    checkColumn(column, "CodexTier.RewardId", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "CodexTier.RewardId")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
