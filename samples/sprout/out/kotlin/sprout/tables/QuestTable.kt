...
class QuestRecord {
    ...
    var id: Int = 0
    /** quest kind */
    var type: QuestType = QuestType.of(0)
    /** title key */
    var titleKey: String = ""
    /** summary key */
...
/** Every row of Quest. */
class QuestTable : Iterable<QuestRecord> {
     ...
     */
    var records: MutableList<QuestRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<QuestRecord> = records.iterator()

    private var byId: HashMap<Int, QuestRecord> = HashMap()
     ...
     */
    fun findById(key: Int): QuestRecord? = byId[key]
     ...
     */
    fun getByIdOrThrow(key: Int): QuestRecord =
     ...
     */
    operator fun get(key: Int): QuestRecord? = findById(key)
     ...
     */
    val entries: Sequence<Pair<Int, QuestRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<QuestRecord>(count)
        val loadedById = HashMap<Int, QuestRecord>(count * 2)

        repeat(count) { loaded.add(QuestRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "Quest.Id", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Quest.Id")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "Quest.Type", KIND_SCALAR, false, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Quest.Type")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                3 -> {
                    checkColumn(column, "Quest.TitleKey", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "Quest.TitleKey")
                    var at = 0
                    while (at < count) {
                ...
                4 -> {
                    checkColumn(column, "Quest.SummaryKey", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "Quest.SummaryKey")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameString(count - at)
                        while (n > 0) {
                ...
                5 -> {
                    checkColumn(column, "Quest.RewardId", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Quest.RewardId")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                6 -> {
                    checkColumn(column, "Quest.UnlockId", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Quest.UnlockId")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                7 -> {
                    checkColumn(column, "Quest.Repeatable", KIND_SCALAR, false, ELEMENT_BOOL)
                    val cursor = ColumnCursor(reader, column, count, "Quest.Repeatable")
