...
class DailyQuestTable : Iterable<DailyQuestRecord> {
     ...
     */
    var records: MutableList<DailyQuestRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<DailyQuestRecord> = records.iterator()

    private var byId: HashMap<Int, DailyQuestRecord> = HashMap()
     ...
     */
    fun findById(key: Int): DailyQuestRecord? = byId[key]
     ...
     */
    fun getByIdOrThrow(key: Int): DailyQuestRecord =
     ...
     */
    operator fun get(key: Int): DailyQuestRecord? = findById(key)
     ...
     */
    val entries: Sequence<Pair<Int, DailyQuestRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<DailyQuestRecord>(count)
        val loadedById = HashMap<Int, DailyQuestRecord>(count * 2)

        repeat(count) { loaded.add(DailyQuestRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "DailyQuest.Id", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "DailyQuest.Id")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "DailyQuest.QuestId", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "DailyQuest.QuestId")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                3 -> {
                    checkColumn(column, "DailyQuest.Points", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "DailyQuest.Points")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                4 -> {
                    checkColumn(column, "DailyQuest.ResetHour", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "DailyQuest.ResetHour")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
