...
class AfkBonusTable : Iterable<AfkBonusRecord> {
     ...
     */
    var records: MutableList<AfkBonusRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<AfkBonusRecord> = records.iterator()

    private var byId: HashMap<Int, AfkBonusRecord> = HashMap()
     ...
     */
    fun findById(key: Int): AfkBonusRecord? = byId[key]
     ...
     */
    fun getByIdOrThrow(key: Int): AfkBonusRecord =
     ...
     */
    operator fun get(key: Int): AfkBonusRecord? = findById(key)
     ...
     */
    val entries: Sequence<Pair<Int, AfkBonusRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<AfkBonusRecord>(count)
        val loadedById = HashMap<Int, AfkBonusRecord>(count * 2)

        repeat(count) { loaded.add(AfkBonusRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "AfkBonus.Id", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "AfkBonus.Id")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "AfkBonus.Hours", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "AfkBonus.Hours")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                3 -> {
                    checkColumn(column, "AfkBonus.Bonus", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "AfkBonus.Bonus")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
