...
class GachaOddsTable : Iterable<GachaOddsRecord> {
     ...
     */
    var records: MutableList<GachaOddsRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<GachaOddsRecord> = records.iterator()

    private var byId: HashMap<Int, GachaOddsRecord> = HashMap()
     ...
     */
    fun findById(key: Int): GachaOddsRecord? = byId[key]
     ...
     */
    fun getByIdOrThrow(key: Int): GachaOddsRecord =
     ...
     */
    operator fun get(key: Int): GachaOddsRecord? = findById(key)
     ...
     */
    val entries: Sequence<Pair<Int, GachaOddsRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<GachaOddsRecord>(count)
        val loadedById = HashMap<Int, GachaOddsRecord>(count * 2)

        repeat(count) { loaded.add(GachaOddsRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "GachaOdds.Id", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "GachaOdds.Id")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "GachaOdds.GachaId", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "GachaOdds.GachaId")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                3 -> {
                    checkColumn(column, "GachaOdds.Grade", KIND_SCALAR, false, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "GachaOdds.Grade")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                4 -> {
                    checkColumn(column, "GachaOdds.Rate", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "GachaOdds.Rate")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
