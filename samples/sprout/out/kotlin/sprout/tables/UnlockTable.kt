...
class UnlockRecord {
    ...
    var id: Int = 0
    /** unlock kind */
    var type: UnlockType = UnlockType.of(0)
    /** condition */
...
/** Every row of Unlock. */
class UnlockTable : Iterable<UnlockRecord> {
     ...
     */
    var records: MutableList<UnlockRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<UnlockRecord> = records.iterator()

    private var byId: HashMap<Int, UnlockRecord> = HashMap()
     ...
     */
    fun findById(key: Int): UnlockRecord? = byId[key]
     ...
     */
    fun getByIdOrThrow(key: Int): UnlockRecord =
     ...
     */
    operator fun get(key: Int): UnlockRecord? = findById(key)
     ...
     */
    val entries: Sequence<Pair<Int, UnlockRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<UnlockRecord>(count)
        val loadedById = HashMap<Int, UnlockRecord>(count * 2)

        repeat(count) { loaded.add(UnlockRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "Unlock.Id", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Unlock.Id")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "Unlock.Type", KIND_SCALAR, false, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Unlock.Type")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                3 -> {
                    checkColumn(column, "Unlock.ConditionId", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Unlock.ConditionId")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                4 -> {
                    checkColumn(column, "Unlock.MessageKey", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "Unlock.MessageKey")
                    var at = 0
                    while (at < count) {
