...
class SettingTable : Iterable<SettingRecord> {
     ...
     */
    var records: MutableList<SettingRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<SettingRecord> = records.iterator()

    private var bySettingKey: HashMap<String, SettingRecord> = HashMap()
     ...
     */
    fun findBySettingKey(key: String): SettingRecord? = bySettingKey[key]
     ...
     */
    fun getBySettingKeyOrThrow(key: String): SettingRecord =
        bySettingKey[key] ?: throw RecordNotFoundException(
    ...
    fun containsSettingKey(key: String): Boolean = bySettingKey.containsKey(key)


    /**
     * The row with this SettingKey, or null when the table has none.
     *
     * findBySettingKey under the spelling Kotlin uses for a keyed collection. It
     ...
     */
    operator fun get(key: String): SettingRecord? = findBySettingKey(key)
     ...
     */
    val entries: Sequence<Pair<String, SettingRecord>>
        get() = records.asSequence().map { record -> record.settingKey to record }
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<SettingRecord>(count)
        val loadedBySettingKey = HashMap<String, SettingRecord>(count * 2)

        repeat(count) { loaded.add(SettingRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "Setting.SettingKey", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "Setting.SettingKey")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "Setting.Value", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "Setting.Value")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameString(count - at)
                        while (n > 0) {
                ...
                3 -> {
                    checkColumn(column, "Setting.DescKey", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "Setting.DescKey")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameString(count - at)
                        while (n > 0) {
