...
class CutsceneRecord {
    ...
    var skipAfterMs: Int = 0

}

/** Every row of Cutscene. */
class CutsceneTable : Iterable<CutsceneRecord> {
     ...
     */
    var records: MutableList<CutsceneRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<CutsceneRecord> = records.iterator()

    private var byId: HashMap<Int, CutsceneRecord> = HashMap()
     ...
     */
    fun findById(key: Int): CutsceneRecord? = byId[key]
     ...
     */
    fun getByIdOrThrow(key: Int): CutsceneRecord =
     ...
     */
    operator fun get(key: Int): CutsceneRecord? = findById(key)
     ...
     */
    val entries: Sequence<Pair<Int, CutsceneRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<CutsceneRecord>(count)
        val loadedById = HashMap<Int, CutsceneRecord>(count * 2)

        repeat(count) { loaded.add(CutsceneRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "Cutscene.Id", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Cutscene.Id")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "Cutscene.Timeline", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "Cutscene.Timeline")
                    var at = 0
                    while (at < count) {
                ...
                3 -> {
                    checkColumn(column, "Cutscene.LengthMs", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Cutscene.LengthMs")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                4 -> {
                    checkColumn(column, "Cutscene.SkipAfterMs", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Cutscene.SkipAfterMs")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
