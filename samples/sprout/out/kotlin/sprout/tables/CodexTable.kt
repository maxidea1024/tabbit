...
class CodexRecord {
    ...
    var id: Int = 0
    /** collection kind */
    var type: CollectionType = CollectionType.of(0)
    /** name key */
    var nameKey: String = ""
    /** icon asset */
    var icon: String = ""
...
/** Every row of Codex. */
class CodexTable : Iterable<CodexRecord> {
     ...
     */
    var records: MutableList<CodexRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<CodexRecord> = records.iterator()

    private var byId: HashMap<Int, CodexRecord> = HashMap()
     ...
     */
    fun findById(key: Int): CodexRecord? = byId[key]
     ...
     */
    fun getByIdOrThrow(key: Int): CodexRecord =
     ...
     */
    operator fun get(key: Int): CodexRecord? = findById(key)
     ...
     */
    val entries: Sequence<Pair<Int, CodexRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<CodexRecord>(count)
        val loadedById = HashMap<Int, CodexRecord>(count * 2)

        repeat(count) { loaded.add(CodexRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "Codex.Id", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Codex.Id")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "Codex.Type", KIND_SCALAR, false, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Codex.Type")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                3 -> {
                    checkColumn(column, "Codex.NameKey", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "Codex.NameKey")
                    var at = 0
                    while (at < count) {
                ...
                4 -> {
                    checkColumn(column, "Codex.Icon", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "Codex.Icon")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameString(count - at)
                        while (n > 0) {
                ...
                5 -> {
                    checkColumn(column, "Codex.RewardId", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Codex.RewardId")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
