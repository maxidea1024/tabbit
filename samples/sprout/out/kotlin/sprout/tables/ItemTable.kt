...
class ItemRecord {
    ...
    var nameKey: String = ""
    /** item kind */
    var type: ItemType = ItemType.of(0)
    /** grade */
    var grade: GradeType = GradeType.of(0)
    /** icon asset */
    var icon: String = ""
    /** stack ceiling */
    var stackLimit: Int = 0
    /** sell price */
    var sellPrice: Int = 0
    /** tradable */
...
/** Every row of Item. */
class ItemTable : Iterable<ItemRecord> {
     ...
     */
    var records: MutableList<ItemRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<ItemRecord> = records.iterator()

    private var byId: HashMap<Int, ItemRecord> = HashMap()
     ...
     */
    fun findById(key: Int): ItemRecord? = byId[key]
     ...
     */
    fun getByIdOrThrow(key: Int): ItemRecord =
     ...
     */
    operator fun get(key: Int): ItemRecord? = findById(key)
     ...
     */
    val entries: Sequence<Pair<Int, ItemRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<ItemRecord>(count)
        val loadedById = HashMap<Int, ItemRecord>(count * 2)

        repeat(count) { loaded.add(ItemRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "Item.Id", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Item.Id")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "Item.NameKey", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "Item.NameKey")
                    var at = 0
                    while (at < count) {
                ...
                3 -> {
                    checkColumn(column, "Item.Type", KIND_SCALAR, false, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Item.Type")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                4 -> {
                    checkColumn(column, "Item.Grade", KIND_SCALAR, false, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Item.Grade")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                5 -> {
                    checkColumn(column, "Item.Icon", KIND_SCALAR, false, ELEMENT_STRING)
                    val cursor = ColumnCursor(reader, column, count, "Item.Icon")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameString(count - at)
                        while (n > 0) {
                ...
                6 -> {
                    checkColumn(column, "Item.StackLimit", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Item.StackLimit")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                7 -> {
                    checkColumn(column, "Item.SellPrice", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Item.SellPrice")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                8 -> {
                    checkColumn(column, "Item.Tradable", KIND_SCALAR, false, ELEMENT_BOOL)
                    val cursor = ColumnCursor(reader, column, count, "Item.Tradable")
