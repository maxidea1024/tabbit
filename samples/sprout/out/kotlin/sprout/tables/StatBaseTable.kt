...
class StatBaseTable : Iterable<StatBaseRecord> {
     ...
     */
    var records: MutableList<StatBaseRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<StatBaseRecord> = records.iterator()

    private var byId: HashMap<Int, StatBaseRecord> = HashMap()
     ...
     */
    fun findById(key: Int): StatBaseRecord? = byId[key]
     ...
     */
    fun getByIdOrThrow(key: Int): StatBaseRecord =
     ...
     */
    operator fun get(key: Int): StatBaseRecord? = findById(key)
     ...
     */
    val entries: Sequence<Pair<Int, StatBaseRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<StatBaseRecord>(count)
        val loadedById = HashMap<Int, StatBaseRecord>(count * 2)

        repeat(count) { loaded.add(StatBaseRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "StatBase.Id", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "StatBase.Id")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "StatBase.Role", KIND_SCALAR, false, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "StatBase.Role")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                3 -> {
                    checkColumn(column, "StatBase.Stat", KIND_SCALAR, false, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "StatBase.Stat")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                4 -> {
                    checkColumn(column, "StatBase.BaseValue", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "StatBase.BaseValue")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
