...
class WeeklyQuestRecord {
    ...
    var resetWeekday: Int = 0

}

/** Every row of WeeklyQuest. */
class WeeklyQuestTable : Iterable<WeeklyQuestRecord> {
     ...
     */
    var records: MutableList<WeeklyQuestRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<WeeklyQuestRecord> = records.iterator()

    private var byId: HashMap<Int, WeeklyQuestRecord> = HashMap()
     ...
     */
    fun findById(key: Int): WeeklyQuestRecord? = byId[key]
     ...
     */
    fun getByIdOrThrow(key: Int): WeeklyQuestRecord =
     ...
     */
    operator fun get(key: Int): WeeklyQuestRecord? = findById(key)
     ...
     */
    val entries: Sequence<Pair<Int, WeeklyQuestRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<WeeklyQuestRecord>(count)
        val loadedById = HashMap<Int, WeeklyQuestRecord>(count * 2)

        repeat(count) { loaded.add(WeeklyQuestRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "WeeklyQuest.Id", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "WeeklyQuest.Id")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "WeeklyQuest.QuestId", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "WeeklyQuest.QuestId")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                3 -> {
                    checkColumn(column, "WeeklyQuest.Points", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "WeeklyQuest.Points")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                4 -> {
                    checkColumn(column, "WeeklyQuest.ResetWeekday", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "WeeklyQuest.ResetWeekday")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
