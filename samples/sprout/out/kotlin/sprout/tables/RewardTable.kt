...
class RewardRecord {
    ...
    /** fixed item counts */
    var itemCounts: MutableList<Int> = ArrayList()

}

/** Every row of Reward. */
class RewardTable : Iterable<RewardRecord> {
     ...
     */
    var records: MutableList<RewardRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<RewardRecord> = records.iterator()

    private var byId: HashMap<Int, RewardRecord> = HashMap()
     ...
     */
    fun findById(key: Int): RewardRecord? = byId[key]
     ...
     */
    fun getByIdOrThrow(key: Int): RewardRecord =
     ...
     */
    operator fun get(key: Int): RewardRecord? = findById(key)
     ...
     */
    val entries: Sequence<Pair<Int, RewardRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<RewardRecord>(count)
        val loadedById = HashMap<Int, RewardRecord>(count * 2)

        repeat(count) { loaded.add(RewardRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "Reward.Id", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Reward.Id")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "Reward.Type", KIND_SCALAR, false, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Reward.Type")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                3 -> {
                    checkColumn(column, "Reward.LootGroupId", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Reward.LootGroupId")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                4 -> {
                    checkColumn(column, "Reward.ItemIds", KIND_ARRAY, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Reward.ItemIds")
                    for (record in loaded) {
                        val elementCount = cursor.nextLength()
                        record.itemIds = ArrayList(elementCount.coerceAtLeast(0))
                        repeat(elementCount) { record.itemIds.add(cursor.nextI32()) }
                    }
                }
                5 -> {
                    checkColumn(column, "Reward.ItemCounts", KIND_ARRAY, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Reward.ItemCounts")
                    for (record in loaded) {
                        val elementCount = cursor.nextLength()
                        record.itemCounts = ArrayList(elementCount.coerceAtLeast(0))
                        repeat(elementCount) { record.itemCounts.add(cursor.nextI32()) }
                    }
                }
                else ->
