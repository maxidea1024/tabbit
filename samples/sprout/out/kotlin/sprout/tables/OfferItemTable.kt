...
class OfferItemTable : Iterable<OfferItemRecord> {
     ...
     */
    var records: MutableList<OfferItemRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<OfferItemRecord> = records.iterator()

    private var byId: HashMap<Int, OfferItemRecord> = HashMap()
     ...
     */
    fun findById(key: Int): OfferItemRecord? = byId[key]
     ...
     */
    fun getByIdOrThrow(key: Int): OfferItemRecord =
     ...
     */
    operator fun get(key: Int): OfferItemRecord? = findById(key)
     ...
     */
    val entries: Sequence<Pair<Int, OfferItemRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<OfferItemRecord>(count)
        val loadedById = HashMap<Int, OfferItemRecord>(count * 2)

        repeat(count) { loaded.add(OfferItemRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "OfferItem.Id", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "OfferItem.Id")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "OfferItem.OfferId", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "OfferItem.OfferId")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                3 -> {
                    checkColumn(column, "OfferItem.ItemId", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "OfferItem.ItemId")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                4 -> {
                    checkColumn(column, "OfferItem.Count", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "OfferItem.Count")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
