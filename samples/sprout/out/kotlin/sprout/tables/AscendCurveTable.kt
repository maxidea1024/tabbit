...
class AscendCurveTable : Iterable<AscendCurveRecord> {
     ...
     */
    var records: MutableList<AscendCurveRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<AscendCurveRecord> = records.iterator()

    private var byId: HashMap<Int, AscendCurveRecord> = HashMap()
     ...
     */
    fun findById(key: Int): AscendCurveRecord? = byId[key]
     ...
     */
    fun getByIdOrThrow(key: Int): AscendCurveRecord =
     ...
     */
    operator fun get(key: Int): AscendCurveRecord? = findById(key)
     ...
     */
    val entries: Sequence<Pair<Int, AscendCurveRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<AscendCurveRecord>(count)
        val loadedById = HashMap<Int, AscendCurveRecord>(count * 2)

        repeat(count) { loaded.add(AscendCurveRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "AscendCurve.Id", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "AscendCurve.Id")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "AscendCurve.Grade", KIND_SCALAR, false, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "AscendCurve.Grade")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                3 -> {
                    checkColumn(column, "AscendCurve.Step", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "AscendCurve.Step")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                4 -> {
                    checkColumn(column, "AscendCurve.Scale", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "AscendCurve.Scale")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
