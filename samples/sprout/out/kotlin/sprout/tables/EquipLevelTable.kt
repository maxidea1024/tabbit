...
class EquipLevelTable : Iterable<EquipLevelRecord> {
     ...
     */
    var records: MutableList<EquipLevelRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<EquipLevelRecord> = records.iterator()

    private var byId: HashMap<Int, EquipLevelRecord> = HashMap()
     ...
     */
    fun findById(key: Int): EquipLevelRecord? = byId[key]
     ...
     */
    fun getByIdOrThrow(key: Int): EquipLevelRecord =
     ...
     */
    operator fun get(key: Int): EquipLevelRecord? = findById(key)
     ...
     */
    val entries: Sequence<Pair<Int, EquipLevelRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<EquipLevelRecord>(count)
        val loadedById = HashMap<Int, EquipLevelRecord>(count * 2)

        repeat(count) { loaded.add(EquipLevelRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "EquipLevel.Id", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "EquipLevel.Id")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "EquipLevel.Grade", KIND_SCALAR, false, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "EquipLevel.Grade")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                3 -> {
                    checkColumn(column, "EquipLevel.Level", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "EquipLevel.Level")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                4 -> {
                    checkColumn(column, "EquipLevel.StatScale", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "EquipLevel.StatScale")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                5 -> {
                    checkColumn(column, "EquipLevel.Cost", KIND_SCALAR, false, ELEMENT_I64, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "EquipLevel.Cost")
