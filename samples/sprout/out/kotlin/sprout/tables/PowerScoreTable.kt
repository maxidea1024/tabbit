...
class PowerScoreTable : Iterable<PowerScoreRecord> {
     ...
     */
    var records: MutableList<PowerScoreRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<PowerScoreRecord> = records.iterator()

    private var byId: HashMap<Int, PowerScoreRecord> = HashMap()
     ...
     */
    fun findById(key: Int): PowerScoreRecord? = byId[key]
     ...
     */
    fun getByIdOrThrow(key: Int): PowerScoreRecord =
     ...
     */
    operator fun get(key: Int): PowerScoreRecord? = findById(key)
     ...
     */
    val entries: Sequence<Pair<Int, PowerScoreRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<PowerScoreRecord>(count)
        val loadedById = HashMap<Int, PowerScoreRecord>(count * 2)

        repeat(count) { loaded.add(PowerScoreRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "PowerScore.Id", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "PowerScore.Id")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "PowerScore.Stat", KIND_SCALAR, false, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "PowerScore.Stat")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                3 -> {
                    checkColumn(column, "PowerScore.Weight", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "PowerScore.Weight")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
