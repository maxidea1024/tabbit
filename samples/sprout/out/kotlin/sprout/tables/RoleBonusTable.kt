...
class RoleBonusTable : Iterable<RoleBonusRecord> {
     ...
     */
    var records: MutableList<RoleBonusRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<RoleBonusRecord> = records.iterator()

    private var byId: HashMap<Int, RoleBonusRecord> = HashMap()
     ...
     */
    fun findById(key: Int): RoleBonusRecord? = byId[key]
     ...
     */
    fun getByIdOrThrow(key: Int): RoleBonusRecord =
     ...
     */
    operator fun get(key: Int): RoleBonusRecord? = findById(key)
     ...
     */
    val entries: Sequence<Pair<Int, RoleBonusRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<RoleBonusRecord>(count)
        val loadedById = HashMap<Int, RoleBonusRecord>(count * 2)

        repeat(count) { loaded.add(RoleBonusRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "RoleBonus.Id", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "RoleBonus.Id")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "RoleBonus.Role", KIND_SCALAR, false, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "RoleBonus.Role")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                3 -> {
                    checkColumn(column, "RoleBonus.Stat", KIND_SCALAR, false, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "RoleBonus.Stat")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                4 -> {
                    checkColumn(column, "RoleBonus.Bonus", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "RoleBonus.Bonus")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
