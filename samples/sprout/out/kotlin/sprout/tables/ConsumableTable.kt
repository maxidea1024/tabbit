...
class ConsumableTable : Iterable<ConsumableRecord> {
     ...
     */
    var records: MutableList<ConsumableRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<ConsumableRecord> = records.iterator()

    private var byId: HashMap<Int, ConsumableRecord> = HashMap()
     ...
     */
    fun findById(key: Int): ConsumableRecord? = byId[key]
     ...
     */
    fun getByIdOrThrow(key: Int): ConsumableRecord =
     ...
     */
    operator fun get(key: Int): ConsumableRecord? = findById(key)
     ...
     */
    val entries: Sequence<Pair<Int, ConsumableRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<ConsumableRecord>(count)
        val loadedById = HashMap<Int, ConsumableRecord>(count * 2)

        repeat(count) { loaded.add(ConsumableRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "Consumable.Id", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Consumable.Id")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "Consumable.ItemId", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Consumable.ItemId")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                3 -> {
                    checkColumn(column, "Consumable.BuffId", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Consumable.BuffId")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                4 -> {
                    checkColumn(column, "Consumable.Cooldown", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "Consumable.Cooldown")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
