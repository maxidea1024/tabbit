...
class StageWaveRecord {
    ...
    var waveOrder: Int = 0
    /** monster ids */
    var monsterIds: MutableList<Int> = ArrayList()
    /** monster levels */
    var monsterLevels: MutableList<Int> = ArrayList()

}

/** Every row of StageWave. */
class StageWaveTable : Iterable<StageWaveRecord> {
     ...
     */
    var records: MutableList<StageWaveRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<StageWaveRecord> = records.iterator()

    private var byId: HashMap<Int, StageWaveRecord> = HashMap()
     ...
     */
    fun findById(key: Int): StageWaveRecord? = byId[key]
     ...
     */
    fun getByIdOrThrow(key: Int): StageWaveRecord =
     ...
     */
    operator fun get(key: Int): StageWaveRecord? = findById(key)
     ...
     */
    val entries: Sequence<Pair<Int, StageWaveRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<StageWaveRecord>(count)
        val loadedById = HashMap<Int, StageWaveRecord>(count * 2)

        repeat(count) { loaded.add(StageWaveRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "StageWave.Id", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "StageWave.Id")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "StageWave.StageId", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "StageWave.StageId")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                3 -> {
                    checkColumn(column, "StageWave.WaveOrder", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "StageWave.WaveOrder")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                4 -> {
                    checkColumn(column, "StageWave.MonsterIds", KIND_ARRAY, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "StageWave.MonsterIds")
                    for (record in loaded) {
                        val elementCount = cursor.nextLength()
                        record.monsterIds = ArrayList(elementCount.coerceAtLeast(0))
                        repeat(elementCount) { record.monsterIds.add(cursor.nextI32()) }
                    }
                }
                5 -> {
                    checkColumn(column, "StageWave.MonsterLevels", KIND_ARRAY, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "StageWave.MonsterLevels")
                    for (record in loaded) {
                        val elementCount = cursor.nextLength()
                        record.monsterLevels = ArrayList(elementCount.coerceAtLeast(0))
                        repeat(elementCount) { record.monsterLevels.add(cursor.nextI32()) }
                    }
                }
                else ->
