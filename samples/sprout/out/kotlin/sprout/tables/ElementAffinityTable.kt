...
class ElementAffinityRecord {
    ...
    var id: Int = 0
    /** attacking element */
    var attacker: ElementType = ElementType.of(0)
    /** defending element */
    var defender: ElementType = ElementType.of(0)
...
class ElementAffinityTable : Iterable<ElementAffinityRecord> {
     ...
     */
    var records: MutableList<ElementAffinityRecord> = ArrayList()
        private set
     ...
     */
    override fun iterator(): Iterator<ElementAffinityRecord> = records.iterator()

    private var byId: HashMap<Int, ElementAffinityRecord> = HashMap()
     ...
     */
    fun findById(key: Int): ElementAffinityRecord? = byId[key]
     ...
     */
    fun getByIdOrThrow(key: Int): ElementAffinityRecord =
     ...
     */
    operator fun get(key: Int): ElementAffinityRecord? = findById(key)
     ...
     */
    val entries: Sequence<Pair<Int, ElementAffinityRecord>>
    ...
    fun read(filename: String) {
        ...
        val loaded = ArrayList<ElementAffinityRecord>(count)
        val loadedById = HashMap<Int, ElementAffinityRecord>(count * 2)

        repeat(count) { loaded.add(ElementAffinityRecord()) }
        ...
        for (column in header.columns) {
            ...
            when (column.tag) {
                1 -> {
                    checkColumn(column, "ElementAffinity.Id", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "ElementAffinity.Id")
                    var at = 0
                    while (at < count) {
                ...
                2 -> {
                    checkColumn(column, "ElementAffinity.Attacker", KIND_SCALAR, false, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "ElementAffinity.Attacker")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                3 -> {
                    checkColumn(column, "ElementAffinity.Defender", KIND_SCALAR, false, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "ElementAffinity.Defender")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
                ...
                4 -> {
                    checkColumn(column, "ElementAffinity.Rate", KIND_SCALAR, false, ELEMENT_I32, ELEMENT_VARINT)
                    val cursor = ColumnCursor(reader, column, count, "ElementAffinity.Rate")
                    var at = 0
                    while (at < count) {
                        var n = cursor.nextSameI32(count - at)
                        while (n > 0) {
