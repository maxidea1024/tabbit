...
public final class AfkBonusTable: Sequence {
    ...
    public func makeIterator() -> Array<AfkBonusRecord>.Iterator {
    ...
    public private(set) var records: [AfkBonusRecord] = []

    private var byId: [Int32: AfkBonusRecord] = [:]

    /// The row with this Id, or nil when the table has none.
    ///
    /// The lookup to reach for when a missing row is an ordinary answer - an optional
    /// reference, a key that came from user input.
    public func findById(_ key: Int32) -> AfkBonusRecord? {
        byId[key]
    }
    ...
    public func getByIdOrThrow(_ key: Int32) throws -> AfkBonusRecord {
        guard let record = byId[key] else {
            throw RecordNotFoundError(
                "there is no record in table `AfkBonus` that corresponds to "
                + "field `Id` value \(key)")
        }
    ...
    public subscript(_ key: Int32) -> AfkBonusRecord? {
        findById(key)
    }
    ...
    /// row with.
    public var entries: LazyMapSequence<[AfkBonusRecord], (key: Int32, row: AfkBonusRecord)> {
    ...
    public func read(
        ...
        var loaded = [AfkBonusRecord]()
        loaded.reserveCapacity(count)

        for _ in 0 ..< count { loaded.append(AfkBonusRecord()) }
        var loadedById = [Int32: AfkBonusRecord](
        ...
        for column in header.columns {
            ...
            switch column.tag {
            case 1:
                try Tcb.checkColumn(column, "AfkBonus.Id", Tcb.kindScalar, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "AfkBonus.Id")
                var at = 0
                while at < count {
            ...
            case 2:
                try Tcb.checkColumn(column, "AfkBonus.Hours", Tcb.kindScalar, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "AfkBonus.Hours")
                var at = 0
                while at < count {
                    var n = try cursor.nextSameI32(count - at)
                    while n > 0 {
                        let i = at
                        loaded[i].hours = cursor.runSameValue
                        at += 1
                        n -= 1
                    }
                }
            case 3:
                try Tcb.checkColumn(column, "AfkBonus.Bonus", Tcb.kindScalar, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "AfkBonus.Bonus")
                var at = 0
                while at < count {
                    var n = try cursor.nextSameI32(count - at)
                    while n > 0 {
