...
public final class PityRecord {
    ...
    /// resets on hit
    public var resetOnHit: Bool = false
}

/// Every row of Pity.
public final class PityTable: Sequence {
    ...
    public func makeIterator() -> Array<PityRecord>.Iterator {
    ...
    public private(set) var records: [PityRecord] = []

    private var byId: [Int32: PityRecord] = [:]

    /// The row with this Id, or nil when the table has none.
    ///
    /// The lookup to reach for when a missing row is an ordinary answer - an optional
    /// reference, a key that came from user input.
    public func findById(_ key: Int32) -> PityRecord? {
        byId[key]
    }
    ...
    public func getByIdOrThrow(_ key: Int32) throws -> PityRecord {
        guard let record = byId[key] else {
            throw RecordNotFoundError(
                "there is no record in table `Pity` that corresponds to "
                + "field `Id` value \(key)")
        }
    ...
    public subscript(_ key: Int32) -> PityRecord? {
        findById(key)
    }
    ...
    /// row with.
    public var entries: LazyMapSequence<[PityRecord], (key: Int32, row: PityRecord)> {
    ...
    public func read(
        ...
        var loaded = [PityRecord]()
        loaded.reserveCapacity(count)

        for _ in 0 ..< count { loaded.append(PityRecord()) }
        var loadedById = [Int32: PityRecord](
        ...
        for column in header.columns {
            ...
            switch column.tag {
            case 1:
                try Tcb.checkColumn(column, "Pity.Id", Tcb.kindScalar, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Pity.Id")
                var at = 0
                while at < count {
            ...
            case 2:
                try Tcb.checkColumn(column, "Pity.Threshold", Tcb.kindScalar, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Pity.Threshold")
                var at = 0
                while at < count {
                    var n = try cursor.nextSameI32(count - at)
                    while n > 0 {
                        let i = at
                        loaded[i].threshold = cursor.runSameValue
                        at += 1
                        n -= 1
                    }
                }
            case 3:
                try Tcb.checkColumn(column, "Pity.Grade", Tcb.kindScalar, false, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Pity.Grade")
                var at = 0
                while at < count {
                    var n = try cursor.nextSameI32(count - at)
                    while n > 0 {
            ...
            case 4:
                try Tcb.checkColumn(column, "Pity.ResetOnHit", Tcb.kindScalar, false, Tcb.elementBool)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Pity.ResetOnHit")
                for record in loaded {
                    record.resetOnHit = try cursor.nextBool()
                }
            default:
