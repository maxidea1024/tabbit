...
/// Every row of Table.
public final class TableTable: Sequence {
    ...
    public func makeIterator() -> Array<TableRecord>.Iterator {
    ...
    public private(set) var records: [TableRecord] = []

    private var byId: [Int32: TableRecord] = [:]

    /// The row with this Id, or nil when the table has none.
    ///
    /// The lookup to reach for when a missing row is an ordinary answer - an optional
    /// reference, a key that came from user input.
    public func findById(_ key: Int32) -> TableRecord? {
        byId[key]
    }
    ...
    public func getByIdOrThrow(_ key: Int32) throws -> TableRecord {
    ...
    public subscript(_ key: Int32) -> TableRecord? {
        findById(key)
    }
    ...
    /// row with.
    public var entries: LazyMapSequence<[TableRecord], (key: Int32, row: TableRecord)> {
    ...
    public func read(
        ...
        var loaded = [TableRecord]()
        loaded.reserveCapacity(count)

        for _ in 0 ..< count { loaded.append(TableRecord()) }
        var loadedById = [Int32: TableRecord](
        ...
        for column in header.columns {
            ...
            switch column.tag {
            case 1:
                try Tcb.checkColumn(column, "Table.Id", Tcb.kindScalar, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Table.Id")
                var at = 0
                while at < count {
            ...
            case 2:
                try Tcb.checkColumn(column, "Table.NameKey", Tcb.kindScalar, false, Tcb.elementString)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Table.NameKey")
                var at = 0
                while at < count {
            ...
            case 3:
                try Tcb.checkColumn(column, "Table.Value", Tcb.kindScalar, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Table.Value")
                var at = 0
                while at < count {
                    var n = try cursor.nextSameI32(count - at)
                    while n > 0 {
