...
public final class PowerScoreTable: Sequence {
    ...
    public func makeIterator() -> Array<PowerScoreRecord>.Iterator {
    ...
    public private(set) var records: [PowerScoreRecord] = []

    private var byId: [Int32: PowerScoreRecord] = [:]

    /// The row with this Id, or nil when the table has none.
    ///
    /// The lookup to reach for when a missing row is an ordinary answer - an optional
    /// reference, a key that came from user input.
    public func findById(_ key: Int32) -> PowerScoreRecord? {
        byId[key]
    }
    ...
    public func getByIdOrThrow(_ key: Int32) throws -> PowerScoreRecord {
    ...
    public subscript(_ key: Int32) -> PowerScoreRecord? {
        findById(key)
    }
    ...
    /// row with.
    public var entries: LazyMapSequence<[PowerScoreRecord], (key: Int32, row: PowerScoreRecord)> {
    ...
    public func read(
        ...
        var loaded = [PowerScoreRecord]()
        loaded.reserveCapacity(count)

        for _ in 0 ..< count { loaded.append(PowerScoreRecord()) }
        var loadedById = [Int32: PowerScoreRecord](
        ...
        for column in header.columns {
            ...
            switch column.tag {
            case 1:
                try Tcb.checkColumn(column, "PowerScore.Id", Tcb.kindScalar, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "PowerScore.Id")
                var at = 0
                while at < count {
            ...
            case 2:
                try Tcb.checkColumn(column, "PowerScore.Stat", Tcb.kindScalar, false, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "PowerScore.Stat")
                var at = 0
                while at < count {
                    var n = try cursor.nextSameI32(count - at)
                    while n > 0 {
            ...
            case 3:
                try Tcb.checkColumn(column, "PowerScore.Weight", Tcb.kindScalar, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "PowerScore.Weight")
                var at = 0
                while at < count {
                    var n = try cursor.nextSameI32(count - at)
                    while n > 0 {
