...
public final class DialogueTable: Sequence {
    ...
    public func makeIterator() -> Array<DialogueRecord>.Iterator {
    ...
    public private(set) var records: [DialogueRecord] = []

    private var byId: [Int32: DialogueRecord] = [:]

    /// The row with this Id, or nil when the table has none.
    ///
    /// The lookup to reach for when a missing row is an ordinary answer - an optional
    /// reference, a key that came from user input.
    public func findById(_ key: Int32) -> DialogueRecord? {
        byId[key]
    }
    ...
    public func getByIdOrThrow(_ key: Int32) throws -> DialogueRecord {
    ...
    public subscript(_ key: Int32) -> DialogueRecord? {
        findById(key)
    }
    ...
    /// row with.
    public var entries: LazyMapSequence<[DialogueRecord], (key: Int32, row: DialogueRecord)> {
    ...
    public func read(
        ...
        var loaded = [DialogueRecord]()
        loaded.reserveCapacity(count)

        for _ in 0 ..< count { loaded.append(DialogueRecord()) }
        var loadedById = [Int32: DialogueRecord](
        ...
        for column in header.columns {
            ...
            switch column.tag {
            case 1:
                try Tcb.checkColumn(column, "Dialogue.Id", Tcb.kindScalar, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Dialogue.Id")
                var at = 0
                while at < count {
            ...
            case 2:
                try Tcb.checkColumn(column, "Dialogue.SceneKey", Tcb.kindScalar, false, Tcb.elementString)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Dialogue.SceneKey")
                var at = 0
                while at < count {
                    var n = try cursor.nextSameString(count - at)
                    while n > 0 {
                        let i = at
                        loaded[i].sceneKey = cursor.runSameText
                        at += 1
                        n -= 1
                    }
                }
            case 3:
                try Tcb.checkColumn(column, "Dialogue.CutsceneId", Tcb.kindScalar, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Dialogue.CutsceneId")
                var at = 0
                while at < count {
                    var n = try cursor.nextSameI32(count - at)
                    while n > 0 {
                        let i = at
                        loaded[i].cutsceneId = cursor.runSameValue
                        at += 1
                        n -= 1
                    }
                }
            case 4:
                try Tcb.checkColumn(column, "Dialogue.Skippable", Tcb.kindScalar, false, Tcb.elementBool)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "Dialogue.Skippable")
                for record in loaded {
                    record.skippable = try cursor.nextBool()
                }
            default:
