...
public final class WorldRecord {
    ...
    /// theme colour
    public var themeColour: String = ""
}

/// Every row of World.
public final class WorldTable: Sequence {
    ...
    public func makeIterator() -> Array<WorldRecord>.Iterator {
    ...
    public private(set) var records: [WorldRecord] = []

    private var byId: [Int32: WorldRecord] = [:]

    /// The row with this Id, or nil when the table has none.
    ///
    /// The lookup to reach for when a missing row is an ordinary answer - an optional
    /// reference, a key that came from user input.
    public func findById(_ key: Int32) -> WorldRecord? {
        byId[key]
    }
    ...
    public func getByIdOrThrow(_ key: Int32) throws -> WorldRecord {
    ...
    public subscript(_ key: Int32) -> WorldRecord? {
        findById(key)
    }
    ...
    /// row with.
    public var entries: LazyMapSequence<[WorldRecord], (key: Int32, row: WorldRecord)> {
    ...
    public func read(
        ...
        var loaded = [WorldRecord]()
        loaded.reserveCapacity(count)

        for _ in 0 ..< count { loaded.append(WorldRecord()) }
        var loadedById = [Int32: WorldRecord](
        ...
        for column in header.columns {
            ...
            switch column.tag {
            case 1:
                try Tcb.checkColumn(column, "World.Id", Tcb.kindScalar, false, Tcb.elementI32, Tcb.elementVarint)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "World.Id")
                var at = 0
                while at < count {
            ...
            case 2:
                try Tcb.checkColumn(column, "World.NameKey", Tcb.kindScalar, false, Tcb.elementString)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "World.NameKey")
                var at = 0
                while at < count {
            ...
            case 3:
                try Tcb.checkColumn(column, "World.Map", Tcb.kindScalar, false, Tcb.elementString)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "World.Map")
                var at = 0
                while at < count {
                    var n = try cursor.nextSameString(count - at)
                    while n > 0 {
                        let i = at
                        loaded[i].map = cursor.runSameText
                        at += 1
                        n -= 1
                    }
                }
            case 4:
                try Tcb.checkColumn(column, "World.ThemeColour", Tcb.kindScalar, false, Tcb.elementString)
                let cursor = try Tcb.ColumnCursor(reader, column, count, "World.ThemeColour")
                var at = 0
                while at < count {
                    var n = try cursor.nextSameString(count - at)
                    while n > 0 {
                        let i = at
                        loaded[i].themeColour = cursor.runSameText
                        at += 1
                        n -= 1
                    }
                }
