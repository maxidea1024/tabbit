...
#ifndef TABBIT_GENERATED_TABLES_TABLE_H
#define TABBIT_GENERATED_TABLES_TABLE_H
...
class TableTable {
 public:
  ...
  typename std::vector<TableRecord>::const_iterator begin() const {
    return records_.begin();
  }

  typename std::vector<TableRecord>::const_iterator end() const {
    return records_.end();
  }
  ...
  const TableRecord* find_by_id(std::int32_t key) const {
  ...
  const TableRecord& get_by_id_or_throw(std::int32_t key) const {
    const TableRecord* found = find_by_id(key);

    if (found == nullptr) {
      throw tabbit::RecordNotFound(
        std::string("there is no record in table `Table` that corresponds to "
  ...
  void read(const std::string& filename) {
    ...
    for (const tabbit::Column& column : header.columns) {
      ...
      switch (column.tag) {
        case 1: {
          tabbit::check_column(column, "Table.Id", tabbit::kKindScalar, false, {tabbit::kElementI32, tabbit::kElementVarint});
          tabbit::TcbColumnCursor cursor(reader, column, header.row_count, "Table.Id");
          std::int32_t value{};
        ...
        case 2: {
          tabbit::check_column(column, "Table.NameKey", tabbit::kKindScalar, false, {tabbit::kElementString});
          tabbit::TcbColumnCursor cursor(reader, column, header.row_count, "Table.NameKey");
          std::string value{};
          for (std::size_t i = 0; i < row_count; ) {
        ...
        case 3: {
          tabbit::check_column(column, "Table.Value", tabbit::kKindScalar, false, {tabbit::kElementI32, tabbit::kElementVarint});
          tabbit::TcbColumnCursor cursor(reader, column, header.row_count, "Table.Value");
          std::int32_t value{};
          for (std::size_t i = 0; i < row_count; ) {
            std::int32_t n = cursor.next_same_i32(
                static_cast<std::int32_t>(row_count - i), value);
 ...
 private:
  ...
  void publish(std::vector<TableRecord>&& records) {
...
#endif  // TABBIT_GENERATED_TABLES_TABLE_H
