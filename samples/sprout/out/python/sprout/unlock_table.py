...
class UnlockRecord:
    ...
    def __init__(self):
        self.id = 0
        self.type_ = UnlockType(0)
        self.condition_id = 0
        self.message_key = ""

    def __repr__(self):
        return "UnlockRecord(id=%r, type_=%r, condition_id=%r, message_key=%r)" % (self.id, self.type_, self.condition_id, self.message_key)
    ...
    def read(self, filename):
        ...
        for column in columns:
            ...
            if column.tag == 1:
                ...
                cursor = tabbit.ColumnCursor(reader, column, count, "Unlock.Type")
                at = 0
                while at < count:
                    n, value = cursor.next_same_i32(count - at)
                    for i in range(at, at + n):
                        records[i].type_ = UnlockType(value)
                    at += n
            elif column.tag == 3:
                tabbit.check_column(column, "Unlock.ConditionId", tabbit.KIND_SCALAR, False, (tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT))
                cursor = tabbit.ColumnCursor(reader, column, count, "Unlock.ConditionId")
                at = 0
                while at < count:
                    n, value = cursor.next_same_i32(count - at)
                    for i in range(at, at + n):
                ...
                cursor = tabbit.ColumnCursor(reader, column, count, "Unlock.MessageKey")
                at = 0
                while at < count:
