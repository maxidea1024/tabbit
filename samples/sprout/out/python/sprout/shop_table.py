...
class ShopRecord:
    ...
    def __init__(self):
        self.id = 0
        self.type_ = ShopType(0)
        self.name_key = ""
        self.refresh_hours = 0
        self.free_refresh = 0

    def __repr__(self):
        return "ShopRecord(id=%r, type_=%r, name_key=%r, refresh_hours=%r, free_refresh=%r)" % (self.id, self.type_, self.name_key, self.refresh_hours, self.free_refresh)
    ...
    def read(self, filename):
        ...
        for column in columns:
            ...
            if column.tag == 1:
                ...
                cursor = tabbit.ColumnCursor(reader, column, count, "Shop.Type")
                at = 0
                while at < count:
                    n, value = cursor.next_same_i32(count - at)
                    for i in range(at, at + n):
                        records[i].type_ = ShopType(value)
                    at += n
            elif column.tag == 3:
                tabbit.check_column(column, "Shop.NameKey", tabbit.KIND_SCALAR, False, (tabbit.ELEMENT_STRING,))
                cursor = tabbit.ColumnCursor(reader, column, count, "Shop.NameKey")
                at = 0
                while at < count:
                ...
                cursor = tabbit.ColumnCursor(reader, column, count, "Shop.RefreshHours")
                at = 0
                while at < count:
                    n, value = cursor.next_same_i32(count - at)
                    for i in range(at, at + n):
                ...
                cursor = tabbit.ColumnCursor(reader, column, count, "Shop.FreeRefresh")
                at = 0
                while at < count:
                    n, value = cursor.next_same_i32(count - at)
                    for i in range(at, at + n):
