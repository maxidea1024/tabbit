...
class StageWaveRecord:
    ...
    def __repr__(self):
        return "StageWaveRecord(id=%r, stage_id=%r, wave_order=%r, monster_ids=%r, monster_levels=%r)" % (self.id, self.stage_id, self.wave_order, self.monster_ids, self.monster_levels)
    ...
    def read(self, filename):
        ...
        for column in columns:
            ...
            if column.tag == 1:
                ...
                cursor = tabbit.ColumnCursor(reader, column, count, "StageWave.StageId")
                at = 0
                while at < count:
                    n, value = cursor.next_same_i32(count - at)
                    for i in range(at, at + n):
                ...
                cursor = tabbit.ColumnCursor(reader, column, count, "StageWave.WaveOrder")
                at = 0
                while at < count:
                    n, value = cursor.next_same_i32(count - at)
                    for i in range(at, at + n):
                ...
                cursor = tabbit.ColumnCursor(reader, column, count, "StageWave.MonsterIds")
                for record in records:
                    element_count = cursor.next_length()
                    record.monster_ids = [cursor.next_i32() for _ in range(element_count)]
            elif column.tag == 5:
                tabbit.check_column(column, "StageWave.MonsterLevels", tabbit.KIND_ARRAY, False, (tabbit.ELEMENT_I32, tabbit.ELEMENT_VARINT))
                cursor = tabbit.ColumnCursor(reader, column, count, "StageWave.MonsterLevels")
                for record in records:
                    element_count = cursor.next_length()
                    record.monster_levels = [cursor.next_i32() for _ in range(element_count)]
            else:
