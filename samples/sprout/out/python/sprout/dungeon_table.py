...
class DungeonRecord:
    ...
    def __repr__(self):
        return "DungeonRecord(id=%r, type_=%r, name_key=%r, element=%r, daily_entries=%r, unlock_id=%r)" % (self.id, self.type_, self.name_key, self.element, self.daily_entries, self.unlock_id)
    ...
    def read(self, filename):
        ...
        for column in columns:
            ...
            if column.tag == 1:
                ...
                cursor = tabbit.ColumnCursor(reader, column, count, "Dungeon.Type")
                at = 0
                while at < count:
                    n, value = cursor.next_same_i32(count - at)
                    for i in range(at, at + n):
                        records[i].type_ = DungeonType(value)
                    at += n
            elif column.tag == 3:
                tabbit.check_column(column, "Dungeon.NameKey", tabbit.KIND_SCALAR, False, (tabbit.ELEMENT_STRING,))
                cursor = tabbit.ColumnCursor(reader, column, count, "Dungeon.NameKey")
                at = 0
                while at < count:
                ...
                cursor = tabbit.ColumnCursor(reader, column, count, "Dungeon.Element")
                at = 0
                while at < count:
                    n, value = cursor.next_same_i32(count - at)
                    for i in range(at, at + n):
                ...
                cursor = tabbit.ColumnCursor(reader, column, count, "Dungeon.DailyEntries")
                at = 0
                while at < count:
                    n, value = cursor.next_same_i32(count - at)
                    for i in range(at, at + n):
                ...
                cursor = tabbit.ColumnCursor(reader, column, count, "Dungeon.UnlockId")
                at = 0
                while at < count:
                    n, value = cursor.next_same_i32(count - at)
                    for i in range(at, at + n):
