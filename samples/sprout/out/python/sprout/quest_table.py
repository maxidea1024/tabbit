...
class QuestRecord:
    ...
    def __repr__(self):
        return "QuestRecord(id=%r, type_=%r, title_key=%r, summary_key=%r, reward_id=%r, unlock_id=%r, repeatable=%r)" % (self.id, self.type_, self.title_key, self.summary_key, self.reward_id, self.unlock_id, self.repeatable)
    ...
    def read(self, filename):
        ...
        for column in columns:
            ...
            if column.tag == 1:
                ...
                cursor = tabbit.ColumnCursor(reader, column, count, "Quest.Type")
                at = 0
                while at < count:
                    n, value = cursor.next_same_i32(count - at)
                    for i in range(at, at + n):
                        records[i].type_ = QuestType(value)
                    at += n
            elif column.tag == 3:
                tabbit.check_column(column, "Quest.TitleKey", tabbit.KIND_SCALAR, False, (tabbit.ELEMENT_STRING,))
                cursor = tabbit.ColumnCursor(reader, column, count, "Quest.TitleKey")
                at = 0
                while at < count:
                ...
                cursor = tabbit.ColumnCursor(reader, column, count, "Quest.SummaryKey")
                at = 0
                while at < count:
                    n, value = cursor.next_same_string(count - at)
                    for i in range(at, at + n):
                ...
                cursor = tabbit.ColumnCursor(reader, column, count, "Quest.RewardId")
                at = 0
                while at < count:
                    n, value = cursor.next_same_i32(count - at)
                    for i in range(at, at + n):
                ...
                cursor = tabbit.ColumnCursor(reader, column, count, "Quest.UnlockId")
                at = 0
                while at < count:
                    n, value = cursor.next_same_i32(count - at)
                    for i in range(at, at + n):
                ...
                cursor = tabbit.ColumnCursor(reader, column, count, "Quest.Repeatable")
