...
impl MaterialTable {
    ...
    /// error it returns.
    pub fn get_by_id_or_error(&self, key: i32) -> tabbit::Result<&MaterialRecord> {
        self.by_id
            .get(&key)
    ...
    pub fn entries(&self) -> impl Iterator<Item = (&i32, &MaterialRecord)> + '_ {
    ...
    pub fn read(&mut self, filename: &Path) -> tabbit::Result<()> {
        ...
        for column in &header.columns {
            ...
            match column.tag {
                1 => {
                    tabbit::check_column(column, "Material.Id", tabbit::KIND_SCALAR, false, &[tabbit::ELEMENT_I32, tabbit::ELEMENT_VARINT])?;
                    let mut cursor = tabbit::TcbColumnCursor::new(&mut reader, column, header.row_count, "Material.Id")?;
                    let mut at = 0usize;
                ...
                2 => {
                    tabbit::check_column(column, "Material.ItemId", tabbit::KIND_SCALAR, false, &[tabbit::ELEMENT_I32, tabbit::ELEMENT_VARINT])?;
                    let mut cursor = tabbit::TcbColumnCursor::new(&mut reader, column, header.row_count, "Material.ItemId")?;
                    let mut at = 0usize;
                    while at < records.len() {
                        let (n, value) = cursor.next_same_i32((records.len() - at) as i32)?;
                        for _ in 0..n {
                ...
                3 => {
                    tabbit::check_column(column, "Material.Tier", tabbit::KIND_SCALAR, false, &[tabbit::ELEMENT_I32, tabbit::ELEMENT_VARINT])?;
                    let mut cursor = tabbit::TcbColumnCursor::new(&mut reader, column, header.row_count, "Material.Tier")?;
                    let mut at = 0usize;
                    while at < records.len() {
                        let (n, value) = cursor.next_same_i32((records.len() - at) as i32)?;
                        for _ in 0..n {
                            records[at].tier = value;
                            at += 1;
                        }
                    }
                }
                4 => {
                    tabbit::check_column(column, "Material.Element", tabbit::KIND_SCALAR, false, &[tabbit::ELEMENT_VARINT])?;
                    let mut cursor = tabbit::TcbColumnCursor::new(&mut reader, column, header.row_count, "Material.Element")?;
                    let mut at = 0usize;
                    while at < records.len() {
                        let (n, value) = cursor.next_same_i32((records.len() - at) as i32)?;
                        for _ in 0..n {
...
impl<'a> IntoIterator for &'a MaterialTable {
    type Item = &'a MaterialRecord;
    type IntoIter = std::slice::Iter<'a, MaterialRecord>;
