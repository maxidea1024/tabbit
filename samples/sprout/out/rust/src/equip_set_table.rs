...
#[derive(Clone, Debug, Default)]
pub struct EquipSetTable {
    records: Vec<EquipSetRecord>,
    by_id: HashMap<i32, usize>,
}

impl EquipSetTable {
    /// Every row, in the order the sheet declared them.
    pub fn records(&self) -> &[EquipSetRecord] {
        &self.records
    }
    ...
    pub fn iter(&self) -> std::slice::Iter<'_, EquipSetRecord> {
        self.records.iter()
    }
    ...
    pub fn find_by_id(&self, key: i32) -> Option<&EquipSetRecord> {
    ...
    /// error it returns.
    pub fn get_by_id_or_error(&self, key: i32) -> tabbit::Result<&EquipSetRecord> {
        self.by_id
            .get(&key)
    ...
    pub fn entries(&self) -> impl Iterator<Item = (&i32, &EquipSetRecord)> + '_ {
    ...
    pub fn read(&mut self, filename: &Path) -> tabbit::Result<()> {
        ...
        let mut records = vec![EquipSetRecord::default(); count];
        ...
        for column in &header.columns {
            ...
            match column.tag {
                1 => {
                    tabbit::check_column(column, "EquipSet.Id", tabbit::KIND_SCALAR, false, &[tabbit::ELEMENT_I32, tabbit::ELEMENT_VARINT])?;
                    let mut cursor = tabbit::TcbColumnCursor::new(&mut reader, column, header.row_count, "EquipSet.Id")?;
                    let mut at = 0usize;
                ...
                2 => {
                    tabbit::check_column(column, "EquipSet.NameKey", tabbit::KIND_SCALAR, false, &[tabbit::ELEMENT_STRING])?;
                    let mut cursor = tabbit::TcbColumnCursor::new(&mut reader, column, header.row_count, "EquipSet.NameKey")?;
                    let mut at = 0usize;
                    while at < records.len() {
                ...
                3 => {
                    tabbit::check_column(column, "EquipSet.MemberIds", tabbit::KIND_ARRAY, false, &[tabbit::ELEMENT_I32, tabbit::ELEMENT_VARINT])?;
                    let mut cursor = tabbit::TcbColumnCursor::new(&mut reader, column, header.row_count, "EquipSet.MemberIds")?;
                    for record in records.iter_mut() {
                        let element_count = cursor.next_length()?.max(0) as usize;
                        record.member_ids = Vec::with_capacity(element_count.min(65536));
                        for _ in 0..element_count {
                            record.member_ids.push(cursor.next_i32()?);
                        }
                    }
                }
                _ => {
...
impl<'a> IntoIterator for &'a EquipSetTable {
    type Item = &'a EquipSetRecord;
    type IntoIter = std::slice::Iter<'a, EquipSetRecord>;
