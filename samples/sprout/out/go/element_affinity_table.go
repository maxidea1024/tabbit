...
func (t *ElementAffinityTable) Records() []ElementAffinityRecord { return t.records }
...
func (t *ElementAffinityTable) All() iter.Seq[*ElementAffinityRecord] {
	records := t.records

	return func(yield func(*ElementAffinityRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *ElementAffinityTable) FindById(key int32) *ElementAffinityRecord {
...
func (t *ElementAffinityTable) GetByIdOrError(key int32) (*ElementAffinityRecord, error) {
	position, found := t.byId[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `ElementAffinity` that corresponds to field `Id` value %v", key)
	}
...
func (t *ElementAffinityTable) ContainsId(key int32) bool {
	_, found := t.byId[key]
	return found
}
...
func (t *ElementAffinityTable) Entries() iter.Seq2[int32, *ElementAffinityRecord] {
	records := t.records

	return func(yield func(int32, *ElementAffinityRecord) bool) {
		for i := range records {
...
func (t *ElementAffinityTable) Read(filename string) error {
	...
	records := make([]ElementAffinityRecord, count)
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "ElementAffinity.Id", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "ElementAffinity.Id")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "ElementAffinity.Attacker", tabbit.KindScalar, false, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "ElementAffinity.Attacker")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 3:
			if tabbit.CheckColumn(reader, column, "ElementAffinity.Defender", tabbit.KindScalar, false, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "ElementAffinity.Defender")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 4:
			if tabbit.CheckColumn(reader, column, "ElementAffinity.Rate", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "ElementAffinity.Rate")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
	...
	if err := reader.Err(); err != nil {
		return fmt.Errorf("ElementAffinity: %w", err)
	}
