...
// DialogueTable holds every row of Dialogue.
...
func (t *DialogueTable) Records() []DialogueRecord { return t.records }

// Count returns how many rows the table holds.
func (t *DialogueTable) Count() int { return len(t.records) }
...
func (t *DialogueTable) All() iter.Seq[*DialogueRecord] {
	records := t.records

	return func(yield func(*DialogueRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *DialogueTable) FindById(key int32) *DialogueRecord {
...
func (t *DialogueTable) GetByIdOrError(key int32) (*DialogueRecord, error) {
	position, found := t.byId[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `Dialogue` that corresponds to field `Id` value %v", key)
	}
...
func (t *DialogueTable) ContainsId(key int32) bool {
	_, found := t.byId[key]
	return found
}
...
func (t *DialogueTable) Entries() iter.Seq2[int32, *DialogueRecord] {
	records := t.records

	return func(yield func(int32, *DialogueRecord) bool) {
		for i := range records {
...
func (t *DialogueTable) Read(filename string) error {
	...
	records := make([]DialogueRecord, count)
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "Dialogue.Id", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Dialogue.Id")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "Dialogue.SceneKey", tabbit.KindScalar, false, tabbit.ElementString) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Dialogue.SceneKey")
				for i := int32(0); i < count; {
		...
		case 3:
			if tabbit.CheckColumn(reader, column, "Dialogue.CutsceneId", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Dialogue.CutsceneId")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 4:
			if tabbit.CheckColumn(reader, column, "Dialogue.Skippable", tabbit.KindScalar, false, tabbit.ElementBool) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Dialogue.Skippable")
				for i := int32(0); i < count; i++ {
					r := &records[i]
					r.Skippable = cursor.NextBool()
				}
			}
		default:
