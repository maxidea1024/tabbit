...
// BuffPoolRecord was generated from samples/sprout/xlsx/Skill.xlsx : BuffPoolTable : A1.
type BuffPoolRecord struct {
	// primary index
	Id int32
	// candidate buffs
	BuffIds []int32
...
func (t *BuffPoolTable) Records() []BuffPoolRecord { return t.records }
...
func (t *BuffPoolTable) All() iter.Seq[*BuffPoolRecord] {
	records := t.records

	return func(yield func(*BuffPoolRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *BuffPoolTable) FindById(key int32) *BuffPoolRecord {
...
func (t *BuffPoolTable) GetByIdOrError(key int32) (*BuffPoolRecord, error) {
	position, found := t.byId[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `BuffPool` that corresponds to field `Id` value %v", key)
	}
...
func (t *BuffPoolTable) ContainsId(key int32) bool {
	_, found := t.byId[key]
	return found
}
...
func (t *BuffPoolTable) Entries() iter.Seq2[int32, *BuffPoolRecord] {
	records := t.records

	return func(yield func(int32, *BuffPoolRecord) bool) {
		for i := range records {
...
func (t *BuffPoolTable) Read(filename string) error {
	...
	records := make([]BuffPoolRecord, count)
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "BuffPool.Id", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "BuffPool.Id")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "BuffPool.BuffIds", tabbit.KindArray, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "BuffPool.BuffIds")
				for i := int32(0); i < count; i++ {
					r := &records[i]
					elementCount := int(cursor.NextLength())
					r.BuffIds = make([]int32, elementCount)
					for j := 0; j < elementCount; j++ {
						r.BuffIds[j] = cursor.NextI32()
					}
				}
			}
		case 3:
			if tabbit.CheckColumn(reader, column, "BuffPool.Weights", tabbit.KindArray, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "BuffPool.Weights")
				for i := int32(0); i < count; i++ {
					r := &records[i]
					elementCount := int(cursor.NextLength())
					r.Weights = make([]int32, elementCount)
					for j := 0; j < elementCount; j++ {
						r.Weights[j] = cursor.NextI32()
					}
				}
			}
		default:
	...
	if err := reader.Err(); err != nil {
		return fmt.Errorf("BuffPool: %w", err)
	}
