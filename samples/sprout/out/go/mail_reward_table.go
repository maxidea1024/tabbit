...
// MailRewardRecord was generated from samples/sprout/xlsx/Reward.xlsx : MailRewardTable : A1.
type MailRewardRecord struct {
	// primary index
	Id int32
	// title key
	TitleKey string
...
func (t *MailRewardTable) Records() []MailRewardRecord { return t.records }
...
func (t *MailRewardTable) All() iter.Seq[*MailRewardRecord] {
	records := t.records

	return func(yield func(*MailRewardRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *MailRewardTable) FindById(key int32) *MailRewardRecord {
...
func (t *MailRewardTable) GetByIdOrError(key int32) (*MailRewardRecord, error) {
	position, found := t.byId[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `MailReward` that corresponds to field `Id` value %v", key)
	}
...
func (t *MailRewardTable) ContainsId(key int32) bool {
	_, found := t.byId[key]
	return found
}
...
func (t *MailRewardTable) Entries() iter.Seq2[int32, *MailRewardRecord] {
	records := t.records

	return func(yield func(int32, *MailRewardRecord) bool) {
		for i := range records {
...
func (t *MailRewardTable) Read(filename string) error {
	...
	records := make([]MailRewardRecord, count)
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "MailReward.Id", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "MailReward.Id")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "MailReward.TitleKey", tabbit.KindScalar, false, tabbit.ElementString) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "MailReward.TitleKey")
				for i := int32(0); i < count; {
		...
		case 3:
			if tabbit.CheckColumn(reader, column, "MailReward.BodyKey", tabbit.KindScalar, false, tabbit.ElementString) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "MailReward.BodyKey")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameString(count - i)
					for ; n > 0; n-- {
						records[i].BodyKey = value
						i++
					}
				}
			}
		case 4:
			if tabbit.CheckColumn(reader, column, "MailReward.RewardId", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "MailReward.RewardId")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 5:
			if tabbit.CheckColumn(reader, column, "MailReward.ExpiryDays", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "MailReward.ExpiryDays")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
						records[i].ExpiryDays = value
						i++
					}
				}
			}
	...
	if err := reader.Err(); err != nil {
		return fmt.Errorf("MailReward: %w", err)
	}
