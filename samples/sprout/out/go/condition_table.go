...
func (t *ConditionTable) Count() int { return len(t.records) }
...
func (t *ConditionTable) All() iter.Seq[*ConditionRecord] {
	records := t.records

	return func(yield func(*ConditionRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *ConditionTable) Entries() iter.Seq2[int32, *ConditionRecord] {
	records := t.records

	return func(yield func(int32, *ConditionRecord) bool) {
		for i := range records {
...
func (t *ConditionTable) Read(filename string) error {
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "Condition.Id", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Condition.Id")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "Condition.Type", tabbit.KindScalar, false, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Condition.Type")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
						records[i].Type = ConditionType(value)
						i++
					}
				}
			}
		case 3:
			if tabbit.CheckColumn(reader, column, "Condition.Value", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Condition.Value")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 4:
			if tabbit.CheckColumn(reader, column, "Condition.TargetId", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Condition.TargetId")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 5:
			if tabbit.CheckColumn(reader, column, "Condition.DescKey", tabbit.KindScalar, false, tabbit.ElementString) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Condition.DescKey")
				for i := int32(0); i < count; {
