...
// DailyQuestRecord was generated from samples/sprout/xlsx/Quest.xlsx : DailyQuestTable : A1.
type DailyQuestRecord struct {
	// primary index
	Id int32
	// backing quest
	QuestId int32
...
// DailyQuestTable holds every row of DailyQuest.
type DailyQuestTable struct {
	records []DailyQuestRecord
	byId    map[int32]int
}

// Records returns every row, in the order the sheet declared them.
func (t *DailyQuestTable) Records() []DailyQuestRecord { return t.records }

// Count returns how many rows the table holds.
func (t *DailyQuestTable) Count() int { return len(t.records) }
...
func (t *DailyQuestTable) All() iter.Seq[*DailyQuestRecord] {
	records := t.records

	return func(yield func(*DailyQuestRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *DailyQuestTable) FindById(key int32) *DailyQuestRecord {
...
func (t *DailyQuestTable) GetByIdOrError(key int32) (*DailyQuestRecord, error) {
	position, found := t.byId[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `DailyQuest` that corresponds to field `Id` value %v", key)
	}
...
func (t *DailyQuestTable) ContainsId(key int32) bool {
	_, found := t.byId[key]
	return found
}
...
func (t *DailyQuestTable) Entries() iter.Seq2[int32, *DailyQuestRecord] {
	records := t.records

	return func(yield func(int32, *DailyQuestRecord) bool) {
		for i := range records {
...
func (t *DailyQuestTable) Read(filename string) error {
	...
	records := make([]DailyQuestRecord, count)
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "DailyQuest.Id", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "DailyQuest.Id")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "DailyQuest.QuestId", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "DailyQuest.QuestId")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 3:
			if tabbit.CheckColumn(reader, column, "DailyQuest.Points", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "DailyQuest.Points")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 4:
			if tabbit.CheckColumn(reader, column, "DailyQuest.ResetHour", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "DailyQuest.ResetHour")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
	...
	if err := reader.Err(); err != nil {
		return fmt.Errorf("DailyQuest: %w", err)
	}
