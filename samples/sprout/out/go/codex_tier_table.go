...
// CodexTierTable holds every row of CodexTier.
type CodexTierTable struct {
	records []CodexTierRecord
	byId map[int32]int
}

// Records returns every row, in the order the sheet declared them.
func (t *CodexTierTable) Records() []CodexTierRecord { return t.records }

// Count returns how many rows the table holds.
func (t *CodexTierTable) Count() int { return len(t.records) }
...
func (t *CodexTierTable) All() iter.Seq[*CodexTierRecord] {
	records := t.records

	return func(yield func(*CodexTierRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *CodexTierTable) FindById(key int32) *CodexTierRecord {
...
func (t *CodexTierTable) GetByIdOrError(key int32) (*CodexTierRecord, error) {
	position, found := t.byId[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `CodexTier` that corresponds to field `Id` value %v", key)
	}
...
func (t *CodexTierTable) ContainsId(key int32) bool {
	_, found := t.byId[key]
	return found
}
...
func (t *CodexTierTable) Entries() iter.Seq2[int32, *CodexTierRecord] {
	records := t.records

	return func(yield func(int32, *CodexTierRecord) bool) {
		for i := range records {
...
func (t *CodexTierTable) Read(filename string) error {
	...
	records := make([]CodexTierRecord, count)
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "CodexTier.Id", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "CodexTier.Id")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "CodexTier.CodexId", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "CodexTier.CodexId")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 3:
			if tabbit.CheckColumn(reader, column, "CodexTier.Required", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "CodexTier.Required")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 4:
			if tabbit.CheckColumn(reader, column, "CodexTier.RewardId", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "CodexTier.RewardId")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
	...
	if err := reader.Err(); err != nil {
		return fmt.Errorf("CodexTier: %w", err)
	}
