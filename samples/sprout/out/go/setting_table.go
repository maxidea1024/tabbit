...
func (t *SettingTable) All() iter.Seq[*SettingRecord] {
	records := t.records

	return func(yield func(*SettingRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *SettingTable) Entries() iter.Seq2[string, *SettingRecord] {
	records := t.records

	return func(yield func(string, *SettingRecord) bool) {
		for i := range records {
			if !yield(records[i].SettingKey, &records[i]) {
				return
			}
		}
	}
