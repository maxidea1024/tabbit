...
// OfferTable holds every row of Offer.
...
func (t *OfferTable) Records() []OfferRecord { return t.records }

// Count returns how many rows the table holds.
func (t *OfferTable) Count() int { return len(t.records) }
...
func (t *OfferTable) All() iter.Seq[*OfferRecord] {
	records := t.records

	return func(yield func(*OfferRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *OfferTable) FindById(key int32) *OfferRecord {
...
func (t *OfferTable) GetByIdOrError(key int32) (*OfferRecord, error) {
	position, found := t.byId[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `Offer` that corresponds to field `Id` value %v", key)
	}
...
func (t *OfferTable) ContainsId(key int32) bool {
	_, found := t.byId[key]
	return found
}
...
func (t *OfferTable) Entries() iter.Seq2[int32, *OfferRecord] {
	records := t.records

	return func(yield func(int32, *OfferRecord) bool) {
		for i := range records {
...
func (t *OfferTable) Read(filename string) error {
	...
	records := make([]OfferRecord, count)
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "Offer.Id", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Offer.Id")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "Offer.NameKey", tabbit.KindScalar, false, tabbit.ElementString) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Offer.NameKey")
				for i := int32(0); i < count; {
		...
		case 3:
			if tabbit.CheckColumn(reader, column, "Offer.Shop", tabbit.KindScalar, false, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Offer.Shop")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 4:
			if tabbit.CheckColumn(reader, column, "Offer.Price", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Offer.Price")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 5:
			if tabbit.CheckColumn(reader, column, "Offer.Limit", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Offer.Limit")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
