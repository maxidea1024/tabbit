...
func (t *EquipSetTable) All() iter.Seq[*EquipSetRecord] {
	records := t.records

	return func(yield func(*EquipSetRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *EquipSetTable) Entries() iter.Seq2[int32, *EquipSetRecord] {
	records := t.records

	return func(yield func(int32, *EquipSetRecord) bool) {
		for i := range records {
