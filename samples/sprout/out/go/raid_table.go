...
// RaidTable holds every row of Raid.
...
func (t *RaidTable) Records() []RaidRecord { return t.records }

// Count returns how many rows the table holds.
func (t *RaidTable) Count() int { return len(t.records) }
...
func (t *RaidTable) All() iter.Seq[*RaidRecord] {
	records := t.records

	return func(yield func(*RaidRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *RaidTable) FindById(key int32) *RaidRecord {
...
func (t *RaidTable) GetByIdOrError(key int32) (*RaidRecord, error) {
	position, found := t.byId[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `Raid` that corresponds to field `Id` value %v", key)
	}
...
func (t *RaidTable) ContainsId(key int32) bool {
	_, found := t.byId[key]
	return found
}
...
func (t *RaidTable) Entries() iter.Seq2[int32, *RaidRecord] {
	records := t.records

	return func(yield func(int32, *RaidRecord) bool) {
		for i := range records {
...
func (t *RaidTable) Read(filename string) error {
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "Raid.Id", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Raid.Id")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "Raid.NameKey", tabbit.KindScalar, false, tabbit.ElementString) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Raid.NameKey")
				for i := int32(0); i < count; {
		...
		case 3:
			if tabbit.CheckColumn(reader, column, "Raid.BossId", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Raid.BossId")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
						records[i].BossId = value
						i++
					}
				}
			}
		case 4:
			if tabbit.CheckColumn(reader, column, "Raid.BossHealth", tabbit.KindScalar, false, tabbit.ElementI64, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Raid.BossHealth")
				for i := int32(0); i < count; i++ {
					r := &records[i]
					r.BossHealth = cursor.NextI64()
				}
			}
		case 5:
			if tabbit.CheckColumn(reader, column, "Raid.TurnLimit", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Raid.TurnLimit")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
						records[i].TurnLimit = value
						i++
					}
				}
			}
		case 6:
			if tabbit.CheckColumn(reader, column, "Raid.SeasonDays", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Raid.SeasonDays")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
						records[i].SeasonDays = value
						i++
					}
				}
			}
