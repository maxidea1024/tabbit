...
// LootGroupRecord was generated from samples/sprout/xlsx/Reward.xlsx : LootGroupTable : A1.
type LootGroupRecord struct {
	// primary index
	Id int32
	// name key
	NameKey string
...
// LootGroupTable holds every row of LootGroup.
type LootGroupTable struct {
	records []LootGroupRecord
	byId    map[int32]int
}

// Records returns every row, in the order the sheet declared them.
func (t *LootGroupTable) Records() []LootGroupRecord { return t.records }

// Count returns how many rows the table holds.
func (t *LootGroupTable) Count() int { return len(t.records) }
...
func (t *LootGroupTable) All() iter.Seq[*LootGroupRecord] {
	records := t.records

	return func(yield func(*LootGroupRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *LootGroupTable) FindById(key int32) *LootGroupRecord {
...
func (t *LootGroupTable) GetByIdOrError(key int32) (*LootGroupRecord, error) {
	position, found := t.byId[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `LootGroup` that corresponds to field `Id` value %v", key)
	}
...
func (t *LootGroupTable) ContainsId(key int32) bool {
	_, found := t.byId[key]
	return found
}
...
func (t *LootGroupTable) Entries() iter.Seq2[int32, *LootGroupRecord] {
	records := t.records

	return func(yield func(int32, *LootGroupRecord) bool) {
		for i := range records {
...
func (t *LootGroupTable) Read(filename string) error {
	...
	records := make([]LootGroupRecord, count)
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "LootGroup.Id", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "LootGroup.Id")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "LootGroup.NameKey", tabbit.KindScalar, false, tabbit.ElementString) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "LootGroup.NameKey")
				for i := int32(0); i < count; {
		...
		case 3:
			if tabbit.CheckColumn(reader, column, "LootGroup.Draws", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "LootGroup.Draws")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 4:
			if tabbit.CheckColumn(reader, column, "LootGroup.AllowDuplicate", tabbit.KindScalar, false, tabbit.ElementBool) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "LootGroup.AllowDuplicate")
				for i := int32(0); i < count; i++ {
					r := &records[i]
					r.AllowDuplicate = cursor.NextBool()
				}
			}
		default:
	...
	if err := reader.Err(); err != nil {
		return fmt.Errorf("LootGroup: %w", err)
	}
