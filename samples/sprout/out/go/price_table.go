...
// PriceRecord was generated from samples/sprout/xlsx/Shop.xlsx : PriceTable : A1.
...
func (t *PriceTable) FindById(key int32) *PriceRecord {
...
func (t *PriceTable) GetByIdOrError(key int32) (*PriceRecord, error) {
	position, found := t.byId[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `Price` that corresponds to field `Id` value %v", key)
	}
...
func (t *PriceTable) ContainsId(key int32) bool {
	_, found := t.byId[key]
	return found
}
...
func (t *PriceTable) Read(filename string) error {
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "Price.Id", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Price.Id")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "Price.Currency", tabbit.KindScalar, false, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Price.Currency")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
						records[i].Currency = CurrencyType(value)
						i++
					}
				}
			}
		...
		case 4:
			if tabbit.CheckColumn(reader, column, "Price.Discount", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Price.Discount")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
