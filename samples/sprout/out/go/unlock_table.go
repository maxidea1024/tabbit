...
// UnlockTable holds every row of Unlock.
...
func (t *UnlockTable) Records() []UnlockRecord { return t.records }

// Count returns how many rows the table holds.
func (t *UnlockTable) Count() int { return len(t.records) }
...
func (t *UnlockTable) All() iter.Seq[*UnlockRecord] {
	records := t.records

	return func(yield func(*UnlockRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *UnlockTable) FindById(key int32) *UnlockRecord {
...
func (t *UnlockTable) GetByIdOrError(key int32) (*UnlockRecord, error) {
	position, found := t.byId[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `Unlock` that corresponds to field `Id` value %v", key)
	}
...
func (t *UnlockTable) ContainsId(key int32) bool {
	_, found := t.byId[key]
	return found
}
...
func (t *UnlockTable) Entries() iter.Seq2[int32, *UnlockRecord] {
	records := t.records

	return func(yield func(int32, *UnlockRecord) bool) {
		for i := range records {
...
func (t *UnlockTable) Read(filename string) error {
	...
	records := make([]UnlockRecord, count)
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "Unlock.Id", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Unlock.Id")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "Unlock.Type", tabbit.KindScalar, false, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Unlock.Type")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
						records[i].Type = UnlockType(value)
						i++
					}
				}
			}
		case 3:
			if tabbit.CheckColumn(reader, column, "Unlock.ConditionId", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Unlock.ConditionId")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 4:
			if tabbit.CheckColumn(reader, column, "Unlock.MessageKey", tabbit.KindScalar, false, tabbit.ElementString) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Unlock.MessageKey")
				for i := int32(0); i < count; {
