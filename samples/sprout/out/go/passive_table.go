...
// PassiveTable holds every row of Passive.
type PassiveTable struct {
	records []PassiveRecord
	byId    map[int32]int
}

// Records returns every row, in the order the sheet declared them.
func (t *PassiveTable) Records() []PassiveRecord { return t.records }

// Count returns how many rows the table holds.
func (t *PassiveTable) Count() int { return len(t.records) }
...
func (t *PassiveTable) All() iter.Seq[*PassiveRecord] {
	records := t.records

	return func(yield func(*PassiveRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *PassiveTable) FindById(key int32) *PassiveRecord {
...
func (t *PassiveTable) GetByIdOrError(key int32) (*PassiveRecord, error) {
	position, found := t.byId[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `Passive` that corresponds to field `Id` value %v", key)
	}
...
func (t *PassiveTable) ContainsId(key int32) bool {
	_, found := t.byId[key]
	return found
}
...
func (t *PassiveTable) Entries() iter.Seq2[int32, *PassiveRecord] {
	records := t.records

	return func(yield func(int32, *PassiveRecord) bool) {
		for i := range records {
...
func (t *PassiveTable) Read(filename string) error {
	...
	records := make([]PassiveRecord, count)
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "Passive.Id", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Passive.Id")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "Passive.CharacterId", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Passive.CharacterId")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 3:
			if tabbit.CheckColumn(reader, column, "Passive.UnlockStep", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Passive.UnlockStep")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 4:
			if tabbit.CheckColumn(reader, column, "Passive.Stat", tabbit.KindScalar, false, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Passive.Stat")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 5:
			if tabbit.CheckColumn(reader, column, "Passive.Bonus", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Passive.Bonus")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
