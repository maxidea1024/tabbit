...
// ItemGradeTable holds every row of ItemGrade.
type ItemGradeTable struct {
	records []ItemGradeRecord
	byId    map[int32]int
}

// Records returns every row, in the order the sheet declared them.
func (t *ItemGradeTable) Records() []ItemGradeRecord { return t.records }

// Count returns how many rows the table holds.
func (t *ItemGradeTable) Count() int { return len(t.records) }
...
func (t *ItemGradeTable) All() iter.Seq[*ItemGradeRecord] {
	records := t.records

	return func(yield func(*ItemGradeRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *ItemGradeTable) FindById(key int32) *ItemGradeRecord {
...
func (t *ItemGradeTable) GetByIdOrError(key int32) (*ItemGradeRecord, error) {
	position, found := t.byId[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `ItemGrade` that corresponds to field `Id` value %v", key)
	}
...
func (t *ItemGradeTable) ContainsId(key int32) bool {
	_, found := t.byId[key]
	return found
}
...
func (t *ItemGradeTable) Entries() iter.Seq2[int32, *ItemGradeRecord] {
	records := t.records

	return func(yield func(int32, *ItemGradeRecord) bool) {
		for i := range records {
...
func (t *ItemGradeTable) Read(filename string) error {
	...
	records := make([]ItemGradeRecord, count)
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "ItemGrade.Id", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "ItemGrade.Id")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "ItemGrade.Grade", tabbit.KindScalar, false, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "ItemGrade.Grade")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 3:
			if tabbit.CheckColumn(reader, column, "ItemGrade.Frame", tabbit.KindScalar, false, tabbit.ElementString) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "ItemGrade.Frame")
				for i := int32(0); i < count; {
		...
		case 4:
			if tabbit.CheckColumn(reader, column, "ItemGrade.SellScale", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "ItemGrade.SellScale")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
						records[i].SellScale = value
						i++
					}
				}
			}
	...
	if err := reader.Err(); err != nil {
		return fmt.Errorf("ItemGrade: %w", err)
	}
