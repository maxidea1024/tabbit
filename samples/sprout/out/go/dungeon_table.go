...
// DungeonTable holds every row of Dungeon.
...
func (t *DungeonTable) Records() []DungeonRecord { return t.records }

// Count returns how many rows the table holds.
func (t *DungeonTable) Count() int { return len(t.records) }
...
func (t *DungeonTable) All() iter.Seq[*DungeonRecord] {
	records := t.records

	return func(yield func(*DungeonRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *DungeonTable) FindById(key int32) *DungeonRecord {
...
func (t *DungeonTable) GetByIdOrError(key int32) (*DungeonRecord, error) {
	position, found := t.byId[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `Dungeon` that corresponds to field `Id` value %v", key)
	}
...
func (t *DungeonTable) ContainsId(key int32) bool {
	_, found := t.byId[key]
	return found
}
...
func (t *DungeonTable) Entries() iter.Seq2[int32, *DungeonRecord] {
	records := t.records

	return func(yield func(int32, *DungeonRecord) bool) {
		for i := range records {
...
func (t *DungeonTable) Read(filename string) error {
	...
	records := make([]DungeonRecord, count)
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "Dungeon.Id", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Dungeon.Id")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "Dungeon.Type", tabbit.KindScalar, false, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Dungeon.Type")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
						records[i].Type = DungeonType(value)
						i++
					}
				}
			}
		case 3:
			if tabbit.CheckColumn(reader, column, "Dungeon.NameKey", tabbit.KindScalar, false, tabbit.ElementString) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Dungeon.NameKey")
				for i := int32(0); i < count; {
		...
		case 4:
			if tabbit.CheckColumn(reader, column, "Dungeon.Element", tabbit.KindScalar, false, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Dungeon.Element")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 5:
			if tabbit.CheckColumn(reader, column, "Dungeon.DailyEntries", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Dungeon.DailyEntries")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 6:
			if tabbit.CheckColumn(reader, column, "Dungeon.UnlockId", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Dungeon.UnlockId")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
