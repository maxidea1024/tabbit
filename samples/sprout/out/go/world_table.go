...
// WorldTable holds every row of World.
...
func (t *WorldTable) Records() []WorldRecord { return t.records }

// Count returns how many rows the table holds.
func (t *WorldTable) Count() int { return len(t.records) }
...
func (t *WorldTable) All() iter.Seq[*WorldRecord] {
	records := t.records

	return func(yield func(*WorldRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *WorldTable) FindById(key int32) *WorldRecord {
...
func (t *WorldTable) GetByIdOrError(key int32) (*WorldRecord, error) {
	position, found := t.byId[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `World` that corresponds to field `Id` value %v", key)
	}
...
func (t *WorldTable) ContainsId(key int32) bool {
	_, found := t.byId[key]
	return found
}
...
func (t *WorldTable) Entries() iter.Seq2[int32, *WorldRecord] {
	records := t.records

	return func(yield func(int32, *WorldRecord) bool) {
		for i := range records {
...
func (t *WorldTable) Read(filename string) error {
	...
	records := make([]WorldRecord, count)
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "World.Id", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "World.Id")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "World.NameKey", tabbit.KindScalar, false, tabbit.ElementString) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "World.NameKey")
				for i := int32(0); i < count; {
		...
		case 3:
			if tabbit.CheckColumn(reader, column, "World.Map", tabbit.KindScalar, false, tabbit.ElementString) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "World.Map")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameString(count - i)
					for ; n > 0; n-- {
						records[i].Map = value
		...
		case 4:
			if tabbit.CheckColumn(reader, column, "World.ThemeColour", tabbit.KindScalar, false, tabbit.ElementString) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "World.ThemeColour")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameString(count - i)
					for ; n > 0; n-- {
