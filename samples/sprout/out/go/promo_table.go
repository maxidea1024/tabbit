...
// PromoTable holds every row of Promo.
...
func (t *PromoTable) Records() []PromoRecord { return t.records }

// Count returns how many rows the table holds.
func (t *PromoTable) Count() int { return len(t.records) }
...
func (t *PromoTable) All() iter.Seq[*PromoRecord] {
	records := t.records

	return func(yield func(*PromoRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *PromoTable) FindById(key int32) *PromoRecord {
...
func (t *PromoTable) GetByIdOrError(key int32) (*PromoRecord, error) {
	position, found := t.byId[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `Promo` that corresponds to field `Id` value %v", key)
	}
...
func (t *PromoTable) ContainsId(key int32) bool {
	_, found := t.byId[key]
	return found
}
...
func (t *PromoTable) Entries() iter.Seq2[int32, *PromoRecord] {
	records := t.records

	return func(yield func(int32, *PromoRecord) bool) {
		for i := range records {
...
func (t *PromoTable) Read(filename string) error {
	...
	records := make([]PromoRecord, count)
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "Promo.Id", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Promo.Id")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "Promo.OfferId", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Promo.OfferId")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 3:
			if tabbit.CheckColumn(reader, column, "Promo.Banner", tabbit.KindScalar, false, tabbit.ElementString) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Promo.Banner")
				for i := int32(0); i < count; {
		...
		case 4:
			if tabbit.CheckColumn(reader, column, "Promo.AvailableDays", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Promo.AvailableDays")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
						records[i].AvailableDays = value
						i++
					}
				}
			}
