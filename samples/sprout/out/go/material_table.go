...
func (t *MaterialTable) Count() int { return len(t.records) }
...
func (t *MaterialTable) All() iter.Seq[*MaterialRecord] {
	records := t.records

	return func(yield func(*MaterialRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *MaterialTable) Entries() iter.Seq2[int32, *MaterialRecord] {
	records := t.records

	return func(yield func(int32, *MaterialRecord) bool) {
		for i := range records {
...
func (t *MaterialTable) Read(filename string) error {
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "Material.Id", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Material.Id")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "Material.ItemId", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Material.ItemId")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 3:
			if tabbit.CheckColumn(reader, column, "Material.Tier", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Material.Tier")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 4:
			if tabbit.CheckColumn(reader, column, "Material.Element", tabbit.KindScalar, false, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Material.Element")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
