...
// WeeklyQuestRecord was generated from samples/sprout/xlsx/Quest.xlsx : WeeklyQuestTable : A1.
type WeeklyQuestRecord struct {
	// primary index
	Id int32
	// backing quest
	QuestId int32
...
// WeeklyQuestTable holds every row of WeeklyQuest.
type WeeklyQuestTable struct {
	records []WeeklyQuestRecord
	byId    map[int32]int
}

// Records returns every row, in the order the sheet declared them.
func (t *WeeklyQuestTable) Records() []WeeklyQuestRecord { return t.records }

// Count returns how many rows the table holds.
func (t *WeeklyQuestTable) Count() int { return len(t.records) }
...
func (t *WeeklyQuestTable) All() iter.Seq[*WeeklyQuestRecord] {
	records := t.records

	return func(yield func(*WeeklyQuestRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *WeeklyQuestTable) FindById(key int32) *WeeklyQuestRecord {
...
func (t *WeeklyQuestTable) GetByIdOrError(key int32) (*WeeklyQuestRecord, error) {
	position, found := t.byId[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `WeeklyQuest` that corresponds to field `Id` value %v", key)
	}
...
func (t *WeeklyQuestTable) ContainsId(key int32) bool {
	_, found := t.byId[key]
	return found
}
...
func (t *WeeklyQuestTable) Entries() iter.Seq2[int32, *WeeklyQuestRecord] {
	records := t.records

	return func(yield func(int32, *WeeklyQuestRecord) bool) {
		for i := range records {
...
func (t *WeeklyQuestTable) Read(filename string) error {
	...
	records := make([]WeeklyQuestRecord, count)
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "WeeklyQuest.Id", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "WeeklyQuest.Id")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "WeeklyQuest.QuestId", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "WeeklyQuest.QuestId")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 3:
			if tabbit.CheckColumn(reader, column, "WeeklyQuest.Points", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "WeeklyQuest.Points")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 4:
			if tabbit.CheckColumn(reader, column, "WeeklyQuest.ResetWeekday", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "WeeklyQuest.ResetWeekday")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
						records[i].ResetWeekday = value
						i++
					}
				}
			}
	...
	if err := reader.Err(); err != nil {
		return fmt.Errorf("WeeklyQuest: %w", err)
	}
