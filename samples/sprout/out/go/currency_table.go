...
func (t *CurrencyTable) Count() int { return len(t.records) }
...
func (t *CurrencyTable) All() iter.Seq[*CurrencyRecord] {
	records := t.records

	return func(yield func(*CurrencyRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *CurrencyTable) Entries() iter.Seq2[int32, *CurrencyRecord] {
	records := t.records

	return func(yield func(int32, *CurrencyRecord) bool) {
		for i := range records {
...
func (t *CurrencyTable) Read(filename string) error {
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "Currency.Id", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Currency.Id")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "Currency.Type", tabbit.KindScalar, false, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Currency.Type")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
						records[i].Type = CurrencyType(value)
						i++
					}
				}
			}
		case 3:
			if tabbit.CheckColumn(reader, column, "Currency.NameKey", tabbit.KindScalar, false, tabbit.ElementString) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Currency.NameKey")
				for i := int32(0); i < count; {
		...
		case 4:
			if tabbit.CheckColumn(reader, column, "Currency.Icon", tabbit.KindScalar, false, tabbit.ElementString) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Currency.Icon")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameString(count - i)
					for ; n > 0; n-- {
		...
		case 5:
			if tabbit.CheckColumn(reader, column, "Currency.Cap", tabbit.KindScalar, false, tabbit.ElementI64, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Currency.Cap")
				for i := int32(0); i < count; i++ {
					r := &records[i]
					r.Cap = cursor.NextI64()
				}
			}
		default:
