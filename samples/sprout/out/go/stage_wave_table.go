...
func (t *StageWaveTable) Records() []StageWaveRecord { return t.records }
...
func (t *StageWaveTable) All() iter.Seq[*StageWaveRecord] {
	records := t.records

	return func(yield func(*StageWaveRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *StageWaveTable) FindById(key int32) *StageWaveRecord {
...
func (t *StageWaveTable) GetByIdOrError(key int32) (*StageWaveRecord, error) {
	position, found := t.byId[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `StageWave` that corresponds to field `Id` value %v", key)
	}
...
func (t *StageWaveTable) ContainsId(key int32) bool {
	_, found := t.byId[key]
	return found
}
...
func (t *StageWaveTable) Entries() iter.Seq2[int32, *StageWaveRecord] {
	records := t.records

	return func(yield func(int32, *StageWaveRecord) bool) {
		for i := range records {
...
func (t *StageWaveTable) Read(filename string) error {
	...
	records := make([]StageWaveRecord, count)
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "StageWave.Id", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "StageWave.Id")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "StageWave.StageId", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "StageWave.StageId")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 3:
			if tabbit.CheckColumn(reader, column, "StageWave.WaveOrder", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "StageWave.WaveOrder")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
						records[i].WaveOrder = value
						i++
					}
				}
			}
		case 4:
			if tabbit.CheckColumn(reader, column, "StageWave.MonsterIds", tabbit.KindArray, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "StageWave.MonsterIds")
				for i := int32(0); i < count; i++ {
					r := &records[i]
					elementCount := int(cursor.NextLength())
					r.MonsterIds = make([]int32, elementCount)
					for j := 0; j < elementCount; j++ {
						r.MonsterIds[j] = cursor.NextI32()
					}
				}
			}
		case 5:
			if tabbit.CheckColumn(reader, column, "StageWave.MonsterLevels", tabbit.KindArray, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "StageWave.MonsterLevels")
				for i := int32(0); i < count; i++ {
					r := &records[i]
					elementCount := int(cursor.NextLength())
					r.MonsterLevels = make([]int32, elementCount)
					for j := 0; j < elementCount; j++ {
						r.MonsterLevels[j] = cursor.NextI32()
					}
				}
			}
		default:
	...
	if err := reader.Err(); err != nil {
		return fmt.Errorf("StageWave: %w", err)
	}
