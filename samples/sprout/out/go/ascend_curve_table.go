...
// AscendCurveTable holds every row of AscendCurve.
type AscendCurveTable struct {
	records []AscendCurveRecord
	byId map[int32]int
}

// Records returns every row, in the order the sheet declared them.
func (t *AscendCurveTable) Records() []AscendCurveRecord { return t.records }

// Count returns how many rows the table holds.
func (t *AscendCurveTable) Count() int { return len(t.records) }
...
func (t *AscendCurveTable) All() iter.Seq[*AscendCurveRecord] {
	records := t.records

	return func(yield func(*AscendCurveRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *AscendCurveTable) FindById(key int32) *AscendCurveRecord {
...
func (t *AscendCurveTable) GetByIdOrError(key int32) (*AscendCurveRecord, error) {
	position, found := t.byId[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `AscendCurve` that corresponds to field `Id` value %v", key)
	}
...
func (t *AscendCurveTable) ContainsId(key int32) bool {
	_, found := t.byId[key]
	return found
}
...
func (t *AscendCurveTable) Entries() iter.Seq2[int32, *AscendCurveRecord] {
	records := t.records

	return func(yield func(int32, *AscendCurveRecord) bool) {
		for i := range records {
...
func (t *AscendCurveTable) Read(filename string) error {
	...
	records := make([]AscendCurveRecord, count)
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "AscendCurve.Id", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "AscendCurve.Id")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "AscendCurve.Grade", tabbit.KindScalar, false, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "AscendCurve.Grade")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 3:
			if tabbit.CheckColumn(reader, column, "AscendCurve.Step", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "AscendCurve.Step")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 4:
			if tabbit.CheckColumn(reader, column, "AscendCurve.Scale", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "AscendCurve.Scale")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
	...
	if err := reader.Err(); err != nil {
		return fmt.Errorf("AscendCurve: %w", err)
	}
