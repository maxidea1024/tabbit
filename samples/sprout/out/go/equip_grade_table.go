...
// EquipGradeRecord was generated from samples/sprout/xlsx/Equipment.xlsx : EquipGradeTable : A1.
type EquipGradeRecord struct {
	// primary index
	Id int32
	// grade
	Grade GradeType
...
// EquipGradeTable holds every row of EquipGrade.
type EquipGradeTable struct {
	records []EquipGradeRecord
	byId    map[int32]int
}

// Records returns every row, in the order the sheet declared them.
func (t *EquipGradeTable) Records() []EquipGradeRecord { return t.records }

// Count returns how many rows the table holds.
func (t *EquipGradeTable) Count() int { return len(t.records) }
...
func (t *EquipGradeTable) All() iter.Seq[*EquipGradeRecord] {
	records := t.records

	return func(yield func(*EquipGradeRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *EquipGradeTable) FindById(key int32) *EquipGradeRecord {
...
func (t *EquipGradeTable) GetByIdOrError(key int32) (*EquipGradeRecord, error) {
	position, found := t.byId[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `EquipGrade` that corresponds to field `Id` value %v", key)
	}
...
func (t *EquipGradeTable) ContainsId(key int32) bool {
	_, found := t.byId[key]
	return found
}
...
func (t *EquipGradeTable) Entries() iter.Seq2[int32, *EquipGradeRecord] {
	records := t.records

	return func(yield func(int32, *EquipGradeRecord) bool) {
		for i := range records {
...
func (t *EquipGradeTable) Read(filename string) error {
	...
	records := make([]EquipGradeRecord, count)
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "EquipGrade.Id", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "EquipGrade.Id")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "EquipGrade.Grade", tabbit.KindScalar, false, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "EquipGrade.Grade")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 3:
			if tabbit.CheckColumn(reader, column, "EquipGrade.OptionSlots", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "EquipGrade.OptionSlots")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 4:
			if tabbit.CheckColumn(reader, column, "EquipGrade.Frame", tabbit.KindScalar, false, tabbit.ElementString) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "EquipGrade.Frame")
				for i := int32(0); i < count; {
	...
	if err := reader.Err(); err != nil {
		return fmt.Errorf("EquipGrade: %w", err)
	}
