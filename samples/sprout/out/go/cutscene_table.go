...
// CutsceneTable holds every row of Cutscene.
type CutsceneTable struct {
	records []CutsceneRecord
	byId map[int32]int
}

// Records returns every row, in the order the sheet declared them.
func (t *CutsceneTable) Records() []CutsceneRecord { return t.records }

// Count returns how many rows the table holds.
func (t *CutsceneTable) Count() int { return len(t.records) }
...
func (t *CutsceneTable) All() iter.Seq[*CutsceneRecord] {
	records := t.records

	return func(yield func(*CutsceneRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *CutsceneTable) FindById(key int32) *CutsceneRecord {
...
func (t *CutsceneTable) GetByIdOrError(key int32) (*CutsceneRecord, error) {
	position, found := t.byId[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `Cutscene` that corresponds to field `Id` value %v", key)
	}
...
func (t *CutsceneTable) ContainsId(key int32) bool {
	_, found := t.byId[key]
	return found
}
...
func (t *CutsceneTable) Entries() iter.Seq2[int32, *CutsceneRecord] {
	records := t.records

	return func(yield func(int32, *CutsceneRecord) bool) {
		for i := range records {
...
func (t *CutsceneTable) Read(filename string) error {
	...
	records := make([]CutsceneRecord, count)
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "Cutscene.Id", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Cutscene.Id")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "Cutscene.Timeline", tabbit.KindScalar, false, tabbit.ElementString) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Cutscene.Timeline")
				for i := int32(0); i < count; {
		...
		case 3:
			if tabbit.CheckColumn(reader, column, "Cutscene.LengthMs", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Cutscene.LengthMs")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
						records[i].LengthMs = value
						i++
					}
				}
			}
		case 4:
			if tabbit.CheckColumn(reader, column, "Cutscene.SkipAfterMs", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Cutscene.SkipAfterMs")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
						records[i].SkipAfterMs = value
						i++
					}
				}
			}
	...
	if err := reader.Err(); err != nil {
		return fmt.Errorf("Cutscene: %w", err)
	}
