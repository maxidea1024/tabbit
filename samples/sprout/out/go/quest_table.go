...
func (t *QuestTable) All() iter.Seq[*QuestRecord] {
	records := t.records

	return func(yield func(*QuestRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *QuestTable) Entries() iter.Seq2[int32, *QuestRecord] {
	records := t.records

	return func(yield func(int32, *QuestRecord) bool) {
		for i := range records {
