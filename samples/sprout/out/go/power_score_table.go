...
// PowerScoreTable holds every row of PowerScore.
type PowerScoreTable struct {
	records []PowerScoreRecord
	byId    map[int32]int
}

// Records returns every row, in the order the sheet declared them.
func (t *PowerScoreTable) Records() []PowerScoreRecord { return t.records }

// Count returns how many rows the table holds.
func (t *PowerScoreTable) Count() int { return len(t.records) }
...
func (t *PowerScoreTable) All() iter.Seq[*PowerScoreRecord] {
	records := t.records

	return func(yield func(*PowerScoreRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *PowerScoreTable) FindById(key int32) *PowerScoreRecord {
...
func (t *PowerScoreTable) GetByIdOrError(key int32) (*PowerScoreRecord, error) {
	position, found := t.byId[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `PowerScore` that corresponds to field `Id` value %v", key)
	}
...
func (t *PowerScoreTable) ContainsId(key int32) bool {
	_, found := t.byId[key]
	return found
}
...
func (t *PowerScoreTable) Entries() iter.Seq2[int32, *PowerScoreRecord] {
	records := t.records

	return func(yield func(int32, *PowerScoreRecord) bool) {
		for i := range records {
...
func (t *PowerScoreTable) Read(filename string) error {
	...
	records := make([]PowerScoreRecord, count)
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "PowerScore.Id", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "PowerScore.Id")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "PowerScore.Stat", tabbit.KindScalar, false, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "PowerScore.Stat")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 3:
			if tabbit.CheckColumn(reader, column, "PowerScore.Weight", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "PowerScore.Weight")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
	...
	if err := reader.Err(); err != nil {
		return fmt.Errorf("PowerScore: %w", err)
	}
