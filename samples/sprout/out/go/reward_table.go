...
// RewardRecord was generated from samples/sprout/xlsx/Reward.xlsx : RewardTable : A1.
...
// RewardTable holds every row of Reward.
...
func (t *RewardTable) Records() []RewardRecord { return t.records }

// Count returns how many rows the table holds.
func (t *RewardTable) Count() int { return len(t.records) }
...
func (t *RewardTable) All() iter.Seq[*RewardRecord] {
	records := t.records

	return func(yield func(*RewardRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *RewardTable) FindById(key int32) *RewardRecord {
...
func (t *RewardTable) GetByIdOrError(key int32) (*RewardRecord, error) {
	position, found := t.byId[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `Reward` that corresponds to field `Id` value %v", key)
	}
...
func (t *RewardTable) ContainsId(key int32) bool {
	_, found := t.byId[key]
	return found
}
...
func (t *RewardTable) Entries() iter.Seq2[int32, *RewardRecord] {
	records := t.records

	return func(yield func(int32, *RewardRecord) bool) {
		for i := range records {
...
func (t *RewardTable) Read(filename string) error {
	...
	records := make([]RewardRecord, count)
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "Reward.Id", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Reward.Id")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "Reward.Type", tabbit.KindScalar, false, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Reward.Type")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 3:
			if tabbit.CheckColumn(reader, column, "Reward.LootGroupId", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Reward.LootGroupId")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 4:
			if tabbit.CheckColumn(reader, column, "Reward.ItemIds", tabbit.KindArray, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Reward.ItemIds")
				for i := int32(0); i < count; i++ {
					r := &records[i]
					elementCount := int(cursor.NextLength())
					r.ItemIds = make([]int32, elementCount)
					for j := 0; j < elementCount; j++ {
						r.ItemIds[j] = cursor.NextI32()
					}
				}
			}
		case 5:
			if tabbit.CheckColumn(reader, column, "Reward.ItemCounts", tabbit.KindArray, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "Reward.ItemCounts")
				for i := int32(0); i < count; i++ {
					r := &records[i]
					elementCount := int(cursor.NextLength())
					r.ItemCounts = make([]int32, elementCount)
					for j := 0; j < elementCount; j++ {
						r.ItemCounts[j] = cursor.NextI32()
					}
				}
			}
		default:
