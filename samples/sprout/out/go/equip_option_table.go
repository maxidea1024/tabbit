...
// EquipOptionTable holds every row of EquipOption.
type EquipOptionTable struct {
	records []EquipOptionRecord
	byId    map[int32]int
}

// Records returns every row, in the order the sheet declared them.
func (t *EquipOptionTable) Records() []EquipOptionRecord { return t.records }

// Count returns how many rows the table holds.
func (t *EquipOptionTable) Count() int { return len(t.records) }
...
func (t *EquipOptionTable) All() iter.Seq[*EquipOptionRecord] {
	records := t.records

	return func(yield func(*EquipOptionRecord) bool) {
		for i := range records {
			if !yield(&records[i]) {
				return
			}
...
func (t *EquipOptionTable) FindById(key int32) *EquipOptionRecord {
...
func (t *EquipOptionTable) GetByIdOrError(key int32) (*EquipOptionRecord, error) {
	position, found := t.byId[key]
	if !found {
		return nil, fmt.Errorf(
			"there is no record in table `EquipOption` that corresponds to field `Id` value %v", key)
	}
...
func (t *EquipOptionTable) ContainsId(key int32) bool {
	_, found := t.byId[key]
	return found
}
...
func (t *EquipOptionTable) Entries() iter.Seq2[int32, *EquipOptionRecord] {
	records := t.records

	return func(yield func(int32, *EquipOptionRecord) bool) {
		for i := range records {
...
func (t *EquipOptionTable) Read(filename string) error {
	...
	records := make([]EquipOptionRecord, count)
	...
	for _, column := range columns {
		...
		switch column.Tag {
		case 1:
			if tabbit.CheckColumn(reader, column, "EquipOption.Id", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "EquipOption.Id")
		...
		case 2:
			if tabbit.CheckColumn(reader, column, "EquipOption.Stat", tabbit.KindScalar, false, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "EquipOption.Stat")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
		...
		case 3:
			if tabbit.CheckColumn(reader, column, "EquipOption.MinRoll", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "EquipOption.MinRoll")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
						records[i].MinRoll = value
						i++
					}
				}
			}
		case 4:
			if tabbit.CheckColumn(reader, column, "EquipOption.MaxRoll", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "EquipOption.MaxRoll")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
						records[i].MaxRoll = value
						i++
					}
				}
			}
		case 5:
			if tabbit.CheckColumn(reader, column, "EquipOption.Weight", tabbit.KindScalar, false, tabbit.ElementI32, tabbit.ElementVarint) {
				cursor := tabbit.NewColumnCursor(reader, column, count, "EquipOption.Weight")
				for i := int32(0); i < count; {
					n, value := cursor.NextSameI32(count - i)
					for ; n > 0; n-- {
	...
	if err := reader.Err(); err != nil {
		return fmt.Errorf("EquipOption: %w", err)
	}
