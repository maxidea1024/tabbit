...
static bool A_PackageParse(A_PackageTable_t* table, tb_reader* reader) {
  int32_t row;
  int32_t at;
  ...
  if (table->count > 0) {
    table->records = (A_PackageRecord_t*)tb_arena_alloc(
...
bool A_PackageLoad(A_PackageTable_t* table, const char* filename,
  ...
  ok = A_PackageParse(&loaded, &reader);
