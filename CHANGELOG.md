...
- **찾은 문제를 로그가 아니라 보고서로 냅니다.** 실행이 끝나면 HTML 한 장과 JSON 한 장을
  ...
  ([사용](doc/cli.md#빌드-리포트--찾은-것을-고칠-사람에게) · [설계](spec/build-report.md)).
  ...
  - **복사 버튼은 보고 전체를 담습니다** — 문장, 그 아래 위치, 구글 시트면 셀을 여는 링크,
    그리고 메시지 ID. 콘솔이 찍는 모양 그대로라 동료에게 그대로 보낼 수 있습니다. 위치만
  ...
  - **보는 사람이 없으면 열지 않습니다.** `CI` 환경 변수 · 리다이렉트된 출력 · `--silent`는
    설정을 이깁니다. 마지막 리포트는 `--show-report`로 변환 없이 다시 엽니다.
...
- **바뀐 것이 없으면 아무것도 하지 않습니다.** 지난 실행이 무엇을 읽고 무엇을 썼는지 기록해
  ...
  ([사용](doc/cli.md#다시-하지-않기--빌드-캐시) · [설계](spec/build-cache.md)).
...
- **로그 한 줄이 수준과 단계를 먼저 말합니다.** `[W] [Cooking   ] ...` 형태이고, 콘솔과 파일이
...
- **시트의 날짜를 어느 시간대로 읽을지 recipe에 적습니다.** `TimeZone`이 그것이고, 저장되는 값은
  언제나 UTC입니다 — 이 설정이 정하는 것은 시트의 벽시계를 어느 시간대의 것으로 읽을지 하나뿐입니다
  ([설정](doc/recipe.md#timezone--시트의-날짜를-어느-시간대로-읽을지) ·
  [설계](spec/datetime-timezone.md)).
