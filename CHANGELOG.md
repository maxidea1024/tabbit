...
- **숫자 셀의 표기가 C#의 숫자 리터럴과 같아졌습니다.** 자릿수 구분자 `_`와 지수 표기가
  ...
  [설계](spec/types/number-literals.md)).
  - `1_000_000` · `0xFF_FF` · `0b1010_1010` · `3.141_592`. `_`는 숫자와 숫자 사이에만
    놓습니다 — `_1000`과 `1000_`은 오류이고, `0x`·`0b` 바로 뒤만 예외입니다(`0b_1010`)
  - **정수 컬럼도 지수 표기를 받습니다 — 그 값이 정수일 때만.** `1e3`은 1000이고 `1e-3`은
    오류입니다. 엑셀이 큰 수를 `1E+15`로 내보내기 때문에 받습니다. 자릿수를 옮겨서 계산하므로
    `double`을 거치는 반올림이 끼어들지 않습니다
  - **`bitset`이 `_`를 받게 되었습니다.** 이전 규격은 거부한다고 적었고 그것이 뒤집힙니다 —
    `0b1010_1010`이 마스크를 적는 방법이기 때문입니다. 부호·천 단위·소수점·지수의 거부는
    그대로입니다
  - **`,`와 `_`를 한 셀에 섞으면 오류입니다.** 값은 갈리지 않지만 적은 사람의 의도가 갈립니다
  - 기존 시트에서 읽히던 값은 하나도 달라지지 않습니다 — 골든이 한 바이트도 움직이지
    않았습니다

- **참과 거짓의 낱말을 recipe에 적을 수 있습니다.** 한국어로 채우는 시트에 `예`·`아니오`라고
  적을 수 있게 되었습니다 ([쓰는 법](doc/recipe/settings.md#truewords--falsewords--참과-거짓의-낱말) ·
  [설계](spec/types/boolean-words.md)).
  - `"TrueWords": ["예", "참", "켜짐"]` · `"FalseWords": ["아니오", "거짓", "꺼짐"]`
  - **내장 낱말에 더해집니다.** `Y`·`YES`·`TRUE`를 적어 둔 시트가 그대로 읽히고, 적은
    낱말만 읽으려면 `"BuiltinBoolWords": false`입니다
  - 양쪽에 같은 낱말을 적거나, 숫자를 낱말로 적거나, `-`를 적으면 **워크북을 열기 전에**
    오류입니다
  - 알 수 없는 셀의 오류 메시지가 무엇이 받아들여지는지 그 자리에서 나열합니다

- **`ArrayDelimiter`가 `DefaultDelimiter`로 바뀌었습니다.** 한 셀 안을 나누는 것이 배열
  하나가 아니게 되었기 때문입니다 — `set`과 `map` 셀의 항목도 이 구분자로 나뉩니다
  ([쓰는 법](doc/recipe/settings.md#defaultdelimiter--한-셀-안의-값-구분자) ·
  [설계](spec/types/value-delimiter.md)).
  - **`Default`가 이름에 있는 이유** — 구분자를 정하는 자리가 이것 말고도 있습니다. 구조체
    선언의 `:sep`과 합성 값 타입의 쉼표가 자기 셀을 정하고, 소스 항목이 자기 시트를 정합니다.
    이것은 그 어느 것도 말하지 않은 나머지 전부의 기본값입니다
  - **옛 이름을 적어 둔 recipe는 그대로 동작합니다.** 읽고, 지금 이름으로 고치라는 경고를
    함께 냅니다. 무시하지 않는 이유는 무시하면 `1|2|3`이 원소 하나가 되고 실행이 성공해
    버리기 때문입니다
  - **둘 다 적으면 오류입니다.** 어느 쪽을 남기려던 것인지는 읽는 쪽이 정할 수 없습니다
...
- **`.tbs` 를 쓰는 동안 오타와 타입 오류를 검출합니다.** VS Code 확장이 문법을 칠하고,
  `tabbit lsp` 가 진단 · 정의로 이동(F12) · 호버 · 자동완성 · 시맨틱 토큰을 답합니다
  ([사용](doc/editor.md) · [설계](spec/ops/lsp.md)).
  - **판정은 하나입니다.** 서버가 변환과 같은 파서를 돌리므로 편집기가 빨간 줄을 긋는 것과
    빌드가 거부하는 것이 같은 답입니다. 편집기에 두 번째 타입 시스템이 생기지 않습니다
  - **워크북을 열지 않습니다.** 타입이 `.tbs` 파일 집합만으로 닫히도록 설계되어 있어서
    가능한 것이고, 시트가 있어야 답할 수 있는 `foreign` 뒤의 테이블 이름은 변환이 답합니다
  - **한 벌은 폴더 하나입니다.** 레시피가 가리키는 단위와 같아서, 서로 다른 프로젝트의
    선언이 합쳐져 없는 중복 오류가 생기지 않습니다
  - **자동완성은 이름을 다시 적지 않습니다.** 스칼라는 `ScalarTypes`, 합성은
    `CompositeTypes`, 메타데이터 키는 `SchemaMetadata` 에 물으므로 표기법에 이름이 늘면
    후보에 저절로 나옵니다
  - **시맨틱 토큰이 이름을 실제 정체로 갈라 칠합니다.** 정규식으로는 되지 않는 것이고, 알아보지
    못한 이름은 색을 잃습니다 — 타입 이름을 잘못 적은 것은 워크북이 있어야 검사되므로 그것이
    편집기가 줄 수 있는 유일한 신호입니다
  - 확장은 `tools/vscode/` 에 있고 `npm run package` 로 만들어 설치합니다. 마켓플레이스에
    올라가 있지 않습니다
...
- **샘플이 전부 합성 데이터가 되었습니다.** 검증에 쓰던 상용 프로젝트의 워크북을 들어내고,
  ...
  ([샘플](samples/readme.md)).
  - **`sprout`** — 출시 전 소규모. 워크북 17개·테이블 71개·109,218행. 이 도구가 설계하지
    않은 규칙으로 쓰인 시트를 그대로 읽는 쪽이고, **변환하면 무엇이 나오는가**의 답입니다 —
  ...
  - **`canopy`** — 라이브 대규모. 워크북 42개·정의된 이름 549개·셀 873만. 테이블의 경계가
    ...
    담습니다. 같은 코퍼스가 `.xlsx`와 `.xlsb` 양쪽에 있어 **두 리더의 결과가 같아야 하는
    게이트**가 됩니다
  - **값이 아니라 분포를 재현합니다.** 정본은 `schema/*.tsv`이고 워크북은 생성기가 만듭니다.
