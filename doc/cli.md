...
|`-e`, `--env <이름>`|**이 실행이 어느 환경의 것인가.** summary에 기록되고, recipe의 `${TABBIT_ENV}`가 이 값으로 채워집니다 — 「[환경 지정](#--env--이-실행이-어느-환경의-것인가)」|
...
|`-v`, `--verbose`|디버그 로그까지 출력|
|`-q`, `--silent`|ERROR 아래로는 출력하지 않음|
|`--debug`|오류 발생 시 콜스택까지 출력|
|`-h`, `--help`|**도움말을 출력하고 종료.** 종료 코드는 0입니다 — 요청은 오용이 아니므로|
|`--version`|버전·커밋·런타임을, 릴리스 빌드면 빌드 시각까지 출력하고 종료|
|`@<파일>`|**인자를 파일에서 읽습니다** — 한 줄에 옵션 하나. 인자를 이것 하나만 줄 때 동작합니다|
...
빌드 시각은 릴리스 워크플로만 넘깁니다. 빌드 중에 시각을 평가하면 그것이 매번 달라지는 컴파일
입력이 되어, 결정론적 빌드가 없어지고 아무것도 고치지 않은 `dotnet build`가 전부 재컴파일이
됩니다.

`+` 뒤는 **커밋 해시**입니다. SDK가 넣어 주므로 로컬 빌드에도 붙고, 빌드 중에 git을 부르지
않습니다. 다만 해시로는 앞뒤를 읽을 수 없어서, 커밋 **날짜**는 아직 넣지 않았습니다 —
[CLI 도움말 스펙 §7.4](../spec/cli-help.md)에 넣는 방법을 적어 두었습니다.

빌드 시각은 실행 첫 줄에는 넣지 않습니다. 그 줄이 답하는 질문은 「어느 빌드가 돌고 있나」이고
버전과 커밋이 이미 그것을 답합니다. 빌드 시각을 원하는 사람은 **바이너리를 손에 들고 이게
무엇인지 묻는 사람**이고, 그 사람이 치는 것이 `--version`입니다.
