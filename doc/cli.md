...
|`--full`|**캐시를 보지 않고 전부 변환.** 캐시의 판정 자체를 의심할 때 쓰는 것입니다 — 「[다시 하지 않기](#다시-하지-않기--빌드-캐시)」|
|`--force-output`|입력 판정은 캐시를 그대로 쓰고 **출력 항목만 전부 실행.** 산출물을 다시 원할 뿐일 때|
|`--cache-dir <경로>`|캐시를 둘 곳. 생략하면 작업 디렉터리의 `.tabbit/`|
|`--detailed-exit-code`|**할 일이 없어서 끝난 실행을 종료 코드로 구분.** 다음 단계가 배포인 파이프라인용 — 「[종료 코드](#종료-코드)」|
|`--verbose`|디버그 로그까지 출력|
...
**두 줄로 나눠 적고 아래에 빈 줄을 둡니다.** 버전 하나와 형식 번호와 런타임과 플랫폼을 한 줄에
담으면 아무것도 찾을 수 없는 줄이 되고, 빈 줄이 없으면 이 줄들이 그 뒤 수백 줄의 앞 두 줄이
됩니다.

넷을 적는 이유는, 받은 로그에 대해 묻게 되는 것이 대개 이 넷 중 하나이기 때문입니다 — 어느
빌드인지, 어느 런타임인지, 어느 운영체제인지, 그리고 **바이너리 형식이 몇 번인지.** 마지막
것이 유일하게 읽는 쪽과 어긋날 수 있는 값입니다. 생성된 리더는 각자 형식 상수의 사본을
가지므로, 클라이언트가 파일을 거부하면 그것은 다른 무엇보다 먼저 버전 불일치입니다.

여기 적히는 빌드 문자열은 **빌드 캐시의 키에 들어가는 것과 같은 값**입니다. 「다른 빌드가 쓴
캐시입니다」라는 보고와 이 줄이 어긋나면 아무도 무엇을 해야 할지 알 수 없으므로, 한 곳에서
나옵니다.

**표준 출력이 남의 입력인 실행에는 적지 않습니다.** `--new-encryption-key`는 키 한 줄만,
`--out` 없는 `--history`·`--stats`는 JSON만 표준 출력에 내보내므로 그대로 파이프할 수 있어야
합니다. 파일 로그에는 언제나 남습니다.
...
**바뀐 것이 없으면 아무것도 하지 않습니다.** 실행마다 워크북을 다시 디코딩하고 테이블을 다시
쿠킹하고 모든 타깃을 다시 내는 것이 기본 동작이었고, 지금은 지난 실행이 무엇을 읽고 무엇을
썼는지 기록해 두었다가 비교합니다. 설계와 근거는 [빌드 캐시](../spec/build-cache.md)에 있습니다.
...
**전체를 다시 하게 되면 그 이유를 함께 출력합니다.** 캐시가 왜 듣지 않았는지 알 수 없으면
`--full`이 습관이 되고, 그러면 캐시가 없는 것과 같아집니다.
...
|도구의 빌드 버전|전부 — 생성기와 템플릿이 실행 파일 안에 있으므로|
|`--target-side` · `--time-zone` · `--env`|전부|
|recipe의 주석·공백·설정 순서|**아무것도**|
|워크북의 수정 시각만 바뀐 것|**아무것도** — 내용을 해시해서 확인합니다|
|`--verbose` · `--silent` · `--debug`|**아무것도**|

수정 시각만 바뀐 것을 걸러내는 것이 실제로 중요합니다. `git checkout`은 받은 파일 전부의
수정 시각을 지금으로 바꾸고, 엑셀은 열었다 저장만 해도 그렇게 합니다 — 시각만 보는 캐시는
체크아웃 한 번을 못 넘깁니다.

**검증은 캐시를 쓰지 않습니다.** `--validate-only`는 게이트이고, 「지난번에 통과했다」로
통과시키면 게이트가 아닙니다. 대신 변환에서 인장이 맞으면 검증도 함께 건너뛰므로, 매 실행
외부 저장소를 확인해야 하는 곳은 `--validate-only`를 따로 돌립니다.

**데이터베이스 타깃과 히스토리는 건너뛰지 않습니다.** 파일 타깃의 산출물은 해시해서 그대로인지
확인할 수 있지만 데이터베이스의 상태는 확인할 수 없습니다 — 파일을 하나도 쓰지 않는 항목은
확인할 것이 없으므로 매번 실행됩니다.

**건너뛴 실행도 낡은 생성 파일은 지웁니다.** 시트에서 테이블을 지웠으면 그 파일은 없어져야
하고, 그것이 「그날 다른 것이 바뀌었는지」에 달려 있을 이유가 없습니다. 지우는 조건은 늘 같습니다 —
타깃이 스윕을 요청한 디렉터리 아래에 있고, 파일 스스로 `Generated by Tabbit`이라고 적고 있는 것.

**캐시는 커밋 대상이 아닙니다.** 절대 경로와 그 기계의 파일 상태를 담으므로 `.gitignore`에
있습니다. CI에서 재사용하려면 `--cache-dir`로 마운트된 경로를 지정합니다.
...
`2`가 플래그 뒤에 있는 이유는 호환입니다. 명령줄 도구를 부르는 것들은 대개 0이 아닌 코드를
실패로 취급하므로, 기본으로 0이 아닌 값을 내면 **캐시가 처음 동작한 날** 뒤에 이어지는 단계가
전부 멈춥니다 — 새 결함으로 보이기에 가장 나쁜 날입니다.

다음 단계가 배포인 파이프라인이 쓰는 형태입니다.
...
**구글 시트는 스코프 하나를 더 씁니다.** 문서는 크기도 수정 시각도 없으므로 Drive에
`version`(서버의 모든 변경에 증가하는 카운터)을 물어봅니다. `drive.metadata.readonly`이고
셀은 한 칸도 읽지 않습니다. 물어볼 수 없으면 **바뀐 것으로 보고 가져오며**, 무엇을 허용하면
되는지 함께 출력합니다.

### 단계별 소요 시간

실행이 끝나면 각 단계가 얼마나 걸렸는지 출력합니다. 어디를 고쳐야 빨라지는지는 짐작이 아니라
이 숫자로 판단할 일입니다.
...
**멈춘 실행도 씁니다.** 오히려 그쪽이 본론입니다 — 콘솔은 스크롤이 지나가면 끝이고, 시트를
고칠 수 있는 사람은 대개 그 화면을 보고 있지 않습니다.

|무엇|어디에|
|--|--|
|사람이 읽는 것|`.report.html` — 워크북·시트로 묶여 접히고, 심각도 토글과 검색이 있습니다. 구글 시트의 위치는 **그 셀로 가는 링크**입니다|
|CI가 읽는 것|`.report.json` — 항목의 키가 메시지 ID이므로 문구를 다듬어도 필터가 깨지지 않습니다. 페이지와 달리 **언제나 전량**입니다|

경고 이상이면 **기본 브라우저로 열립니다.** 열지 않는 경우가 셋 있고, 셋 다 「보는 사람이 그
자리에 없다」의 신호입니다 — `CI` 환경 변수가 설정됨, 출력이 터미널이 아님, `--silent`.
설정은 recipe의 `Report.OpenInBrowser`(`never` · `problems` · `always`)입니다.

직전 실행 것을 다시 열려면 변환 없이 이것만으로 됩니다.

```bash
tabbit --recipe recipe.jsonc --show-report
```

리포트는 **종료 코드에 관여하지 않습니다.** 쓰지 못했으면 경고 한 줄이 나가고 실행의 결과는
그대로입니다. 설계와 근거는 「[빌드 리포트](../spec/build-report.md)」에 있습니다.
