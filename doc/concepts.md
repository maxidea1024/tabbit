...
Tabbit이 읽는 네 가지 선언과, 그것이 각각 무엇으로 생성되는지 설명합니다.
...
셀에 `:table`·`:enum`·`:const`·`:matrix`로 시작하는 **선언 셀**을 적으면 그 자리가
엔티티가 됩니다.
...
| **매트릭스** | `:matrix TownPrice` | **테이블 둘** — 값의 테이블과 열 축의 테이블, 그리고 `At(행키, 열키)` |
...
행과 열이 둘 다 키인 표입니다. 마을마다 상품마다의 가격 보정처럼, 컬럼 이름 자체가 값인
자리입니다.

![매트릭스 TownPrice](figures/concepts-town-price.svg)

읽는 자리는 셋입니다 — **B열이 행 축**, **C열부터가 격자**, **`:col` 줄이 열 축**입니다.
`:field`·`:type`의 C칸 하나가 격자 전체의 것이고, D 이후에 값이 있으면 오류입니다.

**선언 하나에서 테이블이 둘 나옵니다.** 값의 테이블과 열 축의 테이블입니다. 생성된 코드는
`At(행키, 열키)` · `Row(행키)` · `RowKeys` · `ColKeys` 를 냅니다 — 셀이 옵셔널이면 `HasAt` 도
함께입니다.

**두 축의 키가 정수가 아니어도 됩니다.** 위 그림의 열 축은 `foreign Goods` 이고, enum 라벨이나
문자열도 됩니다 — [매트릭스 표](../spec/layout/matrix-tables.md)의 판독 규칙이 정수만 받던
자리를 이 표기가 여는 것입니다.

표기 전체는 [매트릭스 선언](../spec/layout/matrix-declaration.md)에 있습니다.

### 엔티티에 따라 갈리는 배포 경로

| 엔티티 | 배포 |
| --- | --- |
| 테이블 · 매트릭스 | 데이터 파일로 나갑니다. 대개 데이터만 올려도 반영됩니다 |
