...
[행 태그](sheets/layout.md#행-태그--빌드마다-빼는-행)를 쓴 빌드에는 `rowTags`가 더해집니다.
본 행 태그마다 `tag` · `rows`(붙은 행) · `omitted`(그중 뺀 행)이고, **행 태그를 쓰지 않은
프로젝트의 요약에는 이 항목이 없습니다.**

행 태그 이름은 어디에도 선언하지 않으므로 **오타를 알아볼 자리가 여기입니다** — 부른 적 없는
낯선 이름이 `omitted`가 0인 채로 적혀 있으면 그것입니다.
...
저장된 커밋 해시가 아니면 git으로 조회해서 해석합니다.
...
git 태그는 빌드를 돌리지 않은 커밋을 가리키는 경우가 흔합니다.
...
오류로 처리하면 해시를 손으로 찾아야 하고, 조용히 대체하면 요청한 것과 다른 구간을 내게
됩니다.

해석에 필요한 워킹카피는 빌드할 때와 같은 순서로 찾습니다.
`--repository`, 그다음 시트의 소스 디렉터리, 그다음 현재 디렉터리입니다.

체크아웃이 없는 머신에서는 git 태그를 해석할 수 없고, 「스냅샷이 없다」가 아니라 그 이유를
출력합니다.
...
| 옵션 | 의미 |
| --- | --- |
| `--from` / `--to` | 범위. 커밋 해시(앞부분 가능), git 태그, 리비전 표현식. 생략하면 브랜치 처음과 head |
...
| 필드 | 의미 |
| --- | --- |
| `query.notes` | 요청하지 않았는데 수행된 일입니다. git 태그를 커밋으로 해석했거나 스냅샷 없는 커밋을 뒤의 것으로 대체했을 때 문장이 들어옵니다. 숫자가 무엇을 뜻하는지가 달라지므로 비어 있지 않으면 읽어야 합니다 |
...
`from`, `to`, `at`은 CLI와 똑같이 git 태그와 리비전 표현식을 받습니다.
...
git 태그를 대체했다는 안내를 터미널에서만 보고 페이지에서는 못 보는 상황이 생기지 않습니다.
