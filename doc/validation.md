...
**폴더는 다섯이고 단계는 넷입니다.** `rules/shared/`에 둔 것은 어느 시점에도 스스로 돌지 않고,
`pre`·`tables`·`global`·`runtime`의 규칙이 그것을 쓸 수 있을 뿐입니다 — 그래서 하위 폴더를
가져도 되는 유일한 폴더이기도 합니다([§4](#4-폴더-규약)).
...
`Option(key)`가 없는 키에 대해 **오류를 내는 이유**는, 빈 문자열로 말없이 답하면 로케일 비교가
...
**자동 완성이 이 형태를 정했습니다.** 처음 형태는 클래스 없이 최상위 문장을 쓰는 파일이었고,
...
그 파일의 컴파일을 중단합니다. 남는 것은 **평범한 클래스와 평범한 `using` 두 줄**이고, 그것이 지금의 형태입니다.
