...
**원소 옵셔널(비트 7)이 켜진 컬럼**은 값 앞에 비트맵이 하나 더 붙습니다 — 로우별 비트맵 다음, 값 앞이고, 길이는 **실제로 기록된 원소의 총수**입니다(고정 배열이면 `rowCount × count`, 가변 배열이면 행 길이의 합). 로우별 비트맵과 같이 자기 인코딩 바이트를 앞에 답니다. 설계는 [원소가 없을 수 있는 배열](../spec/nullable-array-elements.md)에 있습니다.
