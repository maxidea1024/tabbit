...
|의존 패키지 문서|[의존 패키지](dependencies.md)를 저장소의 모든 `PackageReference`와 대조합니다 — 참조하는데 문서에 없는 것, 문서에 있는데 아무도 참조하지 않는 것, 버전이 다른 것. 손으로 맞추는 목록은 썩고, 실제로 썩어서 없어진 의존이 남아 있었습니다.|
...
`samples/rescue/out/`은 커밋되어 있지만 어떤 게이트도 보지 않기 때문입니다.
...
다른 샘플은 해당하지 않습니다. `samples/named-range/out/`에 커밋된 것은 바이너리와 JSON뿐이라 생성기와
무관합니다.
