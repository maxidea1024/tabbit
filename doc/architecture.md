...
이 분리가 놓인 자리가 `Tables.ReadAllBytesAsync` 시임입니다. 원래 있던 것이고 — 검증 호스트가 메모리에서 테이블을 채울 때 이미 여기에 대입합니다 — 어댑터는 그 자리를 쓸 뿐입니다. 읽기 방식을 바꾸려면 같은 자리에 대입하면 됩니다: 팩 파일, CDN, Addressables 모두 그 자리에서 갈아끼웁니다.
