...
**Unreal 리더가 C++ 리더의 사본이 아닌 이유.** 처음에는 공유했습니다 — 포맷이 정의된 곳이고 이미 코퍼스가 검증하니까요. 그 대가는 엔진에 이미 있는 것을 다시 만드는 것이었습니다. `std::string`으로 받아 `FString`으로 옮기느라 문자열 셀마다 할당 두 번, `Uuid`를 36자 텍스트로 만들어 `FGuid::Parse`에 넘기느라 uuid마다 할당 세 번과 파싱 한 번. 더 나쁜 건 그 테이블 리더가 실패를 **예외로** 알린다는 점이었습니다. Unreal은 모듈을 예외 비활성으로 빌드하므로, 손상된 `.tcb` 하나가 `bool`을 반환하겠다고 선언한 함수 안에서 프로세스를 끝냈습니다.
...
**전체는 푸쉬 전 한 번입니다.** 그 시간의 대부분은 아래 표의 언어별 항목 — 15개 언어의 생성 코드를
...
**전 언어 산출물 비교.** [test/reserved-words/](../test/reserved-words)에 `reserved-words` 픽스처를 **생성기 전부로** 만든 결과가 커밋되어 있습니다. 디렉터리 이름이 그 픽스처의 이름이고, 그것이 이 트리가 실제로 확인하는 범위입니다 — 컬럼 이름이 `class`·`delete`·`operator`·`namespace`·`constructor`·`function`·`int`이어서 **각 언어가 자기 예약어를 어떻게 피하는지**가 드러납니다. 표 2개·4행·enum 1개이고 배열도 참조도 상수셋도 없으므로, 그보다 넓은 것은 자기 픽스처를 가져야 합니다.

게이트가 아니라 읽기 위한 것입니다 — 스위트는 전부 컴파일해 주지만, 생성기들이 무엇을 뱉는지 나란히 놓고 보여주지는 못합니다. 그 차이가 예약어 바깥에서도 문제를 검출합니다: Unreal 타깃이 일반 C++ 리더를 싣고 있던 것도, Dart가 `int int = 0;`을 내던 것도 여기서 드러났습니다.

**검증 규칙도 같은 자리에 있습니다.** [test/reserved-words/validation/](../test/reserved-words/validation)에 그 시트에 대한 규칙 폴더가 있고, recipe가 그것을 가리킵니다 — 나머지는 생성된 것이고 이것은 **손으로 쓴 관점**입니다. 즉 「생성된 것을 실제로 어떻게 받아 쓰는가」가 같은 디렉터리에서 읽힙니다. 규칙은 `rules/` 아래에 있고, 그 옆의 `Validation.csproj`와 `lib/`도 함께 커밋됩니다 — 받은 사람이 변환을 돌리지 않고도 편집기에서 열 수 있어야 하기 때문입니다. 빌드가 남기는 `.build/`만 무시 대상입니다.
