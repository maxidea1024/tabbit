...
마커 열은 테이블의 세로 범위 전체에서 예약됩니다. 담을 수 있는 것은 다음뿐이고, **`:`로 시작하는
미지의 값은 오류**입니다 — 오타가 조용히 지나가지 않게 하기 위해서입니다.

|적는 것|뜻|
|--|--|
|`:table` · `:enum` · `:const` · `:matrix`|엔티티 선언. **직전 엔티티의 종료이기도 합니다**|
|`:field` · `:type` · `:desc` · `:target` · `:variant`|헤더 행 키|
|빈 칸|데이터 행|
|`#` 또는 `//`|**그 행을 변환에서 제외.** 데이터 칸을 고치지 않고 빼고 되돌립니다|
|그 밖|**그 행의 태그.** 빌드마다 골라서 뺍니다 — [행 태그](#행-태그--빌드마다-빼는-행)|
...
|멀티 로우 — `reward[].itemId`|**원소의 타입**을 적습니다. 배열임은 이름의 `[]`에 이미 적혀 있습니다|
...
- 이미 다른 라벨의 이름인 별칭은 거부합니다. 실제 이름이 먼저 답하므로 그 별칭은 아무것도
  가리키지 못합니다.
...
`key`가 정하는 것은 「어느 컬럼으로 행을 가리키는가」이고, **컬럼은 움직이지 않습니다** —
...
|Matrix|`:matrix 이름`|행과 열이 둘 다 키인 격자. **선언 하나가 테이블 둘이 됩니다** — [표기](../../spec/layout/matrix-declaration.md)|
...
|int|`int.Parse`|아래 [숫자 표기](#숫자-표기). 천단위 구분자·`_`·`0x`·`0b`·지수 허용|
|bigint|`long.Parse`|위와 같습니다|
|bitset|자체 파싱|플래그 묶음. **더 엄격합니다** — 부호·천단위·소수점·지수를 거부합니다. [아래](types.md#bitset--플래그-묶음)|
|float|`float.Parse`|아래 [숫자 표기](#숫자-표기). `0x`·`0b`는 정확히 표현되는 값만|
|double|`double.Parse`|위와 같습니다|
|bool|자체 파싱|아래 참고|
...
**C#의 숫자 리터럴과 같습니다.** 접미사(`1.5f`·`100L`)만 없습니다 — 타입 행이 이미 타입을
말하기 때문입니다.

|적는 것|읽히는 값|
|--|--|
|`1,000,000`|1000000 — 천단위 구분자|
|`1_000_000`|1000000 — 자릿수 구분자|
|`0xFF_FF`·`0b1010_1010`|65535·170|
|`1e3`·`1E+15`|1000·1000000000000000|
|`3.141_592`|3.141592|

- **`_`는 숫자와 숫자 사이에만 놓습니다.** `_1000`·`1000_`·`1_.0`은 오류입니다. `0x`·`0b`
  바로 뒤는 예외입니다(`0b_1010`).
- **`,`와 `_`를 한 셀에 섞지 않습니다.** 값은 갈리지 않지만 적은 사람의 의도가 갈립니다.
- **정수 컬럼도 지수 표기를 받습니다 — 그 값이 정수일 때만.** `1e3`은 1000이고 `1e-3`은
  오류입니다. 엑셀이 큰 수를 `1E+15`로 내보내기 때문에 받고, 자릿수를 옮겨서 계산하므로
  `double`을 거치는 반올림이 끼어들지 않습니다.
- **밑수는 표기이지 타입이 아닙니다.** `0xFFFFFFFF`는 `int` 컬럼에서 10진수로 적었을 때와
  똑같이 넘칩니다. 32비트 패턴을 뜻하는 컬럼은 [`bitset`](types.md#bitset--플래그-묶음)입니다.

> 설계와 근거는 [숫자 리터럴 표기](../../spec/types/number-literals.md)에 있습니다.
...
|`Y` `YES` `TRUE` `1`|참|
|`N` `NO` `FALSE` `0`|거짓|
|**recipe가 적은 낱말**|recipe가 정한 대로 — 아래|
|빈 셀|거짓|
...
**시트에 `예`·`아니오`라고 적고 싶으면 recipe에 적습니다.**

```json
"TrueWords":  ["예", "참", "켜짐"],
"FalseWords": ["아니오", "거짓", "꺼짐"]
```

내장 낱말에 **더해집니다** — `TRUE`를 적어 둔 시트가 그대로 읽힙니다. 적은 낱말만 읽으려면
`"BuiltinBoolWords": false`입니다. 자세한 것은
[설정 하나하나](../recipe/settings.md#truewords--falsewords--참과-거짓의-낱말).
...
`#`는 **모든 빌드에서** 뺍니다. 빌드마다 갈라서 빼려면 태그를 씁니다.

### 행 태그 — 빌드마다 빼는 행

마커 열에 낱말을 적으면 그 행의 태그입니다. **적어 두기만 해서는 아무 일도 일어나지
않고**, 레시피의 `ExcludeTags`가 그 태그를 부를 때만 그 행이 빠집니다.
...
```jsonc
"ExcludeTags": [ "wip", "stage=test" ]   // 5행과 6행이 빠집니다
```

|적는 법|내용|
|--|--|
|**여러 개**|쉼표로. `wip, event` — 하나만 걸려도 그 행이 빠집니다|
|**값**|`key=value`. 레시피가 `stage`라고 적으면 `stage`가 붙은 행 전부, `stage=test`라고 적으면 그 값인 행만 빠집니다|
|**대소문자**|가리지 않습니다. `WIP`과 `wip`은 한 태그입니다|
|**레코드 여러 행**|첫 행에 적으면 연장 행까지 함께, 연장 행에 적으면 그 행의 원소만. `#`와 같습니다|
|**enum · 상수셋**|적을 수는 있고 **행이 빠지지는 않습니다.** 그 행들은 데이터가 아니라 생성 코드라 빌드마다 달라지면 안 되기 때문입니다|
|**`#`와 붙여 쓰지 않습니다**|`#wip`은 이름이 `#wip`인 태그로 읽혀 **빼려던 행이 나갑니다.** 오류로 보고합니다 — `#` 하나로 빼거나 `wip` 하나로 태그를 답니다|

**빠진 행은 시트에 없던 행입니다.** 그 행을 가리키는 참조는 없는 키를 가리키는 참조로
보고됩니다 — 손으로 지웠을 때와 같습니다.

**태그 이름은 어디에도 선언하지 않습니다.** 그래서 `wpi`라고 잘못 적어도 오류가 아니라
「아무도 부르지 않는 태그」가 되고, 그 행은 빠지지 않습니다. 변환 요약(`run.rowTags`)에
**본 태그가 전부 나오고 각각 몇 행에 붙어 몇 행을 뺐는지** 적히므로, 0행을 뺀 낯선 이름
하나가 오타입니다.

|이렇게 씁니다|시트에|레시피에|
|--|--|--|
|작업 중인 행|`wip`|릴리스 빌드만 `"wip"`|
|테스트 데이터|`test`|라이브 빌드만 `"test"`|
|미출시 콘텐츠|`season=3`|열기 전까지 `"season=3"`|
|스토어별 행|`store=ios` · `store=android`|각 빌드가 나머지를 적습니다|

[테이블의 행 벌](../../spec/layout/table-row-sets.md)과 다릅니다 — 행 벌은 **벌마다 파일이
나오고 읽는 프로그램이 고르는** 것이고, 태그는 **그 빌드가 담은 한 벌만 파일에 들어가는**
것입니다. 미출시 콘텐츠처럼 배포에 실리면 안 되는 것은 태그 쪽입니다.
