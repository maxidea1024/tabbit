...
그래서 **적합성 코퍼스**가 있습니다. `test/fixtures/xlsx/conformance`가 경계값을 담은 테이블 하나입니다 — 2^53+1, float32의 0.1, varint 1~5바이트, 빈 문자열·빈 배열·비ASCII, .NET 바이트 순서의 uuid, 지그재그 enum, 다른 테이블의 행과 필드를 가리키는 참조 두 개. 회귀 스위트가 이것을 **13개 언어로 각각 컴파일·실행해서 읽고** 익스포터 JSON과 필드 단위로 대조합니다.
