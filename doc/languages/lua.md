...
행, enum, 상수, 접근자 전부에서 선언되지 않은 키를 읽거나 쓰면 그 자리에서 타입 이름과 함께
오류가 납니다.

`row.maxHP` 같은 오타가 조용히 `nil`을 돌려주고 그 `nil`이 멀리서 다른 오류로 나타나는 Lua의
익숙한 사고를 막는 장치입니다.

올바른 접근에는 비용이 없습니다. 메타테이블은 키가 테이블에 없을 때만 호출됩니다.
...
산출물에 함께 나오는 `tabbit/native/tabbit_native.c` 하나입니다.
`lua.h`와 표준 라이브러리 외에는 아무것도 include하지 않으며 Lua 5.1(LuaJIT)부터 5.4까지
컴파일됩니다.

**평문 파일만 읽는 프로젝트는 빌드할 필요가 없습니다.** 리더는 암호 경로에 들어갈 때만
모듈을 `require`하고, 없으면 무엇을 빌드해야 하는지 함께 출력하고 멈춥니다.
...
- **옵셔널은 `nil`이 아닙니다.** 값 필드는 언제나 채워져 있고, 존재 여부는 `hasHp` 같은
  이웃 필드가 정합니다. `if row.hp == nil`은 이 계약에서 이미 잘못이고, 엄격 메타테이블이
  그것을 즉시 오류로 만듭니다.
