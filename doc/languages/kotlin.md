...
그 파일 안의 작은 파서입니다. Java 업데이터를 부르는 게 아니라 **Kotlin 파일**이므로, Kotlin
프로젝트가 Java를 함께 컴파일할 필요가 없습니다.
...
**미사용 import는 경고입니다.** Kotlin은 오류가 아니라 경고라, 생성물이 파일마다 같은 목록을
가져도 빌드가 깨지지 않습니다. Go가 같은 자리에서 오류를 내는 것과 대비됩니다.
