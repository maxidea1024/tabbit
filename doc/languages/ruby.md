...
recipe에 `"WriteUpdater": true`를 적으면 `tabbit/updater.rb`가 함께 나옵니다. CDN이나 버킷에 올려둔 데이터를 받아 로컬 사본을 최신으로 유지하는 코드이고, **배포를 다시 하지 않고 데이터만 패치**하기 위한 것입니다. 기본값이 `false`인 이유는 네트워크를 쓰기 때문입니다.
...
예외를 throw하지 않습니다. 네트워크·디스크·손상된 파일은 모두 호출한 쪽이 다뤄야 할 상황이지 결함이 아니기 때문입니다. 실패하면 `result.error`에 이유가 들어있고, **디스크의 이전 데이터는 손대지 않은 상태**입니다.
