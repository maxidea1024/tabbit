...
- **`foreign A|B[]`은 아직 거부합니다.** 원소마다 다른 테이블일 수 있으면 원소마다 판별자와 좁히는 접근자가 필요한데, 생성되는 리더는 그것을 컬럼 하나와 레코드 멤버에 대해서만 가지고 있습니다. 해석되지 않는 코드를 말없이 만들지 않기 위한 거부이고, 그 목록이 필요하면 연번 컬럼마다 적습니다.
