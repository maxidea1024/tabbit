...
- `foreign[]`은 지원하지 않습니다. 로우마다 가변 개수의 참조를 해석해야 하는데 생성되는 테이블 리더에 그런 형태가 없어서, 해석되지 않는 코드를 말없이 만드는 대신 명시적으로 거부합니다. 고정 개수라면 원소를 컬럼으로 적으세요 — `slot[0]`·`slot[1]`을 `foreign`으로 적으면 **참조의 배열**이 되고, 원소마다 키 하나와 해석 결과 하나를 갖습니다. 대상이 로우 전체(`Piece`)든 그 로우의 값 하나(`Piece.Tier`)든 같습니다.

### 여러 테이블 중 하나를 가리키기 — `Weapon|Armour`

한 컬럼의 값이 **어느 한 테이블의 행이면 되는** 경우가 있습니다. 보상 id 하나가 아이템일 수도
있고 선박일 수도 있는 식입니다. 타입 칸에 `foreign`과 함께 세로줄로 나열하면 됩니다.
...
같은 목록이 여러 컬럼에서 되풀이되면 **그 목록에 이름을 붙일 수 있습니다.** 카탈로그가 하나
늘 때 그 목록을 적은 컬럼을 전부 찾아 고치는 대신, **새 시트가 자기를 집합에 넣습니다.**
...
카탈로그는 자기 선언 셀에 그 이름을 답니다.

|`:table Item(extends=Reward)`|
|--|
|`:table Mount(extends=Reward)`|

그러면 타입 칸에 집합의 이름 하나만 적으면 됩니다.

```
foreign   Item|Mount           목록을 그 자리에 적기
foreign   Reward               집합의 이름으로 — 위와 같은 뜻
```

**둘은 같은 것이 됩니다.** 생성 코드도 파일도 달라지지 않습니다 — 집합은 목록의 이름이고,
아래로 내려가는 것은 목록뿐입니다.

|  |목록을 적기|집합의 이름|
|--|--|--|
|카탈로그를 추가할 때|그 목록을 적은 컬럼 전부|**새 시트에 `extends=` 하나**|
|`abstract struct`의 필드|—|**모든 변종이 갖춰야 하는 컬럼**. 없으면 그 테이블을 가리켜 거부합니다|

- **`abstract struct`의 필드는 공통 표면입니다.** 위 예에서 `Item`과 `Mount`는 둘 다
  `name`과 `icon`을 `string`으로 가져야 합니다. 필드를 하나도 적지 않으면 그 집합은 목록의
  이름일 뿐이고 아무것도 검사하지 않습니다.
- **`extends=`는 `:table`에만 적습니다.** enum과 상수셋에는 행이 없습니다.
- **집합에 테이블이 하나뿐이면 평범한 참조입니다.** 목록의 길이가 1인 것과 같습니다.
- 그 밖의 규칙 — 겹치는 id, 키 타입, 빌드에 없는 대상 — 은 목록을 적었을 때와 같습니다.

> 설계와 근거는 [다형과 참조 배열](../spec/polymorphism.md)에 있습니다.
