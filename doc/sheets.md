...
- **지금은 `json`만 냅니다.** 파일 형식은 로우마다 1비트를 담고 원소마다 담을 자리가 아직 없으므로, `binary`와 코드 생성 타깃은 이런 컬럼을 만나면 **그 타깃과 컬럼의 이름과 함께 거부합니다.** 남은 단계는 [원소가 없을 수 있는 배열](../spec/nullable-array-elements.md)에 있습니다.
