...
시트 하나에 엔티티를 여러 개 정의해도 되고, 편집이 편하도록 여러 시트로 나눠 작성해도 됩니다.
특별히 제한을 두지 않습니다.

한 시트에 엔티티를 몰아서 빈틈없이 배치해도 문제가 없습니다.
다만 보기 좋게 한 칸 정도 띄워 두는 편이 낫습니다.

주의할 점이 하나 있습니다.

엔티티 정의 요소 바로 옆에 엔티티가 아닌 내용을 적으면 엔티티 정의의 일부로 인식될 수 있습니다.
한 칸 띄운 셀에 적어야 합니다.

엔티티 바로 옆에 다른 엔티티를 정의하는 것은 문제가 없습니다.
엔티티 정의 바로 옆에 엔티티가 아닌 내용이 오는 것만 문제가 됩니다.
...
작성 중이라 아직 완성되지 않은 워크북이나 시트가 있을 수 있습니다.

다른 폴더로 옮기는 방법도 있지만 번거롭습니다.

파일 이름, 시트 이름, 필드 이름, 변수 이름 앞에 아래와 같이 접두어를 붙이면 대상에서
제외됩니다.
...
배열의 길이는 어느 쪽이든 행마다 기록됩니다.
serial field의 길이가 모든 행에서 같더라도 그렇고, 그런 컬럼의 길이 스트림은 런 하나로
...
기본이 꺼져 있는 이유는 `FoldSerialFields`와 같습니다.
