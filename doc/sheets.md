...
|무엇을 하려는가|어디를 볼까|
|--|--|
|시트를 처음 만든다|[시트에 엔티티를 배치하는 방법](#시트에-엔티티를-배치하는-방법)|
|컬럼에 무슨 타입을 적을 수 있나|[Supported Data Types](#supported-data-types)|
|값을 여러 개 담고 싶다|[배열이 오는 세 자리](#배열이-오는-세-자리--셀--칸--행)|
|컬럼 여러 개를 한 덩어리로 묶고 싶다|[중첩 필드](#중첩-필드--컬럼-여러-개를-레코드-하나로)|
|칸을 비워도 되나|[빈 칸의 뜻](#빈-칸의-뜻--자리별-총정리)|
|이름을 어떻게 지어야 하나|[Naming Rules](#naming-rules)|
|서버와 클라이언트에 다른 것을 주고 싶다|[Target Side](#target-side-서버클라-분리)|
|왜 안 읽히는지 모르겠다|[헷갈리는 자리](#헷갈리는-자리) · [트러블슈팅](troubleshooting.md)|
|우리가 설계하지 않은 시트를 읽어야 한다|[다른 규칙으로 쓰인 시트](#다른-규칙으로-쓰인-시트)|
...
- **10진수는 2^53까지입니다.** 엑셀의 숫자 셀은 double이라 그보다 큰 값은 셀에 적히는 순간 이미
  ...
  함께 나옵니다.
- **거절하는 것** — 부호(`-1`), 천단위 구분자(`1,000`), 소수점(`1.5`와 **`1.0` 둘 다**),
...
- 폴더는 **recipe가 지정합니다** — `Assets.Roots`. 자세한 것은
  [recipe 레퍼런스](recipe.md#assets--애셋-폴더)에.
...
- **레코드의 멤버에는 붙지 않습니다.** 레코드는 하나의 값이고, 「Id는 있는데 Count는 없다」는
  ...
  [원소 개수](#로우마다-길이가-다른-레코드-배열--옵트인)입니다.
...
> 다른 레이아웃은 이 설정을 읽지 않습니다 — 거기서는 연번이 그저 이름의 일부이고, 규칙이 맞을
> 대상 자체가 없습니다.
...
형태 — 시트를 고치지 않고 그대로 읽는 **`rescue` 레이아웃**이 따로 있습니다. 레이아웃은 소스
...
**레이아웃 규칙과 실제 적용 기록은
[다른 규칙으로 쓰인 시트 읽기](../samples/rescue/doc/적용-기록.md)에 따로 있습니다.** 기존
