...
데이터를 불러올 때 모든 셀의 값을 `Trimming` 한 뒤 불러옵니다. 앞뒤에 공백을 추가해도 제거되므로 주의가 필요합니다. UI 텍스트로 출력할 때 앞뒤 공백으로 레이아웃을 맞추는 방식은 동작하지 않습니다. 필요하다면 공백 대신 다른 문자를 기재하고, 그 문자를 공백으로 치환해 처리합니다.
...
같은 범위에 없어서 이름이 겹치지 않는 경우에도 **연속 밑줄 자체가 보고됩니다** — 밑줄 개수의
차이는 생성 코드 어디에도 전달되지 않으므로, 의도라면 전달되지 않고 오타라면 드러나야 합니다.
...
위 1번의 자동 변환은 **단어 경계를 입력의 대소문자에서 얻습니다.** 그래서 같은 컬럼을
시트마다 `maxHitPoints` · `maxhitpoints` · `maxhitPoints`로 적으면 각각
`MaxHitPoints` · `Maxhitpoints` · `MaxhitPoints`가 되어 **생성 코드에 멤버가 세 개** 생기고,
소비 코드는 테이블마다 어느 표기인지 기억해야 합니다. 타입 선언이 없는 언어에서는 틀린
표기로 읽어도 오류가 아니라 값 없음이라, 증상이 조용한 기능 누락으로 나타납니다.

recipe의 `Naming` 섹션이 이것을 검사합니다 — 종류별로 따라야 할 표기를 선언하고, 선언이
없어도 **한 이름이 여러 표기로 적힌 것**과 **연속 밑줄**은 항상 보고합니다. 값과 도입 절차는
[recipe 파일 — `Naming`](recipe.md#naming--이름의-표기-규약)에 있습니다.
...
모든 언어에서 보증해야 하고, 그중 몇 언어는 **동등성이 값의 것이 아닌 타입**을 시각에 씁니다 —
요구가 없는 형태에 그 비용을 쓰지 않습니다. 시각을 들고 싶으면 평범한 컬럼으로 두고 키는 다른
것으로 잡으세요.
...
- `foreign[]`은 지원하지 않습니다. 로우마다 가변 개수의 참조를 해석해야 하는데 생성되는 테이블 리더에 그런 형태가 없어서, 해석되지 않는 코드를 말없이 만드는 대신 명시적으로 거부합니다. 고정 개수라면 `SerialField`를 사용하세요 — `Slot1`·`Slot2`를 `foreign`으로 적고 접으면 **참조의 배열**이 되고, 원소마다 키 하나와 해석 결과 하나를 갖습니다. 대상이 로우 전체(`Piece`)든 그 로우의 값 하나(`Piece.Tier`)든 같습니다.

### 여러 테이블 중 하나를 가리키기 — `Weapon|Armour`

한 컬럼의 값이 **어느 한 테이블의 행이면 되는** 경우가 있습니다. 보상 id 하나가 아이템일 수도
있고 선박일 수도 있는 식입니다. detail-type에 세로줄로 나열하면 됩니다.

```
foreign   Weapon                 대상 하나
foreign   Weapon|Armour          둘 중 하나
```

그 컬럼은 **키를 그대로 들고**, 생성 코드가 그 옆에 두 가지를 더 냅니다.
...
`None`은 셀이 비어 있거나 값이 어느 대상에도 없는 경우입니다.

- **대상끼리 id가 겹치면 거부합니다.** 두 카탈로그가 같은 id를 들면 접근자가 함께 답하게 되고,
  그 컬럼이 어느 행을 뜻했는지가 데이터에 없어집니다. 대역을 갈라 두거나, 컬럼을 나누세요.
- **키 타입은 대상끼리 같아야 합니다.** 컬럼이 값 하나를 들기 때문입니다.
- **적힌 대상이 하나라도 이 빌드에 없으면 거부합니다.** 대상 하나짜리 `foreign`과 같습니다.
- **점 표기와 섞을 수 없습니다.** `Weapon.Name|Armour.Name`은 받지 않습니다 — 「이 테이블들 중
  하나의, 이 필드」는 아직 정하지 않은 형태입니다.
- **레코드 그룹의 멤버에는 아직 못 씁니다.** 원소 안에서 대상별 접근자가 어떤 이름을 갖는지가
  남아 있습니다 ([설계](../spec/multi-target-accessors.md)).
- Rust와 Unreal은 키만 갖습니다. 그 두 언어는 참조를 로드 시점에 잇지 않기 때문입니다.
...
- 2번 로우의 `Nick`은 **빈 문자열**이고, `Bonus`는 **값이 없습니다**. 생성된 코드에서
  `HasBonus`가 거짓이고 `HasNick`은 참입니다.
- 3번 로우의 `Nick`은 `-` 한 글자입니다.
...
- **연번 컬럼 그룹(`Tag1`·`Tag2`)에서는 그 컬럼의 `?`가 원소의 답입니다.** 칸마다 적는 것이 원소 하나의 타입이므로 그렇습니다 — `Tag2`에 `-`를 적으면 그 원소만 없고, 앞뒤 원소는 그대로입니다. 배열 전체의 없음은 연번 그룹에 표기가 없습니다(컬럼이 모든 로우에 있으므로 뜻할 것이 없습니다). 그것이 필요하면 구분자 배열 `T[]?`입니다.
...
두 방식은 **와이어 포맷이 같습니다.** 배열의 길이는 어느 쪽이든 로우마다 기록됩니다 — serial
field의 길이가 모든 로우에서 같더라도 그렇고, 그런 컬럼의 길이 스트림은 런 하나로 접힙니다
([TCB v107](../spec/tcb-v107-dynamic-arrays.md)).
...
> **원소를 추가하는 것도 그렇습니다.** `Slot3.*`을 시트에 더하면 그 로우들이 원소 셋을 싣고,
