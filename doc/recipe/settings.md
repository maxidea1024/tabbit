...
시트에 적힌 `2022-01-24 10:30:00`에는 **어느 시간대의 10시 30분인지 적혀 있지 않습니다.** 이 설정이
...
|**recipe의 `DefaultDelimiter`**|**타입에도 항목에도 적히지 않은 나머지 전부**|
...
- **숫자는 낱말이 아닙니다.** `0`이나 `1`을 적으면 오류입니다 — 「숫자는 0이 아니면 참」이
  이미 정하는 것이라, 낱말로 적으면 두 규칙이 같은 셀을 두고 갈립니다.
...
- **환경마다 다르게 적으려면 `${TABBIT_ENV}`를 씁니다.** 레시피에 있으므로 빌드 캐시가
  이 값의 변화를 확인합니다 — 태그를 바꾸면 다시 변환합니다.
